...
namespace PX.Commerce.Shopify.API.REST
{
	public abstract class SPRestClientBase : RestClient
	{
		...
		private const string XShopifyAccessToken = "X-Shopify-Access-Token";
		...
		protected SPRestClientBase(IDeserializer deserializer, ISerializer serializer, IRestOptions options, Serilog.ILogger logger)
		{
			...
			//If User setup Access Token, it will be used first; otherwise will use Api Id and Password mode.
			if (string.IsNullOrEmpty(options.AccessToekn) == false)
			{
				this.AddDefaultHeader(XShopifyAccessToken, options.AccessToekn);
				apiIdentifyId = options.AccessToekn;
			}
			else
			{
		...
		{
			var request = new RestRequest(url) { JsonSerializer = _serializer, RequestFormat = DataFormat.Json, ReadWriteTimeout = 300000 };
