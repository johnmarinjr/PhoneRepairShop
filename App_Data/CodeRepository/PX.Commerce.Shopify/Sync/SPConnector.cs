	...
	public class SPConnector : BCConnectorBase<SPConnector>, IConnector
	{
			...
			where TInfo : class
		{
			...
			try
			{
				...
				{
					InventoryLocationRestDataProvider provider = new InventoryLocationRestDataProvider(GetRestClient(shopifyBinding));
			...
			where TInfo : class
		{
			...
			try
			{
				...
				var dataList = (new StoreRestDataProvider(GetRestClient(shopifyBinding))).GetShippingZones();
		...
		{
			return GetRestClient(binding.ShopifyApiBaseUrl, binding.ShopifyApiKey, binding.ShopifyApiPassword, binding.ShopifyAccessToken, binding.StoreSharedSecret, binding.ApiCallLimit);
		}
		public static SPRestClient GetRestClient(String url, String clientID, String password, String accessToken, String sharedSecret, int? apiCallLimit)
		{
			RestOptions options = new RestOptions
			{
				BaseUri = url,
				XAuthClient = clientID,
				XAuthTocken = password,
	...
	public class SPConnectorDescriptor : IConnectorDescriptor
	{
		...
		protected SPRestClient GetSPRestClient(Int32? binding)
		{
				...
				Where<BCBinding.bindingID, Equal<Required<BCBindingShopify.bindingID>>>>.Select(graph, binding))
			{
				...
				password = bcb.ShopifyApiPassword;
				accessToken = bcb.ShopifyAccessToken;
				sharedSecret = bcb.StoreSharedSecret;
				apiLimit = bcb.ApiCallLimit;
			}

			return SPConnector.GetRestClient(url, client, password, accessToken, sharedSecret, apiLimit);
		}
	}
	#endregion
}
