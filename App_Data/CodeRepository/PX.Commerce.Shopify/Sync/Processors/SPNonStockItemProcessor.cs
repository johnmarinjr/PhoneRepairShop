...
namespace PX.Commerce.Shopify
{
	...
	[BCProcessorExternCustomField(BCConstants.MetaFields, ShopifyCaptions.Metafields, nameof(ProductData.Metafields), typeof(ProductData), writeAsCollection: false)]
	[BCProcessorExternCustomField(BCConstants.MetaFields, ShopifyCaptions.Metafields, nameof(ProductVariantData.VariantMetafields), typeof(ProductVariantData), writeAsCollection: false)]
	public class SPNonStockItemProcessor : SPProductProcessor<SPNonStockItemProcessor, SPNonStockItemEntityBucket, MappedNonStockItem>, IProcessor
	{
		...
		{
			ProductData data = productDataProvider.GetByID(syncstatus.ExternID, includedMetafields: true);
		...
		{
			base.MapBucketExport(bucket, existing);
			...
			data.BodyHTML = helper.ClearHTMLContent(impl.Content?.Value);
			data.ProductType = impl.ItemClassDescription?.Value;
			data.Vendor = impl.VendorDetails?.FirstOrDefault(v => v.Default?.Value == true)?.VendorName?.Value;
			...
			SetProductStatus(data, impl.ItemStatus?.Value, availability, visibility);
			...
			{
				data.Metafields = new List<MetafieldData>() { new MetafieldData() { Key = ShopifyCaptions.Product, Value = impl.Id.Value.ToString(), Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global } };
				data.Metafields.Add(new MetafieldData() { Key = nameof(ProductTypes), Value = BCCaptions.NonStockItem, Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global });
			}

			data.Variants = data.Variants ?? new List<ProductVariantData>();
			bool isTaxable;bool.TryParse(helper.GetSubstituteLocalByExtern(GetBindingExt<BCBindingExt>().TaxCategorySubstitutionListID, impl.TaxCategory?.Value, String.Empty), out isTaxable);
			data.Variants.Add(new ProductVariantData()
			{
				...
				Price = impl.CurySpecificPrice.Value,
				Sku = impl.InventoryID?.Value,
				OriginalPrice = impl.CurySpecificMSRP?.Value != null && impl.CurySpecificMSRP?.Value > impl.CurySpecificPrice.Value ? impl.CurySpecificMSRP?.Value : null,
				Barcode = !string.IsNullOrWhiteSpace(impl.SalesUnit?.Value) ? ((impl.CrossReferences?.FirstOrDefault(x => x.AlternateType?.Value == BCCaptions.Barcode && x.UOM?.Value == impl.SalesUnit?.Value) ??
					impl.CrossReferences?.FirstOrDefault(x => x.AlternateType?.Value == BCCaptions.Barcode && string.IsNullOrWhiteSpace(x.UOM?.Value)))?.AlternateID?.Value) : null,
				...
				RequiresShipping = existingProduct?.Variants?.FirstOrDefault(x => x != null && string.Equals(impl.InventoryID?.Value, x?.Sku, StringComparison.OrdinalIgnoreCase)) == null ? impl.RequireShipment.Value ?? false : (bool?)null, 
				VariantMetafields = bucket.VariantMappings.ContainsKey(impl.InventoryID?.Value) ? null : 
					new List<MetafieldData>() { new MetafieldData() { Key = ShopifyConstants.Variant, Value = impl.Id.Value.ToString(), Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global } }
			});
		}
