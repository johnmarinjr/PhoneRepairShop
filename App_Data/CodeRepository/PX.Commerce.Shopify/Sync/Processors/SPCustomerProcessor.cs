...
namespace PX.Commerce.Shopify
{
	...
	public class SPCustomerProcessor : SPLocationBaseProcessor<SPCustomerProcessor, SPCustomerEntityBucket, MappedCustomer>, IProcessor
	{
		...
		public override string ValidateExternCustomField(BCEntity entity, EntityInfo entityInfo, ExternCustomFieldInfo customFieldInfo, string sourceObject, string sourceField, string targetObject, string targetField, EntityOperationType direction)
		{
			...
			return string.Format(BCMessages.InvalidFilter, direction == EntityOperationType.ImportMapping ? "Source" : "Target", ShopifyConstants.MetafieldFormat);
		}
			...
			if (uniqueField != null)
			{
				foreach (PX.Objects.AR.Customer item in helper.CustomerByEmail.Select(uniqueField))
				{
					Customer data = new Customer()
					{
						...
						SendInvoicesbyEmail = item.MailInvoices.ValueField(),
						SendDunningLettersbyEmail = item.MailDunningLetters.ValueField(),
						SendStatementsbyEmail = item.SendStatementByEmail.ValueField(),
						PrintDunningLetters = item.PrintDunningLetters.ValueField(),
		...
		public override IEnumerable<MappedCustomer> PullSimilar(ILocalEntity entity, out String uniqueField)
		{
			...
			else if (uniqueField == null && !string.IsNullOrWhiteSpace(((Customer)entity)?.MainContact?.Phone1?.Value))
			{
				...
				return null;
			IEnumerable<CustomerData> datas = customerDataProvider.GetByQuery(queryField, uniqueField, true);
		...
		public override void FetchBucketsForImport(DateTime? minDateTime, DateTime? maxDateTime, PXFilterRow[] filters)
		{
			...
			filter.Fields = BCRestHelper.PrepareFilterFields(typeof(CustomerData), filters);
		...
		public override void MapBucketImport(SPCustomerEntityBucket bucket, IMappedEntity existing)
		{
			...
			if (customerObj.Extern.Addresses?.Count() > 0)
			{
				...
				{
					addressImpl.State = helper.GetSubstituteLocalByExtern(BCSubstitute.GetValue(Operation.ConnectorType, BCSubstitute.State), addressObj.ProvinceCode, addressObj.ProvinceCode)?.ValueField();
				}
				else
		...
		public override object GetAttribute(SPCustomerEntityBucket bucket, string attributeID)
		{
			...
			return impl.Attributes?.Where(x => string.Equals(x?.AttributeDescription?.Value, attributeID, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
