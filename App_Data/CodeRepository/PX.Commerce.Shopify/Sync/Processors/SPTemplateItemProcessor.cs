...
namespace PX.Commerce.Shopify
{
	...
	[BCProcessorExternCustomField(BCConstants.MetaFields, ShopifyCaptions.Metafields, nameof(ProductVariantData.VariantMetafields), typeof(ProductVariantData), writeAsCollection: true)]
	...
	public class SPTemplateItemProcessor : SPProductProcessor<SPTemplateItemProcessor, SPTemplateItemEntityBucket, MappedTemplateItem>, IProcessor
	{
		...
		public override void MapBucketExport(SPTemplateItemEntityBucket bucket, IMappedEntity existing)
		{
			...
			else
			{
				...
				data.Metafields.Add(new MetafieldData() { Key = ShopifyCaptions.ProductId, Value = impl.InventoryID.Value, Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global });
				data.Metafields.Add(new MetafieldData() { Key = nameof(ProductTypes), Value = BCCaptions.TemplateItem, Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global });
			}
			...
			foreach (var item in obj.Local.Matrix.Where(x => IsVariantActive(x)).Take(ShopifyConstants.ProductVarantsLimit))
			{
				...
				variant.InventoryPolicy = notAvailable == BCCaptions.DisableItem ? InventoryPolicy.Deny : (notAvailable == BCCaptions.EnableSellingItem ? InventoryPolicy.Continue : (InventoryPolicy?)null);
				bool isTaxable;
				...
				else
				{
					...
					variant.RequiresShipping = existingProduct?.Variants?.FirstOrDefault(x => x != null && string.Equals(item.InventoryID?.Value, x?.Sku, StringComparison.OrdinalIgnoreCase)) == null ? impl.RequireShipment.Value ?? false : (bool?)null;
				}
				...
				if (variant.Id == null || variant.Id == 0)
					variant.VariantMetafields = new List<MetafieldData>() { new MetafieldData() { Key = ShopifyConstants.Variant, Value = item.Id.Value.ToString(), Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global } };
				variants.Add(variant);
			}
			if (variants.Count > 0)
			{
				data.Variants = variants.OrderBy(x => x.OptionSortOrder1).ThenBy(x => x.OptionSortOrder2).ThenBy(x => x.OptionSortOrder3).ToList();
			}
			else throw new PXException(ShopifyMessages.NoProductVariants, impl.InventoryID.Value);
