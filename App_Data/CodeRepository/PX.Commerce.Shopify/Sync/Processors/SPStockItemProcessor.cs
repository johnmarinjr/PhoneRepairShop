...
namespace PX.Commerce.Shopify
{
	...
	public class SPStockItemProcessor : SPProductProcessor<SPStockItemProcessor, SPStockItemEntityBucket, MappedStockItem>, IProcessor
	{
		...
		public override void MapBucketExport(SPStockItemEntityBucket bucket, IMappedEntity existing)
		{
			...
			string notAvailable = impl.NotAvailable?.Value;
			...
			{
				data.Metafields = new List<MetafieldData>() { new MetafieldData() { Key = ShopifyCaptions.Product, Value = impl.NoteID.Value.ToString(), Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global } };
				data.Metafields.Add(new MetafieldData() { Key = nameof(ProductTypes), Value = BCCaptions.StockItem, Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global });
			}
			
			data.Variants = data.Variants ?? new List<ProductVariantData>();
			bool isTaxable;
			bool.TryParse(helper.GetSubstituteLocalByExtern(GetBindingExt<BCBindingExt>().TaxCategorySubstitutionListID, impl.TaxCategory?.Value, String.Empty), out isTaxable);
			data.Variants.Add(new ProductVariantData()
			{
				...
				Barcode = !string.IsNullOrWhiteSpace(impl.SalesUOM?.Value) ? ((impl.CrossReferences?.FirstOrDefault(x => x.AlternateType?.Value == BCCaptions.Barcode && x.UOM?.Value == impl.SalesUOM?.Value) ??
				...
				InventoryPolicy = notAvailable == BCCaptions.DisableItem ? InventoryPolicy.Deny : (notAvailable == BCCaptions.EnableSellingItem ? InventoryPolicy.Continue : (InventoryPolicy?)null),
				VariantMetafields = bucket.VariantMappings.ContainsKey(impl.InventoryID?.Value) ? null :
					new List<MetafieldData>() { new MetafieldData() { Key = ShopifyConstants.Variant, Value = impl.NoteID.Value.ToString(), Type = ShopifyConstants.ValueType_SingleString, Namespace = BCObjectsConstants.Namespace_Global } }
			});
