...
namespace PX.Objects.AM.GraphExtensions
{
    ...
    public class ARSalesPriceMaintAMExtension : PXGraphExtension<ARSalesPriceMaint>
    {
		...
		[PXOverride]
		public virtual decimal? AdjustSalesPrice(PXCache sender, ARSalesPriceMaint.SalesPriceItem spItem, int? inventoryID, CurrencyInfo currencyinfo, string uom,
			Func<PXCache, ARSalesPriceMaint.SalesPriceItem, int?, CurrencyInfo, string, decimal?> del)
		{
			decimal? configPrice = null;
            if (!UseBaseCall && TryCalculateConfiguredPrice(sender, inventoryID, currencyinfo, uom, out configPrice))
            {
			...
			return del?.Invoke(sender, spItem, inventoryID, currencyinfo, uom);
		}
