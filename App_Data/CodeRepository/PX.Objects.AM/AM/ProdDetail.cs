...
namespace PX.Objects.AM
{
    public class ProdDetail : PXGraph<ProdDetail, AMProdItem>
    {
        ...
        public INLotSerClass matlLotSerialClass => MatlLineSplittingExt.GetLotSerClass(ProdMatlRecords.Current);
		...
		public class ItemLineSplittingExtension : AMProdItemLineSplittingExtension<ProdDetail> { }
        public ItemLineSplittingExtension ItemLineSplittingExt => FindImplementation<ItemLineSplittingExtension>();

        // Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
        public class ItemAvailabilityExtension : AMProdItemAvailabilityExtension<ProdDetail> { }

        // Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
        public class ItemAvailabilityAllocatedExtension : AMProdItemAvailabilityAllocatedExtension<ProdDetail, ItemAvailabilityExtension> { }
        #endregion

        #region Matl Splitting+Availability Extensions - former LSAMProdMatl lsSelectMatl view
        // Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
        public class MatlLineSplittingExtension : AMProdMatlLineSplittingExtension<ProdDetail> { }
        public MatlLineSplittingExtension MatlLineSplittingExt => FindImplementation<MatlLineSplittingExtension>();

        // Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
        public class MatlItemAvailabilityExtension : AMProdMatlItemAvailabilityExtension<ProdDetail> { }

        // Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
        [PXProtectedAccess(typeof(MatlItemAvailabilityExtension))]
        public abstract class MatlItemAvailabilityAllocatedExtension : AMProdMatlItemAvailabilityAllocatedExtension<ProdDetail, MatlItemAvailabilityExtension> { }
        #endregion
        ...
        [PXDefault]
        [PXFieldDescription]
		protected virtual void _(Events.CacheAttached<AMProdItem.prodOrdID> e) { }
		...
		protected virtual void _(Events.CacheAttached<AMProdStep.sortOrder> e) { }
		...
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXFormula(typeof(Default<AMProdOper.wcID>))]
		[PXDefault(false, typeof(Search<AMWC.controlPoint, Where<AMWC.wcID, Equal<Current<AMProdOper.wcID>>>>))]
		protected virtual void _(Events.CacheAttached<AMProdOper.controlPoint> e) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXFormula(typeof(Selector<AMProdOper.wcID, AMWC.defaultMoveTime>))]
		protected virtual void _(Events.CacheAttached<AMProdOper.moveTime> e) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXFormula(typeof(Selector<AMProdOper.wcID, AMWC.defaultFinishTime>))]
		protected virtual void _(Events.CacheAttached<AMProdOper.finishTime> e) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXFormula(typeof(Selector<AMProdOper.wcID, AMWC.defaultQueueTime>))]
		...
		public override void Persist()
        {
            ...
            if (currentProdItem != null && reschedule && !ProdItemRecords.Cache.IsRowDeleted(currentProdItem))
            {
				...
				IEnumerable<AMProdOper> prodOperList = this.ProdOperRecords.Cache.Deleted.RowCast<AMProdOper>();
				foreach (var prodOperItem in prodOperList)
				{
					DeleteSchdOperDetail(prodOperItem);
				}

				var firstLastOperations = GetFirstLastOperationIds(operations);
				var lastOperChange = currentProdItem.LastOperationID != firstLastOperations?.Item2;
                currentProdItem.FirstOperationID = firstLastOperations?.Item1;
                currentProdItem.LastOperationID = firstLastOperations?.Item2;

				if (lastOperChange && currentProdItem.LastOperationID != null)
				{
        ...
        {
            var currentOrder = ProdItemRecords?.Current;
        ...
        public PXAction<AMProdItem> CreateVendorShipment;
        [PXButton]
        [PXUIField(DisplayName = Messages.CreateVendorShipment)]
        public virtual IEnumerable createVendorShipment(PXAdapter adapter)
        {
            if (this.ProdItemRecords.Current != null)
            {
                var vendorShipGraph = PXGraph.CreateInstance<VendorShipmentEntry>();
                vendorShipGraph.Clear();

                AMVendorShipment vendorShipment = new AMVendorShipment
                {
                    ShipmentType = AMShipType.Shipment,
                    VendorID = this.ProdOperSelected.Current.VendorID,
                    VendorLocationID = this.ProdOperSelected.Current.VendorLocationID,
                    SiteID = this.ProdItemRecords.Current.SiteID
                };

                vendorShipGraph.Document.Cache.Insert(vendorShipment);

                vendorShipGraph.AddShipmentLines(this.ProdItemRecords.Current, this.ProdOperSelected.Current);

                if (vendorShipGraph != null)
                {
                    PXRedirectHelper.TryRedirect(vendorShipGraph, PXRedirectHelper.WindowMode.New);
                }
            }
            return adapter.Get();
        }
        ...
        {
			if(ProdItemRecords.Current?.OrderType == null)
			{
				return adapter.Get(); 
			}
			...
			{
				OrderType = ProdItemRecords.Current.OrderType,
				ProdOrdId = ProdItemRecords.Current.ProdOrdID,
				IncludeOnHold = true
			});
        ...
        protected bool IsApiEnabled
        {
            get
            {
                ...
                var prodItem = (AMProdItem)ProdItemRecords.Cache.Current;
                return ((IsImport || IsContractBasedAPI) && (prodItem.StatusID == ProductionOrderStatus.Released || prodItem.StatusID == ProductionOrderStatus.InProcess));
            }
        }
        ...
        public ProdDetail()
        {
			...
			InquiresDropMenu.AddMenuAction(ProductionScheduleBoardRedirect);
			InquiresDropMenu.AddMenuAction(LateAssignmentEntry);

			PXUIFieldAttribute.SetVisible<AMProdItem.branchID>(ProdItemRecords.Cache, null, true);
		}
        ...
        protected virtual void AMProdItem_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
			...
			ProductionScheduleBoardRedirect.SetEnabled(isOpen && amProdItem.StatusID != ProductionOrderStatus.Completed && amProdItem.Function.GetValueOrDefault() == OrderTypeFunction.Regular);
			LateAssignmentEntry.SetEnabled(amProdItem.Function == OrderTypeFunction.Regular);
            ...
            CreateVendorShipment.SetEnabled((amProdItem.StatusID == ProductionOrderStatus.Released || amProdItem.StatusID == ProductionOrderStatus.InProcess ||
                amProdItem.StatusID == ProductionOrderStatus.Completed) && amProdItem.Hold == false);
        }
        ...
        protected virtual void AMProdOper_OperationCD_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
        {
			...
			var result = cache.Cached.RowCast<AMProdOper>()
				.Where(r => r.OrderType == prodOper.OrderType && r.ProdOrdID == prodOper.ProdOrdID && r.OperationCD == e.NewValue.ToString()
				&& prodOper.OperationID != r.OperationID
				&& cache.GetStatus(r) != PXEntryStatus.Deleted).FirstOrDefault();

			if (!string.IsNullOrEmpty(result?.ProdOrdID))
			{
				var errorMessage = Messages.GetLocal(Messages.DuplicateOperationID, e.NewValue);
				e.NewValue = prodOper.OperationCD;
				e.Cancel = true;
				throw new PXSetPropertyException<AMEstimateOper.operationCD>(errorMessage);
			}
        ...
        protected virtual void _(Events.RowSelected<AMProdOper> e)
        {
			...
			if (!IsImport && !IsContractBasedAPI && ProdItemRecords.Current.LastOperationID == ProdOperRecords?.Current?.OperationID)
			{
                PXUIFieldAttribute.SetEnabled<AMProdOper.controlPoint>(e.Cache, e.Row, false);
			}            
		}
        ...
        protected virtual void _(Events.FieldUpdating<AMProdMatl, AMProdMatl.bFlush> e)
        {
            var newValue = (bool)e.NewValue;
            var prodItem = ProdItemRecords.Current;
            if (newValue == true && prodItem != null)
            {
                if (!(prodItem.Function != OrderTypeFunction.Regular ||
                    ...
                    (prodItem.ParentLotSerialRequired == ParentLotSerialAssignment.OnIssue && 
                        prodItemSplits.Select().Count == 1 && 
                        !string.IsNullOrEmpty(prodItemSplits.Current?.LotSerialNbr)) ||
                        matlLotSerialClass.LotSerTrack == INLotSerTrack.NotNumbered))
                {
                    throw new PXSetPropertyException(Messages.BackflushMaterialNotAllowedWithPreassignLotSerial, prodItem.OrderType, prodItem.ProdOrdID);
                }
            }
        ...
        protected virtual void AMProdMatl_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            ...
            if (RowIsEnableRestricted(e.Row))
            {
                ...
                PXUIFieldAttribute.SetEnabled<AMProdMatl.qtyReq>(cache, null, OrderOnHold || IsApiEnabled);
                PXUIFieldAttribute.SetEnabled<AMProdMatl.descr>(cache, null, OrderOnHold || IsApiEnabled);
                PXUIFieldAttribute.SetEnabled<AMProdMatl.bFlush>(cache, null, (OrderOnHold || IsApiEnabled) && !isDisassembly);
                return;
            }
        ...
        protected virtual void AMProdTool_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            if (RowIsEnableRestricted(e.Row))
            {
                ...
                //Then enable a select few if order is on hold...
                PXUIFieldAttribute.SetEnabled<AMProdTool.qtyReq>(cache, null, OrderOnHold || IsApiEnabled);
            }
        }
        ...
        protected virtual void AMProdOvhd_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            if (RowIsEnableRestricted(e.Row))
            {
                ...
                //Then enable a select few if order is on hold...
                PXUIFieldAttribute.SetEnabled<AMProdOvhd.oFactor>(cache, null, OrderOnHold || IsApiEnabled);
            }
        }
        #endregion
        ...
        public static void ProcessPOCreate(ProdDetail pdGraph, List<AMProdMatl> list, int? vendorID, int? vendorLocationID)
        {
            ...
            POCreateAMExtension.POCreatePOOrders(poGraph, demandList, purchaseDate);
        }
