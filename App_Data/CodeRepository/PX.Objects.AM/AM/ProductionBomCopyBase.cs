...
namespace PX.Objects.AM
{
    public abstract class ProductionBomCopyBase : PXGraph<ProductionBomCopyBase, AMProdItem>
    {
		...
		internal AMProdOper FindCachedOperByCD(AMProdOper operation)
		{
			return FindCachedOperByCD(operation.OrderType, operation.ProdOrdID, operation.OperationCD);
		}
		...
		internal AMProdOper FindCachedOperByCD(string orderType, string prodOrdID, string operationCD)
		{
			return ProcessingGraph.Caches<AMProdOper>()?.Cached.RowCast<AMProdOper>()
				?.Where(r => r.OrderType == orderType && r.ProdOrdID == prodOrdID && r.OperationCD == operationCD)
				?.FirstOrDefault();
		}
