...
namespace PX.Objects.AM
{
    ...
    public class MoveOperationQtyBuilder
    {
		...
		private bool TrySetControlPoints(AMMTran tran, MoveOperationQtyTotals opTotals)
		{
			// Need sorted ascending
            var operationResults = GetOperations(tran);
			...
			var previousControlPointOperationID = 0;
			var prevNextControlPointID = 0;

			var lastOperationID = operationResults.Last()?.GetItem<AMProdOper>()?.OperationID ?? 0;

			foreach (PXResult<AMProdOper, AMWC> operResult in operationResults)
			{
				var oper = operResult.GetItem<AMProdOper>();
				var opTotal = opTotals.GetSetOperationTotal(operResult); 

				if(previousControlPointOperationID != 0)
				{					
					opTotal.PriorControlPointOperationID = previousControlPointOperationID;
					opTotals.UpdateOperation(opTotal);
				}
				if(oper.ControlPoint.GetValueOrDefault() && oper.OperationID != lastOperationID)
				{
					controlPointFound = true;
					previousControlPointOperationID = oper.OperationID.GetValueOrDefault();
				}

				if ((prevNextControlPointID == 0 || prevNextControlPointID == oper.OperationID) && oper.OperationID != lastOperationID)
				{
					prevNextControlPointID = GetNextControlPointID(oper, operationResults);
				}
				if (prevNextControlPointID != 0 && oper.OperationID != lastOperationID)
				{
					opTotal.NextControlPointOperationID = prevNextControlPointID;
                    opTotals.UpdateOperation(opTotal);
				}
			}
		...
		private int GetNextControlPointID(AMProdOper oper, List<PXResult<AMProdOper, AMWC>> operationResults)
		{
			var found = false;

			foreach (var item in operationResults)
			{
                var newOper = item.GetItem<AMProdOper>();
				if (!found && oper.OperationID == newOper.OperationID)
				{
					found = true;
					continue;
				}
				if (found && newOper.ControlPoint.GetValueOrDefault())
				{
					return newOper.OperationID.GetValueOrDefault();
				}
			}
			return 0;
		}
        ...
        private void PostProcessAllTranRows()
        {
            foreach (PXResult<AMMTran, InventoryItem, AMOrderType, AMProdItem, AMProdOper> result in AllTranRows)
            {
				...
				if(TrySetControlPoints(ammTran, opTotals))
				{
					opTotals.HasControlPoints = true;
				}
