...
namespace PX.Objects.AM
{
    public class BOMCostRoll : PXGraph<BOMCostRoll>
    {
		...
		{
			var ignoreSource = Settings.Current?.IgnoreReplenishmentSettings == true;
			...
			var hasBom = !string.IsNullOrWhiteSpace(inSiteExtension?.AMBOMID ?? itemExtension?.AMBOMID);
			if(hasBom && ignoreSource)
			{
				return INReplenishmentSource.Manufactured;
			}

			var repSource = (inItemSite?.ReplenishmentSource ?? itemExtension?.AMReplenishmentSource) ?? INReplenishmentSource.None;
        ...
        protected virtual OperationCosts GetMaterialCost(AMBomCost currentAMBomCost, IEnumerable<PXResult<AMBomMatl, InventoryItem>> material, bool isMultLevel, out List<string> materialMessage)
        {
            ...
            foreach(var result in material)
            {
                ...
                if (isMultLevel)
                {
					...
					var itemSite = InventoryHelper.CacheQueryINItemSite(ItemSiteRecs.Cache, matlRec.InventoryID, matlSiteID);
                    ...
                    var bomItem = PrimaryBomIDManager.GetNotArchivedRevisionBomItem(this,
						bomManager.GetPrimaryAllLevels(inventoryItem, itemSite, matlRec.SubItemID));
