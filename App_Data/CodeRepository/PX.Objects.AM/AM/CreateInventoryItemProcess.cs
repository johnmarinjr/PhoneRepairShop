...
namespace PX.Objects.AM
{
    ...
    public class CreateInventoryItemProcess : PXGraph<CreateInventoryItemProcess>
    {
		...
		public PXSetup<AMEstimateSetup> Setup;
...
#pragma warning restore PX1088
        ...
        [PXDefault]
		protected virtual void _(Events.CacheAttached<AMEstimateItem.estimateID> e) { }
		...
		protected virtual void _(Events.CacheAttached<AMEstimateItem.revisionID> e) { }

		protected virtual void _(Events.FieldVerifying<NonInventoryItem, NonInventoryItem.inventoryCD> e)
        {
                ...
                !string.IsNullOrWhiteSpace(Filter.Current.RevisionID))
            {
                ...
                if (estimateItem != null)
                {
                    ...
                    NonInventoryItem record = new NonInventoryItem
                    {
						...
						MarkupPct = estimateItem.MarkupPct,
						BasePrice = estimateItem.UnitPrice,
		...
		[Obsolete("This method has been deprecated and will be removed in Acumatica ERP 2023R1. Use CreateInventoryItem with InventoryItemMaintBase graph")]
		protected virtual InventoryItem CreateInventoryItem(PXGraph graph, NonInventoryItem row)
		{
			throw new NotImplementedException();
		}

		protected virtual InventoryItem CreateInventoryItem(InventoryItemMaintBase graph, NonInventoryItem row)
        {
            try
            {
                var inventoryItem = graph.Item.Insert(new InventoryItem
                {
				...
				if (row.Level == NonInventoryItem.NonInventoryLevel.Estimate && Setup.Current?.UpdatePriceInfo == true)
				{
					inventoryItem.MarkupPct = row.MarkupPct;
					var curySettings = graph.GetCurySettings(inventoryItem.InventoryID);
					if (curySettings != null)
					{
						curySettings.BasePrice = row.BasePrice;
						graph.ItemCurySettings.Update(curySettings);
					}
				}

				inventoryItem = graph.Item.Update(inventoryItem);

                CopyNotesAndFiles(row, graph.Item.Cache, inventoryItem);
