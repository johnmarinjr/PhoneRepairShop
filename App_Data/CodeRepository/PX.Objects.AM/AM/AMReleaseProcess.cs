...
namespace PX.Objects.AM
{
    ...
    public class AMReleaseProcess : PXGraph<AMReleaseProcess>
    {
                    ...
                    && (ammTran.Qty.GetValueOrDefault() != 0 || ammTran.QtyScrapped.GetValueOrDefault() != 0))
                {
					...
					if(isPreassignedTransaction)
					{
						...
						foreach (PXResult<AMMTranSplitProdMatlLotSerial, AMProdItemSplit> split in splits)
						{
							...
							var itemSplit = (AMProdItemSplit)split;
							if (ammTran.IsScrap == true)
							{
								itemSplit.QtyScrapped += tranSplit.Qty;
								itemSplit.BaseQtyScrapped += tranSplit.BaseQty;
							}
							else
							{
								itemSplit.QtyComplete += tranSplit.Qty;
								itemSplit.BaseQtyComplete += tranSplit.BaseQty;
							}
					...
					else
					{
						...
						if (splitRecord != null)
						{
							...
							if(ammTran.IsScrap != true)
							{
								splitRecord.QtyComplete += ammTran.Qty;
		...
		protected virtual void CheckControlPointQty(MoveOperationQtyBuilder builder)
		{
			...
			{
				var prodItem = result.GetItem<AMProdItem>();
				if (prodItem == null || !orderHash.Add(string.Join(":", prodItem.OrderType, prodItem.ProdOrdID)))
				{
					continue;
				}
		...
		protected virtual void StoreLotSerialRelationship(AMBatch doc)
        {
					...
					.And<AMMTranSplitProdMatlLotSerial.batNbr.IsEqual<@P.AsString>>>.View.ReadOnly.Select(this, doc.DocType, doc.BatNbr))
			{
				...
				var prodItemSplit = prodItemSplits.Cache.LocateElse(result.GetItem<AMProdItemSplit>());
			...
			foreach (AMProdItemSplit item in aMProdItems)
			{
				if (item?.LotSerialNbr != null)
				{
					results = PXSelectGroupBy<AMProdMatlLotSerial
									, Where<AMProdItemSplit.orderType, Equal<Required<AMProdMatlLotSerial.orderType>>
									, And<AMProdItemSplit.prodOrdID, Equal<Required<AMProdMatlLotSerial.prodOrdID>>
									, And<AMProdItemSplit.lotSerialNbr, Equal<Required<AMProdMatlLotSerial.parentLotSerialNbr>>>>>
