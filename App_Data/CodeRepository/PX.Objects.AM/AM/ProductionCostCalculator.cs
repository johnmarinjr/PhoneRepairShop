    ...
    public class ProductionCostCalculator
    {
            ...
            decimal additionalWip, decimal scrapQty, out decimal operExpectedCost)
        {
            ...
            operExpectedCost =
				...
				amProdOper.ScrapAmount.GetValueOrDefault();
        ...
        public static PlannedCosts GetPlannedLaborCost(PXGraph graph, AMProdOper amProdOper, decimal? productionQty, decimal? remainingProductionQty, decimal? totalHours, int? laborTime, decimal? laborCost)
        {
            ...
            var shiftCode = (EPShiftCode)PXSelectJoin<EPShiftCode,
                                InnerJoin<AMShift, On<EPShiftCode.shiftCD, Equal<AMShift.shiftCD>>>,
                                Where<AMShift.wcID, Equal<Required<AMShift.wcID>>>,
                                    OrderBy<Asc<AMShift.shiftCD>>>.SelectWindowed(graph, 0, 1, wc?.WcID);
            return GetPlannedLaborCost(graph, wc, shiftCode, amProdOper, productionQty, remainingProductionQty, totalHours, UomHelper.CostDecimalPrecision, laborTime, laborCost);
        }

        /// <summary>
        /// Get the planned labor unit and total cost for the given operation
        /// </summary>
        public static PlannedCosts GetPlannedLaborCost(PXGraph graph, AMWC wc, EPShiftCode shiftCode, AMProdOper amProdOper, decimal? productionQty, decimal? remainingProductionQty)
        {
            return GetPlannedLaborCost(graph, wc, shiftCode, amProdOper, productionQty, remainingProductionQty, null, UomHelper.CostDecimalPrecision);
        }

        /// <summary>
        /// Get the planned labor unit and total cost for the given operation
        /// </summary>
        public static PlannedCosts GetPlannedLaborCost(PXGraph graph, AMWC wc, EPShiftCode shiftCode, AMProdOper amProdOper, decimal? productionQty, decimal? remainingProductionQty, decimal? totalHours, int decimalPrecision)
        {
            return GetPlannedLaborCost(graph, wc, shiftCode, amProdOper, productionQty, remainingProductionQty, totalHours, decimalPrecision, amProdOper?.ActualLaborTime, amProdOper?.ActualLabor);
        }

        /// <summary>
        /// Get the planned labor unit and total cost for the given operation
        /// </summary>
        public static PlannedCosts GetPlannedLaborCost(PXGraph graph, AMWC wc, EPShiftCode shiftCode, AMProdOper amProdOper, decimal? productionQty, decimal? remainingProductionQty, decimal? totalHours, int decimalPrecision, int? laborTime, decimal? laborCost)
        {
            ...
            var planLaborRate = ShiftDiffType.GetShiftDifferentialCost(graph, wc, shiftCode);
