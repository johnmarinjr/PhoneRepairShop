    ...
    public class VendorShipmentEntry : PXGraph<VendorShipmentEntry, AMVendorShipment>, ICaptionable
    {
        ...
        public PXSelectJoin<AMProdOper,
            InnerJoin<AMProdItem,
                 On<AMProdOper.orderType,
                    Equal<AMProdItem.orderType>,
                 And<AMProdOper.prodOrdID,
                    Equal<AMProdItem.prodOrdID>>>>,
            Where<AMProdOper.vendorID, Equal<Current<AMVendorShipment.vendorID>>,
                And<AMProdOper.qtyComplete, Less<AMProdOper.totalQty>,
                And<AMProdItem.hold, Equal<False>,
                And<AMProdOper.qtytoProd, Greater<AMProdOper.shippedQuantity>,
                And<Where<AMProdItem.statusID, Equal<ProductionOrderStatus.released>,
                    ...
                    Or<AMProdItem.statusID, Equal<ProductionOrderStatus.completed>>>>>>>>>> ShipmentProdOrders;
		...
		[PXSelector(typeof(SearchFor<INLotSerialStatus.lotSerialNbr>
			.In<SelectFrom<INLotSerialStatus>
				.LeftJoin<AMProdItem>.On<AMProdItem.inventoryID.IsEqual<INLotSerialStatus.inventoryID>>
				...
				.Where<INLotSerialStatus.inventoryID.IsEqual<AMVendorShipLine.inventoryID.FromCurrent>
					.And<Brackets
							<Brackets<AMProdItem.orderType.IsEqual<AMVendorShipLine.orderType.FromCurrent>.And<AMProdItem.prodOrdID.IsEqual<AMVendorShipLine.prodOrdID.FromCurrent>>>
								.And<Brackets<AMProdItem.preassignLotSerial.IsEqual<boolTrue>.And<AMProdItemSplit.lotSerialNbr.IsEqual<INLotSerialStatus.lotSerialNbr>>>
									.Or<INLotSerialStatus.qtyOnHand.IsGreater<decimal0>.And<AMProdItem.preassignLotSerial.IsEqual<boolFalse>>>>>
						.Or<Brackets<AMVendorShipLine.lineType.FromCurrent.IsEqual<AMShipLineType.material>.And<INLotSerialStatus.qtyOnHand.IsGreater<decimal0>>>>>>>))]
		...
		public override void Persist()
		{
			using (var ts = new PXTransactionScope())
			{
				...
				else if (Transactions.Cache.Deleted.Any_())
				{
					...
					var rm = PXGraph.CreateInstance<MaterialEntry>();
					//if batch existed and not released, delete the lines and run normally, otherwise set the shipment and lines to released
					foreach (AMMTran deletedShipLine in Transactions.Cache.Deleted)
					{
						AMMTran tran = PXSelect<AMMTran, Where<AMMTran.origDocType, Equal<Required<AMMTran.origDocType>>,
							...
							.Select(this, AMDocType.VendorShipment, deletedShipLine.BatNbr, deletedShipLine.LineNbr);
