...
namespace PX.Objects.AM
{
		...
		And<AMProdItemSplit.prodOrdID, Equal<Current<AMProdItem.prodOrdID>>>>>
	{
		...
		{
			this.MasterQtyField = typeof(AMProdItem.qtytoProd);
			...
			graph.FieldVerifying.AddHandler<AMProdItemSplit.qty>(new PXFieldVerifying(this.AMProdItemSplit_Qty_FieldVerifying));
			graph.FieldUpdated.AddHandler<AMProdItemSplit.lotSerialNbr>(new PXFieldUpdated(this.AMProdItemSplit_LotSerialNbr_FieldUpdated));
		}
		...
		/* 
		Base: Set the available field to not visible
		Override: do nothing		 
		 */
		...
		/*
		Base: If master record has lotsernbr call DefaultLotSerialNbr, then AvailablityFetch
		Override: If master record has lotsernbr call DefaultLotSerialNbr, then if fetchmode is ExcludeCurrent call
			custom AvailabilityFetch, otherwise call normal AvailabilityFetch
		*/
		...
		/*
		AvailabilityFetch - This method calls the standard AvailabilityFetch and then it loops through the
			detail records and subtracts the detail baseqty from qtyavail on the master record
		 */
		private IStatus AvailabilityFetch(PXCache sender, AMProdItem line, AMProdItemSplit detail)
		{
		...
		/*
		Base: If item is lotserial tracked, set Qty to 1
		Override: Call base.VerifySNQuantity instead, this method is normally used to set the qty on the master
			record but here it is being used to set the qty on a detail record.
		 */
		...
		/*Convert - This method takes in an amproditem record and returns an amproditemsplit, the qty
			being the qty left to be assigned
		*/
		...
		/*
		Base: Return the lotserial tracking mode for an inventory item
		Override: Treat all items like non lot tracked items
		*/
		...
		protected override void Master_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			...
			if(((AMProdItem)e.Row)?.PreassignLotSerial == true)
			{
				return;
			}
...
#pragma warning restore PX1048
		...
		{
			var isPreassigned = ((AMProdItem)e.Row)?.PreassignLotSerial == true;
			var row = (AMProdItem)e.Row;
			var isLotTracked = false;
			if(isPreassigned)
			{
				var result = (INLotSerClass)base.ReadInventoryItem(sender, row.InventoryID);
				isLotTracked = result != null && result.LotSerTrack == INLotSerTrack.LotNumbered;
				if(isLotTracked && !string.IsNullOrWhiteSpace(row?.LotSerialNbr))
				{
					// Trigger reset of LotSerialNbr in base
					DetailCounters[row] = new Counters { UnassignedNumber = 1 };
				}
			}
			...
			if (!isPreassigned)
			{
				...
				DetailCounters[row] = new Counters { UnassignedNumber = 0 }; 
			}

			base.Master_RowPersisting(sender, e);

			if(isPreassigned && isLotTracked && string.IsNullOrWhiteSpace(row.LotSerialNbr))
			{
				// User changes lot number used on order...
				foreach (AMProdItemSplit detail in SelectDetail(sender.Graph.Caches[typeof(AMProdItemSplit)], row))
				{
					...
					row.LotSerialNbr = detail.LotSerialNbr;
				}
			}
		}
		...
		public INLotSerClass GetLotSerClass(PXCache sender, ILSMaster line)
		{
			return ReadInventoryItem(sender, line.InventoryID);
		}
		...
		{
			var parent = (AMProdItem)LSParentAttribute.SelectParent(sender, Row ?? OldRow, typeof(AMProdItem));
			if (parent != null)
			{
				if ((Row ?? OldRow) != null && SameInventoryItem((ILSMaster)(Row ?? OldRow), (ILSMaster)parent))
				{
					...
					using (InvtMultScope<AMProdItem> ms = new InvtMultScope<AMProdItem>(parent))
					{
						if (Row != null)
						{
							...
							{
								var hasLotSerialNbrQty = 0m;
								foreach (AMProdItemSplit split in SelectDetail(sender, Row))
								{
									if (split.LotSerialNbr != null)
									{
										hasLotSerialNbrQty += split.BaseQty ?? 0m;
									}
								}
		...
		protected override void LotSerOptions_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			AMProdItem item = (AMProdItem)sender.Graph.Caches[typeof(AMProdItem)].Current;

			bool enableGenerate = (item != null && item.PreassignLotSerial == true);

			PXUIFieldAttribute.SetEnabled<LotSerOptions.startNumVal>(sender, opt, enableGenerate);
			PXUIFieldAttribute.SetEnabled<LotSerOptions.qty>(sender, opt, enableGenerate);
			PXDBDecimalAttribute.SetPrecision(sender, opt, "Qty", (opt.IsSerial == true ? 0 : CommonSetupDecPl.Qty));
			_Graph.Actions[Prefixed("generateLotSerial")].SetEnabled(enableGenerate);
		}

		public override void UpdateNumbers(PXCache sender, object Row)
		{
			PXCache cache = sender.Graph.Caches[typeof(AMProdItemSplit)];

			if (Row is AMProdItem)
				DetailCounters.Remove((AMProdItem)Row);
			foreach (object detail in SelectDetail(cache, (AMProdItem)Row))
			{
				AMProdItemSplit newdetail = PXCache<AMProdItemSplit>.CreateCopy((AMProdItemSplit)detail);

				if (((AMProdItem)Row).LocationID == null && newdetail.LocationID != null && cache.GetStatus(newdetail) == PXEntryStatus.Inserted && newdetail.Qty == 0m)
				{
					cache.Delete(newdetail);
				}
				else
				{
					newdetail.SubItemID = ((AMProdItem)Row).SubItemID ?? newdetail.SubItemID;
					newdetail.SiteID = ((AMProdItem)Row).SiteID;
					newdetail.ExpireDate = ExpireDateByLot(sender, newdetail, (ILSMaster)Row);
					newdetail.LocationID = ((AMProdItem)Row).LocationID;
		...
		protected virtual void AMProdItemSplit_LotSerialNbr_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			var row = (AMProdItemSplit)e.Row;
			...
			AMProdItem parent = (AMProdItem)PXParentAttribute.SelectParent(cache, row, typeof(AMProdItem));
			if(parent?.InventoryID == null)
			{
				return;
			}

			if(cache.Graph.Caches[typeof(AMProdItem)].GetStatus(parent) != PXEntryStatus.Inserted)
			{
				return;
			}

			var oldLotSerialNbr = (string)e.OldValue;
			if(!string.IsNullOrWhiteSpace(oldLotSerialNbr) && parent.LotSerialNbr == oldLotSerialNbr)
			{
				parent.LotSerialNbr = row.LotSerialNbr;
			}
		}

		public void AMProdItemSplit_Qty_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			AMProdItemSplit split = (AMProdItemSplit)e.Row;
			AMProdItem parent = (AMProdItem)PXParentAttribute.SelectParent(sender, split, typeof(AMProdItem));

			if(parent != null && parent.PreassignLotSerial == true)
			{
				PXResult<InventoryItem, INLotSerClass> result = base.ReadInventoryItem(sender, ((AMProdItemSplit)e.Row).InventoryID);
