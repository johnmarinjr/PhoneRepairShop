...
namespace PX.Objects.AM
{
    public class ProductionTransactionHelper
    {
            ...
            bool orderUpdateOnly, bool reverseCost)
        {
            ...
            var wcResult = (PXResult<AMWC, AMShift, EPShiftCode, AMLaborCode>)PXSelectJoin<AMWC,
                InnerJoin<AMShift, On<AMWC.wcID, Equal<AMShift.wcID>>,
                    InnerJoin<EPShiftCode, On<AMShift.shiftCD, Equal<EPShiftCode.shiftCD>>,
            ...
            EPShiftCode shiftCode = wcResult;
            ...
            var wcCost = ShiftDiffType.GetShiftDifferentialCost(AmReleaseGraph, amwc, shiftCode);
		...
		public static void CheckControlPointQuantity(AMMTran tran, MoveOperationQtyTotals operationTotals, out string errorMessage)
		{
			...
			if ((tranTotal.ProdOper.QtyComplete - controlPointOper.ProdOper.QtyComplete) > Math.Abs((sbyte)controlPointOper.CurrentMoveBaseQty)) 
			{
				return;
			}
		...
		public partial class AMMTranCostTotals : IBqlTable, IProdCostTran
		{
			...
			protected Decimal? _BaseQty;
			[PXDecimal()]
			[PXDBCalced(typeof(Mult<Abs<AMMTran.baseQty>, AMMTran.invtMult>), typeof(Decimal))]
			public virtual Decimal? BaseQty
			{
				get
				{
        ...
        public static string PeriodFromDate(PXGraph graph, DateTime? date)
        {
            ...
            var finPeriod = new PX.Objects.GL.FinPeriods.FinPeriodRepository(graph).GetFinPeriodByDate(date,
                orgID ?? PX.Objects.GL.FinPeriods.TableDefinition.FinPeriod.organizationID.MasterValue);
