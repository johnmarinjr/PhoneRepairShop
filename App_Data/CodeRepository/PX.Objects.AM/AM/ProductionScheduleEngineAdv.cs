    ...
    public class ProductionScheduleEngineAdv : ProductionScheduleEngine
    {
        ...
        public virtual void UpdateProdItem(AMSchdItem schdItem)
        {
                ...
                || amProdItem.EndDate.GetValueOrDefault().CompareTo(schdItem.EndDate.GetValueOrDefault()) != 0)
            {
                ...
                amProdItem.ScheduleStatus = schdItem.ScheduleStatus;
            }
		...
		{
			Process(schdItem, false);
		}

        public virtual void Process(AMSchdItem schdItem, bool firm)
        {
            ...
            if(schdItem2 != null)
            {
				if(firm)
				{
					// Let firm process run to sync and for better customization support
					FirmOrder(SchdItems.Update(schdItem2));
					return;
				}
        ...
        private AMSchdItem ProcessSchdItem(AMSchdItem schdItem, PXResultset<AMSchdOper> orderSchdResults, bool scheduleBack, ref DateTime currentSchedDate)
        {
            ...
            foreach (PXResult<AMSchdOper, AMWC, AMProdOper> result in orderSchdResults)
            {
                ...
                if (schdItem.StartDate == null || schdOper.QueueStartDate < schdItem.StartDate)
                {
                    schdItem.StartDate = schdOper.QueueStartDate;
                }
                if (schdItem.EndDate == null || schdOper.MoveEndDate > schdItem.EndDate)
                {
                    schdItem.EndDate = schdOper.MoveEndDate;
                }

				SetWCSchdDetailFloatTime<AMWCSchdDetail.queueTime>(schdOper, schdOper.QueueTime);
				SetWCSchdDetailFloatTime<AMWCSchdDetail.moveTime>(schdOper, schdOper.MoveTime);
            }

            if (schdItem.StartDate == null) { schdItem.StartDate = currentSchedDate; }
            if (schdItem.EndDate == null) { schdItem.EndDate = currentSchedDate; }
            if (schdItem.ScheduleStatus == null) 
            { 
                schdItem.ScheduleStatus = schdItem.FirmSchedule == true ? ProductionScheduleStatus.Firm : ProductionScheduleStatus.Scheduled; 
            }
            ...
            ref DateTime currentSchedDateTime)
        {
...
#if DEBUG
            AMDebug.TraceWriteLine($"----- START Operation {prodOper.OperationCD} ({schdOper.OperationID}) order {schdOper.OrderType}:{schdOper.ProdOrdID} -------------------------------------------------------------");
#endif
			...
			schdOper.QueueStartDate = null;
            schdOper.StartDate = null;
            schdOper.EndDate = null;
			schdOper.MoveEndDate = null;
			schdOper.StatusID = prodOper.StatusID;

			//Calculate the remaining finish time if the current operation is completed
			if(schdOper.StatusID == ProductionOrderStatus.Completed && schdOper.FinishTime > 0)
			{
				var timediff = (TimeSpan)(PX.Objects.AM.Common.Dates.Now - prodOper.ActEndDate);
				schdOper.FinishTime = (schdOper.FinishTime - timediff.Minutes).NotLessZero();
			}
            ...
            if (prodOper?.OperationID != null)
            {
				...
				totalTime += schdOper.FinishTime.GetValueOrDefault();
            ...
            {
				schdOper.QueueStartDate = currentSchedDateTime;
                schdOper.StartDate = currentSchedDateTime;
                schdOper.EndDate = currentSchedDateTime;
				schdOper.MoveEndDate = currentSchedDateTime;
			...
			currentSchedDateTime = ProcessWCQueueMove(workCenterCalendar, scheduleDirection, ref schdOper, currentSchedDateTime,
				scheduleDirection == ReadDirection.Forward ? schdOper.QueueTime : schdOper.MoveTime, true);
            ...
            while (remainingOperationSchdTime > 0)
            {
                ...
                if (runAsMachineSchedule)
                {
					var machineOnlyTime = remainingOperationSchdTime - schdOper.FinishTime.GetValueOrDefault();
					ProcessOperationMachines(machineCalendars, scheduleDirection, ref schdOper, ref currentSchedDateTime, ref machineOnlyTime);
					if(machineOnlyTime == 0)
					{
						runAsMachineSchedule = false;
					}
					remainingOperationSchdTime = machineOnlyTime + schdOper.FinishTime.GetValueOrDefault();
				}
                else
                {
			...
			currentSchedDateTime = ProcessWCQueueMove(workCenterCalendar, scheduleDirection, ref schdOper, currentSchedDateTime,
				scheduleDirection == ReadDirection.Forward ? schdOper.MoveTime : schdOper.QueueTime, false);

#if DEBUG
			AMDebug.TraceWriteLine(
                $"----- END Operation {prodOper.OperationCD} ({schdOper.OperationID}) order {schdOper.OrderType}:{schdOper.ProdOrdID} -------------------------------------------------------------");
#endif
            var newSchdDetail = GetSchdOperDetailCacheInserted(prodOper).ToList();
            if (newSchdDetail != null)
            {
                var totalShiftSchdTime = newSchdDetail.Sum(c => c.RunTime);
                ...
                var startEndDateTimes = GetSchdDetailStartEndDateTimes(GetWCSchdDetailCacheInserted(newSchdDetail));
                if(startEndDateTimes != null)
                {
                    schdOper.StartDate = startEndDateTimes.Item1 ?? schdOper.StartDate;
                    schdOper.EndDate = startEndDateTimes.Item2 ?? schdOper.EndDate;
                }
            }

			if(schdOper.QueueStartDate == null)
			{
				schdOper.QueueStartDate = schdOper.StartDate;
			}

			if(schdOper.MoveEndDate == null)
			{
				schdOper.MoveEndDate = schdOper.EndDate;
			}
		...
		private DateTime ProcessWCQueueMove(WorkCenterCalendarHelper workCenterCalendar, ReadDirection scheduleDirection, ref AMSchdOper schdOper,
			DateTime currentSchedDateTime, int? floatTime, bool isFirst)
		{
			var nextDate = ProcessWCQueueMove(workCenterCalendar, scheduleDirection, currentSchedDateTime, floatTime, out var firstDateTime);

			if ((isFirst && scheduleDirection == ReadDirection.Forward)
				|| (!isFirst && scheduleDirection == ReadDirection.Backward))
			{
				schdOper.QueueStartDate = firstDateTime;
			}
			else
			{
				schdOper.MoveEndDate = nextDate;
			}

			if (floatTime.GetValueOrDefault() > 0 && IsCurrentDateUsedUp(workCenterCalendar.GetWorkingHours(nextDate).WorkCenterDateInfo, nextDate, scheduleDirection))
			{
				nextDate = nextDate.AddDays(AddDays(scheduleDirection)).Date;
			}

			return nextDate;
		}

		protected DateTime ProcessWCQueueMove(WorkCenterCalendarHelper workCenterCalendar, ReadDirection scheduleDirection,
			DateTime currentSchedDateTime, int? floatTime, out DateTime? firstDateTime)
		{
			firstDateTime = null;
			var schedDateTime = currentSchedDateTime;
			var remainingTime = BlockRoundMinutes(floatTime.GetValueOrDefault(), true);
			var nextWorkCenterWorkDay = workCenterCalendar.GetNextWorkDay(schedDateTime, scheduleDirection, true);
			...
			if (nextWorkCenterWorkDay.GetValueOrDefault().Date.CompareTo(schedDateTime.Date) != 0)
			{
				schedDateTime = nextWorkCenterWorkDay.GetValueOrDefault();
			}

			var wcDateInfo = workCenterCalendar.GetWorkingHours(nextWorkCenterWorkDay.GetValueOrDefault());
			var isForward = scheduleDirection == ReadDirection.Forward;
			var shifts = workCenterCalendar.GetShifts(scheduleDirection);
			...
			while(remainingTime > 0)
			{
				...
				{
					var shiftDateInfo = wcDateInfo.ShiftDateInfos[amShift.ShiftCD];
					schedDateTime = AdjusteDateToEarliestWorkingDateTime(shiftDateInfo, schedDateTime, scheduleDirection, workCenterCalendar);

					if(firstDateTime == null)
					{
						firstDateTime = schedDateTime;
					}

					var directionMult = isForward ? 1 : -1;
					var nextTime = schedDateTime.AddMinutes(directionMult * remainingTime);
					var shiftDateTime = schedDateTime.Date + ( isForward ?
						shiftDateInfo.EndTime :
						shiftDateInfo.StartTime);
					if(shiftDateInfo.OvernightTime)
					{
						shiftDateTime = shiftDateTime.AddDays(directionMult);
						getNextDay = false;
					}

					if(isForward && nextTime > shiftDateTime)
					{
						remainingTime = Convert.ToInt32(Math.Ceiling((nextTime - shiftDateTime).TotalMinutes));
						schedDateTime = shiftDateTime;
						continue;
					}
					if(!isForward && nextTime < shiftDateTime)
					{
						remainingTime = Convert.ToInt32(Math.Ceiling((shiftDateTime - nextTime).TotalMinutes));
						schedDateTime = shiftDateTime;
						continue;
					}

					schedDateTime = nextTime;
					remainingTime = 0;
					// we do not need the next shift
					break;
				}

				if (remainingTime > 0 && getNextDay)
				{
					schedDateTime = workCenterCalendar.GetNextWorkDay(schedDateTime, scheduleDirection, false).GetValueOrDefault();
				}
			}
		...
		/// </returns>
		internal static Tuple<DateTime?, DateTime?> GetSchdDetailStartEndDateTimes(IEnumerable<ISchdDetail> schdDetail)
        {
            if(schdDetail == null || !schdDetail.Any())
            {
                return null;
            }

            var orderedSchdDetail = schdDetail.OrderBy(r => r.SchdDate).ThenBy(r => r.StartTime);

            var first = orderedSchdDetail.First();
            var last = orderedSchdDetail.Last();

            var startDateTime = first.SchdDate.GetValueOrDefault().Date + first.StartTime.GetValueOrDefault().TimeOfDay;
            var dateAdd = AMDateInfo.IsOvernightTime(last.StartTime.GetValueOrDefault(), last.EndTime.GetValueOrDefault()) ? 1 : 0;
            var endDateTime = last.SchdDate.GetValueOrDefault().Date.AddDays(dateAdd) + last.EndTime.GetValueOrDefault().TimeOfDay;
        ...
        private void ProcessWorkCenterSchedule(WorkCenterCalendarHelper workCenterCalendar, ReadDirection scheduleDirection, ref AMSchdOper schdOper, ref DateTime currentSchedDateTime, ref int remainingSchdTime)
        {
...
#endif
			var finishTime = Math.Min(schdOper.FinishTime.GetValueOrDefault(), remainingSchdTime);
			var runTimeOnly = (remainingSchdTime - finishTime).NotLessZero();
            ...
            foreach (var amShift in workCenterCalendar.GetShifts(scheduleDirection))
            {
                ...
                var runTimeEfficency = (useCrewEff
                    ? CalcTotalEfficiencyTime(runTimeOnly, amShift.CrewSize, amShift.ShftEff)
                    : runTimeOnly);
				var shiftRemainingSchdTime = runTimeEfficency + finishTime;

                ProcessOperationShift(schdOper, amShift, wcDateInfo.ShiftDateInfos[amShift.ShiftCD],
                    workCenterCalendar, scheduleDirection, ref currentSchedDateTime, ref shiftRemainingSchdTime);

				runTimeEfficency = shiftRemainingSchdTime - finishTime;
				if(shiftRemainingSchdTime < finishTime)
				{
					finishTime = shiftRemainingSchdTime;
					runTimeEfficency = 0;
				}

                remainingSchdTime = (useCrewEff
                    ? CalcTotalEfficiencyTimeReverse(runTimeEfficency, amShift.CrewSize, amShift.ShftEff)
                    : runTimeEfficency) + finishTime;
            }
		...
		protected void SetWCSchdDetailFloatTime<TField>(AMSchdOper schdOper, int? floatTime) where TField : IBqlField
		{
			if(schdOper == null || floatTime.GetValueOrDefault() == 0)
			{
				return;
			}

			var orderByAscending = typeof(TField).Name == typeof(AMWCSchdDetail.queueTime).Name;

			var operSchdDetails = SchdOperDetails.Cache.Cached.RowCast<AMSchdOperDetail>()
				.Where(r => r.OrderType == schdOper.OrderType &&
						r.ProdOrdID == schdOper.ProdOrdID &&
						r.OperationID == schdOper.OperationID &&
						SchdOperDetails.Cache.GetStatus(r) != PXEntryStatus.Deleted)?.ToList();

			if(operSchdDetails == null || operSchdDetails.Count == 0)
			{
				return;
			}

			var schdDetail = orderByAscending
					? operSchdDetails.OrderBy(r => r.SchdDate).First()
					: operSchdDetails.OrderByDescending(r => r.SchdDate).First();
			var schdDate = schdDetail?.SchdDate;
			if(schdDate == null)
			{
				return;
			}

			var wcSchdDetail = GetWCSchdDetail(operSchdDetails.Where(r => r.SchdDate == schdDate.GetValueOrDefault()).ToList(), orderByAscending);
			if(wcSchdDetail == null)
			{
				return;
			}

			WorkCenterSchdDetailRecs.Cache.SetValueExt<TField>(wcSchdDetail, floatTime);
			WorkCenterSchdDetailRecs.Update(wcSchdDetail);
		}

		private AMWCSchdDetail GetWCSchdDetail(List<AMSchdOperDetail> schdOperDetails, bool orderByAscending)
		{
			if(schdOperDetails == null)
			{
				return null;
			}

			AMWCSchdDetail wcSchdDetailReturn = null; 
			foreach (var schdOperDetail in schdOperDetails)
			{
				var wcSchdDetail = WorkCenterSchdDetailRecs.Cache.Inserted.RowCast<AMWCSchdDetail>()
					.Where(r => r.SchdKey == schdOperDetail.SchdKey).FirstOrDefault();

				if(wcSchdDetail?.SchdKey == null)
				{
					continue;
				}

				if(wcSchdDetailReturn == null
					|| (orderByAscending && wcSchdDetail.StartTime.LessThan(wcSchdDetailReturn.StartTime))
					|| (!orderByAscending && wcSchdDetail.StartTime.GreaterThan(wcSchdDetailReturn.StartTime)))
				{
					wcSchdDetailReturn = wcSchdDetail;
				}
			}

			return wcSchdDetailReturn;
		}
        ...
        private void ProcessOperationMachines(List<MachineCalendarHelper> machineCalendars, ReadDirection scheduleDirection, ref AMSchdOper schdOper, ref DateTime currentSchedDateTime, ref int remainingSchdTime)
        {
            ...
            var workCenterSchd = GetWorkCenterSchd(workCenter.WcID, shift.ShiftCD, workCenter.SiteID, workCenterDateInfo, totalMachineWorkingMinutes);
            ...
            CalculateUsedTime(insertedWorkCenterSchdDetails, scheduleDirection, currentSchedDateTime, out var notUsed, out currentSchedDateTime);
		...
		private DateTime AdjusteDateToEarliestWorkingDateTime(AMDateInfo dateInfo, DateTime schedDateTime, ReadDirection scheduleDirection, WorkCenterCalendarHelper workCenterCalendar)
		{
			var newSchedDateTime = AdjusteDateToEarliestDateTime(dateInfo, schedDateTime, scheduleDirection);
			if(workCenterCalendar != null && dateInfo.OvernightTime && scheduleDirection == ReadDirection.Backward)
			{
				// need to confirm the next time used is valid for the shift start day as a working day
				// when the next day is not a work day we need to adjust to this next working day
				var nextWorkDay = workCenterCalendar.GetNextWorkDay(schedDateTime, scheduleDirection);
				if(nextWorkDay != null)
				{
					var daysApart = (nextWorkDay.GetValueOrDefault().Date.AddDays(1) - newSchedDateTime.Date).TotalDays;
					if(daysApart != 0)
					{
						// add days to keep the time
						newSchedDateTime = newSchedDateTime.AddDays(daysApart);
					}
				}
			}

			return newSchedDateTime;
		}
            ...
            ReadDirection scheduleDirection, ref DateTime currentSchedDateTime, ref int remainingSchdTime)
        {
            ...
            var checkWorkCenterSchd = GetWorkCenterSchd(wcShift.WcID, wcShift.ShiftCD, workCenterCalendar.WorkCenter.SiteID, shiftDateInfo);
            ...
            if (checkWorkCenterSchd.AvailableBlocks.GetValueOrDefault() == 0)
            {
                throw new SchedulingOrderDatesException(shiftDateInfo.Date, shiftDateInfo.Date.AddDays(addDays),
                    $"Could not find available time for work center '{wcShift.WcID.TrimIfNotNullEmpty()}' shift '{wcShift.ShiftCD.TrimIfNotNullEmpty()}' on requested date time '{shiftDateInfo.Date}'");
            }
            ...
            ReadDirection scheduleDirection, int remainingSchdTime, DateTime currentSchedDateTime, out List<AMWCSchdDetail> wcSchdDetailList)
        {
            ...
            foreach (var amwcSchdDetail in schdDateDetail.Results)
            {
                ...
                amwcSchdDetail.ShiftCD = wcShift.ShiftCD;
        ...
        protected List<AMWCSchdDetail> GetWCSchdDetail(AMWCSchd workCenterSchd)
        {
            return PXSelect<AMWCSchdDetail,
                ...
                And<AMWCSchdDetail.shiftCD, Equal<Required<AMWCSchdDetail.shiftCD>>,
                And<AMWCSchdDetail.schdDate, Equal<Required<AMWCSchdDetail.schdDate>>>>>
                >.Select(this, workCenterSchd.WcID, workCenterSchd.ShiftCD, workCenterSchd.SchdDate).ToFirstTableList();
        }
        ...
        private static void CreateWorkSchdDaysByMachines(ProductionScheduleEngineAdv schdEngine, List<MachineCalendarHelper> machineCalendars, AMWC workCenter, AMShift firstShift, DateTime? fromDate, DateTime? toDate)
        {
            ...
            while (date.CompareTo(toDate.GetValueOrDefault(Common.Current.BusinessDate(schdEngine))) <= 0)
            {
                ...
                var wcSchd = schdEngine.GetWorkCenterSchd(workCenter.WcID, firstShift.ShiftCD, workCenter.SiteID, workCenterDateInfo, totalMachineWorkingMinutes);
        ...
        private static void CreateWorkSchdDaysByWorkCenter(ProductionScheduleEngineAdv schdEngine, WorkCenterCalendarHelper wcCalHelper, DateTime? fromDate, DateTime? toDate)
        {
            ...
            while (date.CompareTo(toDate.GetValueOrDefault(Common.Current.BusinessDate(schdEngine))) <= 0)
            {
                ...
                {
                    var wcSchd = schdEngine.GetWorkCenterSchd(wcCalHelper.WorkCenter.WcID, amShift.ShiftCD, wcCalHelper.WorkCenter.SiteID, wcDateInfo.ShiftDateInfos[amShift.ShiftCD]);
                    if (wcSchd != null)
                    {
        ...
        private List<AMWCSchdDetail> CreateBreakTimeDetail(AMWCSchd workCenterSchd, List<AMCalendarBreakTime> breakTimes)
        {
            ...
            foreach (var calendarBreakTime in breakTimes)
            {
                ...
                newBreakTimes.Add(WorkCenterSchdDetailRecs.Insert(new AMWCSchdDetail
                {
                    ...
                    ShiftCD = workCenterSchd.ShiftCD,
        ...
        protected override AMWCSchd GetWorkCenterSchd(string workCenterID, string shiftCD, int? siteID, AMDateInfo dateInfo, int workingMinutes)
        {
#if DEBUG
...
#endif
            var wcSchd =  base.GetWorkCenterSchd(workCenterID, shiftCD, siteID, dateInfo, workingMinutes);
            if (wcSchd != null)
            {
        ...
        private void DeleteExistingBreaks(AMWCSchd wcSchd, AMDateInfo dateInfo)
        {
            var breaks = SelectFrom<AMWCSchdDetail>.Where<AMWCSchdDetail.wcID.IsEqual<@P.AsString>
                .And<AMWCSchdDetail.shiftCD.IsEqual<@P.AsString>>
                .And<AMWCSchdDetail.schdDate.IsEqual<@P.AsDateTime>>
                .And<AMWCSchdDetail.isBreak.IsEqual<True>>>.View.Select(this, wcSchd.WcID, wcSchd.ShiftCD, wcSchd.SchdDate);
        ...
        internal IEnumerable<AMWCSchdDetail> GetWCSchdDetailCacheInserted(IEnumerable<ISchdReference> schdDetails)
        {
            if(schdDetails == null)
            {
                yield break;
            ...
            foreach (var schdDetail in schdDetails)
            {
                yield return this.WorkCenterSchdDetailRecs.Cache.Inserted.RowCast<AMWCSchdDetail>()
                    .Where(r => r.SchdKey == schdDetail?.SchdKey).FirstOrDefault();
            }
        }
            ...
            int? operationId, DateTime itemPlanDate)
        {
                ...
                .Select(graph, orderType, prodOrdId, operationId))
            {
                ...
                var result = MaterialAvailability(graph, planDate, material, inventoryItem, (AMProdOper)pxResult, itemPlanDate);
                if (result == null)
                {
                    continue;
                }
            ...
            AMProdMatl prodMatl, InventoryItem inventoryItem, AMProdOper prodOper, DateTime itemPlanDate)
        {
            ...
            if (remainingQty > 0)
            {
				var leadTime = InventoryHelper.GetPurchaseLeadTime(graph, prodMatl.InventoryID, prodMatl.SiteID);
                if(leadTime > 0)
                {
                    var earliestDate = Common.Current.BusinessDate(graph).AddDays(leadTime);
                        if (earliestDate <= planDate)
					{
                            ...
                            var operationDaysLate = earliestDate.Subtract(planDate.Date).Days;
                            return new MaterialAvailabiltyDates(prodMatl, prodOper?.OperationCD, inventoryItem, planDate, allProdMatlSplitsList)
                            {
		...
		/// </summary>
		public virtual void FirmOrder(AMSchdItem schdItem)
		{
			if (!Features.AdvancedPlanningEnabled())
            {
                throw new PXException(Messages.UnableToProcess, this.GetType().Name);
			...
			schdItem.FirmSchedule = true;
			schdItem.ScheduleStatus = ProductionScheduleStatus.Firm;

            UpdateProdItem(SchdItems.Update(schdItem));
		}

		/// <summary>
		/// Undo a firm order only (no scheduling)
		/// </summary>
		public virtual void UndoFirmOrder(AMSchdItem schdItem)
		{
			if (!Features.AdvancedPlanningEnabled())
            {
                throw new PXException(Messages.UnableToProcess, this.GetType().Name);
			...
			schdItem.FirmSchedule = false;
			schdItem.ScheduleStatus = ProductionScheduleStatus.Scheduled;
