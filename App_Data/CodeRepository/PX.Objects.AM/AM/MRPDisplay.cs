...
namespace PX.Objects.AM
{
    ...
    {
		[PXFilterable]
        public PXSelectJoinOrderBy<AMRPDetail,
			LeftJoin<Vendor, On<Vendor.bAccountID, Equal<AMRPDetail.preferredVendorID>>>,
		...
		public PXFilter<MRPDisplayFilters> MRPDisplayFilters;
        ...
        [PXUIField(DisplayName = Messages.MRPDetailInquiry, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton]
		...
		protected virtual IEnumerable detailrecs()
		{
			return EnumerateAndPrepareDetailRecs(SelectFromDetailRecsView());
		}

		public virtual IEnumerable EnumerateAndPrepareDetailRecs(PXResultset<AMRPDetail> detailRecs)
		{
			foreach (PXResult<AMRPDetail> rec in detailRecs)
			{
				yield return rec;
			}
		}

		public virtual PXResultset<AMRPDetail> SelectFromDetailRecsView()
		{
			PXView query = new PXView(this, false, Detailrecs.View.BqlSelect);

			var detailRecs = new PXResultset<AMRPDetail>();
			var startRow = PXView.StartRow;
			var totalRows = 0;
			object[] parameters = null;

			var filter = MRPDisplayFilters.Current;

			query.WhereAnd<Where<AMRPDetail.processed, NotEqual<True>>>();

			if (!IsConsolidateOrdersVisible)
			{
				if (filter.RefNbr != null | filter.ParentRefNbr != null | filter.ProductRefNbr != null)
				{
					if (filter.ProductRefNbr != null)
					{
						query.WhereAnd<
								Where<AMRPDetail.productRefNbr, Equal<Required<AMRPDetail.productRefNbr>>>>();
						parameters = parameters.Append(filter.ProductRefNbr);
					}

					if (filter.ParentRefNbr != null)
					{
						query.WhereAnd<
								Where<AMRPDetail.parentRefNbr, Equal<Required<AMRPDetail.parentRefNbr>>>>();
						parameters = parameters.Append(filter.ParentRefNbr);
					}

					if (filter.RefNbr != null)
					{
						query.WhereAnd<
								Where<AMRPDetail.refNbr, Equal<Required<AMRPDetail.refNbr>>>>();
						parameters = parameters.Append(filter.RefNbr);
					}
				}
			}
			else if (filter.RefNbr != null | filter.ParentRefNbr != null | filter.ProductRefNbr != null)
			{

				var fpRecList = new List<int?>();

				var baseRecs = GetAMRPDetailFPBaseRecs(filter);

				foreach (var rec in baseRecs)
				{
					if (!fpRecList.Contains(rec.RecordID)) { fpRecList.Add(rec.RecordID); }

					bool doRecurse = false;
					if (filter.ProductRefNbr != null) { doRecurse = true; }

					// If consoliated, Add the Parent Consolidation record into list and potentially recursive calls
					filterDetailRecsGetConsChildren(doRecurse, rec, fpRecList, filter);
				}

				query.WhereAnd<Where<AMRPDetail.detailFPRecordID, In<Required<AMRPDetail.detailFPRecordID>>>>();

				parameters = parameters.Append(fpRecList.ToArray());
			}

			using (new PXFieldScope(query, GetDetailRecsFieldScope()))
			{
				foreach (PXResult<AMRPDetail> demand in query.Select(PXView.Currents, parameters,
					PXView.Searches, PXView.SortColumns, PXView.Descendings, PXView.Filters,
					ref startRow, PXView.MaximumRows, ref totalRows))
				{
					detailRecs.Add(demand);
				}
			}
		...
		public virtual IEnumerable<Type> GetDetailRecsFieldScope()
		{
			yield return typeof(AMRPDetail);
			yield return typeof(Vendor.acctName);
		}

		private void filterDetailRecsGetConsChildren(bool doRecurse, AMRPDetailFP rec, List<int?> fpRecList, MRPDisplayFilters filter)
		{
			if (rec.ConsolidatedRecordID != null & !fpRecList.Contains(rec.ConsolidatedRecordID))
			{
				fpRecList.Add(rec.ConsolidatedRecordID);

				if (filter.RefNbr == null)
				{

					var childFilter = new MRPDisplayFilters();
					childFilter.ParentRefNbr = string.Format("CONS, {0}", rec.ConsolidatedRecordID);

					var childRecs = GetAMRPDetailFPBaseRecs(childFilter);
					foreach (var childrec in childRecs)
					{
						if (!fpRecList.Contains(childrec.RecordID)) { fpRecList.Add(childrec.RecordID); }

						if (childrec.ConsolidatedRecordID != null & !fpRecList.Contains(childrec.ConsolidatedRecordID))
						{
							fpRecList.Add(childrec.ConsolidatedRecordID);

						}

						if (doRecurse)
						{
							filterDetailRecsGetConsChildren(doRecurse, childrec, fpRecList, filter);
						}
		...
		private List<AMRPDetailFP> GetAMRPDetailFPBaseRecs(MRPDisplayFilters filter)
		{
			return SelectFrom<AMRPDetailFP>.
				   Where<Brackets<AMRPDetailFP.refNbr.IsEqual<@P.AsString>.Or<@P.AsString.IsNull>>
				   .And<Brackets<AMRPDetailFP.refNbr.IsEqual<@P.AsString>.Or<AMRPDetailFP.parentRefNbr.IsEqual<@P.AsString>.Or<@P.AsString.IsNull>>>>
				   .And<Brackets<AMRPDetailFP.refNbr.IsEqual<@P.AsString>.Or<AMRPDetailFP.parentRefNbr.IsEqual<@P.AsString>.Or<AMRPDetailFP.productRefNbr.IsEqual<@P.AsString>.Or<@P.AsString.IsNull>>>>>>
				   .View.Select(this, filter.RefNbr, filter.RefNbr
								, filter.ParentRefNbr, filter.ParentRefNbr, filter.ParentRefNbr
								, filter.ProductRefNbr, filter.ProductRefNbr, filter.ProductRefNbr, filter.ProductRefNbr)?.FirstTableItems.ToList();
		}
    ...
    public class PlanPurchaseFilter : IBqlTable
    {
        ...
        [LocationActive(typeof(Where<Location.bAccountID, Equal<Current<PlanPurchaseFilter.vendorID>>,
	...
	[Serializable]
	[PXCacheName("MRP Display Filters")]
	public class MRPDisplayFilters : IBqlTable
	{
		#region RefNbr
		public abstract class refNbr : PX.Data.BQL.BqlString.Field<refNbr> { }

		protected String _RefNbr;
		[PXUIField(DisplayName = "Related Document", Visibility = PXUIVisibility.Visible)]
		...
		protected String _ParentRefNbr;
		[PXUIField(DisplayName = "Related Parent Document", Visibility = PXUIVisibility.Visible)]
		public virtual String ParentRefNbr
		{
			get
			{
				return this._ParentRefNbr;
			}
			set
			{
				this._ParentRefNbr = value;
			}
		}
		#endregion
		...
		protected String _ProductRefNbr;
		[PXUIField(DisplayName = "Related Product Document", Visibility = PXUIVisibility.Visible)]
		public virtual String ProductRefNbr
		{
			get
			{
				return this._ProductRefNbr;
			}
			set
			{
				this._ProductRefNbr = value;
			}
		}
		#endregion
