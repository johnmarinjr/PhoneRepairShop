...
namespace PX.Objects.AM
{
    ...
    public class AMConfiguration : IBqlTable, INotable, IProductConfiguration
    {
        ...
        public class PK : PrimaryKeyOf<AMConfiguration>.By<configurationID, revision>
        {
            public static AMConfiguration Find(PXGraph graph, string configurationID, string revision) 
                => FindBy(graph, configurationID, revision);
            public static AMConfiguration FindDirty(PXGraph graph, string configurationID, string revision)
                => PXSelect<AMConfiguration,
                        Where<configurationID, Equal<Required<configurationID>>,
                            And<revision, Equal<Required<revision>>>>>
                    .SelectWindowed(graph, 0, 1, configurationID, revision);
        }

        public static class FK
        {
            public class InventoryItem : PX.Objects.IN.InventoryItem.PK.ForeignKeyOf<AMConfiguration>.By<inventoryID> { }
