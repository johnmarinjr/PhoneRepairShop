...
namespace PX.Objects.AM
{
    ...
    public class LSAMMove : LSAMMTran
    {
		...
		public override void CreateNumbers(PXCache sender, AMMTran Row, decimal BaseQty)
		{
			base.CreateNumbers(sender, Row, BaseQty);
			if (Row.IsLotSerialPreassigned == true)
			{
				BuildPreassignNumbers(sender, Row);
			}
		}

		public override void TruncateNumbers(PXCache sender, AMMTran Row, decimal BaseQty)
		{
			base.TruncateNumbers(sender, Row, BaseQty);
			if (Row.IsLotSerialPreassigned == true)
			{
				BuildPreassignNumbers(sender, Row);
			}
		...
		public void BuildPreassignNumbers(PXCache sender, AMMTran Row)
		{
			...
			.And<AMProdItemSplit.prodOrdID.IsEqual<@P.AsString>>>.View.Select(sender.Graph, Row.OrderType, Row.ProdOrdID))
			{
			...
			{
				foreach (AMMTranSplit detail in SelectDetail(DetailCache, Row))
				{
					if (AssignedSplits.Count == 0)
					{
