    ...
    public class ProductionOrderCopy : ProductionBomCopy
    {
        ...
        protected virtual void CopyOperations(AMOrder order, AMProdItem productionCopyItem)
        {
            ...
            foreach (AMProdOper fromRow in fromRows)
            {
				...
				var foundOper = FindCachedOperByCD(toRow);
				if (foundOper != null)
				{
					var status = ProcessingGraph.Caches<AMProdOper>().GetStatus(foundOper);
					if (status == PXEntryStatus.Deleted || status == PXEntryStatus.InsertedDeleted)
					{
						// Rebuilding order
						ProcessingGraph.Caches<AMProdOper>().SetStatus(foundOper, status == PXEntryStatus.InsertedDeleted ? PXEntryStatus.Inserted : PXEntryStatus.Updated);
					}

					toRow.LineCntrMatl = foundOper.LineCntrMatl;
					toRow.LineCntrOvhd = foundOper.LineCntrOvhd;
					toRow.LineCntrStep = foundOper.LineCntrStep;
					toRow.LineCntrTool = foundOper.LineCntrTool;

					ProcessingGraph.Caches<AMProdOper>().SetDefaultExt<AMProdOper.operationID>(toRow);
					ProcessingGraph.Caches<AMProdOper>().SetDefaultExt<AMProdOper.statusID>(toRow);
					ProcessingGraph.Caches<AMProdOper>().SetDefaultExt<AMProdOper.baseQtyScrapped>(toRow);
					ProcessingGraph.Caches<AMProdOper>().SetDefaultExt<AMProdOper.qtyScrapped>(toRow);
					ProcessingGraph.Caches<AMProdOper>().SetDefaultExt<AMProdOper.baseQtyComplete>(toRow);
					ProcessingGraph.Caches<AMProdOper>().SetDefaultExt<AMProdOper.qtyComplete>(toRow);
					ProcessingGraph.Caches<AMProdOper>().SetDefaultExt<AMProdOper.baseQtytoProd>(toRow);
					ProcessingGraph.Caches<AMProdOper>().SetDefaultExt<AMProdOper.qtytoProd>(toRow);
				}

                var prodCopy = foundOper != null ? (AMProdOper)ProcessingGraph.Caches<AMProdOper>().Update(toRow) : (AMProdOper)ProcessingGraph.Caches<AMProdOper>().Insert(toRow);
