...
namespace PX.Objects.AM
{
    ...
    public class SchedulePlanBomCopy : ProductionBomCopyBase
    {
        ...
        protected virtual AMSchdOper CreateScheduleDetail(AMSchdOper amSchdOper, AMBomOper sourceAmBomOper, decimal phtmpriorlevelqty)
        {
            ...
            while (totalTime > 0)
            {
                ...
                if (schdMinutes > 0)
                {
                    ...
                    GetWorkCenterSchd(amSchdOper.WcID, amShift.ShiftCD, amSchdOper.SiteID, dateInfo);
                    var wcSchdDetail = ProductionScheduleEngine.CreateWCSchdDetail(amSchdOper, currentDateTime, amShift.ShiftCD);
        ...
        protected virtual AMWCSchd GetWorkCenterSchd(string workCenterID, string shiftCD, int? siteID, AMDateInfo dateInfo)
        {
            ...
            {
                throw new ArgumentNullException(nameof(shiftCD));
            }
            ...
            AMWCSchd workCenterSchd = PXSelect<AMWCSchd,
                    ...
                    >.Select(ProcessingGraph, workCenterID, shiftCD, dateInfo.Date);
            ...
            if (workCenterSchd.TotalBlocks.GetValueOrDefault() != totalBlocks)
            {
                ...
                AMDebug.TraceWriteMethodName(
                    $"Adjusting the total blocks for wc '{workCenterID.TrimIfNotNullEmpty()}' shift '{shiftCD.TrimIfNotNullEmpty()}' by '{totalBlocks - workCenterSchd.TotalBlocks.GetValueOrDefault()}'");
#endif
        ...
        protected virtual List<AMPlanMaterial> GetOrderMaterial(DateTime? materialDateTime)
        {
            ...
            foreach (var operationDetail in _operationDetails.OrderedList)
            {
                ...
                if (materialDateTime == null)
                {
                    ...
                    if (amSchdOper != null)
                    {
                        materialPlanDate = amSchdOper.StartDate.GetValueOrDefault(BusinessDate).Date;
                    }
                }
