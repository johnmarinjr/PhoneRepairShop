...
namespace PX.Objects.AM
{
    ...
    public class ConfigurationSelect : PXSelect<AMConfigurationResults>, IRuleProcessor, IRuleConditionValidator
    {
		...
		public virtual void ConfigPersistInsertUpdate()
		{
			...
			{
				ConfigPersistInsertUpdateNoTranScope();

				ts.Complete(_Graph);
			}

			ConfigPersistSetPersisted();
		}

		public virtual void ConfigPersistInsertUpdateNoTranScope()
		{
		...
		public virtual void ConfigPersistSetPersisted()
		{
        ...
        internal virtual void UpdateConfigurationFromKey(PXGraph graph, AMConfigurationResults currentConfigResult, string configKeyFrom)
        {
			...
			try
			{
				...
				RemoveRulesHandlers();
				var locatedToConfigResult = graph.Caches<AMConfigurationResults>().LocateElse(currentConfigResult);
				ConfigurationCopyEngine.UpdateConfigurationFromConfiguration(graph, locatedToConfigResult, fromConfigResult);
				AddRulesHandlers();
				RunRuleFormula(locatedToConfigResult.ConfigResultsID < 0, out var _);
			}
			finally
			{
        ...
        protected virtual void SetConfigurationStatus(PXCache cache, CROpportunity order, CRQuote quote, AMConfigurationResults configurationResults)
        {
            ...
            var isClosed = (order != null && order.IsActive != true) || (quote != null && quote.Status != null && quote.IsDisabled == true);
            if (configurationResults.Closed.GetValueOrDefault() != isClosed)
            {
                configurationResults.Closed = isClosed;
                this.Update(configurationResults);
        ...
        {
            if (cache == null || quote == null || quote.Status == null || configurationResults == null)
            {
                return;
            }
		...
		public static PXResultset<AMBomMatl> GetConfigurationBomMaterialResult(PXGraph graph, string bomid, string revision)
        {
			return PXSelectJoin<
				AMBomMatl,
				InnerJoin<AMBomOperSimple,
					On<AMBomMatl.bOMID, Equal<AMBomOperSimple.bOMID>,
					And<AMBomMatl.revisionID, Equal<AMBomOperSimple.revisionID>,
					And<AMBomMatl.operationID, Equal<AMBomOperSimple.operationID>>>>,
				InnerJoin<InventoryItem,
					On<InventoryItem.inventoryID, Equal<AMBomMatl.inventoryID>>>>,
				Where2<
					Where<AMBomMatl.expDate, IsNull,
						Or<AMBomMatl.expDate, GreaterEqual<Today>>>,
					And<AMBomMatl.bOMID, Equal<Required<AMBomMatl.bOMID>>,
					And<AMBomMatl.revisionID, Equal<Required<AMBomMatl.revisionID>>>>>,
				OrderBy<
					Asc<AMBomOperSimple.operationCD,
					Asc<AMBomMatl.sortOrder,
					Asc<AMBomMatl.lineID>>>>>
				.Select(graph, bomid, revision);
        }
        ...
        protected virtual void AMConfigurationResults_CuryBOMPriceTotal_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
        {
			...
			var configMaint = AMConfiguration.PK.Find(_Graph, row.ConfigurationID, row.Revision);
			if (configMaint == null
				// rollup options which do not include BOMPriceTotal
				|| configMaint.PriceRollup == RollupOptions.Parent || configMaint.PriceRollup == RollupOptions.ChildrenCFG)
			{
				e.NewValue = 0m;
				return;
			}
            ...
            var total = 0m;
            foreach (var result in GetConfigurationBomMaterialResult(_Graph, configMaint.BOMID, configMaint.BOMRevisionID))
            {
				var bomMatl = result.GetItem<AMBomMatl>();
                if (bomMatl?.InventoryID == null)
                {
                    continue;
                }

				var item = result.GetItem<InventoryItem>();
                ...
                catch (Exception exception)
                {
                    ...
                    {
						var oper = AMBomOper.PK.Find(_Graph, bomMatl.BOMID, bomMatl.RevisionID, bomMatl.OperationID);
...
#pragma warning restore PX1044
        ...
        public bool RunRuleFormula(out string message)
        {
			...
			return RunRuleFormula(_IsConfigCreation, out message);
        }

		public bool RunRuleFormula(bool queryCache, out string message)
        {
            return RunRuleFormula(AMRuleFormulaEngine.Run(_Graph, queryCache), false, out message);
        }
		...
		private void SetCurrentResultsFeature(AMConfigResultsOption resultOption)
		{
			if(resultOption?.ConfigResultsID == null)
			{
				return;
			}

			var current = (AMConfigResultsFeature)ResultFeatures.Cache.Current;
			if(current == null || current.ConfigResultsID != resultOption.ConfigResultsID || current.FeatureLineNbr != resultOption.FeatureLineNbr)
			{
				var cacheFeature = ResultFeatures.Cache.Locate(new AMConfigResultsFeature
						{
							ConfigResultsID = resultOption.ConfigResultsID,
							FeatureLineNbr = resultOption.FeatureLineNbr
						});
				if(cacheFeature != null)
				{
					ResultFeatures.Cache.Current = cacheFeature;
					return;
				}

				var foundFeature = AMConfigResultsFeature.PK.Find(_Graph, resultOption.ConfigResultsID, resultOption.FeatureLineNbr);
				if(foundFeature == null)
				{
					return;
				}

				ResultFeatures.Cache.Current = foundFeature;
			}
		}
        ...
        private void ApplyRuleFormula(RuleReturn ret)
        {
            ...
            using (new AMRuleFormulaScope(this))
            {
                ...
                try
                {
					using (new UseCurrentParentScope(ResultOptions.Cache, true, typeof(AMConfigResultsOption.featureLineNbr)))
					{
						foreach (var option in ret.OptionsToUpdate)
						{
							...
							SetCurrentResultsFeature(option);
        ...
        protected void CreateResultsTemplate(PXCache sender, AMConfigurationResults resultConfiguration)
        {
            ...
            try
            {
				foreach (AMConfiguration config in Configuration.Select())
				{
					...
					var item = InventoryItem.PK.Find(_Graph, config?.InventoryID);
					if (item == null)
					{
						continue;
					}
					...
					{
						using (new UseCurrentParentScope(ResultOptions.Cache, true, typeof(AMConfigResultsOption.configResultsID), typeof(AMConfigResultsOption.featureLineNbr)))
						{
							...
							foreach (AMConfigurationFeature configFeature in ConfigFeatures.Select())
							{
								...
								ResultFeatures.Insert(new AMConfigResultsFeature
								{
									FeatureLineNbr = configFeature.LineNbr,
									ConfigResultsID = resultConfiguration.ConfigResultsID,
									TotalQty = 0m
								});
							...
							foreach (AMConfigurationRule configRule in ConfigRules.Select())
							{
								...
								{
									foreach (var option in ResultOptions.Cache.Cached.RowCast<AMConfigResultsOption>()
										.Where(r => r.ConfigResultsID == resultConfiguration.ConfigResultsID && r.FeatureLineNbr == configRule.TargetFeatureLineNbr))
									{
										if (ResultOptions.Cache.GetStatus(option).IsDeleted())
										{
											continue;
										}
        ...
        public void ClearConfigurationCache()
        {
			...
			ResultAttributes.Cache.ClearQueryCache();
            ResultFeatures.Cache.Clear();
			ResultFeatures.Cache.ClearQueryCache();
            ResultOptions.Cache.Clear();
			ResultOptions.Cache.ClearQueryCache();
            ResultRules.Cache.Clear();
			ResultRules.Cache.ClearQueryCache();
        }
    }
}
