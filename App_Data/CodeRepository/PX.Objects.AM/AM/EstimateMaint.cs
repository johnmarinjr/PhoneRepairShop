    ...
    public class EstimateMaint : PXRevisionableGraph<EstimateMaint, AMEstimateItem, AMEstimateItem.estimateID, AMEstimateItem.revisionID>
    {
        ...
        protected virtual void CopyOperations(AMEstimateItem fromEstimateItem, AMEstimateItem toEstimateItem)
        {
                ...
                .Select(this, fromEstimateItem.EstimateID, fromEstimateItem.RevisionID))
            {
				...
				newEstOper.MoveTime = fromEstimateOper.MoveTime;
				newEstOper.FinishTime = fromEstimateOper.FinishTime;
				...
				newEstOper.ControlPoint = fromEstimateOper.ControlPoint;
        ...
        protected virtual void CopyOperations(AMProdItem prodItem)
        {
                ...
                .Select(this, prodItem.OrderType, prodItem.ProdOrdID))
            {
				...
				newEstOper.ControlPoint = prodOper.ControlPoint;
				newEstOper.MoveTime = prodOper.MoveTime;
				newEstOper.FinishTime = prodOper.FinishTime;
				EstimateOperRecords.Update(newEstOper);
            }
        }
        ...
        protected virtual void CopyOperations(AMBomItem bomItem)
        {
                ...
                .Select(this, bomItem.BOMID, bomItem.RevisionID))
            {
				...
				newEstOper.ControlPoint = bomOper.ControlPoint;
				newEstOper.MoveTime = bomOper.MoveTime;
				EstimateOperRecords.Update(newEstOper);
            }
        }
        ...
        [PXButton]
        [PXUIField(DisplayName = "Set as Primary Estimate", MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update)]
        ...
        [PXButton]
        [PXUIField(DisplayName = Messages.CreateInventory)]
        ...
        protected virtual void CreateBomDetails(AMEstimateItem estimateItem, BOMMaint bomMaintGraph)
        {
            ...
            >.Select(this, estimateItem.EstimateID, estimateItem.RevisionID))
            {
				...
				oper.ControlPoint = estimateOper.ControlPoint;
		...
		protected virtual void _(Events.FieldVerifying<AMEstimateOper, AMEstimateOper.operationCD> e)
		{
			if (e.Row == null || e.NewValue == null)
				return;

			var result = e.Cache.Cached.RowCast<AMEstimateOper>()
					.Where(r => r.EstimateID == e.Row.EstimateID && r.RevisionID == e.Row.RevisionID && r.OperationCD == e.NewValue.ToString()
					&& e.Row.OperationID != r.OperationID
					&& e.Cache.GetStatus(r) != PXEntryStatus.Deleted).FirstOrDefault();

			if (!string.IsNullOrEmpty(result?.EstimateID))
			{
				var errorMessage = Messages.GetLocal(Messages.DuplicateOperationID, e.NewValue);
				e.NewValue = e.Row.OperationCD;
				e.Cancel = true;
        ...
        {
            var context = config.GetScreenConfigurationContext<EstimateMaint, AM.AMEstimateItem>();

            var estimateManagementCategory = context.Categories.CreateNew(ActionCategoryNames.EstimateManagement,
                category => category.DisplayName(ActionCategory.EstimateManagement));
            ...
            context.AddScreenConfigurationFor(screen =>
            {
                return screen
                       ...
                       {
                           categories.Add(estimateManagementCategory);
                           categories.Add(printingEmailingCategory);
                       })
                        .WithActions(actions =>
                        {
                            actions.Add(g => g.CreateBOM, c => c.WithCategory(estimateManagementCategory));
                            actions.Add(g => g.CreateProdOrder, c => c.WithCategory(estimateManagementCategory));
                            actions.Add(g => g.CreateInventory, c => c.WithCategory(estimateManagementCategory));
                            actions.Add(g => g.Add2Order, c => c.WithCategory(estimateManagementCategory));
                            actions.Add(g => g.CreateNewRevisionButton, c => c.WithCategory(estimateManagementCategory));
                            actions.Add(g => g.MarkAsPrimary, c => c.WithCategory(estimateManagementCategory));
                            actions.Add(g => g.CopyFrom, c => c.WithCategory(estimateManagementCategory));

                            actions.Add(g => g.ReportSummary, c => c.WithCategory(printingEmailingCategory));
                            actions.Add(g => g.ReportQuote, c => c.WithCategory(printingEmailingCategory));
                        });
            });
        }
        ...
        {
            public const string EstimateManagement = "EstimateManagement";
            public const string PrintingEmailing = "PrintingEmailing";
        }

        public static class ActionCategory
        {
            public const string EstimateManagement = "Estimate Management";
