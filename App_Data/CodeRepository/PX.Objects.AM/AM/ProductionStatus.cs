...
namespace PX.Objects.AM
{
    public class ProductionStatus
    {
        ...
        {
            return IsReleasedTransactionStatus(prodItem?.StatusID, prodItem?.Hold, prodItem?.Function);
        }

        public static bool IsReleasedTransactionStatus(string statusID, bool? hold, int? function)
        {
            if (function == null || string.IsNullOrWhiteSpace(statusID)
                || (function != OrderTypeFunction.Regular && function != OrderTypeFunction.Disassemble)
                || hold == true)
            {
                return false;
            }

            switch (statusID.TrimIfNotNullEmpty())
            {
                case ProductionOrderStatus.Released:
                case ProductionOrderStatus.InProcess:
                case ProductionOrderStatus.Completed:
                ...
                >.Select(graph, productionItem.OrderType, productionItem.ProdOrdID))
                {
                    ...
                    switch (newStatusID)
                    {
                        ...
                        case ProductionOrderStatus.InProcess:
                            break;
                        case ProductionOrderStatus.Completed:
                            productionOperation.ActEndDate = PX.Objects.AM.Common.Dates.Now;
                            break;
                    }
