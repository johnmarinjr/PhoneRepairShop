...
namespace PX.Objects.AM
{
    ...
    public class MRPBucketInq : PXGraph<MRPBucketInq>
    {
        ...
        protected virtual IEnumerable bucketDetailRecords()
        {
            ...
            AMMRPBucketInq bucketInq = PXSelectReadonly<AMMRPBucketInq,
            ...
            if (bucketInq == null)
            {
				BucketRecord.Cache.Clear();
				BucketDetailRecords.Cache.Clear();
				BucketRecord.Cache.ClearQueryCache();
				BucketDetailRecords.Cache.ClearQueryCache();
                FillBuckets();
            }
        ...
        protected static bool IsPlanType(AMRPPlan mrpPlan)
        {
            return mrpPlan.Type == MRPPlanningType.MPS
				   ...
				   || mrpPlan.Type == MRPPlanningType.ReorderPoint
        ...
        protected virtual void SetQuantities(ref AMMRPBucketDetailInq bucketDetailInq)
        {
            ...
            foreach (AMRPPlan mrpPlan in PXSelectReadonly<AMRPPlan,
            ...
            {
				var baseQty = Math.Abs(mrpPlan.BaseQty.GetValueOrDefault());
                if (mrpPlan.SDflag == MRPSDFlag.Supply)
                {
                    if (mrpPlan.IsPlan.GetValueOrDefault())
                    {
                        bucketDetailInq.PlannedSupply += baseQty;
                        continue;
                    }

                    bucketDetailInq.ActualSupply += baseQty;
                    continue;
                }

                if (mrpPlan.IsPlan.GetValueOrDefault())
                {
                    bucketDetailInq.PlannedDemand += baseQty;
                    continue;
                }

                bucketDetailInq.ActualDemand += baseQty;
            }
        ...
        [PXUIField(DisplayName = PX.Objects.IN.Messages.INItemSiteMaint, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton]
        ...
        [PXUIField(DisplayName = PX.Objects.IN.Messages.InventoryAllocDetEnq, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton]
        ...
        [PXUIField(DisplayName = PX.Objects.IN.Messages.InventoryTranDetEnq, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton]
