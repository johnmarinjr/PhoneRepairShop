...
namespace PX.Objects.AM
{
    ...
    public class ProductionScheduleEngine : PXGraph<ProductionScheduleEngine>
    {
		...
		//protected virtual DateTime? GetNextDateByTime(DateTime? currentDateTime, int minutes, CalendarHelper calendarHelper, out int timeConsumedForDate)
		//{
		//	return GetNextDateByTime(currentDateTime, Min, calendarHelper, false, out timeConsumedForDate);
		//}
            ...
            int totalSchdTime, ref DateTime currentDateTime, ref int usedTime, AMWC workCenter, AMShift amShift)
        {
			...
			var firstFloatTime = floatTimeBefore > 0;
            ...
            while (totalTime > 0 || firstFloatTime || finishTimeRemaining > 0)
            {
                cntr++;
				...
				if(totalTime <= 0 && finishTimeRemaining <= 0 && floatTimeAfter > 0)
				{
