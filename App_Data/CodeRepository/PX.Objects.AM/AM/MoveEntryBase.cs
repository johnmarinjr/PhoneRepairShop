...
namespace PX.Objects.AM
{
        ...
        where TWhere : class, IBqlWhere, new()
    {
        ...
        public override void InitCacheMapping(Dictionary<Type, Type> map)
        {
            ...
            this.Caches.AddCacheMapping(typeof(PMLotSerialStatus), typeof(PMLotSerialStatus));
        }
        ...
        protected virtual void ValidateLotSerialInformation()
        {
            ...
            var list = new List<ProdTranLotSerialTotals>();
            var prodList = new HashSet<string>();
            foreach(AMMTran tran in transactions.Select())
            {
				if (tran?.IsLotSerialPreassigned != true)
                {
					continue;
				}
				...
				foreach (AMMTranSplit split in splits.Cache.Cached.RowCast<AMMTranSplit>()
                    ...
                    {
                        var totalIdx = list.FindIndex(x => x.OrderType == tran.OrderType && x.ProdOrdID == tran.ProdOrdID && x.LotSerNbr == split.LotSerialNbr);
                        if(totalIdx != -1)
                        {
                            list[totalIdx].Qty += split.BaseQty ?? 0;
                        }
                        else
                        {
						list.Add(new ProdTranLotSerialTotals
						{
			...
			foreach (var total in list)
            {
                ...
                else if(itemSplit.QtyRemaining < total.Qty)
                {
            ...
            foreach(var key in prodList)
            {
                ...
                foreach(PXResult<AMProdItem, AMProdMatlLotSerial>result in results)
                {
                    ...
                    if(prod != null && prod.ParentLotSerialRequired == ParentLotSerialAssignment.OnCompletion 
                        && prodLotSer?.LotSerialNbr != null && string.IsNullOrWhiteSpace(prodLotSer.ParentLotSerialNbr))
                    {
        ...
        protected virtual void AMMTran_IsScrap_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
        {
			...
			if(row.IsScrap.GetValueOrDefault())
			{
				...
				qty = row.QtyScrapped.GetValueOrDefault();
			}

			row.SiteID = siteId;
			row.LocationID = locationId;
			sender.SetValueExt<AMMTran.qty>(row, qty);
