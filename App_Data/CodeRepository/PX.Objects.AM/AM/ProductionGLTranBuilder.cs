...
namespace PX.Objects.AM
{
    public class ProductionGLTranBuilder
    {
        ...
        public virtual void CreateAmGlLine(
            ...
            int? laborTime)
        {
            CreateAmGlLine(amProdItem, ammTran, tranAmount, offsetAccount, offsetSub, amTranType, costOperationId, transactionDescription, laborTime, null, null);
        }
		...
		{
            CreateAmGlLine(amProdItem, ammTran, tranAmount, offsetAccount, offsetSub, amTranType, costOperationId, transactionDescription, laborTime, referenceCostId, null);
		}
			...
			int? referenceLineId)
        {
            ...
            try
            {
                ...
                var tranAmountAdj = GetAdjustedTranAmount(ammTran, amTranType, referenceCostId.TrimIfNotNull(), costOperationId, tranAmount, referenceLineId);
                ...
                var ammTranGL = _productionCostEntryGraph.transactions.Insert(new AMMTran
                {
					...
					MatlLineId = referenceLineId // reusing field to link to prod cost type records (tool, overhead for example)
                });
		...
		[Obsolete ("Use FindOperWipCompleteRecords(AMMTran")]
		...
		protected virtual List<AMMTran> FindOperWipCompleteRecords(AMMTran sourceTran)
		{
			var ammTranlist = new List<AMMTran>();
			foreach (AMMTran wipComplete in _productionCostEntryGraph.transactions.Select())
			{
				if (wipComplete.OperationID == null)
				{
		...
		public virtual void SetOperationWipCompleteRecords(AMMTran sourceTran, ProductionOperationCostResults costResults)
		{
			...
			var existingWipCompleteRecords = FindOperWipCompleteRecords(sourceTran);
			...
			foreach (var costResult in costResults.OperationResults)
			{
				...
				AMMTran wipCompRow = null;

				foreach (var item in existingWipCompleteRecords.Where(x => x.OperationID == costResult.ProdOper.OperationID.GetValueOrDefault()))
				{
					if (wipCompRow == null)
					{
						wipCompRow = item;
					...
					_productionCostEntryGraph.transactions.Delete(item);
				}
				if (wipCompRow != null)
				{
					if (noWipComp)
		...
		{
			return GetAdjustedTranAmount(tran, tranType, referenceCostID, costOperationId, tranAmount, null);
		}

        internal decimal GetAdjustedTranAmount(AMMTran tran, string tranType, string referenceCostID, int? costOperationId, decimal? tranAmount, int? referenceLineID)
        {
            ...
            foreach (var previousTran in GetRelatedTrans(tran, tranType, costOperationId, referenceCostID, referenceLineID))
            {
        ...
        protected virtual int GetAdjustedLaborTime(AMMTran tran, string tranType, int? costOperationId, int? laborTime)
        {
            ...
            foreach (var previousTran in GetRelatedTrans(tran, tranType, costOperationId, null, null))
            {
        ...
        private IEnumerable<AMMTran> GetRelatedTrans(AMMTran tran, string tranType, int? costOperationId, string referenceCostID, int? referenceLineID)
        {
            ...
            if(!string.IsNullOrWhiteSpace(referenceCostID))
            {
				...
				{
					return results.Where(r => r.ReferenceCostID == referenceCostID && r.MatlLineId == referenceLineID);
				}

                return results.Where(r => r.ReferenceCostID == referenceCostID);
            }
