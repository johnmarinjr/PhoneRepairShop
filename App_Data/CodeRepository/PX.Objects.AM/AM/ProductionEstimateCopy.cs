...
namespace PX.Objects.AM
{
    ...
    public class ProductionEstimateCopy : ProductionBomCopy
    {
        ...
        protected virtual AMProdOper CopyEstimateOper(OperationDetail operationDetail)
        {
			...
			AMEstimateOper oper = AMEstimateOper.PK.Find(ProcessingGraph, operationDetail.BomID, operationDetail.BomRevisionID, operationDetail.BomOperationID);
            if (oper == null)
            {
                return null;
            }
            ...
            try
            {
				var operByCD = FindCachedOperByCD(CurrentProdItem?.OrderType, CurrentProdItem?.ProdOrdID, operationDetail.ProdOperationCD);
				if (operByCD?.OperationCD != null)
				{
					var cacheStatus = ProcessingGraph.Caches<AMProdOper>().GetStatus(operByCD);
					if (cacheStatus == PXEntryStatus.Deleted || cacheStatus == PXEntryStatus.InsertedDeleted)
					{
						// Rebuilding order
						ProcessingGraph.Caches<AMProdOper>().SetStatus(operByCD, cacheStatus == PXEntryStatus.InsertedDeleted ? PXEntryStatus.Inserted : PXEntryStatus.Updated);
					}
				}
				if (operByCD?.OperationCD == null)
				{
					operByCD = (AMProdOper)ProcessingGraph.Caches<AMProdOper>().Insert(new AMProdOper { OperationCD = operationDetail.ProdOperationCD });
				}

				var newProdOper = ProductionBomCopyMap.CopyOper(EstimateBomMap.CopyOperToBom(oper), operByCD);
