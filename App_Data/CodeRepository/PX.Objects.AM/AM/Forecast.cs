...
namespace PX.Objects.AM
{
    ...
    public class Forecast : PXGraph<Forecast>, PXImportAttribute.IPXPrepareItems
    {
        ...
        public bool PrepareImportRow(string viewName, IDictionary keys, IDictionary values)
        {
			...
			var interval = values[nameof(AMForecast.interval)] as string;
			if (!string.IsNullOrWhiteSpace(interval) && !ForecastInterval.IsIntervalKey(interval))
			{
				var newValue = ForecastInterval.ToIntervalKey(interval);
				if(string.IsNullOrWhiteSpace(newValue))
				{
					throw new PXException(Messages.InvalidIntervalImport, interval);
				}

				// we allow for correction to caps or import as single string key vs combo label
				values[nameof(AMForecast.interval)] = newValue;
			}
