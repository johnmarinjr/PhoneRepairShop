...
namespace PX.Objects.AM
{
    ...
    public static class PXExtensionMethods
    {
		...
		/// </summary>
		public static bool IsDeleted(this PXEntryStatus cacheStatus)
		{
			return cacheStatus == PXEntryStatus.Deleted || cacheStatus == PXEntryStatus.InsertedDeleted;
		}
		...
		public static void SetLocatedCacheCurrent<T>(this PXGraph graph, T row) where T : IBqlTable
		{
			if(row == null)
			{
				return;
			}
			graph.Caches[typeof(T)].Current = graph.Caches[typeof(T)].Locate(row);
		}
    }
}
