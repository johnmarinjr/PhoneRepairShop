...
namespace PX.Objects.PR
{
	public class PRPayPeriodMaint : PXGraph<PRPayPeriodMaint, PRPayGroupYear>
	{
		...
		public class SetupValidation : PRSetupValidation<PRPayPeriodMaint> { }
		...
		protected virtual void _(Events.RowSelected<PRPayGroupYear> e)
		{
			...
			if (row.OverrideFinPeriods == false && (row.FinPeriods == 27 || row.FinPeriods == 53))
			{
				PXUIFieldAttribute.SetWarning<PRPayGroupYear.finPeriods>(e.Cache, row, Messages.IrregularNbrPayPeriods);
			}
		...
		public virtual void _(Events.FieldUpdated<PRPayGroupYear, PRPayGroupYear.overrideFinPeriods> e)
		{
			...
			{
				e.Row.FinPeriods = (short?)Periods.Select().Count;
			}
		}
