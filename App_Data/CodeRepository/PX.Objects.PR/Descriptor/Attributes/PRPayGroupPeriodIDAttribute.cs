...
namespace PX.Objects.PR
{
	public class PRPayGroupPeriodIDAttribute : PeriodIDAttribute, IPXFieldVerifyingSubscriber, IPXFieldUpdatedSubscriber, IPXRowSelectedSubscriber
	{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			if (_SelAttrIndex != -1)
			{
				((PXSelectorAttribute)_Attributes[_SelAttrIndex]).SelectorMode |= PXSelectorMode.NoAutocomplete;
			}
		}
		...
		private class PXOrderedSelectorView : PXView
		{
			...
			public override List<object> Select(object[] currents, object[] parameters, object[] searches, string[] sortcolumns, bool[] descendings, PXFilterRow[] filters, ref int startRow, int maximumRows, ref int totalRows)
			{
				...
				if (searches != null)
				{
					searchesList.InsertRange(0, searches);
				}
				if (sortcolumns != null)
				{
					sortcolumnsList.InsertRange(0, sortcolumns);
				}
				if (descendings != null)
				{
					descendingsList.InsertRange(0, descendings);
				}
