	...
	public class PRValidatePaycheckTotals : PXGraph<PRValidatePaycheckTotals>
	{
		...
		public SelectFrom<PRPaymentTaxExt>
			.LeftJoin<PRTaxDetail>.On<PRTaxDetail.FK.Payment.SameAsCurrent
				.And<PRTaxDetail.taxID.IsEqual<PRPaymentTax.taxID>>>
			.Where<PRPaymentTax.FK.Payment.SameAsCurrent
				.And<PRPaymentTax.taxAmount.IsNotEqual<decimal0>>
				.And<PRTaxDetail.taxID.IsNull>>.View TaxSummariesWithDetailMissing;
		...
		public virtual void ValidateTotals(PRPayment payment, bool tryCorrectTotalsOnDiscrepancy)
		{
			...
			Document.Cache.SetStatus(Document.Current, PXEntryStatus.Held);
			...
			VerifySalariedOvertime();
			DoAdditionalValidations(tryCorrectTotalsOnDiscrepancy);
		...
		{
			Dictionary<int?, IEnumerable<PRPaymentTaxExt>> taxSummaries = SummaryTaxes.Select().FirstTableItems
				.GroupBy(x => x.TaxID)
				.ToDictionary(k => k.Key, v => v.AsEnumerable());
			foreach (IGrouping<int?, PRTaxDetailExt> taxDetails in TaxDetails.Select().FirstTableItems.GroupBy(x => x.TaxID))
			{
				...
				decimal taxSummaryAmount = 0;
				if (taxSummaries.ContainsKey(taxDetails.Key))
				{
					taxSummaryAmount = taxSummaries[taxDetails.Key].Sum(x => x.TaxAmount.GetValueOrDefault());
				}

				CheckValues(taxSummaryAmount, taxDetailAmount, Messages.TaxDetailsDontMatch, taxDetails.First().TaxCD, taxDetailAmount, taxSummaryAmount);
			}

			IEnumerable<PRPaymentTaxExt> summariesWithMissingDetail = TaxSummariesWithDetailMissing.Select().FirstTableItems;
			if (summariesWithMissingDetail.Any())
			{
				throw new PXException(Messages.TaxDetailMissing, summariesWithMissingDetail.First().TaxCD);
			}
		}
		...
		protected virtual void ValidateSummaryTaxes(bool tryCorrectTotalsOnDiscrepancy)
		{
			foreach (PRPaymentTaxExt summaryTax in SummaryTaxes.Select())
			{
				...
				PXFormulaAttribute.CalcAggregate<PRPaymentTaxSplit.wageBaseAmount>(TaxSplits.Cache, copy);
		...
		protected virtual void ValidateHeader(bool tryCorrectTotalsOnDiscrepancy)
		{
			...
			PXFormulaAttribute.CalcAggregate<PRPaymentTax.taxAmount>(SummaryTaxes.Cache, copy);
		...
		// No-op in base graph; intended as a way for graph extensions to plug into the paycheck validation
		protected virtual void DoAdditionalValidations(bool tryCorrectTotalsOnDiscrepancy) { }
		...
		public virtual void CheckValues(object val1, object val2, string errorMessage, params object[] errorMessageParams)
		{
			if (!Equals(val1, val2))
			{
		...
		protected virtual void VerifySalariedOvertime()
		{
			foreach (PREarningDetail earning in EarningDetails.Select())
			{
				if (EmployeeType.IsOvertimeEarningForSalariedExempt<PRPayment>(EarningDetails.Cache, earning))
				{
					throw new PXException(Messages.SalariedExemptWithOTReleaseException);
				}
			}
		}
