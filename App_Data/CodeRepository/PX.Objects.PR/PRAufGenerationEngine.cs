	...
	public class PRAufGenerationEngine : PXGraph<PRAufGenerationEngine>
	{
		...
		protected virtual IEnumerable<EmpRecord> CreateEmpRecords(DatRecord reportPeriod)
		{
			...
			foreach (PXResult<EPEmployee, Contact, Address, Country, PREmployeeDirectDeposit> employeeResult 
				in PeriodEmployees.Select(reportPeriod.FirstDate, reportPeriod.LastDate, reportPeriod.FirstDate, reportPeriod.LastDate, shouldGenerateAcaRecords))
			{
		...
		protected virtual IEnumerable<GenRecord> CreateGeneralEmployeeRecords(DatRecord reportPeriod, bool isWeeklyEmployee, bool exemptFromCertifiedReporting)
		{
			...
			foreach (var paymentResult in EmployeePayments
				...
				.OrderBy(x => Array.IndexOf(docTypes, x.Key.docType)))
			{
		...
		protected virtual void CreateTaxEsiRecords(ref Dictionary<int, EsiRecord> records)
		{
			...
			List<PXResult<PREmployeeAttribute, PRCompanyTaxAttribute>> employeeAttributes = EmployeeAttributes.Select().Select(x => (PXResult<PREmployeeAttribute, PRCompanyTaxAttribute>)x).ToList();
			foreach (IGrouping<int?, PXResult<PRPaymentTaxSplit, PRPaymentTax, PRTaxCode>> taxGroup in PaymentTaxSplits.Select()
				.Select(x => (PXResult<PRPaymentTaxSplit, PRPaymentTax, PRTaxCode>)x)
				.GroupBy(x => ((PRTaxCode)x).TaxID))
			{
				PRPaymentTax paymentTax = taxGroup.First();
				PRTaxCode tax = taxGroup.First();
		...
		{
			return CreateJobWeekRecords(general);
		}
		#endregion Avoid breaking chages in 2021R1

		#region Avoid breaking chages in 2022R1
		[Obsolete(Common.Messages.ItemIsObsoleteAndWillBeRemoved2022R2)]
		protected virtual IEnumerable<EjwRecord> CreateJobWeekRecords(DateTime periodStart)
		{
			var payment = (PRPayment)PXResult<PRPayment>.Current;
			var general = new GenRecord(payment.TransactionDate.Value, payment.DocType == PayrollType.Regular);

			return CreateJobWeekRecords(general);
		}
		#endregion Avoid breaking chages in 2022R1

		#region Avoid breaking chages in 2022R2
		[Obsolete(Common.Messages.ItemIsObsoleteAndWillBeRemoved2022R2)]
		protected virtual IEnumerable<EjwRecord> CreateJobWeekRecords(DateTime periodStart, IEnumerable<PXResult<PRPayment, PRPaymentTaxSplit, PRTaxCode>> taxSplits)
		{
			var payment = (PRPayment)PXResult<PRPayment>.Current;
			var general = new GenRecord(payment.TransactionDate.Value, payment.DocType == PayrollType.Regular);
		...
		#endregion Avoid breaking chages in 2022R2
	}
}
