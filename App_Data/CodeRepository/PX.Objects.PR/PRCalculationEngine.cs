...
namespace PX.Objects.PR
{
	...
	public partial class PRCalculationEngine : PXGraph<PRCalculationEngine>
	{
		...
		public SelectFrom<PRTaxCode>
			...
			.Where<PRTaxCode.countryID.IsEqual<PRPayment.countryID.FromCurrent>>.View DeductionTaxList;
		...
		public SelectFrom<PRPayGroupPeriod>
			.Where<PRPayGroupPeriod.payGroupID.IsEqual<P.AsString>
				...
				.And<PRPayGroupPeriod.transactionDate.IsLess<P.AsDateTime>>>
		...
		protected virtual void CalculatePTO(PREarningDetail earningDetail)
		{
			foreach (IGrouping<string, PRPaymentPTOBank> bankGroup in PaymentPTOBanks.Select().FirstTableItems.GroupBy(x => x.BankID))
			{
				...
				if (effectiveBank.IsCertifiedJob == true && effectiveBank.IsCertifiedJob != earningDetail.CertifiedJob)
				{
					continue;
				}
				...
				decimal newAccrualHours = effectiveBank.AccrualAmount.GetValueOrDefault();
				decimal newAccrualMoney = effectiveBank.AccrualMoney.GetValueOrDefault();
				decimal newDisbursementHours = effectiveBank.DisbursementAmount.GetValueOrDefault();
				...
				if (Payments.Current.DocType == PayrollType.Regular)
				{
					...
					if (yearSummary.ProcessedCarryover == false)
					{
						...
						newAccrualHours += effectiveBank.CarryoverAmount.GetValueOrDefault();
						newAccrualMoney += effectiveBank.CarryoverMoney.GetValueOrDefault();
					}
					if (yearSummary.ProcessedFrontLoading == false)
					{
						...
						newAccrualHours += effectiveBank.FrontLoadingAmount.GetValueOrDefault();
					}
					if (earningDetail.IsPayingCarryover == true && earningDetail.TypeCD == effectiveBank.EarningTypeCD)
					{
						...
						newDisbursementHours += effectiveBank.PaidCarryoverAmount.GetValueOrDefault();
					}
				}
				...
				if (earningDetail.TypeCD == effectiveBank.EarningTypeCD && earningDetail.IsPayingCarryover == false)
				{
					...
					newDisbursementHours += effectiveBank.DisbursementAmount.GetValueOrDefault();
					newDisbursementMoney += effectiveBank.DisbursementMoney.GetValueOrDefault();

					if (sourceBank.DisburseFromCarryover == true && newDisbursementHours > history.AvailableHours + effectiveBank.CarryoverAmount)
					{
					...
					else if (sourceBank.AllowNegativeBalance == false && newDisbursementHours > effectiveAvailableHours)
					{
				...
				if (Payments.Current.DocType == PayrollType.Regular || Payments.Current.DocType == PayrollType.Special)
				{
					PTOHelper.PTOHistoricalAmounts history = PTOHelper.GetPTOHistory(this, earningDetail.Date.Value, Payments.Current.EmployeeID.Value, sourceBank);
					decimal bankFromSamePTOYearTotal = bankGroup.Except(effectiveBank).Where(x => yearSummary.StartDate < x.EffectiveEndDate).Sum(x => x.TotalAccrual.GetValueOrDefault() - x.TotalDisbursement.GetValueOrDefault());
					decimal? totalDisbursement = (decimal?)PXFieldState.UnwrapValue(PaymentPTOBanks.GetValueExt<PRPaymentPTOBank.totalDisbursement>(effectiveBank));
					decimal accumulation = newAccrualHours + history.AvailableHours - totalDisbursement.GetValueOrDefault() + bankFromSamePTOYearTotal;
					if (sourceBank.AccrualLimit != null && accumulation > sourceBank.AccrualLimit)
					{
						decimal diff = sourceBank.AccrualLimit.GetValueOrDefault() + totalDisbursement.GetValueOrDefault() - history.AvailableHours
							- effectiveBank.FrontLoadingAmount.GetValueOrDefault() - effectiveBank.CarryoverAmount.GetValueOrDefault() - bankFromSamePTOYearTotal;
						decimal limitedAccrualHours = Math.Max(diff, 0m);
		...
		protected virtual void CalculateAmountPerYearPTO()
		{
			if (Payments.Current.DocType == PayrollType.Regular)
			{
				...
				IEnumerable<PRPaymentPTOBank> ptoBanks = PaymentPTOBanks.Select().FirstTableItems.Where(x => x.AccrualMethod == PTOAccrualMethod.TotalHoursPerYear);
				foreach (PRPaymentPTOBank bank in ptoBanks)
				{
					...
					decimal newAccrualHours = bank.AccrualAmount.GetValueOrDefault();
					PTOHelper.PTOYearSummary yearSummary = GetPTOYearAndPaymentSummary(this, bank.EffectiveStartDate.Value, Payments.Current.EmployeeID.Value, sourceBank);
					if (yearSummary.ProcessedCarryover == false)
					{
						bank.CarryoverAmount += PTOHelper.CalculateHoursToCarryover(this, Payments.Current.EmployeeID, Payments.Current, sourceBank, yearSummary.StartDate, yearSummary.EndDate);
						bank.CarryoverMoney += PTOHelper.CalculateMoneyToCarryover(this, Payments.Current.EmployeeID, Payments.Current, sourceBank, yearSummary.StartDate, yearSummary.EndDate);
						bank.ProcessedCarryover = true;
						newAccrualHours += bank.CarryoverAmount.GetValueOrDefault();
					}
					if (yearSummary.ProcessedFrontLoading == false)
					{
						bank.FrontLoadingAmount += sourceBank.FrontLoadingAmount.GetValueOrDefault();
						bank.ProcessedFrontLoading = true;
						newAccrualHours += bank.FrontLoadingAmount.GetValueOrDefault();
					}

					PTOHelper.PTOHistoricalAmounts history = PTOHelper.GetPTOHistory(this, bank.EffectiveStartDate.Value, Payments.Current.EmployeeID.Value, sourceBank);
					decimal bankFromSamePTOYearTotal = ptoBanks.Except(bank).Where(x => yearSummary.StartDate < x.EffectiveEndDate).Sum(x => x.TotalAccrual.GetValueOrDefault() - x.TotalDisbursement.GetValueOrDefault());
					decimal? totalDisbursement = (decimal?)PXFieldState.UnwrapValue(PaymentPTOBanks.GetValueExt<PRPaymentPTOBank.totalDisbursement>(bank));
					decimal accumulation = newAccrualHours + history.AvailableHours - totalDisbursement.GetValueOrDefault() + bankFromSamePTOYearTotal;
					if (sourceBank.AccrualLimit != null && accumulation > sourceBank.AccrualLimit)
					{
						decimal diff = sourceBank.AccrualLimit.GetValueOrDefault() + totalDisbursement.GetValueOrDefault() - history.AvailableHours
							- bank.FrontLoadingAmount.GetValueOrDefault() - bank.CarryoverAmount.GetValueOrDefault() - bankFromSamePTOYearTotal;
						decimal limitedAccrualHours = Math.Max(diff, 0m);
						decimal limitedAccrualFactor = bank.AccrualAmount != 0 ? limitedAccrualHours / bank.AccrualAmount.Value : 0;
						bank.AccrualAmount = limitedAccrualHours;
						bank.AccrualMoney = Math.Round(bank.AccrualMoney.GetValueOrDefault() * limitedAccrualFactor, 2, MidpointRounding.AwayFromZero);
					}
