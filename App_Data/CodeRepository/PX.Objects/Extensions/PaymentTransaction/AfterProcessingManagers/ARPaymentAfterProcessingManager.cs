...
namespace PX.Objects.Extensions.PaymentTransaction
{
	public class ARPaymentAfterProcessingManager : AfterProcessingManager
	{
		...
		{
			bool ret = true;
			var paymentGraph = CreateGraphIfNeeded(table);
			...
			{
				extTran = paymentGraph.ExternalTran.Select().RowCast<ExternalTransaction>()
					.Where(i=>i.TransactionID == doc.CCActualExternalTransactionID).FirstOrDefault();
			}
			else
			{
				extTran = GetLastProcessedExternalTran(paymentGraph);
			}

			if (extTran != null && extTran.NeedSync == false)
			{
				var state = ExternalTranHelper.GetTransactionState(paymentGraph, extTran);
				if ((state.IsPreAuthorized && !state.IsOpenForReview && doc.IsCCAuthorized == false)
					|| ((state.IsPreAuthorized || state.IsCaptured) && state.IsOpenForReview && doc.IsCCUserAttention == false)
					|| ((state.IsCaptured || state.IsRefunded) && !state.IsOpenForReview && doc.PendingProcessing == true)
					|| (state.IsVoided && (doc.IsCCCaptured == true || doc.IsCCAuthorized == true)))
				{
					ret = false;
				}
			}
		...
		{
			PXResultset<SOAdjust> soAdjusts = graph.GetOrdersToApplyTabExtension()?.SOAdjustments.Select();
			...
			if (soAdjust != null)
			{
				...
				if(soAdjust.CuryAdjgAmt > extTran.Amount)
				{
					...
					graph.GetOrdersToApplyTabExtension(true).SOAdjustments.Update(soAdjust);
					soAdjUpdated = true;
				}
			}
		...
		{
			return doc.Released == false && doc.Hold == false && CCProcessingHelper.IntegratedProcessingActivated(Graph.arsetup.Current);
		}
			...
			if (doc != null && success)
			{
				...
				if (tran != null)
				{
					...
					if (!state.IsDeclined && !state.IsOpenForReview && !state.IsExpired && !state.SyncFailed && !state.NeedSync)
					{
						...
						PXLongOperation.SetCustomInfo(new ClearTransactionCache());
					}
				}
			}
		}
		...
		private bool CreateVoidDocument(ARPaymentEntry graph, bool success)
		{
			...
			if (state.IsVoided && voidedDoc.DocType != extTran.DocType)
			{
				UpdateVoidDocTypeRefNbr(newGraph, extTran);
			}
		...
		private void UpdateVoidDocTypeRefNbr(ARPaymentEntry newGraph, ExternalTransaction extTran)
		{
			var voidedExtTran = newGraph.ExternalTran.Select().RowCast<ExternalTransaction>()
				.Where(i => i.TransactionID == extTran.TransactionID).FirstOrDefault();
			var voidedProcTran = newGraph.ccProcTran.Select().RowCast<CCProcTran>()
				.Where(i => i.TransactionID == extTran.TransactionID).FirstOrDefault();
			var voidedDoc = newGraph.Document.Current;
			if (voidedExtTran != null && voidedProcTran != null)
			{
				voidedExtTran.VoidDocType = voidedDoc.DocType;
				voidedExtTran.VoidRefNbr = voidedDoc.RefNbr;
				voidedProcTran.DocType = voidedDoc.DocType;
				voidedProcTran.RefNbr = voidedDoc.RefNbr;
				newGraph.ExternalTran.Update(voidedExtTran);
				newGraph.ccProcTran.Update(voidedProcTran);
			}
		}
		...
		private void ChangeOriginDocProcessingStatus(ARPaymentEntry paymentGraph, CCTranType tranType, bool success)
		{
			...
			if (oPaymentGraph != null)
			{
				...
				if (oExtTran.TransactionID == tran.TransactionID)
				{
					...
					if (success)
					{
						if (oPayment.Released == true)
						{
							oPayment.IsCCUserAttention = false;
						}
		...
		private void DeactivateExpiredTrans(ARPaymentEntry graph)
		{
			...
			{
				var state = ExternalTranHelper.GetTransactionState(graph, item);
				item.Active = false;
				item.ProcStatus = ExtTransactionProcStatusCode.GetProcStatusStrByProcessingStatus(state.ProcessingStatus);
		...
		private ARPaymentEntry GetGraphWithOriginDoc(ARPaymentEntry graph, CCTranType tranType)
		{
			...
			ARPayment payment = graph.Document.Current;
			if (tran != null && tranType == CCTranType.VoidOrCredit
		...
		private ExternalTransaction GetLastProcessedExternalTran(ARPaymentEntry graph)
		{
			...
			if (extTrans.Count() == 0) return null;

			var procTrans = graph.ccProcTran.Select().RowCast<CCProcTran>().Where(i => i.ProcStatus != CCProcStatus.Opened);
			IExternalTransaction extTran = ExternalTranHelper.GetLastProcessedExtTran(extTrans, procTrans);
			if (extTran == null) return null;

			var retTran = extTrans.Where(i => i.TransactionID == extTran.TransactionID).FirstOrDefault();
			return retTran;
		}
