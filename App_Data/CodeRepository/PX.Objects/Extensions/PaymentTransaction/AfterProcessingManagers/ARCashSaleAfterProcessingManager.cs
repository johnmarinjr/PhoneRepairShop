...
namespace PX.Objects.Extensions.PaymentTransaction
{
	public class ARCashSaleAfterProcessingManager : AfterProcessingManager
	{
		...
		{
			bool ret = true;
			var cashSaleGraph = CreateGraphIfNeeded(table);
			var doc = cashSaleGraph.Document.Current;
			if (doc == null) return ret;

			ExternalTransaction extTran = null;

			var trans = cashSaleGraph.ExternalTran.Select().RowCast<ExternalTransaction>();
			if (doc.CCActualExternalTransactionID != null)
			{
				extTran = trans.Where(i => i.TransactionID == doc.CCActualExternalTransactionID).FirstOrDefault();
			}
			else
			{
				extTran = trans.FirstOrDefault();
			}

			if (extTran != null && extTran.NeedSync == false)
			{
				var state = ExternalTranHelper.GetTransactionState(cashSaleGraph, extTran);
		...
		{
			return ReleaseDoc && cashSale.Released == false && CCProcessingHelper.IntegratedProcessingActivated(Graph.arsetup.Current);
		}
