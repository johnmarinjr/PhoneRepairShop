...
namespace PX.Objects.Extensions.SalesTax
{
         ...
         where TPrimary : class, IBqlTable, new()
    {
        ...
        public void Calculate(PXCache sender, PXRowInsertedEventArgs e)
        {
            ...
            {
				if (inserted.TryGetValue(e.Row, out object copy))
				{
        ...
        public void Calculate(PXCache sender, PXRowUpdatedEventArgs e)
        {
            ...
            if (doc.TaxCalc == TaxCalc.ManualCalc)
            {
				if (updated.TryGetValue(e.Row, out object copy))
				{
					Detail_RowUpdated(sender, new PXRowUpdatedEventArgs(e.Row, copy, false));
					updated.Remove(e.Row);
				}
        ...
        private void TaxSetLineDefault(object taxrow, Detail row)
        {
			...
			DiscPercentsDict.TryGetValue(CurrentDocument, out decimal? DiscPercent);
                    ...
                    || tax.TaxCalcLevel == CSTaxCalcLevel.CalcOnItemAmtPlusTaxAmt))
            {
                ...
                Base.AdjustMinMaxTaxableAmt(taxrev, ref CuryTaxableAmt, ref TaxableAmt);
            ...
            decimal CuryTaxAmtRounded = currencyInfo.RoundCury(CuryTaxAmt);

            if (tax.DeductibleVAT == true)
			{
                taxdet.SetExpenseAmountsForDeductibleVAT(taxrev, CuryTaxAmt, currencyInfo);
				CuryTaxAmtRounded -= (decimal)taxdet.CuryExpenseAmt;
			}

			if (!isExemptTaxCategory)
            {
                SetTaxDetailTaxAmount(cache, taxdet, CuryTaxAmtRounded);
            }
        ...
        protected virtual void _(Events.RowUpdated<Document> e)
        {
			...
			OrigDiscAmtExtCallDict.TryGetValue(e.Row, out bool externallCall);
			if (!externallCall)
                return;
        ...
        {
            if (CurrentDocument?.TaxCalc == TaxCalc.Calc || CurrentDocument?.TaxCalc == TaxCalc.ManualLineCalc)
            {
                ...
                {
                    if (DocumentDetailsView?.SelectSingle() != null)
                    {
                        CalcTaxes(null);
                    }
                }
        ...
        protected string GetTaxCalcMode()
        {
            ...
            var mode = (string)ParentGetValue<Document.taxCalcMode>();
