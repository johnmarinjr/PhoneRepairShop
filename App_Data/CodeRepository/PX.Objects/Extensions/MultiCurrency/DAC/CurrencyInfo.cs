...
namespace PX.Objects.CM.Extensions
{
	...
	public partial class CurrencyInfo : PX.Data.IBqlTable
	{
		...
		protected class CuryIDSelectorAttribute : PXCustomSelectorAttribute
		{
			...
			{
				if (attributeLevel == PXAttributeLevel.Item) return this;
				else return base.Clone(attributeLevel);
			}
		...
		//Calculation methods are located here in order to prevent growth of member methods of IPXCurrencyHelper

		// Acuminator disable once PX1031 InstanceMethodInDac 
		private decimal GetRate()
		{
			if (CuryRate == null) throw new PXRateNotFoundException();
			else if (CuryRate == 0m) return 1;
			else return CuryRate.Value;
		}

		// Acuminator disable once PX1031 InstanceMethodInDac 
		public decimal RoundCury(decimal val) => Math.Round(val, (int)CuryPrecision, MidpointRounding.AwayFromZero);

		// Acuminator disable once PX1031 InstanceMethodInDac 
		public decimal RoundBase(decimal val) => Math.Round(val, (int)BasePrecision, MidpointRounding.AwayFromZero);

		// Acuminator disable once PX1031 InstanceMethodInDac 
		public decimal CuryConvBaseRaw(decimal curyval)
		{
			decimal rate = GetRate();
			bool mult = CuryMultDiv != "D";
			return mult ? curyval * rate : curyval / rate;
		}

		// Acuminator disable once PX1031 InstanceMethodInDac 
		public decimal CuryConvBase(decimal curyval, int? precision)
		{
			decimal baseval = CuryConvBaseRaw(curyval);
			if (precision == null) return baseval;
			else return Math.Round(baseval, (int)precision, MidpointRounding.AwayFromZero);
		}

		// Acuminator disable once PX1031 InstanceMethodInDac 
		public decimal CuryConvBase(decimal curyval) => CuryConvBase(curyval, BasePrecision);
		...
		public decimal CuryConvCuryRaw(decimal baseval)
		{
			decimal rate = GetRate();
			bool mult = CuryMultDiv == "D";
			return mult ? baseval * rate : baseval / rate;
		}

		// Acuminator disable once PX1031 InstanceMethodInDac 
		public decimal CuryConvCury(decimal baseval, int? precision)
		{
			decimal curyval = CuryConvCuryRaw(baseval);
			if (precision == null) return curyval;
			else return Math.Round(curyval, (int)precision, MidpointRounding.AwayFromZero);
		}

		// Acuminator disable once PX1031 InstanceMethodInDac 
		public decimal CuryConvCury(decimal baseval) => CuryConvCury(baseval, CuryPrecision);
