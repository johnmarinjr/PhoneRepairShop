...
namespace PX.Objects.AR
{
	...
	public class ARTaxAttribute : TaxAttribute
	{
		...
		public override object Insert(PXCache cache, object item)
		{
			...
			PXRowInserted taxInsertedHandler = delegate (PXCache sender, PXRowInsertedEventArgs e)
			{
				...
				PXSelect<ARTax,
					Where<ARTax.tranType, Equal<Current<ARRegister.docType>>,
						And<ARTax.refNbr, Equal<Current<ARRegister.refNbr>>>>>
					.StoreCached(cache.Graph, new PXCommandKey(new object[] { taxTranType, taxRefNbr }), taxLinesList);
					//.StoreResult(sender.Graph, taxLinesList, PXQueryParameters.ExplicitParameters(taxTranType, taxRefNbr));
			};
		...
		public override object Update(PXCache cache, object item)
		{
			...
			PXRowUpdated taxUpdatedHandler = delegate (PXCache sender, PXRowUpdatedEventArgs e)
			{
				var comparer = sender.GetComparer();
				int ndx = taxLinesList.FindIndex(t => comparer.Equals(t, e.OldRow));
				if (ndx >= 0)
				{
					taxLinesList[ndx] = e.Row;
				}
			...
			cache.Graph.RowUpdated.AddHandler<ARTax>(taxUpdatedHandler);
			...
			{
				cache.Graph.RowUpdated.RemoveHandler<ARTax>(taxUpdatedHandler);
			}
		}
		...
		{
			Currency currency = MultiCurrencyCalculator.GetCurrentCurrency(graph);
			...
			{
				return currency.ARInvoiceRounding != RoundingType.Currency;
			}
			else
			{
				return ((ARSetup)graph.Caches<ARSetup>().Current).InvoiceRounding != RoundingType.Currency;
			}
		}
		...
		protected virtual decimal? RoundAmount(PXGraph graph, decimal? value)
		{
			...
			{
				return ARReleaseProcess.RoundAmount(value, currency.ARInvoiceRounding, currency.ARInvoicePrecision);
			}
			else
			{
				PXCache setupCache = graph.Caches[typeof(ARSetup)];
				ARSetup setup = (ARSetup)setupCache.Current;
				return ARReleaseProcess.RoundAmount(value, setup.InvoiceRounding, setup.InvoicePrecision);
			}
		}
		...
		protected override void ParentSetValue(PXGraph graph, string fieldname, object value)
		{
			...
			if (fieldname == _CuryDocBal && cache.Current != null && IsRoundingNeeded(graph))
			{
				...
				MultiCurrencyCalculator.CuryConvBase(ParentCache(graph), ParentRow(graph), (decimal)oldval, out decimal oldbaseval);
				MultiCurrencyCalculator.CuryConvBase(ParentCache(graph), ParentRow(graph), (decimal)value, out decimal baseval);
		...
		protected override List<object> SelectTaxes<Where>(PXGraph graph, object row, PXTaxCheck taxchk, params object[] parameters)
		{
			...
			switch (taxchk)
			{
				case PXTaxCheck.Line:
					...
					tails = CollectInvolvedTaxes<Where>(graph, taxDetails, selectTaxes, currents, parameters, selectParameters);
					...
					foreach (ARTax record in taxDetails)
					{
				...
				case PXTaxCheck.RecalcTotals:
					...
					foreach (ARTaxTran record in taxDetails)
					{
						InsertTax(graph, taxchk, record, tails, ret);
					}
					return ret;
	...
	public class ARCashTranIDAttribute : CashTranIDAttribute
	{
		...
		public static CATran DefaultValues(PXCache sender, CATran catran_Row, ARPayment orig_Row)
		{
			...
			catran_Row.CuryTranAmt = (ARPaymentType.DrCr(parentDoc.DocType) == DrCr.Credit ? -1 : 1) * parentDoc.CuryOrigDocAmt - parentDoc.CuryConsolidateChargeTotal;
	...
	public class ARCashSaleCashTranIDAttribute : CashTranIDAttribute
	{
		...
		public override CATran DefaultValues(PXCache sender, CATran catran_Row, object orig_Row)
		{
			...
			{
				case ARDocType.CashSale:
					catran_Row.CuryTranAmt = parentDoc.CuryOrigDocAmt-parentDoc.CuryConsolidateChargeTotal;
					catran_Row.DrCr = DrCr.Debit;
					break;
	...
	public class PaymentRefAttribute : PXEventSubscriberAttribute, IPXRowPersistingSubscriber, IPXFieldDefaultingSubscriber, IPXFieldVerifyingSubscriber
	{
		...
		public virtual void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (det != null && det.ARAutoNextNbr == true)
			{
				...
				bool? isMigratedRecord = _IsMigratedRecord == null ? null : (bool?)sender.GetValue(e.Row, _IsMigratedRecord.Name);
