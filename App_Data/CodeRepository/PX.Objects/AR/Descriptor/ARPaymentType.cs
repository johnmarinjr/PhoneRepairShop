	...
	public class ARPaymentType : ARDocType
	{
		...
		public new class SOListAttribute : PXStringListAttribute
		{
			public SOListAttribute()
				: base(
				new string[] { Payment, CreditMemo, Prepayment, Refund },
				new string[] { Messages.Payment, Messages.CreditMemo, Messages.Prepayment, Messages.Refund })
			{ }

			public static void SetPaymentList<Field>(PXCache cache, bool refund)
				where Field : IBqlField
			{
				SetList<Field>(cache, null,
					refund ? new string[] { Refund } : new string[] { Payment, CreditMemo, Prepayment },
					refund ? new string[] { Messages.Refund } : new string[] { Messages.Payment, Messages.CreditMemo, Messages.Prepayment });
			}
		}
