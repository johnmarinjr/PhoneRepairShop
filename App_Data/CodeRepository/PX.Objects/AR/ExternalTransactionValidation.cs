	...
	public class ExternalTransactionValidation : PXGraph<ExternalTransactionValidation>
	{
		...
		public PXFilteredProcessing<ExternalTransaction, ExternalTransactionFilter,
						Where<ExternalTransaction.refNbr, IsNotNull,
							And<ExternalTransaction.docType, In3<ARDocType.payment, ARDocType.prepayment, ARDocType.refund, ARDocType.cashSale>,
		...
		public static void ValidateCCPayment(PXGraph graph, List<IExternalTransaction> list, bool isMassProcess)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				foreach (PXResult<ExternalTransaction, Standalone.ARRegister, ARPayment, ARPaymentByVoidLink, Standalone.ARCashSale> item in resultSet)
				{
					if (item == null)
						continue;
					...
					try
					{
							...
							&& arCashSale != null && arCashSale.RefNbr != null)
						{
							...
							ARCashSaleEntryPaymentTransaction ext = arCashSaleGraph.GetExtension<ARCashSaleEntryPaymentTransaction>();
									...
									&& arPayment != null && arPayment.RefNbr != null)
						{
							...
							ARPaymentEntryPaymentTransaction ext = arPaymentGraph.GetExtension<ARPaymentEntryPaymentTransaction>();
							...
							if (docByVoidLink?.RefNbr != null && ext.CanValidate())
							{
								ext.validateCCPayment.Press();
							}
							else
			...
			if (failed)
			{
				throw new PXOperationCompletedWithErrorException(Messages.DocumentsNotValidated);
			}
		}
		#endregion
