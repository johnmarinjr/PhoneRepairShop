	...
	public class ARInvoiceEntryExternalTax : ExternalTax<ARInvoiceEntry, ARInvoice>
	{
		...
		public virtual CommitTaxRequest BuildCommitTaxRequest(ARInvoice invoice)
		{
			...
			foreach (PXResult<ARTran, InventoryItem, Account> res in select.View.SelectMultiBound(new object[] { invoice }))
			{
				...
				line.Quantity = tran.LineType == SOLineType.Freight && tran.TranAmt != 0m && tran.Qty == 0m
								? 1m
		...
		private IAddressLocation GetDropShipOriginAddress(ARTran tran, SOLine soLine)
		{
				...
				&& soLine.POCreate == true && soLine.POSource == INReplenishmentSource.DropShipToOrder)
			{
				...
				Address vendorAddress = PXSelectJoin<Address,
						InnerJoin<Location, On<Location.defAddressID, Equal<Address.addressID>>,
						InnerJoin<PO.POReceipt,
							On<PO.POReceipt.receiptType, Equal<Required<PO.POReceipt.receiptType>>,
							And<PO.POReceipt.receiptNbr, Equal<Required<PO.POReceipt.receiptNbr>>,
							And<PO.POReceipt.vendorLocationID, Equal<Location.locationID>>>>>>>
						.Select(Base, receiptType, receiptNbr);
		...
		private IAddressLocation GetDropShipDestinationAddress(ARTran tran, SOLine soLine)
		{
			if (soLine != null && tran.SOOrderLineNbr != null
				&& soLine.POCreate == true && soLine.POSource == INReplenishmentSource.DropShipToOrder)
			{
