...
namespace PX.Objects.AR
{
	public class CustomerPaymentMethodMaint : PXGraph<CustomerPaymentMethodMaint, CustomerPaymentMethod>
	{
		...
		protected virtual void CustomerPaymentMethod_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetVisible<CustomerPaymentMethod.expirationDate>(cache, row, isExpirationDateVisible);
			...
			if (!string.IsNullOrEmpty(row.PaymentMethodID))
			{
				...
				PXUIFieldAttribute.SetVisible<CustomerPaymentMethod.cCProcessingCenterID>(cache, row, integratedProcessing || row.CCProcessingCenterID != null);
				if (!string.IsNullOrEmpty(row.CCProcessingCenterID))
				{
					bool visibleCustId = isTokenizedMethodActive || row.CustomerCCPID != null;
			...
			if (def != null)
			{
				if (def.IsIdentifier == true)
				{
					...
					if (!this.CustomerPaymentMethod.Current.Descr.Contains(id))
					{
						...
						this.CustomerPaymentMethod.Cache.SetValueExt<CustomerPaymentMethod.descr>(parent, FormatDescription(parent.CardType, id));
