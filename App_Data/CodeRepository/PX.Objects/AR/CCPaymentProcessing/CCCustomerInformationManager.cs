	...
	public class CCCustomerInformationManager
	{
			...
			)
		{
			...
			while ((attempt <= (procCenter.SyncRetryAttemptsNo ?? 0) + 1) && newCard == null)
			{
				...
				if (newCard != null)
				{
					SetPaymentDetailsFromCreditCardData(newCard, payment, paymentDetail, cim);
				}
				attempt++;
			}
			if (newCard == null)
		...
		private static void SetPaymentDetailsFromCreditCardData(CreditCardData newCard, ICCPaymentProfileAdapter payment, ICCPaymentProfileDetailAdapter paymentDetail, CCCustomerInformationManager cim)
		{
			...
			{
				string maskedCardNum = CCProcessingHelper.FormatMaskedCardNumber(newCard.CardNumber, GetDisplayCardTypeName(newCard));
				...
				, ICCPaymentProfileDetailAdapter paymentDetail)
		{
			...
			if ((isIDFilled || isOtherDetsFilled) && !isHF || tryGetProfile)
			{
				...
				if (cardData != null && !string.IsNullOrEmpty(cardData.PaymentProfileID))
				{
					...
					{
						string maskedCardNum = CCProcessingHelper.FormatMaskedCardNumber(cardData.CardNumber, GetDisplayCardTypeName(cardData));
		...
		public static TranProfile GetOrCreatePaymentProfileByTran(PXGraph graph, ICCPaymentProfileAdapter adapter, string tranId)
		{
			...
			{
				ret = cim.GetOrCreateCustomerProfileFromTransaction(tranId, new CreateTranPaymentProfileParams() { LocalCustomerId = customer.AcctCD.Trim(), Description = customer.AcctName});
				return ret;
			}
			var cardLimit = processingCenter.CreditCardLimit;
				int customerProfileCount = CCProcessingHelper.CustomerProfileCountPerCustomer(graph,
			...
			if (processingCenter.CreateAdditionalCustomerProfiles == true && cardLimit != null && cardLimit > 0)
			{
				...
				{
					ret = cim.GetOrCreateCustomerProfileFromTransaction(tranId, new CreateTranPaymentProfileParams() { LocalCustomerId = prefix + customer.AcctCD.Trim(), Description = customer.AcctName });
				}
				else
				{
