...
namespace PX.Objects.AR.CCPaymentProcessing.Helpers
{
	public static class TranValidationHelper
	{
		...
		public static void CheckActiveTransactionStateForPayment(Common.ICCPayment checkedPayment, TransactionData tranData, ExternalTransactionState activeState)
		{
			...
			string docTypeRefNbr = checkedPayment.DocType + checkedPayment.RefNbr;
			var docLabel = TranValidationHelper.GetDocumentName(checkedPayment.DocType);
			...
			if (activeTran == null && tranData.TranType == CCTranType.Void)
			{
				throw new TranValidationException(Messages.ERR_CCNoSuccessfulTransactionToVoid, tranData.TranID, checkedPayment.RefNbr, docLabel);
			}
		...
		public static void CheckSharedTranIsSuitableForRefund(Common.ICCPayment checkedPayment, TransactionData tranData, AdditionalParams prms)
		{
			...
			if (tranNumber == null)
			{
				throw new TranValidationException(Messages.ERR_VoidTranNotLinkedOrigTran, tranData.TranID, checkedPayment.RefTranExtNbr);
			}
		...
		public static void CheckPaymentProfile(TransactionData tranData, AdditionalParams prms)
		{
			...
			if (prms.PMInstanceId > 0)
			{
				...
				else
				{
					...
					CheckCustomer(tranData, prms.CustomerID, res.Item1);
				}
			}
			else
			{
