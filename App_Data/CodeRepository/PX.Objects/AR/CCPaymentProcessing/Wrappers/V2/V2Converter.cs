	...
	public class V2Converter
	{
		...
		static V2Converter()
		{
			MapperConfiguration mapperConf = new MapperConfiguration(cfg =>
			{
				...
				cfg.CreateMap<CCCardType, V2.CCCardType>()
					.AfterMap((i, v2) => {
						if (!Enum.IsDefined(v2.GetType(), v2))
						{
							throw new PXException(CCProcessingBase.Messages.UnexpectedCardType, i);
						}
					});
				cfg.CreateMap<V2.CCCardType, CCCardType>()
				.AfterMap((v2, o) => {
					if (!Enum.IsDefined(o.GetType(), o))
					{
						throw new PXException(CCProcessingBase.Messages.UnexpectedCardType, v2);
					}
				});
			});
		...
		{
			return Mapper.Map<CCCardType>(v2TranType);
		}
