...
namespace PX.Objects.AR.CCPaymentProcessing.Wrappers
{
	public class HostedFromProcessingWrapper
	{
		...
		private class V2HostedFormProcessor : V2ProcessorBase, IHostedFromProcessingWrapper, IHostedPaymentFormProcessingWrapper
		{
			...
			{
				var processor = GetProcessor<V2.ICCProfileFormProcessor>();
				var formPrepareOptions = new V2.ProfileFormPrepareOptions
				{
					Customer = V2ProcessingInputGenerator.GetCustomerData(_provider.GetCustomerDataReader()),
					Address = V2ProcessingInputGenerator.GetAddressData(_provider.GetCustomerDataReader())
				};
				var redirectOptions = V2PluginErrorHandler.ExecuteAndHandleError(() => processor.PrepareProfileForm(formPrepareOptions));
				throw new PXPluginRedirectException<PXPluginRedirectOptions>(redirectOptions);
			}

			public V2.ProfileFormResponseProcessResult ProcessProfileFormResponse(string response)
			{
				var processor = GetProcessor<V2.ICCProfileFormProcessor>();
				var formPrepareOptions = new V2.ProfileFormPrepareOptions
				{
				...
				var options = new V2.ProfileFormResponseProcessOptions
				{
					PrepareOptions = formPrepareOptions,
					Response = response
				};
				return V2PluginErrorHandler.ExecuteAndHandleError(() => {
					return processor.ProcessProfileFormResponse(options);
				});
			}
			...
			public PXPluginRedirectOptions PreparePaymentForm(V2.PaymentFormPrepareOptions inputData)
			{
				var processor = GetProcessor<V2.ICCPaymentFormProcessor>();
				return V2PluginErrorHandler.ExecuteAndHandleError(() => {
					return processor.PreparePaymentForm(inputData);
				});
			}

			public V2.PaymentFormResponseProcessResult ProcessPaymentFormResponse(V2.PaymentFormPrepareOptions inputData, string response)
			{
				var processor = GetProcessor<V2.ICCPaymentFormProcessor>();
				var options = new V2.PaymentFormResponseProcessOptions
				{
					PrepareOptions = inputData,
					Response = response
				};
				return V2PluginErrorHandler.ExecuteAndHandleError(() => {
					return processor.ProcessPaymentFormResponse(options);
				});
			}
