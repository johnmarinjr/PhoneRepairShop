...
namespace PX.Objects.AR
{
		...
		where TLine : class, IBqlTable, IHasMinGrossProfit, new()
	{
			...
			where uOMField : IBqlField
		{
			...
			var itemCurySettings = InventoryItemCurySettings.PK.Find(sender.Graph, inventoryItem.InventoryID, inItemCost.CuryID);
			decimal minPrice = INUnitAttribute.ConvertToBase<inventoryIDField, uOMField>(sender, line, PXPriceCostAttribute.MinPrice(inventoryItem, inItemCost, itemCurySettings), INPrecision.UNITCOST);
			return minPrice;
		}
			...
			where uOMField : IBqlField
		{
			...
			if (line != null && line.InventoryID != null && line.UOM != null && curyNewUnitPrice >= 0 && line.IsFree != true && line.SiteID != null)
			{
				var site = INSite.PK.Find(sender.Graph, line.SiteID);
				...
				var r = (PXResult<InventoryItem, INItemCost>)
					...
					LeftJoin<INItemCost, On<INItemCost.inventoryID, Equal<InventoryItem.inventoryID>,
						And<INItemCost.curyID, Equal<Required<INSite.baseCuryID>>>>>,
					Where<InventoryItem.inventoryID, Equal<Required<ARSalesPrice.inventoryID>>>>
					.Select(sender.Graph, site?.SiteID, line.InventoryID);
			...
			where uOMField : IBqlField
		{
			...
			if (line != null && unitPrice > 0 && newDiscPct > 0 && line.IsFree != true && line.SiteID != null)
			{
				var site = INSite.PK.Find(sender.Graph, line.SiteID);
				...
				var r = (PXResult<InventoryItem, INItemCost>)
					...
					LeftJoin<INItemCost, On<INItemCost.inventoryID, Equal<InventoryItem.inventoryID>,
						And<INItemCost.curyID, Equal<Required<INSite.baseCuryID>>>>>,
					Where<InventoryItem.inventoryID, Equal<Required<ARSalesPrice.inventoryID>>>>
					.Select(sender.Graph, site?.BaseCuryID, line.InventoryID);
				InventoryItem item = r;
				INItemCost cost = r;
			...
			where uOMField : IBqlField
		{
			...
			if (line != null && Math.Abs(line.Qty.GetValueOrDefault()) > 0 && newDisc > 0 && unitPrice > 0 && line.IsFree != true && line.SiteID != null)
			{
				var site = INSite.PK.Find(sender.Graph, line.SiteID);
