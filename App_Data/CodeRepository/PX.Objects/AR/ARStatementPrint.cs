	...
	public class ARStatementPrint : PXGraph<ARStatementDetails>
	{
		...
		public static void MarkAsDoNotPrint(PrintParameters filter, IEnumerable<DetailsResult> list)
		{
			...
			foreach (DetailsResult detailsResult in list)
			{
				if (filter.ShowAll != true)
				{
					...
					{
						if (arSetup?.PrepareStatements == AR.ARSetup.prepareStatements.ConsolidatedForAllCompanies
							|| (arSetup?.PrepareStatements == AR.ARSetup.prepareStatements.ForEachBranch && statement.BranchID == filter.BranchID)
							|| (arSetup?.PrepareStatements == AR.ARSetup.prepareStatements.ConsolidatedForCompany && PXAccess.GetBranch(statement.BranchID).Organization.OrganizationID == filter.OrganizationID))
						{
		...
		public static void PrintStatements(PrintParameters filter, IEnumerable<DetailsResult> list)
		{
			...
			foreach (DetailsResult detailsResult in list)
			{
				...
				{
					parametersDictionary[ARStatementReportParams.Parameters.BranchID] = PXAccess.GetBranchCD(filter.BranchID);
				}
				if (arSetup?.PrepareStatements == AR.ARSetup.prepareStatements.ConsolidatedForCompany)
				{
					parametersDictionary[ARStatementReportParams.Parameters.OrganizationID] = PXAccess.GetOrganizationCD(filter.OrganizationID);
				}
					...
					detailsResult.CuryID))
				{
					if ((statement.Printed != true)
						&& (arSetup?.PrepareStatements == AR.ARSetup.prepareStatements.ConsolidatedForAllCompanies
							|| (arSetup?.PrepareStatements == AR.ARSetup.prepareStatements.ForEachBranch && statement.BranchID == filter.BranchID)
							|| (arSetup?.PrepareStatements == AR.ARSetup.prepareStatements.ConsolidatedForCompany && PXAccess.GetBranch(statement.BranchID).Organization.OrganizationID == filter.OrganizationID)))
					{
				...
				else 
				{
					if (statement.IsParentCustomerStatement)
					{
						...
						ARStatementKey statementKey = new ARStatementKey(
							arSetup?.PrepareStatements == AR.ARSetup.prepareStatements.ConsolidatedForAllCompanies ? int.MaxValue : statement.BranchID.Value,
						...
						if (!sentStatements.Contains(statementKey))
						{
							...
							Activity.SendNotification(ARNotificationSource.Customer, notificationCD, statement.BranchID, parameters, true);
