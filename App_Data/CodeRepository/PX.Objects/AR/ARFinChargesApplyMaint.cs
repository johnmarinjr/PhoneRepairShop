...
namespace PX.Objects.AR
{
	...
	public class ARFinChargesApplyMaint : PXGraph<ARFinChargesApplyMaint>
	{
		...
		private static void CreateFCDoc(List<ARFinChargesDetails> list, ARFinChargesApplyParameters filter)
		{
			...
			Numbering numbering = PXSelect<Numbering, Where<Numbering.numberingID, Equal<Required<Numbering.numberingID>>>>.Select(ie, ie.ARSetup.Current.FinChargeNumberingID);
			if (numbering != null && numbering.UserNumbering == true)
			{
				throw new PXException(Messages.ARSetupOverdueChargeNumberingIDCannotBeManual, numbering.NumberingID);
			}
				...
				try
				{
					...
					if ((i == list.Count - 1) || (Compare(fcDetail, list[i + 1]) != 0))
					{
						if (ie.finChargeTrans.Any())
						{
							...
							var currentBranch = PXAccess.GetBranch(ie.Accessinfo.BranchID);

							if (PXAccess.FeatureInstalled<CS.FeaturesSet.multipleBaseCurrencies>()
									&& inv.BranchID == null
									&& cust.BaseCuryID != null
									&& cust.BaseCuryID != currentBranch.BaseCuryID)
							{
								var message = PXMessages.LocalizeFormatNoPrefix(Messages.BranchCustomerDifferentBaseCuryCreated, currentBranch.BranchCD.Trim(), cust.BaseCuryID);
