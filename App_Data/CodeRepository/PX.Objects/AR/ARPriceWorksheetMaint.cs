...
namespace PX.Objects.AR
{
    ...
    public class ARPriceWorksheetMaint : PXGraph<ARPriceWorksheetMaint, ARPriceWorksheet>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		...
		public ARPriceWorksheetMaint()
        {
			...
			var importAttribute = Details.View.Attributes.OfType<PXImportAttribute>().First();
			importAttribute.RowImporting += ImportAttributeRowImporting;
		}
        ...
        protected virtual void ProcessReleaseForNonPromotionalSalesPrices(PXResultset<ARSalesPrice> salesPrices, ARPriceWorksheetDetail priceLine)
        {
			...
			bool? discountable = Document.Current.Discountable;
                    ...
                    Where<InventoryItem.itemStatus, NotEqual<INItemStatus.inactive>,
                        ...
                        And2<
                            AreSame<ARSalesPrice.siteID, Required<ARSalesPrice.siteID>>,
								...
								And<ARSalesPrice.discountable, Equal<Required<ARSalesPrice.discountable>>,
								And<
                                    Where2<
                    ...
                    PXSelectJoin<InventoryItem,
                        ...
                        LeftJoin<INItemCost, 
                            On<INItemCost.inventoryID, Equal<InventoryItem.inventoryID>,
                                And<INItemCost.curyID, Equal<Current<CalculatePricesFilter.baseCuryID>>>>>>,
                                                                                                          ...
                                                                                                          ARPriceWorksheetDetail worksheetDetail)
        {
	        ...
	        decimal? stdCost = itemSite?.StdCost ?? curyItem.StdCost;
	        decimal? recCost = itemSite?.RecPrice ?? curyItem.RecPrice;
        ...
        protected virtual void ARPriceWorksheet_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
			...
			PXUIFieldAttribute.SetEnabled<ARPriceWorksheet.discountable>(sender, tran, allowEdit && tran.IsFairValue == true);
            ...
            if (det != null && det.PriceType != null)
            {
                if (det.PriceType == PriceTypes.Customer)
                {
					...
					if (string.IsNullOrEmpty(customerCD))
					{
						if (det.CustomerID != null)
						{
							...
							if (customer != null)
							{
								customerCD = customer.AcctCD.TrimEnd();
                ...
                else
                {
                    if (e.ReturnState == null)
                        e.ReturnState = det.CustPriceClassID?.TrimEnd();
                    if (det.PriceCode == null)
                        det.PriceCode = det.CustPriceClassID?.TrimEnd();
                }
            }
        }
		...
		private decimal GetItemPrice(PXGraph graph, string priceType, string priceCode, int? inventoryID, string toCuryID, string uom, DateTime? curyEffectiveDate)
		{
			...
			return itemPrice == null
				? 0m
				: ConvertSalesPrice(this, new CMSetupSelect(this).Current.ARRateTypeDflt, Accessinfo.BaseCuryID,
		...
		{
            var itemCurySettngs = InventoryItemCurySettings.PK.Find(this, inventoryItem?.InventoryID, Accessinfo.BaseCuryID);

			if (itemCurySettngs == null)
			{
				return null;
			}
			if (string.IsNullOrEmpty(uom) || uom == inventoryItem.BaseUnit)
			{
				return itemCurySettngs.BasePrice;
			}
			else
			{
		...
		protected virtual void ImportAttributeRowImporting(object sender, PXImportAttribute.RowImportingEventArgs e)
		{
			if (e.Mode != PXImportAttribute.ImportMode.Value.InsertAllRecords)
			{
				...
				var duplicate = DuplicateFinder.Find(e.Values);
				if (duplicate != null)
				{
					if (e.Keys.Contains(nameof(ARPriceWorksheetDetail.LineID)))
						e.Keys[nameof(ARPriceWorksheetDetail.LineID)] = duplicate.LineID;
					else
						e.Keys.Add(nameof(ARPriceWorksheetDetail.LineID), duplicate.LineID);
				}
				}
			}
	...
	public partial class ARAddItemSelected : IBqlTable
    {
        ...
        [InventoryIncludingTemplates(BqlField = typeof(InventoryItem.inventoryID), IsKey = true)]
        [PXDefault()]
    ...
    public partial class CopyPricesFilter : IBqlTable
    {
		...
		#endregion
		#region Discountable
		public abstract class discountable : IBqlField { }

		[PXBool]
		[PXDefault(false, PersistingCheck = PXPersistingCheck.Nothing)]
		[PXUIField(DisplayName = "Discountable", Enabled = false, Visible = false, Visibility = PXUIVisibility.Invisible)]
