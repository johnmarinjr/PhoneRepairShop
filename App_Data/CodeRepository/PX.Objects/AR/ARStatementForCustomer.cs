...
namespace PX.Objects.AR
{
    ...
    public class ARStatementForCustomer : PXGraph<ARStatementForCustomer>
    {
        ...
        public partial class ARStatementForCustomerParameters : IBqlTable
        {
			...
			[Branch(addDefaultAttribute: false, useDefaulting: false, Required = false)]
        ...
        protected virtual IEnumerable details()
        {
			...
			if (customer != null)
            {
				...
				PXSelectBase<ARStatement> sel = new PXSelectJoin<ARStatement,
                        InnerJoin<Branch, On<ARStatement.branchID, Equal<Branch.branchID>>,
                        InnerJoin<GL.DAC.Organization, On<Branch.organizationID, Equal<GL.DAC.Organization.organizationID>>>>,
                ...
                List<object> Parameters = new List<object> { header.CustomerID};

				if (header.BranchID != null)
				{
					sel.WhereAnd<Where<ARStatement.branchID, Equal<Required<ARStatement.branchID>>>>();
                    Parameters.Add(header.BranchID);

                }
                else if (header.OrgBAccountID != null)
                {
                    sel.WhereAnd<Where<GL.DAC.Organization.bAccountID, Equal<Required<GL.DAC.Organization.bAccountID>>>>();
                    Parameters.Add(header.OrgBAccountID);
                }

                foreach (ARStatement st in sel.Select(Parameters.ToArray()))
				{
                    ...
                    else
                    {
                        ...
                        string baseCuryID = PXAccess.FeatureInstalled<FeaturesSet.multipleBaseCurrencies>() ?
                            PXOrgAccess.GetBaseCuryID(PXAccess.GetBranchCD(st.BranchID))
                            : company.BaseCuryID;

                        res.ResetToBaseCury(baseCuryID);
            ...
            if (this.Details.Current != null && this.Filter.Current != null)
            {
                if (this.Filter.Current.CustomerID.HasValue)
                {
					...
					if (customer != null)
                    {
						...
						{
							parameters[ARStatementReportParams.Parameters.BranchID] = PXAccess.GetBranchCD(Filter.Current.BranchID);
						}
						else if (Filter.Current.OrganizationID != null)
						{
							parameters[ARStatementReportParams.Parameters.OrganizationID] = PXAccess.GetOrganizationCD(Filter.Current.OrganizationID);
						}
		...
		protected virtual void _(Events.FieldDefaulting<ARStatementForCustomerParameters, ARStatementForCustomerParameters.orgBAccountID> e)
		{
			if (PXAccess.FeatureInstalled<FeaturesSet.branch>())
			{
				PXAccess.MasterCollection.Branch currentBranch = PXAccess.GetBranch(Accessinfo.BranchID);

				if (currentBranch != null && e.NewValue == null)
				{
					switch (ARSetup.Current.PrepareStatements)
					{
						case AR.ARSetup.prepareStatements.ForEachBranch:
							e.NewValue = currentBranch.BranchCD.Trim();
							break;
						case AR.ARSetup.prepareStatements.ConsolidatedForCompany:
							e.NewValue = currentBranch.Organization.OrganizationCD.Trim();
							break;
						default:
							e.NewValue = null;
							e.Cancel = true;
