...
namespace PX.Objects.AR
{
	...
	public class ARInvoiceEntry : ARDataEntryGraph<ARInvoiceEntry, ARInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
			...
			string reportID)
		{
			...
			if (invoice != null)
			{
				ReportNotificationGenerator reportNotificationGenerator = ReportNotificationGeneratorFactory(reportID);
		...
		public PXSelectJoin<
			ARAdjust,
				LeftJoin<Standalone.ARRegisterAlias,
					On<Standalone.ARRegisterAlias.noteID, Equal<ARAdjust.invoiceID>,
		...
		public PM.PMActivityList<ARInvoice> Activity;
		...
		protected virtual bool AskUserApprovalIfUnreleasedCreditMemoAlreadyExists(ARInvoice doc, out ARAdjust creditMemo)
		{
			...
			if (creditMemo != null)
			{
				string localizedMsg = PXMessages.LocalizeFormatNoPrefix(Messages.UnreleasedCreditMemoExists, creditMemo.AdjgRefNbr);
				return Document.View.Ask(localizedMsg, MessageButtons.YesNo) == WebDialogResult.Yes;
			}
		...
		protected virtual void _(Events.RowUpdated<CurrencyInfo> e)
		{
			...
			if (Document.Current?.DocType == ARDocType.CreditMemo)
				...
				Adjustments.Select().RowCast<ARAdjust2>().ForEach(adjustment =>
						{
							if (adjustment != null)
							{
		...
		string notificationCD)
		{
			foreach (ARInvoice doc in adapter.Get().RowCast<ARInvoice>())
			{
				...
				using (var ts = new PXTransactionScope())
				{
					...
					{
						Activity.SendNotification(PMNotificationSource.Project, notificationCD, doc.BranchID, parameters, adapter.MassProcess);
					}
					else
					{
						Activity.SendNotification(ARNotificationSource.Customer, notificationCD, doc.BranchID, parameters, adapter.MassProcess);
					}
					this.Save.Press();
		...
		public virtual IEnumerable adjustments_1()
		{
			...
			BqlCommand select = Adjustments_Crm.View.BqlSelect;
			if (Document.Current.DocType == ARDocType.Invoice)
			{
				select = select.WhereNew<Where<ARInvoice.docType.FromCurrent.IsNotEqual<ARDocType.creditMemo>
			...
			foreach (PXResult<ARAdjust, Standalone.ARRegisterAlias, ARInvoice, ARPayment, CurrencyInfo, ExternalTransaction> res in 
				mapper.Select(select.CreateView(this, mergeCache: true)))
			{
				ARAdjust adj = res;
		...
		protected virtual void ARShippingAddress_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (row != null)
			{
				if (!IsTaxZoneDerivedFromCustomer() && !string.IsNullOrEmpty(row.PostalCode) && oldRow.PostalCode != row.PostalCode && Document.Current.Released != true)
				{
		...
		public virtual ARInvoiceState GetDocumentState(PXCache cache, ARInvoice doc)
		{
			...
			ARInvoiceState res = new ARInvoiceState
			{
				...
				IsRetainageReversing = (doc.IsOriginalRetainageDocument() || doc.IsChildRetainageDocument()) && doc.IsRetainageReversing == true,
		...
		protected virtual void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool creditMemoAdjustmentsEnabled = Transactions.Cache.AllowUpdate 
			                                    && !state.IsDocumentScheduled 
			                                    && !state.PaymentsByLinesAllowed
		...
		protected virtual void _(Events.RowUpdating<ARInvoice> e)
		{
			...
			{
				ARInvoice origDocument = PXSelect<ARInvoice,
				...
				{
					e.Cache.RaiseExceptionHandling<ARInvoice.finPeriodID>(e.NewRow, PeriodIDAttribute.FormatForDisplay(e.NewRow.FinPeriodID),
