...
namespace PX.Objects.AR
{
	...
	public class CCTransactionsHistoryEnq : PXGraph<CCTransactionsHistoryEnq>
	{
		...
		public partial class CCTransactionsHistoryFilter : IBqlTable
		{
			...
			#endregion
			#region Descr
			public abstract class descr : PX.Data.BQL.BqlInt.Field<descr> { }
			[PXDBString]
			[PXDefault]
			[PXSelector(typeof(Search2<CustomerPaymentMethod.descr,
								...
								And<Where<CustomerPaymentMethod.cardType, Equal<Current<CCTransactionsHistoryFilter.cardType>>,
									Or<Current<CCTransactionsHistoryFilter.cardType>, IsNull>>>
									>>>>),
		...
		public PXSelectJoin<
			...
			InnerJoin<CustomerPaymentMethod, 
				On<CustomerPaymentMethod.pMInstanceID, Equal<CCProcTran.pMInstanceID>>,
			InnerJoin<Customer, 
				On<CustomerPaymentMethod.bAccountID, Equal<Customer.bAccountID>>,
			LeftJoin<ARPayment, 
				On<ARPayment.docType, Equal<CCProcTran.docType>, 
				And<ARPayment.refNbr, Equal<CCProcTran.refNbr>>>>>>> 
			CCTransView;
		...
		public virtual IEnumerable cCTrans()
		{
			...
			if (filter != null && string.IsNullOrEmpty(filter.PaymentMethodID) == false &&
				filter.StartDate.HasValue && filter.EndDate.HasValue && filter.Descr != null)
			{
				GetInternalData(filter.PaymentMethodID);

				if (string.IsNullOrEmpty(filter.PaymentMethodID) == false)
				{
					CCTransView.WhereAnd<Where<CustomerPaymentMethod.paymentMethodID, Equal<Current<CCTransactionsHistoryFilter.paymentMethodID>>>>();
				}
				if (string.IsNullOrEmpty(filter.ProcessingCenterID) == false)
				{
					CCTransView.WhereAnd<Where<CCProcTran.processingCenterID, Equal<Current<CCTransactionsHistoryFilter.processingCenterID>>>>();
				}
				if (filter.StartDate.HasValue)
				{
					CCTransView.WhereAnd<Where<CCProcTran.startTime, GreaterEqual<Current<CCTransactionsHistoryFilter.startDate>>>>();
				}
				if (filter.EndDate.HasValue)
				{
					CCTransView.WhereAnd<Where<CCProcTran.startTime, LessEqual<Current<CCTransactionsHistoryFilter.endDatePlusOne>>>>();
				}

				if (string.IsNullOrEmpty(filter.Descr) == false)
				{
					CCTransView.WhereAnd<Where<CustomerPaymentMethod.descr, Equal<Current<CCTransactionsHistoryFilter.descr>>>>();
				}

				foreach (PXResult<CCProcTran, CustomerPaymentMethod, Customer, ARPayment> it in CCTransView.Select(CardNumber_DetailID, NameOnCard_DetailID))
				{
		...
		public virtual void CCTransactionsHistoryFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<CCTransactionsHistoryFilter.descr>(sender, e.Row, !string.IsNullOrEmpty(row.PaymentMethodID));
			...
			PXUIFieldAttribute.SetEnabled<CCTransactionsHistoryFilter.cardType>(sender, e.Row, !string.IsNullOrEmpty(row.PaymentMethodID));
		}
		...
		public virtual void CCTransactionsHistoryFilter_CardType_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			CCTransactionsHistoryFilter row = (CCTransactionsHistoryFilter)e.Row;
			row.Descr = null;
		}
		...
		public virtual void CCTransactionsHistoryFilter_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			PXResult<CustomerPaymentMethod> cnt = PXSelectJoinGroupBy<CustomerPaymentMethod, InnerJoin<Customer,
														...
														Where<CustomerPaymentMethod.paymentMethodID, Equal<Current<CCTransactionsHistoryFilter.paymentMethodID>>,
															...
															And<CustomerPaymentMethod.isActive, Equal<True>,
															And<Where<CustomerPaymentMethod.cardType, Equal<Required<CCTransactionsHistoryFilter.cardType>>,
																Or<Required<CCTransactionsHistoryFilter.cardType>, IsNull>>>>>>,
														Aggregate<Count<CustomerPaymentMethod.pMInstanceID>>>.Select(this, row.DBSearchName, row.CardType, row.CardType);
			int count = 0;
		...
		{
			PXGraph target = null;
			if (docType == ARDocType.Payment || docType == ARDocType.VoidPayment || docType == ARDocType.Refund)
			{
