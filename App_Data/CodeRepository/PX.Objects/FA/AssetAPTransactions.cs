...
namespace PX.Objects.FA
{
	...
	public class AssetGLTransactions : PXGraph<AssetGLTransactions>
	{
		...
		public class GLTransactionsViewExtension : AdditionsViewExtensionBase<AssetGLTransactions>
		{
			...
			{
				return GetFAAccrualTransactions(Base.Filter.Current, GLTransactions.Cache);
			}
			...
			[PXOverride]
			public virtual void ClearInserted()
			{
				GLTransactions.Cache.Clear();
			}
		}
		#endregion
		...
		protected virtual void GLTranFilter_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			if (!sender.ObjectsEqual<GLTranFilter.accountID, GLTranFilter.subID>(e.Row, e.OldRow)
				 || PXAccess.FeatureInstalled<FeaturesSet.multipleBaseCurrencies>() && !sender.ObjectsEqual<GLTranFilter.branchID>(e.Row, e.OldRow))
			{
				ClearInserted();
			}
		}
		...
		protected virtual void FATran_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			FAAccrualTran ext = (FAAccrualTran)Caches[typeof(FAAccrualTran)].Current;
			if (ext == null)
			{
		...
		protected virtual void _(Events.FieldUpdated<FATran, FATran.targetAssetID> e)
		{
			FATran row = (FATran)e.Row;

			if (row == null || e.NewValue == null || row.NewAsset == true) return;

			FixedAsset asset = PXSelectorAttribute.Select<FixedAsset.assetID>(e.Cache, row, e.NewValue) as FixedAsset;

			if (row.TranType == FATran.tranType.PurchasingPlus && asset != null)
			{
				e.Cache.SetValueExt<FATran.branchID>(row, asset.BranchID);
			}
		}
		...
		protected virtual void _(Events.FieldVerifying<FATran, FATran.branchID> e)
		{
			FATran row = (FATran)e.Row;

			if (row == null || e.NewValue == null || row.NewAsset == true) return;

			FixedAsset asset = PXSelectorAttribute.Select<FixedAsset.assetID>(e.Cache, row, row.TargetAssetID) as FixedAsset;

			if (asset != null && row.TranType == FATran.tranType.PurchasingPlus)
			{
				e.NewValue = asset.BranchID;
			}
		}
		#endregion
		...
		public override void Persist()
		{
			...
			GLTransactionsViewExtension glTransactionExtension = GetExtension<GLTransactionsViewExtension>();

			foreach (FAAccrualTran ex in glTransactionExtension.GLTransactions.Select())
			{
			...
			foreach (FAAccrualTran ext in glTransactionExtension.GLTransactions.Select().RowCast<FAAccrualTran>().Where(ext => ext.Selected == true))
			{
				...
				glTransactionExtension.GLTransactions.Cache.MarkUpdated(ext);
			}
	...
	public class AdditionsFATran : PXGraph<AdditionsFATran>
	{
		...
		protected virtual void InsertFATransactions(FixedAsset asset, DateTime? date, Decimal? amt, GLTran gltran)
		{
			foreach (FABookBalance bal in PXSelect<FABookBalance, Where<FABookBalance.assetID, Equal<Current<FixedAsset.assetID>>>>.SelectMultiBound(this, new object[] { asset }))
			{
				...
				if (string.IsNullOrEmpty(bal.CurrDeprPeriod) && asset.Depreciable == true)
				{
	...
	public partial class GLTranFilter : IBqlTable
	{
		...
		public abstract class branchBaseCuryID : BqlString.Field<branchBaseCuryID> { }
		...
		public virtual string BranchBaseCuryID
		{
			get;
			set;
		}
