...
namespace PX.Objects.FA
{
	...
	public class AccBalanceByAssetInq : PXGraph<AccBalanceByAssetInq>
	{
		...
		public virtual BqlCommand GetSelectCommand(AccBalanceByAssetFilter filter)
		{
			BqlCommand query = new PXSelectJoin<FATran,
			...
			{
				query = query.WhereAnd<Where<Branch.organizationID, Equal<Current2<AccBalanceByAssetFilter.organizationID>>, And<MatchWithBranch<Branch.branchID>>>>();
			}
			if (filter.BranchID != null)
			{
				query = query.WhereAnd<Where<Branch.branchID, Equal<Current<AccBalanceByAssetFilter.branchID>>>>();
			}
		...
		public virtual IEnumerable amts(PXAdapter adapter)
		{
			...
			BqlCommand select = GetSelectCommand(filter);

			Dictionary<int?, Amounts> dict = new Dictionary<int?, Amounts>();
			foreach (PXResult<FATran, FixedAsset, FADetails, Branch, FALocationHistoryCurrent, FALocationHistory> res
				in select
