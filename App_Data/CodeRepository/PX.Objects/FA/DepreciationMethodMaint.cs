...
namespace PX.Objects.FA
{
	...
	public class DepreciationTableMethodMaint : PXGraph<DepreciationTableMethodMaint, FADepreciationMethod>
	{
		...
		protected virtual void FADepreciationMethod_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXDefaultAttribute.SetPersistingCheck<FADepreciationMethod.parentMethodID>(sender, method,
				method.RecordType == FARecordType.AssetType ? PXPersistingCheck.NullOrBlank: PXPersistingCheck.Nothing);
			PXUIFieldAttribute.SetRequired<FADepreciationMethod.parentMethodID>(sender, method.RecordType == FARecordType.AssetType);
			...
			PXUIFieldAttribute.SetEnabled<FADepreciationMethod.usefulLife>(sender, method, method.RecordType != FARecordType.AssetType);
		...
		protected virtual void FADepreciationMethod_RecordType_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			sender.SetDefaultExt<FADepreciationMethod.parentMethodID>(meth);
		}
		...
		protected virtual void FADepreciationMethod_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			FADepreciationMethod method = (FADepreciationMethod)e.Row;

			if (method.DisplayTotalPercents > 100m)
			{
				sender.RaiseExceptionHandling<FADepreciationMethod.displayTotalPercents>(method, null, new PXSetPropertyException(CS.Messages.Entry_LE, 100));
			}
		}

		protected virtual void FADepreciationMethod_ParentMethodID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			FADepreciationMethod method = (FADepreciationMethod)e.Row;
			if (method == null) return;

			if (method.ParentMethodID == null)
			{
				sender.SetValueExt<FADepreciationMethod.usefulLife>(method, null);
			}
			else
			{
				FADepreciationMethod item = PXSelect<FADepreciationMethod, Where<FADepreciationMethod.recordType, Equal<FARecordType.classType>, And<FADepreciationMethod.methodID, Equal<Current<FADepreciationMethod.parentMethodID>>>>>.SelectSingleBound(this, null);
				sender.SetValueExt<FADepreciationMethod.usefulLife>(method, item.UsefulLife);
			}
		}
		#endregion
