...
namespace PX.Objects.FA
{
	public class AssetClassMaint : PXGraph<AssetClassMaint, FixedAsset>
	{
		...
		protected virtual void _(Events.CacheAttached<FixedAsset.baseCuryID> e) { }
		...
		protected virtual void FABookSettings_DepreciationMethodID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (bookSettings.BookID != null)
			{
				...
				if (yearSetup == null)
				{
					string errorMessage = PXMessages.LocalizeFormat(Messages.BookCalendarNotExist,
														(PXSelectorAttribute.Select<FABookSettings.bookID>(sender, bookSettings) as FABook).BookCode);
					sender.RaiseExceptionHandling<FABookSettings.depreciationMethodID>(bookSettings, deprMethod.DepreciationMethod, new PXSetPropertyException(errorMessage));
				}
				else
