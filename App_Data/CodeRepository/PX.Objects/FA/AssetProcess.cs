...
namespace PX.Objects.FA
{
	...
	public class AssetTranRelease : PXGraph<AssetTranRelease>
	{
		...
		public static DocumentList<Batch> ReleaseDoc(List<FARegister> list, bool isMassProcess, bool AutoPost)
		{
			...
			Dictionary<(string, string), HashSet<int>> batchbind = new Dictionary<(string, string), HashSet<int>>();
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				try
				{
					...
					{
						(string, string) key = (je.BatchModule.Current.BatchNbr, je.BatchModule.Current.Module);

						if (batchbind.ContainsKey(key))
							batchbind[key].Add(i);
						else
							batchbind.Add(key, new HashSet<int>() { i });
					}
			...
			for (int i = 0; i < batchlist.Count; i++)
			{
				...
				{
					HashSet<int> indexesFARegister = batchbind[(batch.BatchNbr, batch.Module)];

					if (isMassProcess)
					{
						failed = true;
						
						foreach (int index in indexesFARegister)
						{
							PXProcessing<FARegister>.SetError(index, e);
						}
						
					}
					else
					{
						throw new PXMassProcessException(indexesFARegister.First(), e);
					}
				}
			}
			if (failed)
	...
	public class AssetProcess : PXGraph<AssetProcess>
	{
		...
		public static FixedAsset GetSourceForNewAccounts(PXGraph graph, FixedAsset asset)
		{
			...
			{
				Standalone.FADetails detailsLite = AssetMaint.FetchDetailsLite(graph, asset);

				var location = FAHelper.GetCurrentLocation(graph, detailsLite);
		...
		public static void SuspendBalanceForPeriods(TransactionEntry graph, FABookBalance bookbal, int Periods)
		{
			...
			hist = FAHelper.InsertFABookHist(graph, hist, ref bookbal);
		...
		{
			ThrowDisabled_Transfer(
				graph,
				(TransferProcess.TransferFilter)graph.Caches<TransferProcess.TransferFilter>().Current,
				asset,
				det);
		}

		public static void ThrowDisabled_Transfer(PXGraph graph, TransferProcess.TransferFilter filter, FixedAsset asset, FADetailsTransfer det)
		{
			PXAccess.MasterCollection.Branch assetBranch = PXAccess.GetBranch(asset.BranchID);
			PXAccess.MasterCollection.Branch branchTo = PXAccess.GetBranch(filter.BranchTo);
			if (assetBranch != null && branchTo != null && assetBranch.BaseCuryID != branchTo.BaseCuryID)
			{
				throw new PXSetPropertyException(Messages.AssetBaseCurrencyDiffersFromDestinationBranch,
					PXErrorLevel.RowWarning,
					assetBranch.BaseCuryID,
					asset.AssetCD,
					branchTo.BaseCuryID,
					branchTo.BranchCD);
			}
		...
		public static List<FABookBalance> PrepareDisposal(TransactionEntry docgraph, FixedAsset asset, FADetails assetdet, bool IsMassProcess, bool deprBeforeDisposal)
		{
			...
			CalculateAsset(books, null); // (AC-211595) the FabookBalance object might be persisted with temporary DeprTo values which set by SetDeprTo function
			ResetDeprTo(books, origin);

			books.ForEach(bal => docgraph.EnsureRowPersistence(bal)); // (AC-211595) so we must persist restored DeprTo values
			docgraph.Actions.PressSave();
		...
		public static DocumentList<FARegister> SplitAsset(FixedAsset asset, DateTime? splitDate, string splitPeriodID, bool deprBeforeSplit, Dictionary<FixedAsset, decimal> splits)
		{
				...
				foreach (FABookBalance sbal in PXSelect<FABookBalance, Where<FABookBalance.assetID, Equal<Required<FixedAsset.assetID>>>>.Select(docgraph, split.Key.AssetID))
				{
						...
						OrderBy<Asc<FABookHist.finPeriodID>>>.Select(docgraph, bal.AssetID, bal.BookID, lastPeriod ?? bal.DeprToPeriod))
					{
								...
								|| bal.Status == FixedAssetStatus.FullyDepreciated))
						{
							...
							FABookBalance bookBalance = sbal;
							newhist = FAHelper.InsertFABookHist(docgraph, newhist, ref bookBalance);
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (docgraph.Trans.Cache.IsInsertedUpdatedDeleted)
				{
					...
					foreach (FABookBalance balanceFromDB in SelectFrom<FABookBalance>
						.Where<FABookBalance.assetID.IsEqual<@P.AsInt>>
						.View.ReadOnly.Select(docgraph, asset.AssetID))
					{
						PXTimeStampScope.PutPersisted(docgraph.bookbalances.Cache, balanceFromDB, balanceFromDB.tstamp);
					}
		...
		public static void AcquireAsset(TransactionEntry docgraph, int BranchID, IDictionary<FABookBalance, OperableTran> booktrn, FARegister register)
		{
			...
			foreach (KeyValuePair<FABookBalance, OperableTran> book in booktrn)
			{
				...
				if (book.Value.Op == PXDBOperation.Update && (!docgraph.UpdateGL || register.Released == false))
				{
					...
					bookBal.MaxHistoryPeriodID = null;
					docgraph.EnsureRowPersistence(bookBal);
				...
				if (string.IsNullOrEmpty(book.Key.LastDeprPeriod))
				{
					if (book.Value.Op == PXDBOperation.Delete || book.Value.Op == PXDBOperation.Update)
					{
						...
						if (book.Value.Op != PXDBOperation.Delete)
						{
							bookBal.MaxHistoryPeriodID = null;
							docgraph.EnsureRowPersistence(bookBal);
						}
					...
					if ((book.Value.Op == PXDBOperation.Insert && asset.SplittedFrom == null) || book.Value.Op == PXDBOperation.Update)
					{
						...
						hist = FAHelper.InsertFABookHist(docgraph, hist, ref bookBal);
					...
					case PXDBOperation.Insert:
						...
						if (!docgraph.UpdateGL)
						{
							...
							foreach (FABookHist hist in PXSelect<
								...
								.Select(hist => hist = FAHelper.InsertFABookHist(docgraph, hist, ref bookBal)))
							{
								hist.Closed = true;
							}
		...
		public static bool DepreciateAsset(IEnumerable<FABookBalance> books, DateTime? DateTo, string PeriodTo, bool IsMassProcess, bool IncludeLastPeriod)
		{
			...
			foreach (FABookBalance item in books)
			{
				...
				try
				{
					...
					{
						var result 
							= (PXResult<FABookBalance, FixedAsset,FADetails,FADepreciationMethod,Branch,OrganizationFinPeriod>)
							PXSelectJoin<FABookBalance, 
								InnerJoin<FixedAsset, On<FixedAsset.assetID, Equal<FABookBalance.assetID>>,
								InnerJoin<FADetails, 
									On<FADetails.assetID, Equal<FixedAsset.assetID>>,
								InnerJoin<FADepreciationMethod,
									On<FADepreciationMethod.methodID, Equal<FABookBalance.depreciationMethodID>>,
								LeftJoin<Branch,
									On<Branch.branchID, Equal<FixedAsset.branchID>>,
								LeftJoin<OrganizationFinPeriod,
									On<OrganizationFinPeriod.organizationID, Equal<Branch.branchID>, 
									And<OrganizationFinPeriod.finPeriodID, Equal<Required<OrganizationFinPeriod.finPeriodID>>>>
										>>>>>,
								Where<FABookBalance.assetID, Equal<Current<FABookBalance.assetID>>, 
							And<FABookBalance.bookID, Equal<Current<FABookBalance.bookID>>>>>
								.SelectSingleBound(docgraph, new object[] { item }, PeriodTo);
						
						FixedAsset cache = result;
						FABookBalance bookbal = result;
						FADetails detail = result;
						FADepreciationMethod method = result;
						OrganizationFinPeriod finperiod = result;

						if (finperiod != null && finperiod.FAClosed != true)
						{
							PXSelectJoin<
									...
									Where<OrganizationFinPeriod.finPeriodID,
										Equal<Required<OrganizationFinPeriod.finPeriodID>>,
										And<OrganizationFinPeriod.fAClosed, NotEqual<True>>>>
								.StoreResult(
									docgraph,
									new List<object> { new PXResult<OrganizationFinPeriod, Branch, FixedAsset>(result, result, result)},
									PXQueryParameters.ExplicitParameters(item.AssetID, PeriodTo)
									);
						}
						// open period by assetID	
	                    OrganizationFinPeriod periodTo = docgraph
						...
						#region Tune up query cache
						// 1176647572
						PXSelect<FixedAsset, 
								Where<FixedAsset.assetID, Equal<Required<FixedAsset.assetID>>>>
							.StoreResult(docgraph, cache);
						// 596019361	
						PXSelect<FixedAsset, 
								Where<FixedAsset.assetID, Equal<Current<FATran.assetID>>>>
							.StoreResult(docgraph, cache);
						
						PXSelectorAttribute.StoreCached<FABookBalance.assetID>(docgraph.bookbalances.Cache, bookbal, cache );
						docgraph.bookbalances.StoreResult(bookbal);
						#endregion
							...
							.SelectMultiBound(docgraph, new object[] { bookbal }, bookbal.CurrDeprPeriod, BookPeriodTo))
						{
							...
							docgraph.Document.Current.DocDesc = PXMessages.LocalizeNoPrefix(Messages.FADocumentDepreciationDescription);
							...
							FATran prev_tran;
							{
								...
								PXSelectorAttribute.StoreCached<FATran.assetID>(docgraph.Trans.Cache, tran, asset );
								PXSelectorAttribute.StoreCached<FATran.bookID>(docgraph.Trans.Cache, tran, book );
								PXSelectorAttribute.StoreCached<FATran.finPeriodID>(docgraph.Trans.Cache, tran, period );
		...
		public virtual void ProcessAssetTran(JournalEntry je, FARegister doc, DocumentList<Batch> created)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
																																	...
																																	And<FATran.released, Equal<False>>>>.Select(this, doc.RefNbr))
				{
					...
					PXCache<FATran>.StoreOriginal(this, fatran);
					PXCache<FixedAsset>.StoreOriginal(this, asset);
					
					PXSelect<FixedAsset, 
						Where<FixedAsset.assetID, Equal<Required<FixedAsset.assetID>>>>
						.StoreResult(this, asset);
					...
					if (fatran.TranType == "P+" || fatran.TranType == "P-")
					{
							...
							.Select(this, fatran.BookID, GetDeprFromPeriod(je, bookbal, asset, det), fatran.FinPeriodID, PXAccess.GetParentOrganizationID(fatran.BranchID)))
						{
							...
							createdHist = FAHelper.InsertFABookHist(
								this,
								createdHist,
								ref bookbal);
					...
					hist = FAHelper.InsertFABookHist(
						this,
						hist,
						ref bookbal);
					...
					switch (fatran.TranType)
					{
						...
						case "TD":
							if (!string.IsNullOrEmpty(bookbal.DeprToPeriod) && string.CompareOrdinal(fatran.TranPeriodID, bookbal.DeprToPeriod) > 0)
							{
								...
								{
									throw new PXException(
										Messages.ActiveAssetTransferedPastDeprToPeriod,
										asset.AssetCD,
										FinPeriodIDAttribute.FormatForError(fatran.TranPeriodID),
										FinPeriodIDAttribute.FormatForError(bookbal.DeprToPeriod),
										book.BookCode);
								}
								//do not create open history outside depreciation schedule of the asset
								bookhist.Cache.SetStatus(hist, PXEntryStatus.Notchanged);
							...
							{
								if (!string.IsNullOrEmpty(bookbal.CurrDeprPeriod) 
									&& !string.IsNullOrEmpty(bookbal.LastDeprPeriod)
									&& string.CompareOrdinal(bookbal.CurrDeprPeriod, fatran.TranPeriodID) > 0)
								{
									throw new PXException(
										Messages.ActiveAssetTransferedBeforePeriod, 
										asset.AssetCD,
										FinPeriodIDAttribute.FormatForError(fatran.TranPeriodID),
										FinPeriodIDAttribute.FormatForError(bookbal.CurrDeprPeriod),
										book.BookCode);
					...
					PXCache<FABookBalance>.StoreOriginal(this, bookbal);
					switch (doc.Origin)
					{
						case FARegister.origin.Purchasing:
							...
							if(bookbal.Status == FixedAssetStatus.FullyDepreciated && string.CompareOrdinal(fatran.FinPeriodID, bookbal.LastPeriod) > 0)
							{
								bookbal.LastPeriod = fatran.FinPeriodID;
							}
						...
						case FARegister.origin.Depreciation:
							...
							else
							{
								...
								hist = FAHelper.InsertFABookHist(this, hist, ref bookbal);
							}
							break;
						...
						case FARegister.origin.DisposalReversal:
							...
							//check reversal period is greater than disposal period
							bookbal.LastPeriod = (string.CompareOrdinal(hist.FinPeriodID, bookbal.DisposalPeriodID) > 0) ? fatran.FinPeriodID : bookbal.DeprToPeriod;
							bookbalance.Update(bookbal);
		...
		private static void SegregateBatch(JournalEntry je, int? BranchID, DateTime? DocDate, string FinPeriodID, string descr, DocumentList<Batch> created)
		{
			...
			if (fabatch != null)
			{
				...
				PXCache<Batch>.StoreOriginal(je, fabatch);
		...
		protected static void InsertTransferAssetTransaction(PXCache transactCache, ref FARegister register, FABookBalance bal, FixedAsset asset, FALocationHistory location, FAAccounts oldAccounts, FAAccounts newAccounts, bool? previousUnderConstructionValue)
		{
			...
			var tran = new FATran
			{
				...
				FinPeriodID = bal.UpdateGL == true ? finPeriodID : null,
	...
	public static class FAHelper
	{
		public static string CloseFABookHistory(this PXGraph graph, FABookBalance bal, IEnumerable<string> periodIDs)
		{
			...
			foreach (string id in periodIDs.Where(id => !string.IsNullOrEmpty(id)))
			{
				...
				closeHist = InsertFABookHist(graph, closeHist, ref bal);
		...
		[Obsolete(Common.InternalMessages.MethodIsObsoleteAndWillBeRemoved2022R1)]
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static FALocationHistory GetCurrentLocation(this PXGraph graph, Standalone.FADetails details)
		{
			FALocationHistory current = PXSelect<FALocationHistory,
				Where<FALocationHistory.assetID, Equal<Current<Standalone.FADetails.assetID>>,
					And<FALocationHistory.revisionID, Equal<Current<Standalone.FADetails.locationRevID>>>>>
				.SelectSingleBound(graph, new object[] { details });
			return current;
		}
		...
		{
			FABookHist result = (FABookHist)graph.Caches<FABookHist>().Insert(keyedHistory);

			if (result != null
				&& (bookBalance.MaxHistoryPeriodID == null
					|| string.Compare(keyedHistory.FinPeriodID, bookBalance.MaxHistoryPeriodID) > 0))
			{
				PXCache<FABookBalance> cache = graph.Caches<FABookBalance>();
				PXEntryStatus status = cache.GetStatus(bookBalance);

				bookBalance.MaxHistoryPeriodID = keyedHistory.FinPeriodID;
				graph.EnsureRowPersistence(bookBalance);
			}
			return result;
		}
		...
		{
			FABookHistory maxPeriodHistory = SelectFrom<FABookHistory>
				...
				.View
				.SelectSingleBound(graph, null, assetID, bookID);
			string maxDatabaseFinPeriodID = maxPeriodHistory?.FinPeriodID;

			string maxInsertedFinPeriodID = graph
				.Caches<FABookHist>()
				.Inserted
				.Cast<FABookHist>()
				.Max(hist => hist.FinPeriodID);

			return string.Compare(maxDatabaseFinPeriodID, maxInsertedFinPeriodID) > 0
				? maxDatabaseFinPeriodID
				: maxInsertedFinPeriodID;
		}
	}
}
