	...
	public class TransactionEntry : PXGraph<TransactionEntry, FARegister>
	{
		...
		public PXSelect<FABookBalance, 
			Where<FABookBalance.assetID, Equal<Required<FABookBalance.assetID>>, 
				And<FABookBalance.bookID, Equal<Required<FABookBalance.bookID>>>>> bookbalances;
		...
		public static void SegregateRegister(PXGraph graph, int BranchID, string Origin, string PeriodID, DateTime? DocDate, string descr, DocumentList<FARegister> created)
		{
			...
			if (register.RefNbr != null)
			{
				...
				newreg.TranAmt = register.TranAmt;
				PXCache<FARegister>.StoreOriginal(graph, newreg);
		...
		protected virtual void FARegister_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetVisibility<FATran.srcBranchID>(this.Trans.Cache, null,
				reg.Origin == FARegister.origin.Transfer ? PXUIVisibility.Visible: PXUIVisibility.Invisible);
			PXUIFieldAttribute.SetVisible<FATran.srcBranchID>(this.Trans.Cache, null,
				reg.Origin == FARegister.origin.Transfer);
			PXUIFieldAttribute.SetEnabled<FATran.srcBranchID>(this.Trans.Cache, null,
				reg.Origin == FARegister.origin.Transfer);
