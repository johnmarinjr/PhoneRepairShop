...
namespace PX.Objects.CS
{
	...
	public class HashedList<T>: ICollection<T>
			...
			where TAddressID : IBqlField
		{
			...
			if (SourceRow != null)
			{
                ...
                if (PXResult.Unwrap<TAddress>(SourceRow).AddressID == null || sender.GetValue(DocumentRow, FieldName) == null)
				{
					...
					if (address is IAddressLocation addressLocation)
					{
						addressLocation.Latitude = PXResult.Unwrap<Address>(SourceRow).Latitude;
						addressLocation.Longitude = PXResult.Unwrap<Address>(SourceRow).Longitude;
					}
	...
	public class TermsAttribute : PXEventSubscriberAttribute, IPXFieldUpdatedSubscriber
	{
		...
		protected virtual void CalcDisc(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			    ...
			    sender.GetValue(e.Row, _CuryDocBal.Name) != null && !sender.Graph.IsCopyPasteContext)
			{
				...
				{
					CuryDiscBal = CM.TemporaryHelpers.MultiCurrencyCalculator.RoundCury(sender, e.Row, CuryDocBal * (decimal)terms.DiscPercent / 100);
				}
	...
	[PXRestrictorAttribute(
		typeof(Where<Location.isActive.IsEqual<True>>),
		Messages.InactiveLocation,
	...
	public class LocationActiveAttribute : LocationIDBaseAttribute
	{
		...
		public LocationActiveAttribute(Type WhereType)
			: base(WhereType)
		{
		}

		public LocationActiveAttribute(Type WhereType, Type JoinType)
			: base(WhereType, JoinType)
		{
		}
	}

	public abstract class LocationIDBaseAttribute : AcctSubAttribute
	{
			...
			: base()
		{
			...
			_Attributes.Add(GetSelectorAttribute(searchType));
			_SelAttrIndex = _Attributes.Count - 1;
		}
		...
		protected virtual PXDimensionSelectorAttribute GetSelectorAttribute(Type searchType)
		{
	...
	public class ValidatedAddressAttribute : PXEventSubscriberAttribute, IPXRowUpdatedSubscriber, IPXRowSelectedSubscriber
	{
		...
		public virtual void RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			int? bAccountAddressIDOld = (int?)sender.GetValue(e.OldRow, _BAccountAddressID);
			int? bAccountAddressIDNew = (int?)sender.GetValue(e.Row, _BAccountAddressID);

			if (isValidatedOld == true && isValidatedNew == true && (bAccountAddressIDOld == bAccountAddressIDNew))
			{
