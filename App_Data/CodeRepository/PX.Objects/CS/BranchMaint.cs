...
namespace PX.Objects.CS
{
	...
	public class BranchMaint : OrganizationUnitMaintBase<BranchMaint.BranchBAccount, Where<True, Equal<True>>>
	{
		...
		public partial class BranchBAccount : BAccount
		{
			...
			[PXDBString(50, IsUnicode = true, BqlField = typeof(Branch.carrierFacility))]
		...
		public BranchMaint()
		{
			bool multiBranchFeatureEnabled = PXAccess.FeatureInstalled<FeaturesSet.branch>();

			BAccount.Cache.AllowInsert = multiBranchFeatureEnabled;

			Next.SetVisible(multiBranchFeatureEnabled);
			Previous.SetVisible(multiBranchFeatureEnabled);
			Last.SetVisible(multiBranchFeatureEnabled);
			First.SetVisible(multiBranchFeatureEnabled);
			Insert.SetVisible(multiBranchFeatureEnabled);

			ChangeID.SetCategory(CS.ActionCategories.Other);
		...
		public override void Persist()
		{
			...
			if (resetPageCache && requestRelogin)
			{
				...
				{
					if (!this.UnattendedMode) throw new PXRedirectRequiredException(this, "Branch", true);
				}
			}
		}
