...
namespace PX.Objects.CS
{
	...
	public class FeatureRestrictorAttribute : PXAggregateAttribute, IPXRowSelectedSubscriber, IPXFieldVerifyingSubscriber
	{
		...
		{
			if ((bool?) e.NewValue == true && e.Row != null)
			{
				string localization = LocalizationFeaturesService.GetLocalizationForFeature(_FieldName);
				if (localization == null) return;

				ValidateLocalizationFeatureIsInstalled();
				ValidateOnlyOneLocalizationFeatureIsEnabled(sender, e);
			}
		}

		private void ValidateOnlyOneLocalizationFeatureIsEnabled(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			var allFeatures = sender.Fields.Where(f => f != _FieldName).ToList();

			foreach (var featureName in allFeatures)
			{
				string anotherLocalization = LocalizationFeaturesService.GetLocalizationForFeature(featureName);
				if(anotherLocalization == null) continue;

				var isAnotherLocalizationFeatureEnabled = (bool?) sender.GetValue(e.Row, featureName) ?? false;

				if (isAnotherLocalizationFeatureEnabled)
				{
					throw new PXSetPropertyException(Messages.OnlyOneLocalizationFeatureCanBeEnabled, PXErrorLevel.Error);
				}
			}
		}

		private void ValidateLocalizationFeatureIsInstalled()
		{
			string localization = LocalizationFeaturesService.GetLocalizationForFeature(_FieldName);
			var installedLocalizations = LocalizationFeaturesService.GetInstalledLocalizations();

			if (!installedLocalizations.Contains(localization))
			{
				throw new PXSetPropertyException(Messages.NeedToInstallLocalizationFeature, PXErrorLevel.Error);
			}
		}
