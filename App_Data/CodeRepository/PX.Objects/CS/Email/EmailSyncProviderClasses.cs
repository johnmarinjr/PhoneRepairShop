...
namespace PX.Objects.CS.Email
{
	...
	public class PXSyncCache
	{
		...
		public Dictionary<string, int> UsersCache
		{
			get
			{
				if (_UsersCache == null)
				{
					lock (_locker)
					{
						_UsersCache = new Dictionary<string, int>(StringComparer.InvariantCultureIgnoreCase);
						foreach (PXDataRecord rec in PXDatabase.SelectMulti<Contact>(
							new PXDataField<Contact.eMail>(),
							new PXDataField<Contact.contactID>(),
							new PXDataFieldValue<Contact.contactType>(ContactTypesAttribute.Employee)))
						{
							if (String.IsNullOrEmpty(rec.GetString(0))) continue;

							_UsersCache[rec.GetString(0)] = rec.GetInt32(1).Value;
						}
					}
				}
				return _UsersCache;
		...
		private Dictionary<Int32, int> _EmployeeCache;
		public Dictionary<Int32, int> EmployeeCache
		{
			get
			{
				if (_EmployeeCache == null)
				{
					lock (_locker)
					{
						_EmployeeCache = new Dictionary<Int32, int>();
						foreach (PXDataRecord rec in PXDatabase.SelectMulti<CR.BAccount>(
							new PXDataField<CR.BAccount.bAccountID>(),
							new PXDataField<CR.BAccount.defContactID>(),
							new PXDataFieldValue<CR.BAccount.type>(BAccountType.EmployeeType)))
						{
							if (rec.GetInt32(0) == null || rec.GetInt32(1) == null) continue;

							_EmployeeCache[rec.GetInt32(0).Value] = rec.GetInt32(1).Value;
						}
					}
				}
			...
			get
			{
				if (_CountryCache == null)
				{
					lock (_locker)
					{
						...
						foreach (PXDataRecord rec in PXDatabase.SelectMulti<Country>(
							new PXDataField<Country.countryID>(),
							new PXDataField<Country.description>()))
						{
						...
						foreach (PXDataRecord rec in PXDatabase.SelectMulti<State>(
							new PXDataField<State.countryID>(),
							new PXDataField<State.stateID>(),
							new PXDataField<State.name>()))
						{
	...
	public static class PXExchangeConversionHelper
	{
		...
		public static string ParseReminderMinutesBeforeStart(string reminderMinutesBeforeStart)
		{
			...
			{
				case "0":
					return RemindAtListAttribute.AtTheTimeOfEvent;
				case "5":
					return RemindAtListAttribute.Before5minutes;
				case "15":
					return RemindAtListAttribute.Before15minutes;
				case "30":
					return RemindAtListAttribute.Before30minutes;
				case "60":
					return RemindAtListAttribute.Before1hour;
				case "120":
					return RemindAtListAttribute.Before2hours;
				case "1440":
					return RemindAtListAttribute.Before1day;
				case "4320":
					return RemindAtListAttribute.Before3days;
				case "10080":
					return RemindAtListAttribute.Before1week;
				default:
					return RemindAtListAttribute.DateTimeFromExchange;
			}
		}
