...
namespace PX.Objects.CS.Email
{
	public class EmailsSyncMaint : PXGraph<EmailsSyncMaint>
	{
		...
		protected IEnumerable clearLog(PXAdapter adapter)
		{
			...
			PXLongOperation.StartOperation(this, () => ClearLogInChunks(account));
		...
		{
			var graph = PXGraph.CreateInstance<EmailsSyncMaint>();

			var minValue = new SelectFrom<EMailSyncLog>
				.Where<EMailSyncLog.address.IsEqual<@P.AsString>.And<EMailSyncLog.serverID.IsEqual<@P.AsInt>>>
				.AggregateTo<Min<EMailSyncLog.eventID>>
				.View.ReadOnly(graph)
				.SelectSingle(account.Address, account.ServerID);
			var maxValue = new SelectFrom<EMailSyncLog>
				.Where<EMailSyncLog.address.IsEqual<@P.AsString>.And<EMailSyncLog.serverID.IsEqual<@P.AsInt>>>
				.AggregateTo<Max<EMailSyncLog.eventID>>
				.View.ReadOnly(graph)
				.SelectSingle(account.Address, account.ServerID);
			if (minValue?.EventID == null || maxValue?.EventID == null)
			{
				return;
			}

			const int increment = 5000;
			int minEventID = minValue.EventID.Value;
			int maxEventID = maxValue.EventID.Value;

			for (int i = minEventID; i <= maxEventID; i += increment)
			{
				int maxEventIDToDelete = Math.Min(i + increment, maxEventID);
				...
				PXDatabase.Delete<EMailSyncLog>(
					...
					new PXDataFieldRestrict<EMailSyncLog.eventID>(PXDbType.Int, null, minEventID, PXComp.GE),
					new PXDataFieldRestrict<EMailSyncLog.eventID>(PXDbType.Int, null, maxEventIDToDelete, PXComp.LE));
			}
		}
