...
namespace PX.Objects.GL
{
  ...
  protected virtual void Batch_RowInserting(PXCache cache, PXRowInsertingEventArgs e)
		...
		/// </summary>
		private void CheckBatchBalances(Batch batch, PXCache cache)
		{
		...
		public static bool IsTransactionReclassifiable(GLTran tran, string batchType, string ledgerBalanceType, int? nonProjectID)
		{
			return (tran.ReclassBatchNbr == null
					...
					&& (tran.ProjectID == nonProjectID || tran.ProjectID == null || PXAccess.FeatureInstalled<FeaturesSet.projectAccounting>())
	...
	public class PostGraph : PXGraph<PostGraph>
	{
		...
		public virtual void IntegrityCheckProc(Ledger ledger, string startingPeriod)
		{
			...
			if (FinPeriods.FinPeriodUtils.PeriodInYear(startingPeriod) == FinPeriods.FinPeriodUtils.FirstPeriodOfYear)
			{
		...
		{			
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (PXResult<GLTran, CATran, CurrencyInfo, Account, Ledger> res in transWithdata)
				{
					...
					if(tran.BranchID != b.BranchID && !InterbranchFeatureEnabled && (ledger.BalanceType == LedgerBalanceType.Report || ledger.BalanceType == LedgerBalanceType.Statistical))
					{
		...
		public virtual void RowPersisted(PXCache sender, PXRowPersistedEventArgs e)
		{
			...
			if (hist != null && hist.AccountID == reacct && FinPeriodUtils.PeriodInYear(hist.FinPeriodID) == "01"  && e.TranStatus == PXTranStatus.Open)
			{
				...
				{									
						...
						AcctHist,
					LeftJoin<GLHistoryByCurrentPeriod,
						On<GLHistoryByCurrentPeriod.ledgerID, Equal<AcctHist.ledgerID>,
						And<GLHistoryByCurrentPeriod.branchID, Equal<AcctHist.branchID>,
						And<GLHistoryByCurrentPeriod.accountID, Equal<Required<Account.accountID>>,
						And<GLHistoryByCurrentPeriod.subID, Equal<AcctHist.subID>,
						And<GLHistoryByCurrentPeriod.finPeriodID, Equal<Required<GLHistoryByCurrentPeriod.finPeriodID>>>>>>>,
						...
						And<PX.Data.Substring<AcctHist3.finPeriodID, int1, int4>, Greater<PX.Data.Substring<GLHistoryByCurrentPeriod.lastActivityPeriod, int1, int4>>,
						And<AcctHist3.finPeriodID, LessEqual<Required<GLHistoryByCurrentPeriod.finPeriodID>>>>>>>>,
						LeftJoin<AcctHist2, //Net Income
						On<AcctHist2.ledgerID, Equal<GLHistoryByCurrentPeriod.ledgerID>,
						And<AcctHist2.branchID, Equal<GLHistoryByCurrentPeriod.branchID>,
						And<AcctHist2.accountID, Equal<GLHistoryByCurrentPeriod.accountID>,
						And<AcctHist2.subID, Equal<GLHistoryByCurrentPeriod.subID>,
						And<AcctHist2.finPeriodID, Equal<GLHistoryByCurrentPeriod.lastActivityPeriod>,
						...
						// Here is a performance optimization for MySQL server.
						// This way our platform core will translate all Where conditions
						// inside the GLHistoryByPeriod projection.
						// 
						And<GLHistoryByCurrentPeriod.subID, Equal<Required<GLHistoryByCurrentPeriod.subID>>,
						And<GLHistoryByCurrentPeriod.ledgerID, Equal<Required<GLHistoryByCurrentPeriod.ledgerID>>,
						And<GLHistoryByCurrentPeriod.branchID, Equal<Required<GLHistoryByCurrentPeriod.branchID>>,
						And<GLHistoryByCurrentPeriod.finPeriodID, Equal<Required<GLHistoryByCurrentPeriod.finPeriodID>>>>>>>>>>>>>
					.Update(sender.Graph, niacct, prevFinPeriod?.FinPeriodID, prevFinPeriod?.FinPeriodID,
							reacct, hist.SubID, hist.LedgerID, hist.BranchID, hist.FinPeriodID,
							hist.SubID, hist.LedgerID, hist.BranchID, prevFinPeriod?.FinPeriodID);   // GLHistoryByPeriod parameters
				}
