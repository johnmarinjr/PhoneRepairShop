	...
	public class GLBudgetEntry : PXGraph<GLBudgetEntry>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		...
		protected IEnumerable<GLBudgetLine> GetTreeArticle(Guid? groupID)
			...
			foreach (PXResult<GLBudgetLine, Account, Sub> result in PXSelectJoin<GLBudgetLine,
					LeftJoin<Account, On<Account.accountID, Equal<GLBudgetLine.accountID>>,
		...
		protected IEnumerable<GLBudgetLine> GetListArticle()
		{
			...
			foreach (PXResult<GLBudgetLine, Account, Sub> result in PXSelectJoin<GLBudgetLine,
					...
					Where<GLBudgetLine.branchID, Equal<Current<BudgetFilter.branchID>>,
						And<GLBudgetLine.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
						And<GLBudgetLine.finYear, Equal<Current<BudgetFilter.finYear>>,
						And2<Where<Sub.subCD, Like<Current<BudgetFilter.subCDWildcard>>,
						Or<Sub.subID, IsNull>
						>,
						And<Match<Current<AccessInfo.userName>>>>>>>,
				OrderBy<
		...
		private void CheckBudgetLinesForDuplicates()
		{
			...
			var items = cache.Cached.RowCast<GLBudgetLine>().OrderBy(a => cache.GetStatus(a));
			var duplicateCount = 0;

			foreach (GLBudgetLine item in items)
			{
				if (item.IsGroup == false && 
					cache.GetStatus(item) != PXEntryStatus.Deleted &&
					cache.GetStatus(item) != PXEntryStatus.InsertedDeleted &&
			...
			if (duplicateCount > 0) throw new PXException(Messages.DuplicateAccountSubEntry);
		}
		...
		protected void RemoveDuplicates()
		{
			var cache = BudgetArticles.Cache;
			var listCached = new HashSet<GLBudgetLineKey>();
			var items = cache.Cached.RowCast<GLBudgetLine>().OrderBy(item => cache.GetStatus(item));
		...
		private bool isImportStarted = false;

		public virtual bool PrepareImportRow(string viewName, IDictionary keys, IDictionary values)
		{
			if (!isImportStarted && BudgetArticles.Cache.IsDirty)
			{
				throw new PXException(Messages.SaveOrCancelDataModificationsBeforeImport);
			}
		...
		public virtual void distributeOK()
		{
			...
			{
				if (!(bool)article.IsGroup && (article.AccountID.HasValue && AccountDefinition.IsActive(article.AccountID.Value)
					&& article.SubID.HasValue && SubDefinition.IsActive(article.SubID.Value)))
				{
		...
		protected virtual void AllocationFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e, int fieldNbr)
		{
			...
			if (article != null)
			{
				...
				state.Enabled = state.Enabled
					&& article.AccountID.HasValue && AccountDefinition.IsActive(article.AccountID.Value)
					&& article.SubID.HasValue && SubDefinition.IsActive(article.SubID.Value);
				e.ReturnValue = val;
			}
		...
		protected virtual void BudgetFilter_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (Filter.Current.BranchID != null && Filter.Current.LedgerID != null && Filter.Current.FinYear != null && 
				...
				if (IsImport)
				{
					if (row.BranchID != null && row.FinYear != null && row.LedgerID != null &&
						(row.BranchID != old.BranchID || row.FinYear != old.FinYear || row.LedgerID != old.LedgerID)
					)
					{
						IndexesIsPrepared = false;
						PrepareIndexes();
						_BudgetArticlesIndex = null;
					}
				}
		...
		protected virtual void GLBudgetLine_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (article.AccountID != null && article.SubID != null)
			{
				PXUIFieldAttribute.SetReadOnly(sender, article, !AccountDefinition.IsActive(article.AccountID.Value)
					|| !SubDefinition.IsActive(article.SubID.Value));
			}
		}
		...
		private void PrepareIndexes(bool preload = true)
		{
			...
			{
				if ((_ArticleIndex?.IsEmpty ?? true) || !IndexesIsPrepared)
				{
					...
					_ArticleIndex = new GLBudgetLineIdx(PXSelect<GLBudgetLine,
						...
						And<GLBudgetLine.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
						And<GLBudgetLine.finYear, Equal<Current<BudgetFilter.finYear>>>>>>.Select(this));
					foreach (var a in _ArticleIndex.GetAll())	// clear previouse details
					{
						a.Allocated = null;
					}
		...
		private class AccountDefinition : IPrefetchable
		{
			...
			public void Prefetch()
			{
				foreach (PXDataRecord record in PXDatabase.SelectMulti<Account>(new PXDataField<Account.accountID>(),	new PXDataField<Account.accountCD>(), new PXDataField<Account.active>()))
				{
					int? accountID = record.GetInt32(0);
					string accountCD = record.GetString(1);
					bool? active = record.GetBoolean(2);
					_IDCD.Add(accountID??0, accountCD);
					_CDID.Add(accountCD.Trim(), accountID??0);
					_IDActive.Add(accountID ?? 0, active ?? false);
				}
			}
			...
			public static bool IsActive(int id)
			{
				bool active = false;
				var instance = PXDatabase.GetSlot<AccountDefinition>(typeof(AccountDefinition).FullName, typeof(Account));
				instance._IDActive.TryGetValue(id, out active);
				return active;
			}
		}
		...
		protected class SubDefinition : IPrefetchable
		{
			...
			public void Prefetch()
			{
				foreach (PXDataRecord record in PXDatabase.SelectMulti<Sub>(new PXDataField<Sub.subID>(), new PXDataField<Sub.subCD>(), new PXDataField<Sub.active>()))
				{
					...
					_IDActive.Add(subID ?? 0, active ?? false);
				}
