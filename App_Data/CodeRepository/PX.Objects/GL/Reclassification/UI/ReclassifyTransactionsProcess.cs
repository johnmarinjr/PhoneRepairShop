...
namespace PX.Objects.GL.Reclassification.UI
{
	public class ReclassifyTransactionsProcess : ReclassifyTransactionsBase<ReclassifyTransactionsProcess>
	{
		...
		public ReclassifyTransactionsProcess()
		{
			...
			bool projectIDVisibility = PXAccess.FeatureInstalled<FeaturesSet.projectAccounting>();

			PXUIFieldAttribute.SetVisibility<GLTran.projectID>(GLTranForReclass.Cache, null, projectIDVisibility ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
			PXUIFieldAttribute.SetVisible<GLTran.projectID>(GLTranForReclass.Cache, null, projectIDVisibility);
		...
		protected virtual void GLTranForReclassification_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<GLTranForReclassification.newProjectID>(cache, tran, canEdit);
			PXUIFieldAttribute.SetEnabled<GLTranForReclassification.newTaskID>(cache, tran, canEdit);
			PXUIFieldAttribute.SetEnabled<GLTranForReclassification.newCostCodeID>(cache, tran, canEdit);
		...
		private void SetErrorToNewFieldsIfNeed(GLTranForReclassification tran)
		{
			foreach (var fieldNameErrorPair in tran.FieldsErrorForInvalidFromValues)
			{
				if (fieldNameErrorPair.Value != null)
				{
					...
					else if (fieldNameErrorPair.Key == typeof(GLTranForReclassification.newProjectID).Name)
					{
						curValue = tran.NewProjectID;
					}
					else if (fieldNameErrorPair.Key == typeof(GLTranForReclassification.newTaskID).Name)
					{
						curValue = tran.NewTaskID;
					}
					else if (fieldNameErrorPair.Key == typeof(GLTranForReclassification.newCostCodeID).Name)
					{
						curValue = tran.NewCostCodeID;
					}
		...
		protected virtual void GLTranForReclassification_NewAccountID_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
		{
			...
			VerifyAccountGroup(cache, e, account);
		}

		private static void VerifyAccountGroup(PXCache cache, PXFieldVerifyingEventArgs e, Account account)
		{
			var row = (GLTranForReclassification)e.Row;
			if (row.NewProjectID != null && row.NewProjectID != ProjectDefaultAttribute.NonProject())
			{
				var origAccount = (Account)PXSelectorAttribute.Select<GLTranForReclassification.newAccountID>(cache, e.Row, row.AccountID);
				ActiveProjectAttribute.VerifyAccountIsInAccountGroup<GLTranForReclassification.accountID>(cache, e, origAccount, true);
				ActiveProjectAttribute.VerifyAccountIsInAccountGroup<GLTranForReclassification.newAccountID>(cache, e, account, true);
			}
		}
		...
		protected virtual void GLTranForReclassification_NewProjectID_FieldUpdating(PXCache cache, PXFieldUpdatingEventArgs e)
			=> GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newProjectID>(e.Row, e.NewValue, null);

		protected virtual void GLTranForReclassification_NewTaskID_FieldUpdating(PXCache cache, PXFieldUpdatingEventArgs e)
			=> GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newTaskID>(e.Row, e.NewValue, null);

		protected virtual void GLTranForReclassification_NewCostCodeID_FieldUpdating(PXCache cache, PXFieldUpdatingEventArgs e)
			=> GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newCostCodeID>(e.Row, e.NewValue, null);
		...
		private void VerifyProjectFields(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			if (tran.NewProjectID == null && tran.ProjectID != null)
			{
				cache.RaiseExceptionHandling<GLTranForReclassification.newProjectID>(e.Row, null,
					new PXSetPropertyException(ErrorMessages.FieldIsEmpty, PXErrorLevel.Error,
						$"[{nameof(GLTranForReclassification.newProjectID)}]"));
			}

			if (tran.NewTaskID == null && tran.TaskID != null && tran.NewProjectID != ProjectDefaultAttribute.NonProject())
			{
				cache.RaiseExceptionHandling<GLTranForReclassification.newTaskID>(e.Row, null,
					new PXSetPropertyException(ErrorMessages.FieldIsEmpty, PXErrorLevel.Error,
						$"[{nameof(GLTranForReclassification.newTaskID)}]"));
			}
			...
			if (tran.NewCostCodeID == null && tran.CostCodeID != null)
			{
				cache.RaiseExceptionHandling<GLTranForReclassification.newCostCodeID>(e.Row, null,
					new PXSetPropertyException(ErrorMessages.FieldIsEmpty, PXErrorLevel.Error,
						$"[{nameof(GLTranForReclassification.newCostCodeID)}]"));
			}
		}
		...
		private void VerifyAndRememberErorrForNewFields(GLTranForReclassification tran)
		{
			...
			VerifyAndRememberErorr<GLTranForReclassification.newProjectID>(tran.NewProjectID, tran);
			VerifyAndRememberErorr<GLTranForReclassification.newTaskID>(tran.NewTaskID, tran);
			VerifyAndRememberErorr<GLTranForReclassification.newCostCodeID>(tran.NewCostCodeID, tran);
		}
		...
		protected virtual void LoadOptions_FromTaskID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			var options = (LoadOptions)e.Row;

			var fromTaskIDState = (PXFieldState)cache.GetStateExt<LoadOptions.fromTaskID>(options);
			var toTaskIDState = (PXFieldState)cache.GetStateExt<LoadOptions.toTaskID>(options);

			var fromTaskCD = (string)fromTaskIDState.Value;
			var toTaskCD = (string)toTaskIDState.Value;

			if (string.CompareOrdinal(fromTaskCD, toTaskCD) > 0)
			{
				cache.SetValueExt<LoadOptions.toTaskID>(e.Row, fromTaskCD);
			}
		}
		...
		protected virtual void LoadOptions_ToTaskID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (toTaskCD != null && String.CompareOrdinal(fromTaskCD, toTaskCD) > 0)
			{
				cache.SetValueExt<LoadOptions.fromTaskID>(e.Row, toTaskCD);
			}
		}

		protected virtual void LoadOptions_FromCostCodeID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			var options = (LoadOptions)e.Row;

			var fromCostCodeIDState = (PXFieldState)cache.GetStateExt<LoadOptions.fromCostCodeID>(options);
			var toCostCodeIDState = (PXFieldState)cache.GetStateExt<LoadOptions.toCostCodeID>(options);

			var fromCostCodeCD = (string)fromCostCodeIDState.Value;
			var toCostCodeCD = (string)toCostCodeIDState.Value;

			if (string.CompareOrdinal(fromCostCodeCD, toCostCodeCD) > 0)
			{
				cache.SetValueExt<LoadOptions.toCostCodeID>(e.Row, fromCostCodeCD);
			}
		}

		protected virtual void LoadOptions_ToCostCodeID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			var options = (LoadOptions)e.Row;
			...
			if (toCostCodeCD != null && String.CompareOrdinal(fromCostCodeCD, toCostCodeCD) > 0)
			{
				cache.SetValueExt<LoadOptions.fromCostCodeID>(e.Row, toCostCodeCD);
			}
		}
		...
		public virtual IEnumerable Replace(PXAdapter adapter)
		{
			...
			if (ReplaceOptionsView.AskExt() == WebDialogResult.OK)
			{
				if (options.NewBranchID == null
					...
					&& options.NewProjectID == null
					&& options.NewTaskID == null
					&& options.NewCostCodeID == null)
				{
					return adapter.Get();
				}
				...
				trans = OptionallySetUpProjectOptionsToReplace(options, trans);
				...
				foreach (var tran in trans)
				{
					...
					ReplaceProjectAttributes(options, tran);
		...
		private static IEnumerable<GLTranForReclassification> OptionallySetUpProjectOptionsToReplace(ReplaceOptions options, IEnumerable<GLTranForReclassification> trans)
		{
			if (options.WithProjectID != null)
			{
				trans = trans.Where(tran => tran.NewProjectID == options.WithProjectID);
			}

			if (options.WithTaskID != null)
			{
				trans = trans.Where(tran => tran.NewTaskID == options.WithTaskID);
			}

			if (options.WithCostCodeID != null)
			{
				trans = trans.Where(tran => tran.NewCostCodeID == options.WithCostCodeID);
			}

			return trans;
		}

		private static void ReplaceProjectAttributes(ReplaceOptions options, GLTranForReclassification tran)
		{
			if (options.NewProjectID != null && options.NewProjectID != tran.NewProjectID)
			{
				tran.NewProjectID = options.NewProjectID;
			}

			if (options.NewTaskID != null && options.NewTaskID != tran.NewTaskID)
			{
				tran.NewTaskID = options.NewTaskID;
			}

			if (options.NewCostCodeID != null && options.NewCostCodeID != tran.NewCostCodeID)
			{
        ...
        private GLTranForReclassification SplitTransaction(PXCache cache, GLTranForReclassification originalTran)
        {
            ...
            var splitTran = (GLTranForReclassification)GLTranForReclass.Cache.CreateCopy(originalTran);

            splitTran.LineNbr = State.CurrentSplitLineNbr;
            splitTran.ParentKey = parentKey;
            splitTran.CuryNewAmt = 0.0m;
            splitTran.SourceCuryDebitAmt = originalTran.CuryDebitAmt;
            splitTran.SourceCuryCreditAmt = originalTran.CuryCreditAmt;
            splitTran.CuryDebitAmt = 0m;
            splitTran.CuryCreditAmt = 0m;
            splitTran.NewBranchID = originalTran.BranchID;
            splitTran.NewAccountID = originalTran.AccountID;
            splitTran.NewSubID = originalTran.SubID;
            splitTran.NewTranDesc = originalTran.TranDesc;
            splitTran.NewTranDate = originalTran.TranDate;
			SetUpProjectFieldsForSplitTransaction(originalTran, splitTran);

			splitTran.ReclassRowType = ReclassRowTypes.AddingNew;
            splitTran.IsSplitted = false;
            splitTran.Selected = false;

            State.IncSplitLineNbr();

            Batch.Current = (Batch)PXParentAttribute.SelectParent(cache, originalTran);
            splitTran = GLTranForReclass.Insert(splitTran);
			...
			if (splitTran.ProjectID != originalTran.ProjectID)
			{
				SetUpProjectFieldsForSplitTransaction(originalTran, splitTran);
			}

			GLTranForReclass.Cache.SetStatus(splitTran, PXEntryStatus.Updated);

            return splitTran;
        }

		private static void SetUpProjectFieldsForSplitTransaction(GLTranForReclassification originalTran, GLTranForReclassification splittedTran)
		{
        ...
        private bool IsAllowProcess(GLTranForReclassification tranForReclass)
        {
			...
			var projectNotNull = (tranForReclass.NewProjectID == tranForReclass.ProjectID || tranForReclass.NewProjectID != null || tranForReclass.NewProjectID == ProjectDefaultAttribute.NonProject()) &&
								 (tranForReclass.NewTaskID == tranForReclass.TaskID || tranForReclass.NewTaskID != null || tranForReclass.NewProjectID == ProjectDefaultAttribute.NonProject()) &&
								 (tranForReclass.NewCostCodeID == tranForReclass.CostCodeID || tranForReclass.NewCostCodeID != null || PXAccess.FeatureInstalled<FeaturesSet.costCodes>() == false) ||
								 PXAccess.FeatureInstalled<FeaturesSet.projectAccounting>() == false;
		...
		{
			return ltran.NewBranchID == rtran.NewBranchID && ltran.NewAccountID == rtran.NewAccountID && ltran.NewSubID == rtran.NewSubID && IsDuplicatedProjectAttributes(ltran, rtran);
		}

		private bool IsDuplicatedProjectAttributes(GLTranForReclassification ltran, GLTranForReclassification rtran)
		{
			return ltran.NewProjectID == rtran.NewProjectID && ltran.NewTaskID == rtran.NewTaskID && ltran.NewCostCodeID == rtran.NewCostCodeID;
		}
		...
		public static void TryOpenForReclassificationOfDocument(PXView askView, string module, string batchNbr, string docType, string refNbr)
		{
			...
			foreach (var row in dataRows)
			{
				...
				if (JournalEntry.IsTransactionReclassifiable(tran, null, null, ProjectDefaultAttribute.NonProject()) && IsTransactionReclassifiableByProjectSettings(graph, tran, ProjectDefaultAttribute.NonProject()))
				{
			...
			where TTran : GLTran
		{
			...
			foreach (var tran in trans)
			{
				bool isReclassifyable = JournalEntry.IsTransactionReclassifiable(tran, getBatchTypeDelegate(tran), ledger?.BalanceType, ProjectDefaultAttribute.NonProject())
					&& IsTransactionReclassifiableByProjectSettings(graph, tran, ProjectDefaultAttribute.NonProject());
		...
		public static bool IsTransactionReclassifiableByProjectSettings(PXGraph graph, GLTran tran, int? nonProjectID)
		{
			if (!PXAccess.FeatureInstalled<FeaturesSet.projectAccounting>() || tran.ProjectID == nonProjectID || tran.ProjectID == null)
			{
				return true;
			}
			...
			return SelectFrom<PMProject>
				.LeftJoin<PMTran>
					.On<PMTran.tranID.IsEqual<@P.AsInt>
						.And<PMTran.projectID.IsEqual<PMProject.contractID>>>
				.Where<PMProject.contractID.IsEqual<@P.AsInt>
						...
						.And<Where<PMTran.tranID.IsNull
								.Or<PMTran.allocated.IsEqual<False>>
								.And<PMTran.billed.IsEqual<False>>>>>.View.Select(graph, tran.PMTranID, tran.ProjectID).Any();
		}
		...
		public static void TryOpenForReclassification<TTran>(PXGraph graph,
            ...
            where TTran : GLTran
        {
			TryOpenForReclassification(graph, trans, null, tran => batchType, askView, 
        ...
        private void InitTranForReclassEditableFieldsFromTran(GLTranForReclassification tranForReclass, GLTran tran)
		{
			...
			InitTranForReclassEditableProjectFieldsFromTran(tranForReclass, tran);
		}

		private void InitTranForReclassEditableProjectFieldsFromTran(GLTranForReclassification tranForReclass, GLTran tran)
		{
		...
		private void InitTranForReclassEditableFields(GLTranForReclassification tranForReclass)
		{
			...
			tranForReclass.NewProjectID = tranForReclass.ProjectID;
		...
		{
			PXSelectBase<GLTranForReclassification> query = new PXSelectJoin<GLTranForReclassification,
																		InnerJoin<Account,
																		...
																		LeftJoin<PMProject,
																			On<PMProject.contractID, Equal<GLTranForReclassification.projectID>>,
																		LeftJoin<PMTask,
																			On<PMTask.taskID, Equal<GLTranForReclassification.taskID>>,
																		LeftJoin<PMCostCode,
																			On<PMCostCode.costCodeID, Equal<GLTranForReclassification.costCodeID>>,
																		LeftJoin<PMTran, 
																			On<PMTran.tranID, Equal<GLTran.pMTranID>>>>>>>>>>>,
																		Where<PMProject.contractID, IsNull, Or<PMProject.nonProject, Equal<True>,
																			Or2<FeatureInstalled<FeaturesSet.projectAccounting>, And<PMProject.baseType, Equal<CT.CTPRType.project>, 
																			And<PMProject.isActive, Equal<True>, And<PMProject.status, Equal<ProjectStatus.active>,
																			And<Where<PMTran.tranID, IsNull, 
																			Or<PMTran.allocated, NotEqual<True>,
																			And<PMTran.billed, NotEqual<True>>>>>>>>>>>,
			...
			OptionallyApplyProjectOptions(loadOptions, query, pars);
		...
		private void OptionallyApplyProjectOptions(LoadOptions loadOptions, PXSelectBase<GLTranForReclassification> query, List<object> pars)
		{
			if (loadOptions.ProjectID != null)
			{
				query.WhereAnd<Where<PMProject.contractID, Equal<Required<PMProject.contractID>>>>();
				pars.Add(loadOptions.ProjectID);
			}

			if (loadOptions.FromTaskID != null)
			{
				var project = PMTask.PK.Find(this, loadOptions.FromTaskID);
				query.WhereAnd<Where<PMTask.taskCD, GreaterEqual<Required<PMTask.taskCD>>>>();
				pars.Add(project.TaskCD);
			}

			if (loadOptions.ToTaskID != null)
			{
				var project = PMTask.PK.Find(this, loadOptions.ToTaskID);
				query.WhereAnd<Where<PMTask.taskCD, LessEqual<Required<PMTask.taskCD>>>>();
				pars.Add(project.TaskCD);
			}

			if (loadOptions.FromCostCodeID != null)
			{
				var project = PMCostCode.PK.Find(this, loadOptions.FromCostCodeID);
				query.WhereAnd<Where<PMCostCode.costCodeCD, GreaterEqual<Required<PMCostCode.costCodeCD>>>>();
				pars.Add(project.CostCodeCD);
			}

			if (loadOptions.ToCostCodeID != null)
			{
				var project = PMCostCode.PK.Find(this, loadOptions.ToCostCodeID);
        ...
        public partial class LoadOptions : IBqlTable
		{
			...
			#endregion
			#region FromTaskID
			public abstract class fromTaskID : PX.Data.BQL.BqlInt.Field<fromTaskID> { }

			[BaseProjectTask(typeof(projectID), BatchModule.GL, IsDBField = false, DisplayName = "From Project Task", AllowInactive = false)]
			...
			[BaseProjectTask(typeof(projectID), BatchModule.GL, IsDBField = false, DisplayName = "To Project Task", AllowInactive = false)]
			...
			#endregion
			#region FromCostCodeID
			public abstract class fromCostCodeID : PX.Data.BQL.BqlInt.Field<fromCostCodeID> { }

			[CostCode(DisplayName = "From Cost Code", IsDBField = false)]
			public virtual int? FromCostCodeID
			{
				get;
				set;
			}
			...
			public virtual int? ToCostCodeID
			{
				get;
				set;
			}
		...
		public partial class ReplaceOptions : IBqlTable
		{
			...
			#endregion
			#region WithProjectID
			public abstract class withProjectID : PX.Data.BQL.BqlInt.Field<withProjectID> { }

			[ActiveProject(DisplayName = "Project", IsDBField = false)]
			public virtual int? WithProjectID
			{
				get;
				set;
			}
			#endregion
			#region WithTaskID
			public abstract class withTaskID : PX.Data.BQL.BqlInt.Field<withTaskID> { }

			[BaseProjectTask(typeof(withProjectID), BatchModule.GL, IsDBField = false, DisplayName = "Project Task", AllowInactive = false)]
			public virtual int? WithTaskID
			{
				get;
				set;
			}
			#endregion
			#region WithCostCodeID
			public abstract class withCostCodeID : PX.Data.BQL.BqlInt.Field<withCostCodeID> { }

			[CostCode(typeof(withAccountID), typeof(withTaskID), IsDBField = false, DisplayName = "Cost Code")]
			public virtual int? WithCostCodeID
			{
				get;
				set;
			}
			...
			[Branch(searchType: typeof(Search2<
				Branch.branchID,
				InnerJoin<Organization,
						On<Branch.organizationID, Equal<Organization.organizationID>>>,
				Where<Branch.baseCuryID, EqualBaseCuryID<Current<ReplaceOptions.withBranchID>>,And<MatchWithBranch<Branch.branchID>>>>),
				useDefaulting: false, DisplayName = "Branch", IsDBField = false, Required = false)]
			...
			[BaseProjectTask(typeof(newProjectID), BatchModule.GL, IsDBField = false, DisplayName = "Project Task", AllowInactive = false)]
			...
			[CostCode(typeof(newAccountID), typeof(newTaskID), IsDBField = false, DisplayName = "Cost Code")]
