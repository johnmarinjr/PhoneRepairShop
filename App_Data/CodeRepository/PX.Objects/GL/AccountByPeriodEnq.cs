...
namespace PX.Objects.GL
{
	...
	public partial class GLTranR : GLTran, ISignedBalances
	{
		...
		[PXDefault(StringColor.Default)]
		public StringColor MLScoreColor { get; set; }
	...
	public class AccountByPeriodEnq : PXGraph<AccountByPeriodEnq>
	{
		...
		public Ledger CurrentLedger => PXSelectReadonly<Ledger, Where<Ledger.ledgerID, Equal<Current<AccountByPeriodFilter.ledgerID>>>>.Select(this);
		...
		public virtual IEnumerable ReclassifyAll(PXAdapter adapter)
		{
			...
			ReclassifyTransactionsProcess.TryOpenForReclassification<GLTranR>(this, trans,
				CurrentLedger,
				tran => tran.BatchType,
				GLTranEnq.View,
				...
				if (filter.ShowSummary == false)
				{
					if (filter.IncludeReclassified == false)
					{
						...
						.Select(result =>
						{
							...
							ADL.Sub sub = result;
							GL.Sub glSub = GL.Sub.PK.Find(this, sub.SubID);
							if (GroupHelper.IsAccessibleToUser(this.Caches[typeof(GL.Sub)], glSub, Accessinfo.UserName, forceUnattended: true))
							{
							SetOrigAmounts(tran);
		...
		protected virtual void GLTranR_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			bool tranSelectableForReclass = CannotBeReclassified(tran);
		...
		{
			 return !JournalEntry.IsTransactionReclassifiable(tran, tran.BatchType, CurrentLedger.BalanceType, ProjectDefaultAttribute.NonProject())
