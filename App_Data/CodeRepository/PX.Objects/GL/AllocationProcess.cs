...
namespace PX.Objects.GL
{
	...
	public class AllocationProcess : PXGraph<AllocationProcess>
	{
		...
		protected virtual void ProcessAllocation(GLAllocation allocation, string masterPostPeriod, DateTime? aDate) 
		{
			...
			{
				_subMask = (this.Caches[typeof(Sub)].GetStateExt<Sub.subCD>(null) as PXStringState)?.InputMask;
				List<string> notCreatedSubCDList = new List<string>();
				foreach (GLAllocationAccountHistory src in allocationTrans.Values)
				{
					List<string> notCreatedSubCd = new List<string>();
					...
					switch (allocation.AllocMethod)
					{
						...
						case Constants.AllocationMethod.ByPercent:
							this.RetrievePredefinedDestribution(out destAccounts, allocation, src, out notCreatedSubCd);
							break;
						case Constants.AllocationMethod.ByAcctPTD:
						case Constants.AllocationMethod.ByAcctYTD:
							this.RetrieveDestributionByAcctState(out destAccounts, allocation, masterPostPeriod, src, out notCreatedSubCd);
							break;
					...
					else if (notCreatedSubCd.Count > 0)
					{
						foreach (string s in notCreatedSubCd)
						{
							if (!notCreatedSubCDList.Contains(s))
							{
								notCreatedSubCDList.Add(s);
							}
						}
					}
					else
					{
						int? iAcctSrc = src.ContrAccontID ?? src.AccountID;
						decimal multiplier = AccountRules.IsCreditBalance(acctDefs[(int)iAcctSrc].Type) ? Decimal.MinusOne : Decimal.One;
						DistributeAmount(batchGraph, allocation, (decimal)src.AllocatedAmount * multiplier, destAccounts, allocLedger);
					}
				}
				if (notCreatedSubCDList.Count > 5)
				{
					PXTrace.WriteError(Messages.AmountCannotBeDistributed, notCreatedSubCDList.Aggregate((a, b) => a + ", " + b));
					throw new PXException(Messages.AmountCannotBeDistributedTrace);
				}
				else if (notCreatedSubCDList.Count > 0)
				{
					throw new PXException(Messages.AmountCannotBeDistributed, notCreatedSubCDList.Aggregate((a, b) => a + ", " + b));
				}
			}
			else
			{
		...
		protected virtual void RetrievePredefinedDestribution(out AccountWeightedList aAccounts, GLAllocation allocation) 
		{
			...
			PXSelectBase<GLAllocationDestination> destSelect = new PXSelect<GLAllocationDestination,
																			Where<GLAllocationDestination.gLAllocationID,
																			Equal<Required<GLAllocationDestination.gLAllocationID>>>>(this);
			foreach (PXResult<GLAllocationDestination> iRes in destSelect.Select(allocation.GLAllocationID))
			{
				GLAllocationDestination iDest = iRes;
				Account acctDef = Account.UK.Find(this, iDest.AccountCD);
				Sub subDef = Sub.UK.Find(this, iDest.SubCD);
				if (acctDef.Active != true)
				{
					throw new PXException(Messages.AllocationDestinationAccountsContainInactiveAccount);
				}
				aAccounts.Add(iDest.BranchID.Value, acctDef.AccountID.Value, subDef.SubID.Value, iDest.Weight.Value);
			}
		}

		protected virtual void RetrievePredefinedDestribution(out AccountWeightedList aAccounts, GLAllocation allocation, GLAllocationAccountHistory srcAcc, out List<string> notCreatedSubAccounts)
		{
			aAccounts = new AccountWeightedList();
			...
			{
				GLAllocationDestination iDest = iRes;
				Account acctDest = Account.UK.Find(this, iDest.AccountCD);
				Sub subDest = Sub.UK.Find(this, iDest.SubCD);

				if (acctDest == null)
				{
					acctDest = Account.PK.Find(this, srcAcc.AccountID);
				}
				if (acctDest.Active != true)
				{
					throw new PXException(Messages.AllocationDestinationAccountsContainInactiveAccount);
				}
				else if (subDest != null)
				{
					aAccounts.Add(iDest.BranchID.Value, acctDest.AccountID.Value, subDest.SubID.Value, iDest.Weight.Value);
				}
				else
				{
					// parse subacoount mask
					Sub subSrc = Sub.PK.Find(this, srcAcc.SubID);
					string newSubCD = "";
					...
					for (int i = 1; i <= segCount; i++)
					{
						...
						String segvalue = (iDest.SubCD == null) ? "" : iDest.SubCD.Substring(keylength, iDest.SubCD.Length < keylength + list[0].Length ? iDest.SubCD.Length - keylength : list[0].Length);

						if (segvalue.Trim() == "")
						{
							// replace blank segment with value from Source
							String segSrcValue = subSrc.SubCD.Substring(keylength, subSrc.SubCD.Length < keylength + list[0].Length ? subSrc.SubCD.Length - keylength : list[0].Length);
							newSubCD += segSrcValue;
						}
						else
						{
							newSubCD += segvalue;
					...
					// check if new SubCd exists in a system and add new one if lookup mode = by value
					int? newSubID = SubAccountMaint.FindSubIDByCD(this, newSubCD);
					if (newSubID == null)
					{
						if (definition.LookupModes[SubAccountAttribute.DimensionName] == DimensionLookupMode.BySegmentsAndAllAvailableSegmentValues)
						{
							// subaccount does not exist in a system -> create
							Sub subAcc = new Sub();
							subAcc.SubCD = newSubCD;
							subAcc.Active = true;

							subMaint.SubRecords.Insert(subAcc);
							subMaint.Save.Press();

							//newSubID = SubAccountMaint.FindSubIDByCD(this, newSubCD);
							newSubID = Sub.UK.Find(this, newSubCD).SubID;
							aAccounts.Add(iDest.BranchID.Value, acctDest.AccountID.Value, (int)newSubID, iDest.Weight.Value);
						}
						else
						{
							aAccounts = new AccountWeightedList();
							notCreatedSubAccounts.Add(_subMask == null ? newSubCD : PX.Common.Mask.Format(_subMask, newSubCD));
						}
					}
					else
					{
		...
		protected virtual bool RetrieveDestributionByAcctState(out AccountWeightedList aAccounts, GLAllocation allocation, string masterPostPeriodID, GLAllocationAccountHistory srcAcc, out List<string> notCreatedSubAccounts)
		{
			PXSelectBase<GLAllocationDestination> destSelect = new PXSelect<GLAllocationDestination,
			...
			{
				GLAllocationDestination iDest = iRes;
				Account destAccount = Account.UK.Find(this, iDest.AccountCD);
				Sub destSubAccount = Sub.UK.Find(this, iDest.SubCD);
				//AccountWeight acctWeight = null;//new AccountWeight(iDest.BranchID.Value, destAccount.AccountID.Value, destSubAccount.SubID.Value, 0.0m);

				if (destAccount == null)
				{
					destAccount = Account.PK.Find(this, srcAcc.AccountID);
				}
				if (destAccount.Active != true)
				{
					throw new PXException(Messages.AllocationDestinationAccountsContainInactiveAccount);
				}
				else if (destSubAccount == null)
				{
					// parse subacount mask
					Sub subSrc = Sub.PK.Find(this, srcAcc.SubID);
					string newSubCD = "";
					PXDimensionAttribute.Definition definition = PX.Common.PXContext.GetSlot<PXDimensionAttribute.Definition>();
					int segCount = definition.Dimensions[SubAccountAttribute.DimensionName].Count();
					...
					if (newSubID == null)
					{
						if (definition.LookupModes[SubAccountAttribute.DimensionName] == DimensionLookupMode.BySegmentsAndAllAvailableSegmentValues)
						{
							...
							//newSubID = Sub.UK.Find(this, newSubCD).SubID;
							destSubAccount = Sub.UK.Find(this, newSubCD);
							//acctWeight = new AccountWeight(iDest.BranchID.Value, destAccount.AccountID.Value, (int)newSubID, 0.0m);
						}
						else
						{
							aAccounts = new AccountWeightedList();
				...
				AccountWeight acctWeight = new AccountWeight(iDest.BranchID.Value, destAccount.AccountID.Value, destSubAccount.SubID.Value, 0.0m);
