...
{
	[Serializable]
	public partial class JournalWithSubEntry : PXGraph<JournalWithSubEntry, GLDocBatch>, PXImportAttribute.IPXPrepareItems
	{
		[InjectDependency]
		...
		public PXSelect<CM.Extensions.CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Current<GLDocBatch.curyInfoID>>>> currencyinfoEx;
			...
			switch (doc.TranModule)
			{
				case GL.BatchModule.AP:
					setupField = APInvoiceType.NumberingAttribute.GetNumberingIDField(doc.TranType) ??
								 APPaymentType.NumberingAttribute.GetNumberingIDField(doc.TranType) ??
								 AP.Standalone.APQuickCheckType.NumberingAttribute.GetNumberingIDField(doc.TranType);
				...
				case GL.BatchModule.AR:
					setupField = ARInvoiceType.NumberingAttribute.GetNumberingIDField(doc.TranType) ?? 
								 ARPaymentType.NumberingAttribute.GetNumberingIDField(doc.TranType) ??
								 AR.Standalone.ARCashSaleType.NumberingAttribute.GetNumberingIDField(doc.TranType);
			...
			if (String.IsNullOrEmpty(source.TranType) == false && source.TranDate.HasValue && source.IsChildTran == false && String.IsNullOrEmpty(source.RefNbr))
			{
					...
					&& source.BAccountID.HasValue && source.LocationID.HasValue)
				{
					...
					if (String.IsNullOrEmpty(source.RefNbr))
					{
						...
						//Probably, will not be required afer switching GL to new CM
						CurrencyInfo new_CuryInfo = (CurrencyInfo)currencyinfo.Cache.Insert(new CurrencyInfo
						{
							CuryID = source.CuryID
						});
						currencyinfoEx.Insert(CM.Extensions.CurrencyInfo.GetEX(new_CuryInfo));

						APRegister refDoc = (APRegister)cache.Insert(new APRegister
						{
							...
							CuryID = source.CuryID,
							CuryInfoID = new_CuryInfo.CuryInfoID
						});
					...
					&& source.BAccountID.HasValue && source.LocationID.HasValue)
				{
					...
					if (String.IsNullOrEmpty(source.RefNbr))
					{
						...
						ARRegister refDoc = (ARRegister)cache.Insert(new ARRegister
						{
							DocType = source.TranType,
							DocDate = source.TranDate,
						...
						&& source.CashAccountID.HasValue)
				{
					...
					if (String.IsNullOrEmpty(source.RefNbr))
					{
						...
						CAAdj refDoc = (CAAdj)cache.Insert(new CAAdj
						{
					...
					&& (source.DebitAccountID.HasValue || source.CreditAccountID.HasValue))
				{
					...
					if (String.IsNullOrEmpty(source.RefNbr))
					{
						...
						Batch refDoc = (Batch)cache.Insert(new Batch
						{
		...
		protected virtual void TryToSetDefaultCashAccountForAP(GLTranDoc aRow, bool isCredit)
		{
			switch (aRow.TranType)
			{
				...
				case AP.APPaymentType.Check:
					if (isCredit)
					{
						CashAccount cashAccount = FindDefaultAPCashAccount(aRow.PaymentMethodID, Location.Select(aRow.BAccountID, aRow.LocationID), aRow.BranchID);
						if (cashAccount != null)
						{
							this.SetCashAccount(cashAccount, isCredit);
						}
				...
				CashAccount acct = PXSelectJoin<CashAccount, InnerJoin<PaymentMethodAccount,
														On<PaymentMethodAccount.cashAccountID, Equal<CashAccount.cashAccountID>,
														 ...
														 Where<PaymentMethodAccount.paymentMethodID, Equal<Required<PaymentMethodAccount.paymentMethodID>>,
															...
															And<CashAccount.branchID, Equal<Required<GLTranDoc.branchID>>>>>>>.Select(this, aPaymentMethodID, aBranchID);
				return acct;
			}
			return null;
		}
		...
		protected virtual int? FindDefaultARAccount(GLTranDoc aRow, bool isCredit, Location location)
		{
			switch (aRow.TranType)
			{
				...
				case AR.ARInvoiceType.CashSale:
					if (isCredit == false)
					{
						CashAccount cashAccount = FindDefaultARCashAccount(aRow.PaymentMethodID, aRow.PMInstanceID, aRow.CuryID, aRow.BranchID);
						if (cashAccount != null)
						{
							this.SetCashAccount(cashAccount, isCredit);
							return cashAccount.AccountID;
		...
		{
			return FindDefaultARCashAccount(aPaymentMethodID, aPMInstanceID, BatchModule.Current.CuryID, null);
		}

		[Obsolete]
		protected virtual CashAccount FindDefaultARCashAccount(string aPaymentMethodID, int? aPMInstanceID, string curyID)
		{
			return FindDefaultARCashAccount(aPaymentMethodID, aPMInstanceID, curyID, null);
		}

		protected virtual CashAccount FindDefaultARCashAccount(string aPaymentMethodID, int? aPMInstanceID, string curyID, int? branchID)
		{
			...
			if (aPMInstanceID.HasValue)
			{
				cashAcct = PXSelectJoin<CashAccount, InnerJoin<CustomerPaymentMethod, On<CashAccount.cashAccountID, Equal<CustomerPaymentMethod.cashAccountID>>>,
										Where<CustomerPaymentMethod.pMInstanceID, Equal<Required<CustomerPaymentMethod.pMInstanceID>>,
											   And<CustomerPaymentMethod.isActive, Equal<True>,
											   And<Match<Current<AccessInfo.userName>>>>>>.Select(this, aPMInstanceID.Value);
			...
			if (cashAcct == null)
			{
				cashAcct = PXSelectJoin<CashAccount,
											...
											And<PaymentMethodAccount.useForAR, Equal<True>,
												...
												And2<Match<Current<AccessInfo.userName>>,
												And<CashAccount.branchID, Equal<Required<GLTranDoc.branchID>>>>>>>>, OrderBy<Desc<PaymentMethodAccount.aRIsDefault>>>.Select(this, aPaymentMethodID, curyID, branchID);
			}
			return cashAcct;
		}
		#endregion
		...
		protected virtual int? FindDefaultSubAccount(GLTranDoc aRow, bool isCredit)
		{
			...
			if (cashAccount == null && keyAccount != null && IsPaymentType(aRow))
			{
				TryToSetDefaultCashAccount(aRow, isCredit);
		...
		private void TryToSetDefaultCashAccount(GLTranDoc aRow, bool isCredit)
		{
			if (aRow.TranModule == GL.BatchModule.AP)
			{
				TryToSetDefaultCashAccountForAP(aRow, isCredit);
			}
			if (aRow.TranModule == GL.BatchModule.AR)
			{
				TryToSetDefaultCashAccountForAR(aRow, isCredit);
			}
		}
		...
		private bool IsPaymentType(GLTranDoc aRow)
		{
			if (aRow.TranModule == GL.BatchModule.AP)
			{
				return aRow.TranType == AP.APInvoiceType.QuickCheck ||
						aRow.TranType == AP.APPaymentType.Prepayment ||
						aRow.TranType == AP.APPaymentType.Check ||
						aRow.TranType == AP.APInvoiceType.VoidQuickCheck ||
						aRow.TranType == AP.APPaymentType.Refund;
			}
			if (aRow.TranModule == GL.BatchModule.AR)
			{
				return aRow.TranType == AR.ARPaymentType.CreditMemo ||
						aRow.TranType == AR.ARPaymentType.CashSale ||
						aRow.TranType == AR.ARPaymentType.Prepayment ||
						aRow.TranType == AR.ARPaymentType.Payment ||
						aRow.TranType == AR.ARPaymentType.CashReturn ||
						aRow.TranType == AR.ARPaymentType.Refund;
			}
		...
		{
			if (!(e.Row is APAdjust adj)) return;

			//TODO: add condition to prevent additional calls?
			//Condition if (adj.AdjdCuryInfoID == null) is bad condition
			...
			try
			{
						...
						And<APInvoice.refNbr, Equal<Required<APInvoice.refNbr>>>>>>.Select(this, adj.AdjdDocType, adj.AdjdRefNbr))
				{
					...
					APAdjust_AdjdRefNbr_FieldUpdated(new APRegisterAdapter<APInvoice>(doc), curyInfo, adj);
					return;
				}
										...
										And<GLTranDoc.refNbr, Equal<Required<GLTranDoc.refNbr>>>>>>>>>.Select(this, adj.VendorID, adj.AdjdDocType, adj.AdjdRefNbr))
				{
					GLTranDoc doc = res;
					CurrencyInfo curyInfo = res;
					APAdjust_AdjdRefNbr_FieldUpdated(new GlRegisterAdapter<GLTranDoc>(doc), curyInfo, adj);
					return;
				}
						...
						Where<APPayment.vendorID, Equal<Current<GLTranDocAP.bAccountID>>, And<APPayment.docType, Equal<Required<APPayment.docType>>, And<APPayment.refNbr, Equal<Required<APPayment.refNbr>>>>>>.Select(this, adj.AdjdDocType, adj.AdjdRefNbr))
				{
					APPayment doc = res;
					CurrencyInfo curyInfo = res;
					APAdjust_AdjdRefNbr_FieldUpdated(new APRegisterAdapter<APPayment>(doc), curyInfo, adj);
				}
			}
		...
		private void APAdjust_AdjdRefNbr_FieldUpdated<T>(RegisterAdapter<T> invoice, CurrencyInfo info, APAdjust adj)
			where T : class, IBqlTable, IInvoice, new()
		{
			...
			currencyinfoEx.Insert(CM.Extensions.CurrencyInfo.GetEX(info_copy));
			...
			CM.Extensions.APPaymentBalanceCalculator aPPaymentBalanceCalculator = new CM.Extensions.APPaymentBalanceCalculator(CurrencyInfo_CuryInfoID);
			aPPaymentBalanceCalculator.CalcBalances(adj, invoice.Entity, false, true, null);
			...
			aPPaymentBalanceCalculator.CalcBalances(adj, invoice.Entity, true, true, null);
		}
		...
		protected virtual void APAdjust_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (AdjdDocDateIsAfterParentTranDate(doc, parentDoc) && (doc.AdjgDocType != APDocType.Check && doc.AdjgDocType != APDocType.VoidCheck && doc.AdjgDocType != APDocType.Prepayment || apsetup.Current.EarlyChecks == false))
			{
		...
		{
			APAdjust adj = row;
			CM.Extensions.APPaymentBalanceCalculator ApAdjBalanceCalculator = new CM.Extensions.APPaymentBalanceCalculator(CurrencyInfo_CuryInfoID);

			foreach (APInvoice voucher in APInvoice_VendorID_DocType_RefNbr.Select(adj.VendorID, adj.AdjdDocType, adj.AdjdRefNbr))
			{
				ApAdjBalanceCalculator.CalcBalances(adj, voucher, isCalcRGOL, DiscOnDiscDate, null);
				return;
			}
			...
			if (protoInvoice0 != null)
			{
				...
				ApAdjBalanceCalculator.CalcBalances(adj, protoInvoice1, isCalcRGOL, DiscOnDiscDate, null);
				return;
			}

			foreach (APPayment payment in APPayment_VendorID_DocType_RefNbr.Select(adj.VendorID, adj.AdjdDocType, adj.AdjdRefNbr))
			{
				ApAdjBalanceCalculator.CalcBalances(adj, payment, isCalcRGOL, DiscOnDiscDate, null);
			}
		}
		...
		{
			if (!(e.Row is ARAdjust adj)) return;
			...
			try
			{
						...
						And<ARInvoice.refNbr, Equal<Required<ARInvoice.refNbr>>>>>>.Select(this, adj.AdjdDocType, adj.AdjdRefNbr))
				{
					ARInvoice doc = res;
					CurrencyInfo curyInfo = res;
					ARAdjust_AdjdRefNbr_FieldUpdated(new ARRegisterAdapter<ARInvoice>(doc), curyInfo, adj);
					return;
				}
										...
										And<GLTranDoc.refNbr, Equal<Required<GLTranDoc.refNbr>>>>>>>>>.Select(this, adj.CustomerID, adj.AdjdDocType, adj.AdjdRefNbr))
				{
					GLTranDoc doc = res;
					CurrencyInfo curyInfo = res;
					ARAdjust_AdjdRefNbr_FieldUpdated(new GlRegisterAdapter<GLTranDoc>(doc), curyInfo, adj);
					return;
				}
						...
						Where<ARPayment.customerID, Equal<Current<GLTranDocAR.bAccountID>>, And<ARPayment.docType, Equal<Required<ARPayment.docType>>, And<ARPayment.refNbr, Equal<Required<ARPayment.refNbr>>>>>>.Select(this, adj.AdjdDocType, adj.AdjdRefNbr))
				{
					ARPayment doc = res;
					CurrencyInfo curyInfo = res;
					ARAdjust_AdjdRefNbr_FieldUpdated(new ARRegisterAdapter<ARPayment>(doc), curyInfo, adj);
				}
			}
			catch (PXSetPropertyException ex)
			{
		...
		private void ARAdjust_AdjdRefNbr_FieldUpdated<T>(T invoice, CurrencyInfo aInvoiceInfo, ARAdjust adj)
			where T : class, IRegister, IInvoice
		{
			...
			CurrencyInfo info_copy = PXCache<CurrencyInfo>.CreateCopy(aInvoiceInfo);
			info_copy.CuryInfoID = null;
			info_copy = (CurrencyInfo)currencyinfo.Cache.Insert(info_copy);
			info_copy.SetCuryEffDate(currencyinfo.Cache, docDate);
			currencyinfoEx.Insert(CM.Extensions.CurrencyInfo.GetEX(info_copy));
		...
		protected virtual void ARAdjust_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			GLTranDocAR parentDoc = ARPayments.Search<GLTranDocAR.tranType, GLTranDocAR.refNbr>(doc.AdjgDocType, doc.AdjgRefNbr);
			PXCache parentCache = ARPayments.Cache;
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Delete)
			{
				return;
			}
			...
			if (AdjdDocDateIsAfterParentTranDate(doc, parentDoc))
			{
			...
			if (doc.AdjdFinPeriodID.CompareTo(parentDoc.FinPeriodID) > 0)
			{
		...
		{
			if (doc.AdjdDocDate == null) throw new ArgumentException("doc.AdjdDocDate was null");
			if (parentDoc.TranDate == null) throw new ArgumentException("parentDoc.TranDate was null");
			...
			where T : class, IInvoice
		{
			...
			CuryHelper curyHelper = new CuryHelper(CurrencyInfo_CuryInfoID);
			new CM.Extensions.PaymentBalanceCalculator(curyHelper).CalcBalances(adj.AdjgCuryInfoID, adj.AdjdCuryInfoID, invoice, adj);

			if (DiscOnDiscDate)
			{
				PaymentEntry.CalcDiscount(adj.AdjgDocDate, invoice, adj);
			}
			PaymentEntry.WarnDiscount(this, adj.AdjgDocDate, invoice, adj);
	...
	public class GLBatchDocRelease : PXGraph<GLBatchDocRelease>
	{
		...
		private ARRegister ObtainCorrespondingARRegisterEntry(GLTranDoc doc)
		{
			if (string.IsNullOrEmpty(doc.RefNbr)) return null;
			else
			{
				PXResult<ARRegister, AR.Standalone.ARInvoice, AR.Standalone.ARPayment, Batch> res = PXSelectJoin<ARRegister,
				...
				Where<ARRegister.docType, Equal<Required<ARRegister.docType>>,
					...
					.SelectWindowed(this, 0, 1, doc.TranType, doc.RefNbr)
					.FirstOrDefault() as PXResult<ARRegister, AR.Standalone.ARInvoice, AR.Standalone.ARPayment, Batch>;

				ARRegister arRegister = res;
				if (arRegister != null) AssertNotAlreadyExists(res, res, res);
				return arRegister;
			}
		}

		private static void AssertNotAlreadyExists(AR.Standalone.ARInvoice arInvoice, AR.Standalone.ARPayment arPayment, Batch batch)
		{
			if (!String.IsNullOrEmpty(arInvoice?.RefNbr)
				|| !String.IsNullOrEmpty(arPayment?.RefNbr)
				|| !String.IsNullOrEmpty(batch?.BatchNbr))
				throw new PXException(Messages.DocumentAlreadyExists);
		}
		...
		protected virtual PXGraph CreateAR(GLTranDoc doc, List<GLTranDoc> aList, List<Batch> toPost)
		{
			PXGraph result = null;
			bool multiLine = aList.Count > 1;
			ARRegister arRegister = ObtainCorrespondingARRegisterEntry(doc);
			...
			if (JournalWithSubEntry.IsARInvoice(doc) && JournalWithSubEntry.IsARPayment(doc))
			{
				...
				Copy(copy, doc);
				copy.Hold = true;
				if (_suppressARPrintVerification && qcGraph.arsetup.Current.PrintBeforeRelease == true)
				{
					copy.DontPrint = true;
				}
				if (_suppressARPrintVerification && qcGraph.arsetup.Current.EmailBeforeRelease == true)
				{
					copy.DontEmail = true;
				}
				row = qcGraph.Document.Update(copy);
		...
		protected virtual PXGraph CreateCA(GLTranDoc doc, List<GLTranDoc> aList, List<Batch> toPost)
		{
			...
			if (doc.TranModule == GL.BatchModule.CA && doc.TranType == CA.CATranType.CAAdjustment)
			{
				...
				var caGraphMultiCurrency = caGraph.GetExtension<CA.MultiCurrency.CATranEntryMultiCurrency>();
				...
				CloneCuryInfo(caGraphMultiCurrency.currencyinfo, doc.CuryInfoID);
				foreach (GLTranDoc iRow in aList)
				{
					if (multiLine && Object.ReferenceEquals(doc, iRow)) continue;
					...
					OptionallyUpdateProjectFields(caGraph, iRow, tran, tranCopy);
				}
		...
		private void OptionallyUpdateProjectFields(CATranEntry caGraph, GLTranDoc iRow, CASplit tran, CASplit tranCopy)
		{
			if (tran.ProjectID == tranCopy.ProjectID || tranCopy.ProjectID != ProjectDefaultAttribute.NonProject())
			{
				return;
			}

			CopyProjectFields(tran, iRow);
			caGraph.CASplitRecords.Update(tranCopy);
		}
		...
		protected virtual void Copy(GLTran aDest, GLTranDoc aSrc, bool debit)
		{
			...
			var length = aSrc.ExtRefNbr?.Length ?? 0;
			aDest.RefNbr = aSrc.ExtRefNbr?.Substring(0, length > RefNbrLength ? RefNbrLength : length);
		}
		#endregion
		...
		protected void CloneCuryInfo(PXSelectBase<CM.Extensions.CurrencyInfo> aCuryInfoView, long? aCuryInfoID)
		{
			if (aCuryInfoID == null) return;
			...
			foreach (CM.Extensions.CurrencyInfo info in aCuryInfoView.Select())
			{
				CM.Extensions.CurrencyInfo new_info = CM.Extensions.CurrencyInfo.GetEX(tranCuryInfo);
		...
		private static void SetFinPeriodID(GLTranDoc doc, APPayment row)
		{
			row.FinPeriodID = doc.FinPeriodID;
			row.AdjFinPeriodID = doc.FinPeriodID;
		}

		private static void SetFinPeriodID(GLTranDoc doc, ARPayment row)
		{
			row.FinPeriodID = doc.FinPeriodID;
			row.AdjFinPeriodID = doc.FinPeriodID;
		}

		private static void SetFinPeriodID(GLTranDoc doc, CAAdj row)
		{
			row.FinPeriodID = doc.FinPeriodID;
		}
		#endregion
