...
namespace PX.Objects.GL
{
	...
	public class FinPeriodStatusProcess : PXGraph<FinPeriodStatusProcess>
	{
		...
		{
			if (RunningFlagScope<FinPeriodStatusProcess>.IsRunning) return;

			FinPeriodStatusProcess graph = this.Clone();

			using(new RunningFlagScope<FinPeriodStatusProcess>())
			{
				graph.Filter.Current = (FinPeriodStatusProcessParameters)this.Filter.Cache.CreateCopy(this.Filter.Current);
			}
			...
			switch (parameters.Action)
			{
				case FinPeriodStatusProcessParameters.action.Close:
					...
					if(subledgerTitles.Any())
					{
						...
						WebDialogResult result = Filter.Ask("ConfirmProcessingKey", "", message, MessageButtons.OKCancel);
