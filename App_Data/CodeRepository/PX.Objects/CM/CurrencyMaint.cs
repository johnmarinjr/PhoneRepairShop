...
namespace PX.Objects.CM
{
	public class CurrencyMaint : PXGraph<CurrencyMaint, CurrencyList>
	{
		...
		protected void _(Events.RowSelected<CurrencyList> e)
		{
			...
			CuryListRecords.Cache.AllowDelete = true;
			CuryRecords.Cache.AllowDelete = true;
			...
			if (IsBaseCurrency(currencyList.CuryID)) 
			{
...
;			}
		...
		{
			return GetSettingsByFeatures(curyID, isFinancial);
		}
		protected virtual SettingsByFeatures GetSettingsByFeatures(string curyID, bool? isFinancial)
		{
			var isBaseCury = IsBaseCurrency(curyID);
			...
			else // If the Multiple Base Currencies feature is off
			{
				...
				if ((f_finStandart || f_invoiceRounding) && isBaseCury)
				{
		...
		protected void _(Events.RowSelected<Currency> e)
		{
			...
			SettingsByFeatures f_setting = GetSettingsByFeatures(currencyRow.CuryID, currencyListRow?.IsFinancial ?? false);
		...
		protected void _(Events.RowPersisting<Currency> e)
		{
			...
			var cache = e.Cache;
			SettingsByFeatures f_setting = GetSettingsByFeatures(row.CuryID, currencyListRow?.IsFinancial ?? false);
