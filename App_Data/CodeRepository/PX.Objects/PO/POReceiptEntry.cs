	...
	public class POReceiptEntry : PXGraph<POReceiptEntry, POReceipt>, IGraphWithInitialization
	{
			...
			PXLongOperation.StartOperation(this, () =>
			{
				...
				foreach (POReceipt receipt in adapter.Get<POReceipt>())
				{
					...
					try
					{
						...
						{
							Activity.SendNotification(APNotificationSource.Vendor, notificationCD, receipt.BranchID, parameters, adapter.MassProcess);
							Save.Press();
		...
		protected virtual void POReceiptLine_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row.Released != true)
			{
				...
				bool isProjectDropShip = POLineType.IsProjectDropShip(row.LineType);
            ...
            if ((row.AllowComplete == null || row.AllowOpen == null) && fromPO)
            {
                ...
                var query = new PXSelect<POLineR,
                    Where<POLineR.orderType, Equal<Required<POLineR.orderType>>,
                        And<POLineR.orderNbr, Equal<Required<POLineR.orderNbr>>,
                            And<POLineR.lineNbr, Equal<Required<POLineR.lineNbr>>>>>>(this);

                POLineR source = query.Select(row.POType, row.PONbr, row.POLineNbr);
                query.StoreResult(source);
        ...
        public virtual void ReleaseReceipt(INReceiptEntry docgraph, AP.APInvoiceEntry invoiceGraph, POReceipt aDoc, DocumentList<INRegister> aINCreated, DocumentList<AP.APInvoice> aAPCreated, bool aIsMassProcess)
		{
			...
			foreach (PXResult<POReceiptLine> res in GetLinesToReleaseQuery().Select(aDoc.ReceiptType, aDoc.ReceiptNbr))
			{
				...
				ValidateProjectDropShipLineOnRelease(line);
				...
				{
					if ((!POLineType.IsProjectDropShip(line.LineType) && (line.IsStockItem() || (line.IsNonStockItem() && insetup.Current.UpdateGL == true && line.POAccrualAcctID != null))) ||
			...
			List<INItemPlan> posupply, HashSet<PXResult<INItemPlan, INPlanType>> podemand)
		{
			...
			foreach (PXResult<INItemPlan, INPlanType> demandres in
				...
				.Select(this, poLine.PlanID).AsEnumerable()
		...
		protected virtual void ReleaseReturnWithNoExactCost(INIssueEntry docgraph, AP.APInvoiceEntry invoiceGraph, POReceipt aDoc, DocumentList<INRegister> aCreated, DocumentList<AP.APInvoice> aInvoiceCreated, bool aIsMassProcess)
		{
			...
			foreach (PXResult<POReceiptLine, POReceiptLineSplit, POLineUOpen, INTran, INItemPlan, POReceiptLine2> res in PXSelectJoin<POReceiptLine,
				LeftJoin<POReceiptLineSplit,
					On<POReceiptLineSplit.FK.ReceiptLine>,
				...
				LeftJoin<INTran,
					On<INTran.pOReceiptType, Equal<POReceiptLine.receiptType>,
					And<INTran.pOReceiptNbr, Equal<POReceiptLine.receiptNbr>,
				...
				Where2<
					POReceiptLine.FK.Receipt.SameAsCurrent,
					And<INTran.refNbr, IsNull>>,
				OrderBy<Asc<POReceiptLine.receiptNbr, Asc<POReceiptLine.lineNbr>>>>.Select(this))
			{
				...
				bool postToIN = ((isStock || isNonStock && insetup.Current.UpdateGL == true) ||
					(insetup.Current.UpdateGL == true && POLineType.IsProjectDropShip(line.LineType) && line.DropshipExpenseRecording == DropshipExpenseRecordingOption.OnReceiptRelease));
				if (postToIN)
				{
		...
		{
			POReceipt receiptCopy = (POReceipt)Document.Cache.CreateCopy(Document.Current);
			...
			{			
				...
				if (!isManualCostMode)
				{
					...
					tranCost = (costLayer.QtyOnHand <= baseQty) ? costLayer.TotalCost
						: currencyInfo.RoundBase((costLayer.TotalCost * baseQty / costLayer.QtyOnHand) ?? 0m);
				}
			}
			else
			{
				if (splitBasedTran)
				{
					...
					decimal? fairTranCost = currencyInfo.RoundBase((tranCost * split.BaseQty / rctLine.BaseReceiptQty) ?? 0m);
