...
namespace PX.Objects.PO
{
	...
	public partial class POReceipt : PX.Data.IBqlTable, PX.Data.EP.IAssign
	{
		...
		public static class FK
		{
			...
			public class IntercompanyShipment : SO.SOShipment.PK.ForeignKeyOf<POReceipt>.By<intercompanyShipmentNbr> { }
        ...
        public abstract class vendorID : PX.Data.BQL.BqlInt.Field<vendorID>
		{
			public class PreventEditBAccountVOrgBAccountID<TGraph> :
				PreventEditBAccountRestrictToBase<BAccount.vOrgBAccountID, TGraph, POReceipt, 
					SelectFrom<POReceipt>
						.Where<POReceipt.receiptType.IsNotEqual<POReceiptType.transferreceipt>.
							And<POReceipt.vendorID.IsEqual<BAccount.bAccountID.FromCurrent>>>>
				where TGraph : PXGraph
			{
				protected override string GetErrorMessage(BAccount baccount, POReceipt document, string documentBaseCurrency)
				{
					return PXMessages.LocalizeFormatNoPrefix(Messages.CannotChangeRestricToIfPOReceiptExists,
							documentBaseCurrency, baccount.AcctCD, document.ReceiptNbr);
				}
			}
		...
		[LocationActive(typeof(Where<Location.bAccountID, Equal<Current<POReceipt.vendorID>>,
		...
		[PXSearchable(SM.SearchCategory.PO, "{0}: {1} - {3}", new Type[] { typeof(POReceipt.receiptType), typeof(POReceipt.receiptNbr), typeof(POReceipt.vendorID), typeof(Vendor.acctName) },
		   ...
		   MatchWithJoin = typeof(InnerJoin<BAccountR, On<BAccountR.bAccountID, Equal<POReceipt.vendorID>>>),
		...
		[LocationActive(typeof(Where<Location.bAccountID, Equal<Current<POReceipt.shipToBAccountID>>>), DescriptionField = typeof(Location.descr))]
	...
	public static class POReceiptType
	{
		...
		{
			private static (string, string)[] ValuesToLabels = new[]
			{
			...
			protected ListAttribute(bool all): base(
				all
					? (new[] { (All, Messages.All) }).Concat(ValuesToLabels).ToArray()
					: ValuesToLabels)
			{ }
