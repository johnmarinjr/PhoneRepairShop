...
using Currency = PX.Objects.CM.Extensions.Currency;
...
namespace PX.Objects.PO
{
	...
	public partial class POVendorInventory : PX.Data.IBqlTable
	{
		...
		{
			public class PreventEditBAccountVOrgBAccountID<TGraph> : PreventEditOf<BAccount.vOrgBAccountID>.On<TGraph>.IfExists<
					SelectFrom<POVendorInventory>
					.Where<POVendorInventory.vendorID.IsEqual<BAccount.bAccountID.FromCurrent>>>
				where TGraph : PXGraph
			{
				...
				{
					BAccount vendor = arg.Row as BAccount;
					var newBranchID = arg.NewValue as int?;
					var newBranchBaseCuryID = PXAccess.GetBranchByBAccountID(newBranchID)?.BaseCuryID ??
						PXAccess.GetOrganizationByBAccountID(newBranchID)?.BaseCuryID;
					var inventory = IN.InventoryItem.PK.Find(Base, (firstPreventingEntity as POVendorInventory)?.InventoryID);

					if (newBranchBaseCuryID == vendor?.BaseCuryID)
						return null;

					if (vendor?.BaseCuryID != null)
					{
						return PXMessages.LocalizeFormatNoPrefix(IN.Messages.CannotChangeRestricToIfPOVendorInventoryExists,
							vendor.BaseCuryID, vendor.AcctCD, inventory?.InventoryCD);
					}
					else
					{
						return PXMessages.LocalizeFormatNoPrefix(IN.Messages.CannotChangeRestricToIfPOVendorInventoryExistsNull,
							vendor.AcctCD, inventory?.InventoryCD);
					}
				}
			}
