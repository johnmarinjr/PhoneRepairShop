...
namespace PX.Objects.PO.GraphExtensions.APInvoiceEntryExt
{
	public class APInvoicePOValidation : PXGraphExtension<APInvoiceEntry>
	{
		...
		public class POLineDTO
		{
			...
			public virtual Int32? InventoryID => _poLineRevision == null ? _poLine.InventoryID : _poLineRevision.InventoryID;
			...
			public virtual String LineType => _poLineRevision == null ? _poLine.LineType : _poLineRevision.LineType;
		...
		protected virtual void Validate(APTran tran, PXCache cache)
		{
			...
			if (tran.LineType != POLineType.Freight)
			{
				bool isReceivedMoreThenOrdered = poLine.ReceivedQty > poLine.OrderQty * poLine.RcptQtyMax / 100m;
				message = poLine.OrderType == POOrderType.RegularSubcontract ?
                ...
                {
					PXUIFieldAttribute.SetError<APTran.uOM>(cache, tran, ex.Message);
				}
			}
			else
			{
				message = Messages.APInvoicePOUnbilledAmountValidationWarning;
			}
		...
		protected virtual void Prefetch(APInvoice doc)
		{
			...
			if (doc.Released != true)
			{
				...
				using (var fieldScope = new PXFieldScope(poLinesQuery.View, GetPOLinesQueryFieldScope()))
				{
					foreach (PXResult<POLine, POOrder, APTran> record in poLinesQuery.View.SelectMulti(doc.DocType, doc.RefNbr))
					{
						...
						POLineQuery.StoreResult(
							new List<object>(1) { new PXResult<POLine, POOrder>(poLine, record) },
							PXQueryParameters.ExtractFromRecord(poLine));
					}
				}
			}
			else
			{
					...
					.Select(Base, doc.DocType, doc.RefNbr))
				{
					...
					POLineRevisionQuery.StoreResult(
						new List<object>(1) { revision.OrderNbr != null ? new PXResult<POLineBillingRevision>(revision) : null },
						PXQueryParameters.ExplicitParameters(tran.TranType, tran.RefNbr, tran.POOrderType, tran.PONbr, tran.POLineNbr));
				}
			}
		...
		protected virtual void _(Events.RowPersisting<APTran> e)
		{
			...
			switch (tran.POAccrualType)
			{
				case POAccrualType.Receipt:
					POReceiptLine rctLine = POReceiptLine.PK.Find(Base, tran.ReceiptType, tran.ReceiptNbr, tran.ReceiptLineNbr);
