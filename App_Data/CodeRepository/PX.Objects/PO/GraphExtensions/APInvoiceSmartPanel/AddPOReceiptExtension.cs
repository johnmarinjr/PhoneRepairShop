...
namespace PX.Objects.PO.GraphExtensions.APInvoiceSmartPanel
{
	...
	public class AddPOReceiptExtension : PXGraphExtension<LinkLineExtension, APInvoiceEntry>
	{
					...
					Base.Document.Current.Prebooked == false)
			{
				using (new APInvoiceEntry.SkipUpdAdjustments(Base.Document.Current.DocType + Base.Document.Current.RefNbr))
				{
					...
					if (receipts.Any())
					{
						...
						{
							Base.InvoicePOReceipt(rc, null, usePOParameters: false, keepOrderTaxes: false, errorIfUnreleasedAPExists: false);
						}
						var orders = PXSelectJoinGroupBy<POOrder,
							...
							Where<
								POOrderReceipt.receiptType, Equal<Required<POOrderReceipt.receiptType>>,
								And<POOrderReceipt.receiptNbr, In<Required<POOrderReceipt.receiptNbr>>>>,
							Aggregate<GroupBy<POOrder.orderType, GroupBy<POOrder.orderNbr>>>>
							.Select(Base, receipts[0].ReceiptType, receipts.Select(r => r.ReceiptNbr).ToArray())
		...
		public virtual void POReceipt_ReceiptNbr_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			{
				PXResultset<POOrderReceiptLink> receiptLinks = PXSelectGroupBy<POOrderReceiptLink,
					Where<POOrderReceiptLink.receiptType, Equal<Required<POReceipt.receiptType>>,
						And<POOrderReceiptLink.receiptNbr, Equal<Required<POReceipt.receiptNbr>>>>,
					Aggregate<GroupBy<POOrderReceiptLink.curyID>>>.Select(Base, row.ReceiptType, row.ReceiptNbr);
