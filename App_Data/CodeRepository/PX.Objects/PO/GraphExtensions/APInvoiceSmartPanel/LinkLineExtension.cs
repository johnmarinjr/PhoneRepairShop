						...
						if (linkLineFilter.Current.SelectedMode == LinkLineFilter.selectedMode.Receipt)
						{
							foreach (POReceiptLineS receipt in linkLineReceiptTran.Cache.Updated)
							{
								if (receipt.Selected == true)
								{
									...
									if (apTran.POOrderType == POOrderType.ProjectDropShip)
									{
										apTran.ProjectID = receipt.ProjectID;
										apTran.TaskID = receipt.TaskID;
										apTran.CostCodeID = receipt.CostCodeID;
										apTran.DropshipExpenseRecording = receipt.DropshipExpenseRecording;
									}
									break;
								}
							}
							...
							foreach (POLineS order in linkLineOrderTran.Cache.Updated)
							{
								if (order.Selected == true)
								{
									...
									if (apTran.POOrderType == POOrderType.ProjectDropShip)
									{
										...
										apTran.DropshipExpenseRecording = order.DropshipExpenseRecording;
									}
									break;
								}
							}
