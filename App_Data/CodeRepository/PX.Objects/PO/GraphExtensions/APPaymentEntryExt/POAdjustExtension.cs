...
namespace PX.Objects.PO.GraphExtensions.APPaymentEntryExt
{
	public class POAdjustExtension : PXGraphExtension<APPaymentEntry.MultiCurrency, APPaymentEntry>
	{
		...
		/// <summary>
		/// Overrides <see cref="APPaymentEntry.MultiCurrency.GetChildren"/>.
		/// </summary>
		[PXOverride]
		public virtual PXSelectBase[] GetChildren(Func<PXSelectBase[]> baseMethod)
		{
			return baseMethod().Union(new PXSelectBase[] { POAdjustments }).ToArray();
		}
		...
		protected virtual void _(Events.FieldVerifying<POAdjust, POAdjust.curyAdjgAmt> eventArgs)
		{
			...
			if (order != null && order.Cancelled != true)
			{
				bool exceed;
				decimal newValue = (decimal?)eventArgs.NewValue ?? 0;

				if (eventArgs.Row.CuryAdjgAmt >= newValue || eventArgs.Row.CuryAdjgAmt == null && newValue <= 0)
					return;
				...
				else
				{
					...
					decimal baseCuryValue = currencyInfo.CuryConvBase(newValue);
		...
		protected virtual void _(Events.FieldDefaulting<POAdjust, POAdjust.curyAdjgAmt> eventArgs)
		{
			...
			if (order != null)
			{
				...
				{
					CurrencyInfo currencyInfo = Base.FindImplementation<IPXCurrencyHelper>().GetDefaultCurrencyInfo();
					decimal curyamount = currencyInfo.CuryConvCury(order.UnprepaidTotal ?? 0);
		...
		{
			if (eventArgs.Row == null || eventArgs.OldValue == eventArgs.NewValue
				|| eventArgs.Row.AdjdOrderType == null || eventArgs.Row.AdjdOrderNbr == null)
				return;

			eventArgs.Cache.SetDefaultExt<POAdjust.adjdCuryInfoID>(eventArgs.Row);
		...
		protected virtual void SetPOAdjustAdjdAmt(POAdjust row)
		{
			...
			CurrencyInfo poCurrencyInfo = Base.FindImplementation<IPXCurrencyHelper>().GetCurrencyInfo(row.AdjdCuryInfoID);
			if (poCurrencyInfo.CuryID == Base.Document.Current.CuryID)
			{
			...
			{
				decimal orderCuryAmt = poCurrencyInfo.CuryConvCury(row.AdjgAmt ?? 0);
		...
		protected virtual List<AdjustmentStubGroup> AddPOAdjustments(IEnumerable<AdjustmentStubGroup> adjustmentStubs)
		{
			...
			var result = adjustmentStubs.Where(r =>
					!poadjustments.Any(adj => adj.AdjdDocType == r.GroupedStubs.Key.AdjdDocType
						&& adj.AdjdRefNbr == r.GroupedStubs.Key.AdjdRefNbr))
				.Union(poadjustments.GroupBy(adj =>
		...
		protected virtual void InsertUpdatePOAdjust(APAdjust apadjust)
		{
			...
			foreach (PXResult<POOrderPrepayment, POOrder> row in prepaymentRequests)
			{
				...
				{
					CurrencyInfo currencyInfo = Base.FindImplementation<IPXCurrencyHelper>().GetDefaultCurrencyInfo();
					decimal curyamount = currencyInfo.CuryConvCury(apadjust.AdjAmt ?? 0);
