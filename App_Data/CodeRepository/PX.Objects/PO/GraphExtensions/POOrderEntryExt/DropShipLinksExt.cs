...
namespace PX.Objects.PO.GraphExtensions.POOrderEntryExt
{
	public class DropShipLinksExt : PXGraphExtension<POOrderEntry>
	{
		...
		public virtual IEnumerable CreateSalesOrder(PXAdapter adapter)
		{
			...
			if (CreateSOFilter.AskExt(LineMessagesDialogInitializer, true) == WebDialogResult.OK)
			{
				...
				PXLongOperation.StartOperation(Base, () =>
				{
					...
					var doc = new SOOrder();
					doc.OrderType = CreateSOFilter.Current.OrderType;
					doc.CustomerOrderNbr = CreateSOFilter.Current.CustomerOrderNbr;
					doc.Hold = true;

					if (!string.IsNullOrWhiteSpace(CreateSOFilter.Current.OrderNbr))
						doc.OrderNbr = CreateSOFilter.Current.OrderNbr;
					...
					ContactAttribute.CopyRecord<SOOrder.shipContactID>(graph.Document.Cache, graph.Document.Current, Base.Shipping_Contact.Current, true);
					AddressAttribute.CopyRecord<SOOrder.shipAddressID>(graph.Document.Cache, graph.Document.Current, Base.Shipping_Address.Current, true);
		...
		{
			if (soline != null && line.LineType.IsIn(POLineType.GoodsForDropShip, POLineType.NonStockForDropShip))
			{
