...
namespace PX.Objects.PO.WMS
{
	...
	public class RPACartSupport : CartBase
	{
		...
		public class AlterPutAwayModeLogic : PXGraphExtension<ReceivePutAway.PutAwayMode.Logic, ReceivePutAway, ReceivePutAway.Host>
		{
			...
			/// Overrides <see cref="ReceivePutAway.PutAwayMode.Logic.CanSwitchReceipt"/>
			/// </summary>
			[PXOverride]
			public virtual bool get_CanSwitchReceipt(Func<bool> base_CanSwitchReceipt) => base_CanSwitchReceipt() && !CartBasis.IsCartRequired();
				...
				return CartBasis.IsCartRequired()
					? 	SelectFrom<INRegister>.
						...
						Where<
							...
							And<POCartReceipt.receiptType.IsEqual<RPAScanHeader.receiptType.FromCurrent>>.
			...
			public class AlterRefNbrStateLogic : PXGraphExtension<ReceivePutAway.PutAwayMode.ReceiptState.Logic, ReceivePutAway, ReceivePutAway.Host>
			{
				...
				public ReceivePutAway Basis => Base1;
				public RPACartSupport CartBasis => Basis.Get<RPACartSupport>();
				public ReceivePutAway.PutAwayMode.ReceiptState.Logic Target => Base2;
				...
				{
					if (CartBasis.CartID > 0 && Basis.SiteID != Target.SiteID)
					{
						error = Validation.Fail(CartState.Msg.InvalidSite, CartBasis.SelectedCart?.CartCD);
						return false;
					}

					var existingLink = Lazy.By<POReceiptSplitToCartSplitLink>(() => CartBasis.CartSplitLinks.Select());
					if (CartBasis.CartID > 0 && existingLink.Value != null && existingLink.Value.ReceiptNbr != receipt.ReceiptNbr)
					{
						error = Validation.Fail(CartState.Msg.CartAlreadyHasReceipt, existingLink.Value.ReceiptNbr);
						return false;
					}
					...
					POReceiptLineSplit notPutSplit =
						...
						Where<
							POReceiptLineSplit.FK.Receipt.SameAsCurrent.
							And<POReceiptLineSplit.putAwayQty.IsLess<POReceiptLineSplit.qty>>>.
						View.SelectSingleBound(Basis, new[] { receipt });
					...
					if (notPutSplit == null)
					{
						INRegister notReleasedTransfer =
							...
							Where<
								...
								And<INRegister.released.IsEqual<False>>.
								And<INRegister.FK.POReceipt.SameAsCurrent>>.
