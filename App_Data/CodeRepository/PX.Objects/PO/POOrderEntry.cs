	...
	public class POOrderEntry : PXGraph<POOrderEntry, POOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public class MultiCurrency : MultiCurrencyGraph<POOrderEntry, POOrder>
		{
			...
			public  virtual void POOrder_VendorID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e, PXFieldUpdated baseMethod)
			{
				...
				if ((e.ExternalCall || order?.CuryID == null || order.OverrideCurrency == true) && !Base.IsCopyPasteContext)
				{
			...
			if (this.Document.Current.Hold == true)
			{
				...
				foreach (POSiteStatusSelected line in sitestatus.Cache.Cached)
				{
					if (line.Selected == true && line.QtySelected > 0)
					{
						...
						switch (this.Document.Current.OrderType)
						{
				...
				foreach (POOrder order in orders)
				{
					...
					try
					{
						...
						using (var ts = new PXTransactionScope())
						{
							...
							orderEntry.Value.Activity.SendNotification(APNotificationSource.Vendor, notificationCD, order.BranchID, parameters);
		...
		private bool IsForInventory(POLine row)
		{
			return row.LineType == POLineType.GoodsForInventory ||
				...
				row.LineType == POLineType.GoodsForProject;
		}
		...
		protected virtual void POOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			addInvBySite.SetEnabled(vendorSelected);
		...
		[Obsolete("Empty handler for backward compatibility")]
		...
		public virtual bool UpdateClosedState(POLine blanketLine)
        {
			...
			POLine line = (POLine)Transactions.Cache.Locate(blanketLine)
				?? SelectFrom<POLine>
					.Where<POLine.pOType.IsEqual<@P.AsString.ASCII>
					.And<POLine.pONbr.IsEqual<@P.AsString>>
					.And<POLine.pOLineNbr.IsEqual<@P.AsInt>>>
					.View
					.SelectWindowed(this, 0, 1, blanketLine.OrderType, blanketLine.OrderNbr, blanketLine.LineNbr);
						...
						if (project != null)
						{
							switch (project.DropshipExpenseAccountSource)
							{
								case DropshipExpenseAccountSourceOption.Task:
									{
										var task = PMTask.PK.Find(this, row.TaskID);
										if (task != null)
										{
				...
				default:
					...
					if (item != null)
					{
						if (Document.Current != null && Document.Current.OrderType == POOrderType.ProjectDropShip)
						{
							...
							var pmTask = PMTask.PK.Find(this, row.TaskID);
							var postClass = INPostClass.PK.Find(this, item.PostClassID);
		...
		protected virtual void POLine_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			PXDefaultAttribute.SetPersistingCheck<POLine.siteID>(sender, e.Row, !POLineType.IsProjectDropShip(row.LineType) && (isForInventory || isNonStock && !POLineType.IsService(row.LineType)) ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
		...
		public virtual void FillPOLineFromDemand(POLine dest, POFixedDemand demand, string OrderType, SOLineSplit3 solinesplit)
		{
			...
			if (solinesplit != null && demand.AlternateID != null && demand.InventoryID != null)
			{
				...
				{
					if (dest.AlternateID != null && dest.InventoryID != null)
					{
