...
namespace PX.Objects.PO.LandedCosts
{
	public abstract class AllocationServiceBase : PXGraph<AllocationServiceBase>
	{
			...
			Int32? branchID)
		{
			...
			CurrencyInfo receiptCuryInfo = graph.FindImplementation<IPXCurrencyHelper>().GetCurrencyInfo(allocationItem.ReceiptLine.CuryInfoID);
			...
			foreach (POReceiptLineSplit split in splits)
			{
				...
				handledSplitsAmt = receiptCuryInfo.RoundBase(handledSplitsAmt);
			...
			if (!hasSplits)
			{
				...
				shareAmt = receiptCuryInfo.RoundBase(shareAmt);
				if (shareAmt != 0m)
				{
		...
		public virtual INTran GetOriginalInTran(PXGraph graph, string receiptType, string receiptNbr, int? lineNbr)
		{
			if (receiptType == null || receiptNbr == null || lineNbr == null)
				return null;
			...
			return
					PXSelectReadonly<INTran, Where<INTran.docType, NotEqual<INDocType.adjustment>,
						...
						And<INTran.pOReceiptLineNbr, Equal<Required<INTran.pOReceiptLineNbr>>>>>>>>.SelectWindowed(graph, 0, 1, receiptType, receiptNbr, lineNbr);
		}
