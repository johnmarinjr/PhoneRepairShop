...
namespace PX.Objects.GDPR
{
	public class GDPRPersonalDataProcessBase : PXGraph<GDPRPersonalDataProcessBase>
	{
		...
		public void ProcessImpl(IEnumerable<IBqlTable> entities, bool topLayer, Guid? topParentNoteID, bool eraseAll = false)
		{
				...
				foreach (KeyValuePair<Type, List<BqlCommand>> tableLink in mapping)
				{
					...
					foreach (BqlCommand select in selects)
					{
						...
						if (bqlPseudonymizationStatus != null)
						{
							if (eraseAll)
							{
								currentSelect = currentSelect.WhereAnd(BqlCommand.Compose(typeof(Where<,>), bqlPseudonymizationStatus, typeof(NotEqual<>), typeof(PXPseudonymizationStatusListAttribute.erased)));
							}
							else
							{
