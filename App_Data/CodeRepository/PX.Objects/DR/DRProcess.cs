...
namespace PX.Objects.DR
{
		...
		: PXGraph<DRProcess>, IDREntityStorage, IBusinessAccountProvider, IInventoryItemProvider
	{
		...
		protected virtual void DRScheduleDetail_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (row.IsResidual != true && String.IsNullOrEmpty(row.DefCode))
				sender.RaiseExceptionHandling<DRScheduleDetail.defCode>(row, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty));
		}
		...
		public List<Batch> RunRecognition(List<DRRecognition.DRBatch> list, DateTime? recDate)
		{
			...
			Exceptions.Clear();
				...
				foreach (DRRecognition.DRTranKey drTranKey in drBatch.Trans)
				{
					try
					{
						PXResult<DRSchedule> res = PXSelectReadonly2<DRSchedule,
								...
								InnerJoin<DRScheduleTran,
									On<DRScheduleTran.scheduleID, Equal<DRSchedule.scheduleID>,
								...
								Where<DRSchedule.scheduleID, Equal<Required<DRSchedule.scheduleID>>>
						>.Select(je, drTranKey.ComponentID, drTranKey.DetailLineNbr, drTranKey.LineNbr, drTranKey.ScheduleID).FirstOrDefault();

						DRSchedule schedule = PXResult.Unwrap<DRSchedule>(res);
						DRScheduleDetail scheduleDetail = PXResult.Unwrap<DRScheduleDetail>(res);
						DRScheduleTran drTran = PXResult.Unwrap<DRScheduleTran>(res);

						if (!ScheduleDetailForComponent.ContainsKey(new Tuple<int, int, int>(drTranKey.ScheduleID.Value, drTranKey.ComponentID.Value, drTranKey.DetailLineNbr.Value)))
						{
							ScheduleDetailForComponent[new Tuple<int, int, int>(drTranKey.ScheduleID.Value, drTranKey.ComponentID.Value, drTranKey.DetailLineNbr.Value)] = scheduleDetail;
						}
						#region Tran 1
						tran = new GLTran();
						tran.SummPost = false;
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (DRScheduleTran drTran in drTranList)
					{
						...
						Transactions.Cache.DisableReadItem = true;
						...
						ScheduleDetail.Cache.DisableReadItem = true;
		...
		{
			return PXSelect<InventoryItem, Where<InventoryItem.inventoryID,
				Equal<Required<InventoryItem.inventoryID>>>>.Select(this, inventoryItemID);
		}
		...
		DRSchedule IDREntityStorage.Insert(DRSchedule schedule)
		{
			...
			schedule.CuryInfoID = null;
			return ((IDREntityStorage)this).UpdateCuryInfo(this.Schedule.Insert(schedule), curyInfoID);
		}
		...
		{
			CurrencyInfo sourceInfo =
				curyInfoID == null ? null :
				PXSelect<CurrencyInfo,
						Where<CurrencyInfo.curyInfoID, Equal<Required<CurrencyInfo.curyInfoID>>>>
					.Select(this, curyInfoID);

			if (sourceInfo != null)
			{
				this.Caches<CurrencyInfo>().Clear();
				CurrencyInfo curyInfo = PXCache<CurrencyInfo>.CreateCopy((CurrencyInfo)sourceInfo);
				curyInfo.CuryInfoID = null;
				curyInfo.IsReadOnly = false;
				curyInfo = PXCache<CurrencyInfo>.CreateCopy(
					(CurrencyInfo)this.Caches<CurrencyInfo>().Insert(curyInfo));
				schedule.CuryInfoID = curyInfo.CuryInfoID;
				schedule.CuryID = curyInfo.CuryID;
				schedule.BaseCuryID = curyInfo.BaseCuryID;
			}
			else
			{
				CurrencyInfo curyInfo =
					PXSelect<CurrencyInfo,
							Where<CurrencyInfo.curyInfoID, Equal<Required<CurrencyInfo.curyInfoID>>>>
						.Select(this, schedule.CuryInfoID);
				curyInfo.CuryID = schedule.CuryID ?? schedule.BaseCuryID;
				curyInfo.BaseCuryID = schedule.BaseCuryID;
				this.Caches<CurrencyInfo>().Update(curyInfo);
				schedule.CuryID = curyInfo.CuryID;
				schedule.BaseCuryID = curyInfo.BaseCuryID;
			}
			return schedule;
		...
		#region ScheduleDetailForComponent
		...
		{
			DRScheduleDetail scheduleDetail;
			ScheduleDetailForComponent.TryGetValue(new Tuple<int, int, int>(scheduleID.Value, componentID.Value, detailLineNbr.Value), out scheduleDetail);
			if (scheduleDetail == null)
			{
				scheduleDetail = PXSelect<
					DRScheduleDetail,
					Where<
						DRScheduleDetail.scheduleID, Equal<Required<DRScheduleDetail.scheduleID>>,
						And<DRScheduleDetail.componentID, Equal<Required<DRScheduleDetail.componentID>>,
				...
				if (scheduleDetail != null)
				{
					ScheduleDetailForComponent[new Tuple<int, int, int>(scheduleID.Value, componentID.Value, detailLineNbr.Value)] = scheduleDetail;
				}
			}
