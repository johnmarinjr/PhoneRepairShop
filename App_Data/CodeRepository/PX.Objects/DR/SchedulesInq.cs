...
namespace PX.Objects.DR
{
	...
	public class SchedulesInq : PXGraph<SchedulesInq>
	{
		...
		public PXSelectJoin<SchedulesInqResult,
				...
				Where<DRDeferredCode.accountType, Equal<Current<SchedulesFilter.accountType>>>> Records;
		...
		{
			var ret = new PXDelegateResult() { IsResultSorted = true, IsResultTruncated = true, IsResultFiltered = true };
		...
		public virtual BqlCommand ComposeBQLCommandForRecords(SchedulesFilter filter)
		{
			...
			if (filter.OrgBAccountID != null || PXAccess.FeatureInstalled<FeaturesSet.multipleBaseCurrencies>())
			{
				select.WhereAnd<Where<SchedulesInqResult.branchID, Inside<Current2<SchedulesFilter.orgBAccountID>>>>(); //MatchWithOrg
			}
		...
		public partial class SchedulesInqResult : DRScheduleDetail
		{
			...
			[PXDBInt(IsKey = true)]
			[PXUIField(DisplayName = Messages.ScheduleNbr, TabOrder = 1)]
			...
			[PXDBCalced(typeof(Switch<
								Case<Where<CurrentValue<SchedulesFilter.accountType>, Equal<AccountType.income>,
										And<SchedulesInqResult.docType, Equal<ARDocType.creditMemo>,
										Or<CurrentValue<SchedulesFilter.accountType>, Equal<AccountType.expense>,
