	...
	public class DRTerms
	{
		...
		public static Term GetDefaultItemTerms(PXGraph graph, InventoryItem item)
		{
			...
			else if (deferralCode.MultiDeliverableArrangement == true)
			{
				...
				components.Sort(SortDecendingByTermUOM);
		...
		private static int SortDecendingByTermUOM(PXResult<INComponent> a, PXResult<INComponent> b)
		{
			...
			int result = ((IComparable)(SortOrder(docB.DefaultTermUOM))).CompareTo(SortOrder(docA.DefaultTermUOM));

			return result == 0 ? ((IComparable)(docB.DefaultTerm)).CompareTo(docA.DefaultTerm): result;
		}
