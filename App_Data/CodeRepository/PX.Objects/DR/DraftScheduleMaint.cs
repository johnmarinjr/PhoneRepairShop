	...
	public class DraftScheduleMaint : PXGraph<DraftScheduleMaint, DRSchedule>
	{
		...
		public PXSelectJoin<
			...
			Where<
				...
				And<Where<DRScheduleDetail.isResidual, Equal<False>,
					Or2<Where<DRScheduleDetail.defCode, IsNotNull>,
						And<FeatureInstalled<FeaturesSet.aSC606>>>>>>>
			Components;
		...
		protected virtual void DRSchedule_RefNbr_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (origCurrencyInfo !=null && schedule.BaseCuryID != origCurrencyInfo.BaseCuryID)
			{
				sender.SetValue<DRSchedule.baseCuryID>(schedule,
					origCurrencyInfo?.BaseCuryID ?? Accessinfo.BaseCuryID);
			}
			...
			if (origCurrencyInfo != null)
			{
				...
				scheduleCopy = Schedule.Update(scheduleCopy);
				//Sync curyID
				scheduleCopy.CuryID = info.CuryID;
			}
			else if(e.OldValue != null)
			{
				CurrencyInfo.Cache.Clear();
				CurrencyInfo info = (CurrencyInfo)CurrencyInfo.Cache.CreateInstance();
				info.IsReadOnly = false;
				schedule.CuryID = this.Accessinfo.BaseCuryID;
				schedule.BaseCuryID = this.Accessinfo.BaseCuryID;
				info = CurrencyInfo.Insert(info);
				schedule.CuryInfoID = info.CuryInfoID;
				schedule.CuryID = info.CuryID;
				schedule.BaseCuryID = info.BaseCuryID;
			}
		}
		...
		protected virtual void DRSchedule_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [This is an old code, that has been working for a long time]
