...
namespace PX.Objects.TX
{
	public class TaxHistorySumManager
	{
		...
		public static void UpdateTaxHistorySums(PXGraph graph, RoundingManager rmanager, string taxPeriodId, int? revisionId, int? organizationID, int? branchID, Func<TaxReportLine, bool> ShowTaxReportLine = null)
		{
			...
			{
				PXResultset<TaxHistory> lines = GetTaxHistoryLines(graph, rmanager.CurrentVendor.BAccountID, rmanager.CurrentTaxReportRevisionID, taxPeriodId, revisionId);
				...
				if (organizationID == null)
				{
					...
					organizationID = branch?.Organization.OrganizationID;
					baseCuryID = branch.BaseCuryID;
				}
				else
				{
					var org = PXAccess.GetOrganizationByID(organizationID);
					baseCuryID = org.BaseCuryID;
				}

				TaxPeriod period = TaxYearMaint.GetTaxPeriodByKey(graph, organizationID, rmanager.CurrentVendor.BAccountID,
					taxPeriodId);

				PXResult<Currency, CurrencyRateByDate> curyWithRateSet = GetCurrencyAndRateByDate(graph, rmanager.CurrentVendor, baseCuryID, period);
				Currency currency = curyWithRateSet;
				CurrencyRateByDate rateByDate = currency.CuryID != baseCuryID ? curyWithRateSet : null;
		...
		private static PXResultset<TaxHistory> GetTaxHistoryLines(PXGraph graph, int? curVendorBaccountID, int? curTaxReportRevisionID, string taxPeriodId, int? revisionId)
		{
			...
			.Select(graph, curVendorBaccountID, curTaxReportRevisionID, taxPeriodId, revisionId);

			return lines;
		}

		private static PXResult<Currency, CurrencyRateByDate> GetCurrencyAndRateByDate(PXGraph graph, Vendor curVendor, string baseCuryID, TaxPeriod period)
		{
			...
			var curyWithRateSet = (PXResult<Currency, CurrencyRateByDate>)
				...
				.SelectWindowed(graph, StartRow, TotalRows, baseCuryID, curVendor.CuryRateTypeID, 
								period.EndDate, period.EndDate, curVendor.CuryID ?? baseCuryID);

			return curyWithRateSet;
		}

		public static decimal RecalcCurrency(int? decimalPlaces, CurrencyRate rate, decimal value)
		{
			...
			return Math.Round(convertedValue, decimalPlaces ?? 2, MidpointRounding.AwayFromZero);
		}

		public static PXResultset<TaxReportLine, TaxHistory> GetPreviewReport(PXGraph graph, Vendor vendor, int taxReportRevisionID,
																			  ...
																			  Func<TaxReportLine, bool> ShowTaxReportLine = null)
        {
            ...
            RoundingManager rmanager = new RoundingManager(vendor, taxReportRevisionID);

            Dictionary<int, List<int>> taxAggregatesDict = TaxReportMaint.AnalyseBuckets(graph,
                vendorbAccountID, taxReportRevisionID,
		...
		private class TaxBucketsCalculation
		{
										 ...
										 Func<TaxReportLine, bool> showTaxReportLine)
			{
				...
				aggregatesTable = TaxReportMaint.AnalyseBuckets(graph, curVendorBaccountID, roundingManager.CurrentTaxReportRevisionID, taxTypeForCalculation, CalcWithZones, showTaxReportLine);
			...
			private TaxHistory CreateDeltaHistory(TaxHistory original, decimal? roundedAmount)
			{
				TaxHistory delta = new TaxHistory
				{
					...
					TaxReportRevisionID = original.TaxReportRevisionID,
				...
				else
				{
					...
					delta.FiledAmt = RecalcCurrency(currency.DecimalPlaces, rateByDate, delta.ReportFiledAmt.GetValueOrDefault());
				}
