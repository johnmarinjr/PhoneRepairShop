...
namespace PX.Objects.TX
{
	public abstract class TaxAttribute : TaxBaseAttribute
	{
		...
		protected virtual void Tax_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (e.ExternalCall && IsTaxRowAmountUpdated(sender, e))
			{
				...
				if (parentCache.Current != null)
				{
					...
					MultiCurrencyCalculator.CuryConvBase(ParentCache(sender.Graph), ParentRow(sender.Graph), discrepancy, out decimal discrepancyBase);
			...
			if ((_TaxCalc != TaxCalc.NoCalc && e.ExternalCall || _TaxCalc == TaxCalc.ManualCalc))
			{
				...
				{
					if (taxSumOld.TaxID != taxSum.TaxID || IsTaxRowQuantityUpdated(sender, e))
					{
		...
		{
			decimal? oldTaxableQuantity = sender.GetValue(e.OldRow, TaxableQtyFieldNameForTaxDetail) as decimal?;
			decimal? newTaxableQuantity = sender.GetValue(e.Row, TaxableQtyFieldNameForTaxDetail) as decimal?;
			return oldTaxableQuantity != newTaxableQuantity;
		}
		...
		protected virtual void VerifyTaxID(PXCache sender, object row, bool externalCall)
		{
			...
			if (nomatch)
			{
				sender.RaiseExceptionHandling("TaxID", row, ((TaxDetail)row).TaxID,
					new PXSetPropertyException(TX.Messages.NoLinesMatchTax, PXErrorLevel.RowError, ((TaxDetail)row).TaxID));
			}
			sender.Current = row;
		}
		...
		public class CalcTaxable
		{
				...
				Decimal aCuryTotal)
			{
				...
				decimal baseLvl2 = MultiCurrencyCalculator.RoundCury(cache, row, aCuryTotal / (1 + rateLvl2 / 100), _precision);
				decimal baseLvl1 = MultiCurrencyCalculator.RoundCury(cache, row, baseLvl2 / (1 + (rateLvl1 + rateInclusive) / 100), _precision);
				...
				foreach (PXResult<Tax, TaxRev> iRes in orderedTaxList)
				{
					...
					switch (tax.TaxCalcLevel)
					{
						case CSTaxCalcLevel.Inclusive:
							break;
						case CSTaxCalcLevel.CalcOnItemAmt:
							curyTaxTotal += multiplier * MultiCurrencyCalculator.RoundCury(cache, row, (baseLvl1 * taxRev.TaxRate / 100m) ?? 0m, _precision);
							break;
						case CSTaxCalcLevel.CalcOnItemAmtPlusTaxAmt:
							curyTax2Total += multiplier * MultiCurrencyCalculator.RoundCury(cache, row, (baseLvl2 * taxRev.TaxRate / 100m) ?? 0m, _precision);
							break;
					}
				}
