...
namespace PX.Objects.TX
{
													 ...
													 IComparable
	{
		...
		protected virtual TaxDetail CalculateTaxSum(PXCache sender, object taxrow, object row)
		{
			...
			taxdet.CuryExpenseAmt = MultiCurrencyCalculator.RoundCury(sumcache, taxdet, CuryExpenseAmt, Precision);
		...
		private TaxDetail TaxSummarize(PXCache sender, object taxrow, object row, bool calcTaxes)
		{
			if (taxrow == null)
			{
				throw new PXArgumentException("taxrow", ErrorMessages.ArgumentNullException);
			...
			PXCache sumcache = sender.Graph.Caches[_TaxSumType];
			TaxDetail taxdet = (TaxDetail)((PXResult)taxrow)[0];
			var copy = sumcache.CreateCopy(taxdet);
			...
			if (taxSum != null)
			{
				if (!calcTaxes)
				{
					sumcache.SetValue(taxSum, _CuryTaxableAmt, sumcache.GetValue(copy, _CuryTaxableAmt));
					sumcache.SetValue(taxSum, _CuryTaxAmt, sumcache.GetValue(copy, _CuryTaxAmt));

					sumcache.MarkUpdated(taxSum);
					return taxSum;
				}
		...
		protected virtual void CalcTotals(PXCache sender, object row, bool CalcTaxes)
		{
			...
			{
				TaxDetail taxdet = TaxSummarize(sender, taxrow, row, CalcTaxes);
		...
		protected virtual void ClearChildTaxAmts(PXCache cache, object childRow)
		{
			...
			{
				childCache.MarkUpdated(childRow);
			}
		}
