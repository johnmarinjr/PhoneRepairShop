...
namespace PX.Objects.TX
{
	...
	internal class TaxReportLinesByTaxZonesReloader
	{
		...
		public void ReloadTaxReportLinesForTaxZones()
		{
			...
			{
				WebDialogResult result = taxReportGraph.Report.Ask(Messages.CreateReportLinesForNewTaxZones, messageDisplayed,
		...
		{
			TaxReport vendor = taxReportGraph?.Report.Current;
			...
			Dictionary<int, TaxReportLine> templateLinesByLineNumber =
				...
				Where<TaxReportLine.vendorID, Equal<Required<TaxReport.vendorID>>,
					And<TaxReportLine.taxReportRevisionID, Equal<Required<TaxReport.revisionID>>,
					And<TaxReportLine.tempLineNbr, IsNull,
					And<TaxReportLine.tempLine, Equal<True>>>>>>
				.Select(taxReportGraph, vendor.VendorID, vendor.RevisionID)
