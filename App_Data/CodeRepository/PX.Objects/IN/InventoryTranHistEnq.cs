...
*/
    ...
    public class InventoryTranHistEnq : PXGraph<InventoryTranHistEnq>
    {
		...
		[PXSelector(typeof(Search<PO.POReceipt.receiptNbr, Where<PO.POReceipt.receiptType.IsEqual<INTran.pOReceiptType.FromCurrent>>>))]
	    ...
	    protected virtual IEnumerable internalResultRecords()
	    {
		    ...
		    bool summaryByDay = filter.SummaryByDay ?? false;
		    bool includeUnreleased = filter.IncludeUnreleased ?? false;
		    bool filterByLotSerial = !string.IsNullOrEmpty(filter.LotSerialNbr) && PXAccess.FeatureInstalled<FeaturesSet.lotSerialTracking>();
            ...
            PXSelectBase<INTranSplit> cmdBeginBalanceLS = new PXSelectJoinGroupBy<INTranSplit,
                InnerJoin<INTran, On<INTranSplit.FK.Tran>,
                InnerJoin<INSubItem, On<INTranSplit.FK.SubItem>,
                InnerJoin<INSite, On<INTran.FK.Site>>>>,
                Where<INTranSplit.inventoryID, Equal<Current<InventoryTranHistEnqFilter.inventoryID>>,
                    And<INTranSplit.tranDate, Less<Required<INTranSplit.tranDate>>,
                    And<Match<INSite, Current<AccessInfo.userName>>>>>,
                Aggregate<GroupBy<INTranSplit.invtMult, Sum<INTranSplit.baseQty>>>>(this);
		    ...
		    if (!SubCDUtils.IsSubCDEmpty(filter.SubItemCD) && PXAccess.FeatureInstalled<FeaturesSet.subItem>())
		    {
                ...
                cmdBeginBalanceLS.WhereAnd<Where<INSubItem.subItemCD, Like<Current<InventoryTranHistEnqFilter.subItemCDWildcard>>>>();
            }
		    ...
		    if (filter.SiteID != null && PXAccess.FeatureInstalled<FeaturesSet.warehouse>())
		    {
                ...
                cmdBeginBalanceLS.WhereAnd<Where<INTranSplit.siteID, Equal<Current<InventoryTranHistEnqFilter.siteID>>>>();
            }

            if (PXAccess.FeatureInstalled<FeaturesSet.multipleBaseCurrencies>())
            {
                cmd.WhereAnd<Where<INSite.branchID, InsideBranchesOf<Current<InventoryTranHistEnqFilter.orgBAccountID>>>>();
                cmdBegBalanceNew.WhereAnd<Where<INSite.branchID, InsideBranchesOf<Current<InventoryTranHistEnqFilter.orgBAccountID>>>>();
                cmdBeginBalanceLS.WhereAnd<Where<INSite.branchID, InsideBranchesOf<Current<InventoryTranHistEnqFilter.orgBAccountID>>>>();
            }
            ...
            if ((filter.LocationID ?? -1) != -1 && PXAccess.FeatureInstalled<FeaturesSet.warehouseLocation>()) // there are cases when filter.LocationID = -1
		    {
                ...
                cmdBeginBalanceLS.WhereAnd<Where<INTranSplit.locationID, Equal<Current<InventoryTranHistEnqFilter.locationID>>>>();
            }

		    if (filterByLotSerial)
		    {
			    cmd.WhereAnd<Where<INTranSplit.lotSerialNbr, Like<Current<InventoryTranHistEnqFilter.lotSerialNbrWildcard>>>>();
                cmdBeginBalanceLS.WhereAnd<Where<INTranSplit.lotSerialNbr, Like<Current<InventoryTranHistEnqFilter.lotSerialNbrWildcard>>>>();
            }

		    if (!includeUnreleased)
		    {
			    cmd.WhereAnd<Where<INTranSplit.released, Equal<True>>>();
                cmdBeginBalanceLS.WhereAnd<Where<INTranSplit.released, Equal<True>>>();
            }
		    ...
		    if (filter.StartDate != null)
		    {
                if (filterByLotSerial)
                {
                    foreach (INTranSplit split in cmdBeginBalanceLS.Select(filter.StartDate))
                    {
                        cumulativeQty += (split.InvtMult * split.BaseQty) ?? 0m;
                    }
                }
                else
                {
