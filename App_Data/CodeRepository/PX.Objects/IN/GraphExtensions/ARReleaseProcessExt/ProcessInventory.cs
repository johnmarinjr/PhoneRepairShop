			...
			if (n.InventoryID != null && (n.LineType.IsIn(null, SOLineType.MiscCharge) || n.LineType.IsIn(SOLineType.Inventory, SO.SOLineType.NonInventory) && !tranCostSet))
			{
				...
				if (result != null)
				{
					...
					else if (n.SOShipmentType != SOShipmentType.DropShip)
					{
						...
						{
							var branchID = Branch.PK.Find(Base, n.BranchID);
							var itemCurySettings = InventoryItemCurySettings.PK.Find(Base, item.InventoryID, branchID.BaseCuryID);
							decimal stdCost = (itemCurySettings?.StdCost ?? 0m);
