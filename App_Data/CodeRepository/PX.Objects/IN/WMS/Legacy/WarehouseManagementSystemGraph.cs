...
// TODO: remove the whole file once AM team got their WMS-classes moved to the modern WMS-engine
namespace PX.Objects.IN
{
		...
		where TWMSHeader : WMSHeader, IBqlTable, new()
	{
		...
		[PXButton(CommitChanges = true, IsLockedOnToolbar = true), PXUIField(DisplayName = "Reset")]
		...
		[PXButton(CommitChanges = true, IsLockedOnToolbar = true), PXUIField(DisplayName = "Remove")]
		...
		[PXButton(CommitChanges = true, IsLockedOnToolbar = true), PXUIField(DisplayName = "Set Qty")]
		...
		protected virtual void _(Events.RowSelected<TWMSHeader> e)
		{
			...
			ScanModePick.SetVisible(Base.IsMobile && wmsFulfillment && false);
			ScanModePack.SetVisible(Base.IsMobile && wmsFulfillment && false);
			ScanModeShip.SetVisible(Base.IsMobile && wmsFulfillment && false);
			...
			ScanModeReceive.SetVisible(Base.IsMobile && wmsReceiving && false);
			ScanModePutAway.SetVisible(Base.IsMobile && wmsReceiving && false);
			...
			if (Base.IsMobile) // they are hidden on attribute level
				Info.Cache.AdjustUIReadonly()
					.For<WMSInfo.mode>(ui => ui.Visible = true)
					.SameFor<WMSInfo.message>()
					.SameFor<WMSInfo.prompt>();
		}
		#endregion
		...
		protected virtual bool CanHandleScan(TWMSHeader doc)
			=> doc.Barcode.StartsWith(ScanRedirects.Marker) || doc.ScanState == ScanStates.RefNbr || !DocumentLoaded || DocumentIsEditable;
	...
	public class WMSInfo : IBqlTable
	{
		...
		#region Instructions
		[PXString(255, IsUnicode = true)]
		[PXUIField(DisplayName = "Instructions", Enabled = false, Visible = false)]
		public virtual string Instructions { get; set; }
		public abstract class instructions : PX.Data.BQL.BqlString.Field<instructions> { }
		#endregion
		#region Prompt
		[PXString(255, IsUnicode = true)]
