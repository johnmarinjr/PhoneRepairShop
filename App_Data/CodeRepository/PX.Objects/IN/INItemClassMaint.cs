...
namespace PX.Objects.IN
{
	public class INItemClassMaint : PXGraph<INItemClassMaint, INItemClass>, IGraphWithInitialization, ICreateMatrixHelperFactory
	{
		...
		[INParentItemClass(defaultStkItemFromParent: true, InsertCurySettings = true)]
		...
		public
			SelectFrom<INItemClassCurySettings>.
			Where<INItemClassCurySettings.itemClassID.IsEqual<INItemClass.itemClassID.FromCurrent>.
				And<INItemClassCurySettings.curyID.IsEqual<AccessInfo.baseCuryID.FromCurrent>>>.
			View itemclasscurysettings;
		...
		public
			...
			Where<INItemClassRep.curyID.IsEqual<AccessInfo.baseCuryID.FromCurrent>.
				And<INItemClassRep.itemClassID.IsEqual<INItemClass.itemClassID.AsOptional>>>.
			View replenishment;
		...
		public override void Configure(PXScreenConfiguration config)
		{
			...
			context.AddScreenConfigurationFor(screen =>
			{
				return screen
					.WithActions(actions =>
					{
						actions.Add(g => g.applyToChildren, a => a.WithCategory(otherCategory));
						actions.Add(g => g.resetGroup, a => a.WithCategory(otherCategory));
		...
		protected virtual void _(Events.FieldUpdated<INItemClass, INItemClass.stkItem> e)
		{
			...
			if (e.Cache.GetStatus(e.Row) == PXEntryStatus.Inserted)
			{
				...
				var parentAttribute = e.Cache.GetAttributesReadonly<INItemClass.parentItemClassID>()
					.OfType<INParentItemClassAttribute>().FirstOrDefault();

				if (parentAttribute == null)
					throw new PXException(Common.Messages.CannotFindAttribute, e.Cache.DisplayName, nameof(INParentItemClassAttribute));

				parentAttribute?.CopyCurySettings(this, e.Row);
		...
		protected virtual void InitDetailsFromParentItemClass(INItemClass itemClass)
		{
			...
			using (new ReadOnlyScope(classunits.Cache, replenishment.Cache, Mapping.Cache))
			{
				...
				FieldVerifying.AddHandler<INItemClassRep.replenishmentSourceSiteID>(cancelVerifying);

				try
				{
					var replenishmentRows = SelectFrom<INItemClassRep>.
						Where<INItemClassRep.itemClassID.IsEqual<@P.AsInt>>.
						View.Select(this, itemClass.ParentItemClassID);

					foreach (INItemClassRep rep in replenishmentRows)
					{
				...
				{
					FieldVerifying.RemoveHandler<INItemClassRep.replenishmentSourceSiteID>(cancelVerifying);
				}
		...
		protected static void UpdateChildren(int? itemClassID)
		{
			...
			graph.itemclasscurysettings.Current = graph.itemclasscurysettings.Select();
		...
		protected virtual void UpdateChildItemClass(INItemClass child)
		{
			...
			bool updated = PXDatabase.Update<INItemClassCurySettings>(
				new PXDataFieldRestrict<INItemClassCurySettings.itemClassID>(child.ItemClassID),
				new PXDataFieldRestrict<INItemClassCurySettings.curyID>(Accessinfo.BaseCuryID),
				new PXDataFieldAssign<INItemClassCurySettings.dfltSiteID>(itemclasscurysettings.Current?.DfltSiteID)
			);

			if (!updated && itemclasscurysettings.Current?.DfltSiteID != null)
			{
				var cache = itemclasscurysettings.Cache;
				var fields = cache.Fields.
					Where(f => cache.GetAttributesReadonly(f).OfType<PXDBFieldAttribute>().Any() &&
						!f.Equals(nameof(INItemClassCurySettings.tstamp), StringComparison.OrdinalIgnoreCase) &&
						!f.Equals(nameof(INItemClassCurySettings.itemClassID), StringComparison.OrdinalIgnoreCase))
					.Select(
						f => new PXDataFieldAssign(f, itemclasscurysettings.Cache.GetValue(itemclasscurysettings.Current, f)))
					.Append(
						new PXDataFieldAssign<INItemClassCurySettings.itemClassID>(child.ItemClassID));

				PXDatabase.Insert<INItemClassCurySettings>(fields.ToArray());
			}
		}
			...
			foreach (INItemClassRep rep in replenishmentTemplate)
			{
				...
				if (replenishmentExisting.Any(r => r.ReplenishmentClassID == rep.ReplenishmentClassID))
				{
					var updateParams = new List<PXDataFieldParam>(crudParamsTemplate)
					{
						...
						new PXDataFieldRestrict<INItemClassRep.curyID>(rep.CuryID)
					};
				...
				else
				{
					var insertParams = new List<PXDataFieldAssign>(crudParamsTemplate)
					{
						...
						new PXDataFieldAssign<INItemClassRep.curyID>(rep.CuryID)
					};
