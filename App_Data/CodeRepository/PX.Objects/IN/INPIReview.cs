...
namespace PX.Objects.IN
{
	...
	public class INPIEntry : INPIController, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
			...
			foreach (var creditLine in creditLines.OrderBy(l => (int)l.LineNbr))
			{
				...
				foreach (var costLayer in costLayers)
				{
					if (costLayer.QtyOnHand <= restQty)
					{
						...
						tranTotalCostNotRounded = -costLayer.TotalCost.Value;
					...
					{
						tranQty = -restQty;
						tranTotalCostNotRounded = -(costLayer.TotalCost.Value / costLayer.QtyOnHand.Value) * restQty;
					...
					var projectedTran = CreateProjectedTran(costLayer, creditLine, itemSiteSettings);
		...
		protected virtual ProjectedTranRec CreateProjectedTran(INCostStatus costLayer, INPIDetail line, INItemSiteSettings itemSiteSettings)
        {
			ProjectedTranRec tran = CreateProjectedTran(line, true);
			tran.OrigRefNbr = itemSiteSettings.ValMethod == INValMethod.FIFO ? costLayer.ReceiptNbr : null;
	...
	public class INPIReview : INPIEntry  // (= PIEntry + some extended functionality)
	{
		...
		{
			var context = config.GetScreenConfigurationContext<INPIReview, INPIHeader>();
