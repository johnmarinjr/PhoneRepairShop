...
namespace PX.Objects.IN
{
	...
	public class INReplenishmentProjection : TM.OwnedFilter.ProjectionAttribute
	{
		...
		protected override Type GetSelect(PXCache sender)
		{
			...
			commands.AddRange(new [] {
				...
				typeof(InnerJoin<,,>), typeof(INSite),
					typeof(On<S.INItemSite.FK.Site>),
	...
	{
		#region Keys
		[Obsolete(Common.Messages.ItemIsObsoleteAndWillBeRemoved2021R1)]//rename UK to PK after that
		...
		public class UK : PrimaryKeyOf<INReplenishmentItem>.By<inventoryCD, siteCD, subItemCD>
		{
			public static INReplenishmentItem Find(PXGraph graph, string inventoryID, string siteID, string subItemCD) => FindBy(graph, inventoryID, siteID, subItemCD);
		}
		...
		[AP.VendorNonEmployeeActive(DisplayName = "Preferred Vendor ID", Required = false, DescriptionField = typeof(Vendor.acctName), BqlField=typeof(INItemSite.preferredVendorID))]
		...
		#endregion
		#region QtyProcessRounded
		[PXBool()]
		public virtual bool? QtyProcessRounded { get; set; }
		public abstract class qtyProcessRounded : BqlBool.Field<qtyProcessRounded> { }
		#endregion
	...
	public class INReplenishmentCreate: PXGraph<INReplenishmentCreate>
	{
		...
		protected virtual void INReplenishmentItem_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			{
				ManageQtyProcessRoundedWarning(rec);
			}
		...
		protected virtual bool ManageQtyProcessRoundedWarning(INReplenishmentItem rec)
		{
			return false;
		}
		...
		protected static void ReplenishmentCreateProc(List<INReplenishmentItem> list, INReplenishmentFilter filter)
		{
			...
			foreach (INReplenishmentItem rec in list)
			{
				...
				if (rec.QtyProcess > 0)
				{
					...
					int? replenishmentSourceSiteID = PXAccess.FeatureInstalled<FeaturesSet.warehouse>() && rec.ReplenishmentSourceSiteID != rec.SiteID ? rec.ReplenishmentSourceSiteID : null;
