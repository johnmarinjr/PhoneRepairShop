...
namespace PX.Objects.IN
{
	public class INReceiptEntry : INRegisterEntryBase, PXImportAttribute.IPXPrepareItems
	{
		...
		protected virtual void _(Events.RowSelected<INRegister> e)
		{
			...
			LineSplittingExt.lsselect.AllowInsert = e.Row.Released == false && e.Row.OrigModule == GL.BatchModule.IN && !isTransfer;
			LineSplittingExt.lsselect.AllowUpdate = e.Row.Released == false;
			LineSplittingExt.lsselect.AllowDelete = e.Row.Released == false && e.Row.OrigModule == GL.BatchModule.IN && !isTransfer;
		...
		public override void CopyPasteGetScript(bool isImportSimple, List<PX.Api.Models.Command> script, List<PX.Api.Models.Container> containers)
		{
			...
			var transfer = INRegister.PK.Find(this, INDocType.Transfer, receipt.Current?.TransferNbr);
			if (transfer?.OrigModule != null && transfer.OrigModule != BatchModule.IN)
			{
				var tranferFieldIndex = script.FindIndex(c => 
						c.ObjectName == nameof(receipt)
						&& c.FieldName == nameof(INRegister.TransferNbr));
				if (tranferFieldIndex >= 0)
				{
					script.RemoveAt(tranferFieldIndex);
					containers.RemoveAt(tranferFieldIndex);
				}
			}
		}
		...
		public class LineSplittingExtension : GraphExtensions.INRegisterLineSplittingExtension<INReceiptEntry> { }
		#endregion

		#region ItemAvailability
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class ItemAvailabilityExtension : GraphExtensions.INRegisterItemAvailabilityExtension<INReceiptEntry> { }
		#endregion
		...
		public class TransferNbrSelectorAttribute : PXSelectorAttribute
		{
			...
			{
				bool isSelectorCacheAttachedExecuted = (_outerview != null);
				if (isSelectorCacheAttachedExecuted)
				{
				_outerview.WhereAnd(whr);
				return _outerview.BqlSelect;
			}
				else return base.WhereAnd(sender, whr);
			}
		}
