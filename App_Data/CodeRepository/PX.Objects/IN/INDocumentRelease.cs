...
namespace PX.Objects.IN
{
    ...
    public class INReleaseProcess : PXGraph<INReleaseProcess>
    {
        ...
        protected virtual TransitLocationStatus TwoStepTransferNonLot(INTran tran, INTranSplit split)
        {
        ...
        public virtual LotSerialStatus AccumulatedLotSerialStatus(INTran tran, INTranSplit split, INLotSerClass lsclass)
        {
        ...
        public virtual TransitLotSerialStatus AccumulatedTransitLotSerialStatus(INTran tran, INTranSplit split, INLotSerClass lsclass, INTransitLine tl)
        {
        ...
        protected virtual LotSerialStatus OneStepTransferLot(INTran tran, INTranSplit split, InventoryItem item, INLotSerClass lsclass)
        {
            LotSerialStatus lsitem = AccumulatedLotSerialStatus(tran, split, lsclass);
        ...
        protected virtual TransitLotSerialStatus TwoStepTransferLot(INTran tran, INTranSplit split, InventoryItem item, INLotSerClass lsclass)
        {
            TransitLotSerialStatus lstritem = null;
            if (IsIngoingTransfer(tran))
            {
                var trl = new INTransitLine();
                trl.TransferNbr = tran.OrigRefNbr;
                ...
                lstritem = AccumulatedTransitLotSerialStatus(tran, split, lsclass, trl);
                ...
                {
                    TwoStepTransferLotComplement(tran, split, item, lsclass, res.ReceiptDate.Value);
                }
            }
            else
            {
                ...
                lstritem = AccumulatedTransitLotSerialStatus(tran, split, lsclass, ntl);
        ...
        protected virtual LotSerialStatus TwoStepTransferLotComplement(INTran tran, INTranSplit split, InventoryItem item, INLotSerClass lsclass, DateTime receiptDate)
        {
            LotSerialStatus complementlsitem = AccumulatedLotSerialStatus(tran, split, lsclass);
            complementlsitem.ReceiptDate = receiptDate;
		...
		{
			return
				tran.TranType == INTranType.Transfer && layer.CostSiteID == INTransitSiteID;
		}
		...
		public virtual void ValidateOrigLCReceiptIsReleased(INRegister doc)
		{
			...
			{
				var ortran = LandedCostAllocationService.Instance.GetOriginalInTran(this, unattachedLCtran.POReceiptType, unattachedLCtran.POReceiptNbr, unattachedLCtran.POReceiptLineNbr);
...
namespace PX.Objects.IN.Overrides.INDocumentRelease
{
    ...
    [ItemStatsAccumulator(
        LastCostDateField = typeof(ItemStats.lastCostDate), LastCostField = typeof(ItemStats.lastCost),
        MinCostField = typeof(ItemStats.minCost), MaxCostField = typeof(ItemStats.maxCost),
        QtyOnHandField = typeof(ItemStats.qtyOnHand), TotalCostField = typeof(ItemStats.totalCost))]
    ...
    [ItemStatsAccumulator(
        LastCostDateField = typeof(ItemCost.lastCostDate), LastCostField = typeof(ItemCost.lastCost),
        MinCostField = typeof(ItemCost.minCost), MaxCostField = typeof(ItemCost.maxCost),
        QtyOnHandField = typeof(ItemCost.qtyOnHand), TotalCostField = typeof(ItemCost.totalCost))]
    [PXDisableCloneAttributes()]
    [PXBreakInheritance]
    [PXHidden]
    public class ItemCost : INItemCost
    {
        ...
        public new abstract class lastCostSiteID : PX.Data.BQL.BqlInt.Field<lastCostSiteID> { }
        [PXInt]
        public virtual Int32? LastCostSiteID
        {
            get;
            set;
        }
        #endregion
        #region TranUnitCost
        public new abstract class tranUnitCost : PX.Data.BQL.BqlDecimal.Field<tranUnitCost> { }
        [PXDecimal]
	...
	public class INTranCostUpdateAccum : PXAccumulatorAttribute
    {
        ...
        protected override bool PrepareInsert(PXCache sender, object row, PXAccumulatorCollection columns)
        {
            ...
            if (itcrow.ResetOversoldFlag ?? false)
            {
                columns.AppendException(string.Empty, new PXAccumulatorRestriction<INTranCostUpdate.oversoldQty>(PXComp.EQ, 0));
                if (itcrow.ValMethod != INValMethod.Standard)
                columns.AppendException(string.Empty, new PXAccumulatorRestriction<INTranCostUpdate.oversoldTranCost>(PXComp.EQ, 0));
            }
            else
            {
                columns.AppendException(string.Empty, new PXAccumulatorRestriction<INTranCostUpdate.oversoldQty>(PXComp.GE, 0));
                if (itcrow.ValMethod != INValMethod.Standard)
                {
                columns.AppendException(string.Empty, new PXAccumulatorRestriction<INTranCostUpdate.oversoldTranCost>(PXComp.GE, 0));
                columns.AppendException(string.Empty,
                    new PXAccumulatorRestriction<INTranCostUpdate.oversoldQty>(PXComp.GT, 0),
                    new PXAccumulatorRestriction<INTranCostUpdate.oversoldTranCost>(PXComp.EQ, 0));
            }
            }
            return true;
        }
        ...
        public override bool PersistInserted(PXCache sender, object row)
        {
            ...
            catch (PXRestrictionViolationException)
            {
				...
				var it = INTran.PK.Find(sender.Graph, ((INTranCostUpdate)row).DocType, ((INTranCostUpdate)row).RefNbr, ((INTranCostUpdate)row).LineNbr);
    ...
    [PXDisableCloneAttributes()]
    [Serializable]
	public partial class SiteStatus : INSiteStatus, IQtyAllocated, ICostStatus
	{
		#region Keys
    ...
    [Serializable]
    public partial class LotSerialStatus : INLotSerialStatus, IQtyAllocated, ILotSerial
    {
	    #region Keys
    ...
    public partial class SiteLotSerial : INSiteLotSerial, IQtyAllocatedBase
    {
        ...
        #endregion
		#region NegActualQty
		public abstract class negActualQty : PX.Data.BQL.BqlBool.Field<negActualQty> { }
		[PXBool]
		...
		[PXDefault(TypeCode.Decimal, "0.0", typeof(Coalesce<
			...
			Search<INItemCost.lastCost,
				Where<INItemCost.inventoryID, Equal<Current<OversoldCostStatus.inventoryID>>,
                    And<INItemCost.curyID, Equal<Current<INRegister.branchBaseCuryID>>>>>>))]
    ...
    public class ItemStatsAccumulatorAttribute : PXAccumulatorAttribute
    {
        ...
        protected override bool PrepareInsert(PXCache sender, object row, PXAccumulatorCollection columns)
        {
            ...
            if (lastCostDate == INItemStats.MinDate.get())
            {
				...
				columns.Update(LastCostField, 0m, PXDataFieldAssign.AssignBehavior.Initialize);
				// It is needed for correct work of DBLastChangedDateTime.
                columns.Update(LastCostDateField, lastCostDate, PXDataFieldAssign.AssignBehavior.Initialize);
            }
            else
            {
                var lastCost = (decimal?)sender.GetValue(row, LastCostField.Name);
                columns.Update(LastCostField, lastCost, PXDataFieldAssign.AssignBehavior.Replace);

                columns.Update(LastCostDateField, lastCostDate, PXDataFieldAssign.AssignBehavior.Replace);
            }

            var minCost = (decimal?)sender.GetValue(row, MinCostField.Name);
            if (minCost == 0m)
            {
                columns.Update(MinCostField, minCost, PXDataFieldAssign.AssignBehavior.Initialize);
            }
            else
            {
                columns.Update(MinCostField, minCost, PXDataFieldAssign.AssignBehavior.Minimize);
            }

            var maxCost = (decimal?)sender.GetValue(row, MaxCostField.Name);
            columns.Update(MaxCostField, maxCost, PXDataFieldAssign.AssignBehavior.Maximize);

            var qtyOnHand = (decimal?)sender.GetValue(row, QtyOnHandField.Name);
            columns.Update(QtyOnHandField, qtyOnHand, PXDataFieldAssign.AssignBehavior.Summarize);

            var totalCost = (decimal?)sender.GetValue(row, TotalCostField.Name);
            columns.Update(TotalCostField, totalCost, PXDataFieldAssign.AssignBehavior.Summarize);
    ...
    public class SiteLotSerialAccumulatorAttribute : StatusAccumulatorAttribute
    {
        ...
        protected override bool PrepareInsert(PXCache sender, object row, PXAccumulatorCollection columns)
        {
            ...
            if (bal.LotSerTrack == INLotSerTrack.SerialNumbered && bal.LotSerAssign == INLotSerAssign.WhenUsed)
                return false;
			...
			else if (this.ValidateAvailQty(sender.Graph))
            {
				...
				if (bal.QtyHardAvail < 0m
					&& bal.LotSerTrack == INLotSerTrack.LotNumbered && bal.LotSerAssign == INLotSerAssign.WhenReceived)
				{
					columns.Restrict<SiteLotSerial.qtyHardAvail>(PXComp.GE, -bal.QtyHardAvail);
				}
            }
			if (bal.NegActualQty != true && bal.QtyActual < 0m && bal.LotSerAssign == INLotSerAssign.WhenReceived)
			{
            ...
            catch (PXLockViolationException)
            {
				...
				else if (this.ValidateAvailQty(sender.Graph))
                {
						...
						&& bal.LotSerTrack == INLotSerTrack.LotNumbered && bal.LotSerAssign == INLotSerAssign.WhenReceived)
					{
						...
						{
							throw new PXException(
								sender.Graph is INRegisterEntryBase
