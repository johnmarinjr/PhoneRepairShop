...
namespace PX.Objects.IN
{
	...
	public class INPIClassMaint : PXGraph<INPIClassMaint, INPIClass>
	{
		...
		{
			// IncludeZeroItems field depends on SelectedMethod, so it should be copied at the end.
			int includeZeroItemsIndex = script.FindIndex(_ => _.FieldName == nameof(INPIClass.IncludeZeroItems));
			if (includeZeroItemsIndex == -1)
				return;

			Api.Models.Command cmdIncludeZeroItems = script[includeZeroItemsIndex];
			Api.Models.Container cntIncludeZeroItems = containers[includeZeroItemsIndex];

			script.Remove(cmdIncludeZeroItems);
			containers.Remove(cntIncludeZeroItems);

			script.Add(cmdIncludeZeroItems);
			containers.Add(cntIncludeZeroItems);
		}
	}
