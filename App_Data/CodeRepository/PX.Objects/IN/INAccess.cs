	...
	public class INAccessItem : PX.SM.BaseAccess
	{
		...
		)
		{
			...
			var query = new PXSelect<InventoryItem,
				...
				And<Where2<
					Match<Current<PX.SM.RelationGroup.groupName>>,
					Or<Match<Required<InventoryItem.groupMask>>>>>>>>(this);

			var startRow = PXView.StartRow;
			var totalRows = 0;

			var parameters = PXView.Parameters.Concat(new object[] { new byte[0] }).ToArray();

			foreach (InventoryItem item in query.View.Select(
				PXView.Currents, parameters, PXView.Searches, PXView.SortColumns, PXView.Descendings, PXView.Filters,
