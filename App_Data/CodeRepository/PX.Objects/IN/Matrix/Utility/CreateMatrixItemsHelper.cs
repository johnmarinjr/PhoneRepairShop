...
namespace PX.Objects.IN.Matrix.Utility
{
	public class CreateMatrixItemsHelper
	{
			...
			List<INMatrixGenerationRule> idGenRules, List<INMatrixGenerationRule> descrGenRules)
		{
			...
			var itemCurySettings = InventoryItemCurySettings.PK.Find(_graph, template.InventoryID, _graph.Accessinfo.BaseCuryID);
			newItem.DfltSiteID = itemCurySettings?.DfltSiteID;
		...
		public virtual string GetAttributeCaption(INMatrixGenerationRule genRule, InventoryItem template, MatrixInventoryItem newItem, string locale)
		{
			...
			{
				CSAnswers answer = CSAnswers.PK.FindDirty(_graph, template.NoteID, genRule.AttributeID);
			...
			Action<MatrixInventoryItem, InventoryItem> beforeSave = null)
		{
			...
			IEnumerable<InventoryItemCurySettings> templateCurySettings =
				graph.ItemCurySettings.Select(templateItem.InventoryID)
			...
			foreach (MatrixInventoryItem itemToCreateUpdate in itemsToCreateUpdate)
			{
				...
				AssignCurySettings(graph, templateCurySettings);
		...
		{
			if (!userExcludedFields.Contains(nameof(InventoryItem.body)))
			{
				PXDBLocalizableStringAttribute.CopyTranslations
					<InventoryItem.body, InventoryItem.body>(_graph, templateItem, item);
			}
			...
			var excludeFields = new string[]
			{
				...
				nameof(InventoryItem.dfltReceiptLocationID),
				nameof(InventoryItem.dfltShipLocationID),
				nameof(InventoryItem.dfltSiteID),
				nameof(InventoryItem.lastStdCost),
				nameof(InventoryItem.pendingStdCost),
				nameof(InventoryItem.pendingStdCostDate),
				nameof(InventoryItem.preferredVendorID),
				nameof(InventoryItem.preferredVendorLocationID),
				nameof(InventoryItem.recPrice),
				nameof(InventoryItem.stdCost),
				nameof(InventoryItem.stdCostDate),
		...
		protected virtual void AssignDummyAttribute(InventoryItemMaintBase graph, InventoryItem template, CSAnswers[] answers)
		{
			...
			if ((template.DefaultColumnMatrixAttributeID == dummyAttribute ||
				 ...
				 CSAnswers.PK.FindDirty(graph, graph.Item.Current.NoteID, dummyAttribute) == null)
			{
		...
		protected virtual void AssignCurySettings(InventoryItemMaintBase graph,
			IEnumerable<InventoryItemCurySettings> templateCurySettingsRows)
		{
			...
			var curySettingsRows = graph.ItemCurySettings.Select(graph.Item.Current.InventoryID)
				.RowCast<InventoryItemCurySettings>().ToArray();

			foreach (var templateCurySettings in templateCurySettingsRows)
			{
				InventoryItemCurySettings curySetting =
					curySettingsRows.FirstOrDefault(cs
						=> cs.CuryID == templateCurySettings.CuryID);

				bool createCurySettings = (curySetting == null);

				var excludeFields = createCurySettings ? excludeFieldsCreate : excludeFieldsUpdate;
				if (excludeFields == null)
				{
					excludeFields = new string[]
					{
						nameof(InventoryItemCurySettings.inventoryID),
						nameof(InventoryItemCurySettings.lastStdCost),
						nameof(InventoryItemCurySettings.stdCost),
						nameof(InventoryItemCurySettings.stdCostDate),
						nameof(InventoryItemCurySettings.preferredVendorID),
						nameof(InventoryItemCurySettings.preferredVendorLocationID),
						nameof(InventoryItemCurySettings.Tstamp),
						nameof(InventoryItemCurySettings.createdByID),
						nameof(InventoryItemCurySettings.createdByScreenID),
						nameof(InventoryItemCurySettings.createdDateTime),
						nameof(InventoryItemCurySettings.lastModifiedByID),
						nameof(InventoryItemCurySettings.lastModifiedByScreenID),
						nameof(InventoryItemCurySettings.lastModifiedDateTime),
					};

					var userExcludedFields = GetExcludedFields(templateCurySettings.InventoryID, typeof(InventoryItemCurySettings));

					if (createCurySettings)
					{
						skipNewRows = userExcludedFields.Any(f => new[] {
							nameof(InventoryItemCurySettings.curyID),
							}.Contains(f, StringComparer.OrdinalIgnoreCase));
				...
				if (createCurySettings)
				{
					...
					curySetting = graph.ItemCurySettings.Insert(
						new InventoryItemCurySettings { CuryID = templateCurySettings.CuryID });
				}
				var curySettingsCache = graph.ItemCurySettings.Cache;
				var copy = (InventoryItemCurySettings)curySettingsCache.CreateCopy(curySetting);
				curySettingsCache.RestoreCopy(copy, templateCurySettings);

				foreach (string excludeField in excludeFields)
				{
					curySettingsCache.SetValue(copy, excludeField,
						curySettingsCache.GetValue(curySetting, excludeField));
				}

				curySetting = graph.ItemCurySettings.Update(copy);
			}
		}
		...
		{
			var select = new PXSelect<ExcludedField,
			...
			var result = select.SelectMain(templateID, tableName.FullName)
				.Select(excludeField => excludeField.FieldName)
				.ToList();

			AddRelatedExcludedFields(result, templateID, tableName, select);

			return result;
		}

		protected virtual void AddRelatedExcludedFields(List<string> result, int? templateID, Type tableName, PXSelectBase<ExcludedField> select)
		{
			// Because of the integration between InventoryItem and InventoryItemCurySettings we have to
			// exclude fields which added for InventoryItem if it's InventoryItemCurySettings.
			// Remove this code after removing the integration.
			if (tableName == typeof(InventoryItemCurySettings))
			{
				result.AddRange(
					select.SelectMain(templateID, typeof(InventoryItem).FullName)
						.Where(excludeField =>
							_graph.Caches[typeof(InventoryItemCurySettings)].Fields.Contains(excludeField.FieldName))
						.Select(excludeField => excludeField.FieldName));
			}
		}
