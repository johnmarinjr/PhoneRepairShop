...
namespace PX.Objects.IN
{
	public class INTransferEntry : INRegisterEntryBase
	{
		...
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXCustomizeBaseAttribute(typeof(LocationAvailAttribute), nameof(LocationAvailAttribute.Enabled), true)]
		[PXDefault]
		...
		protected virtual void _(Events.RowSelected<INRegister> e)
		{
			if (e.Row == null)
				return;
			bool isEditableDocument = ((INRegister)e.Row).Released == false && ((INRegister)e.Row).OrigModule == GL.BatchModule.IN;
			PXUIFieldAttribute.SetEnabled(e.Cache, e.Row, isEditableDocument);
			...
			LineSplittingExt.lsselect.AllowInsert = e.Row.Released == false && e.Row.OrigModule == BatchModule.IN && e.Row.SiteID != null;
			LineSplittingExt.lsselect.AllowUpdate = e.Row.Released == false;
			LineSplittingExt.lsselect.AllowDelete = e.Row.Released == false && e.Row.OrigModule == BatchModule.IN;
			
			PXUIFieldAttribute.SetEnabled<INTran.toLocationID>(transactions.Cache, null, isEditableDocument); // Should be after setting lsselect.AllowInsert because LSSelect disables/enables all fields.
		...
		public class LineSplittingExtension : GraphExtensions.INRegisterLineSplittingExtension<INTransferEntry> { }
		#endregion
		...
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class ItemAvailabilityExtension : GraphExtensions.INRegisterItemAvailabilityExtension<INTransferEntry> { }
		#endregion
