...
namespace PX.Objects.IN
{
	public class InventoryItemMaint : InventoryItemMaintBase
	{
			...
			protected virtual IEnumerable UpdateCost(PXAdapter adapter)
			{
				...
				if (row?.PendingStdCostDate != null && Base.ItemSettings.Current?.ValMethod == INValMethod.Standard)
				{
					...
					if (layer == null)
					{
						...
						curySettings.Cache.MarkUpdated(row);
						...
						if (string.Equals(row.CuryID, CurrencyCollection.GetBaseCurrency()?.CuryID, StringComparison.OrdinalIgnoreCase))
						{
							...
							Base.ItemSettings.Cache.MarkUpdated(item);
						}
						...
						foreach (INItemSite itemSite in Base.itemsiterecords.Select())
						{
							...
							Base.itemsiterecords.Cache.MarkUpdated(itemSite);
						}
		...
		#endregion
		#region SiteStatus
		[AnyInventory(IsKey = true, DirtyRead = true)]
		...
		public override void InitCacheMapping(Dictionary<Type, Type> map)
		{
			...
			this.Caches.AddCacheMapping(typeof(SiteStatus), typeof(SiteStatus));
		}
		...
		private bool IsWhenUsedQtyStillPresent()
		{
			INSiteStatus sitestatus =
				SelectFrom<INSiteStatus>
				.Where<INSiteStatus.FK.InventoryItem.SameAsCurrent
				.And<
					INSiteStatus.qtyOnHand.IsNotEqual<decimal0>
					...
					.Or<INSiteStatus.qtySOShipped.IsNotEqual<decimal0>>
					.Or<INSiteStatus.qtySOShipping.IsNotEqual<decimal0>>>>
		...
		protected virtual void _(Events.RowDeleting<InventoryItem> e)
		{
			...
			if (sitestat != null)
				throw new PXException(Messages.ItemHasStockRemainder, e.Row.InventoryCD, nonemptysitestatuses.GetValueExt<INSiteStatus.siteID>(sitestat));
		}
		...
		protected virtual void SetLastCostEnabled()
		{
			...
			bool lastCostEnabled = Item.Current.ValMethod.IsNotIn(INValMethod.Standard, INValMethod.Specific) && (!PXAccess.FeatureInstalled<FeaturesSet.warehouse>() || itemsiterecords.SelectSingle() != null);
		...
		{
			foreach (PXResult<INItemSite, INSite> rec in itemsiterecords.Select(r.CuryID))
			{
