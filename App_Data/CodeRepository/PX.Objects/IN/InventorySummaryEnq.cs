...
namespace PX.Objects.IN
{
	...
	public class InventorySummaryEnq : PXGraph<InventorySummaryEnq>
	{
		...
		protected virtual void AppendCostLocationLayerJoin(PXSelectBase<INLocationStatus> cmd)
		{			
		}

		protected virtual void AppendCostLotSerialLayerJoin(PXSelectBase<INLotSerialStatus> cmd)
		{
		}
		...
		protected virtual ICostStatus GetCostStatus(PXResult res)
        {
		...
		protected virtual IEnumerable iSERecordsFetch()
		{
			...
			using (new PXFieldScope(cmd_lss.View, fieldScope.ToArray()))
			{
				foreach (PXResult<INLotSerialStatus, INLocation, InventoryItem, INSite, INSubItem> res in cmd_lss.Select())
				{
					...
					InventorySummaryEnquiryResult ret = lss_rec.CopyToNew<InventorySummaryEnquiryResult>();
					...
					if (lscs_rec?.InventoryID != null)
					{
						...
						ret.TotalCost = PXDBCurrencyAttribute.BaseRound(this, ret.QtyOnHand.GetValueOrDefault() * lscs_rec.UnitCost.GetValueOrDefault());
					}
					else
					{
						ICostStatus costStatus = GetCostStatus(res);
						ret.UnitCost = CalculateUnitCost(costStatus, decPlPrcCst);
						ret.TotalCost = PXDBCurrencyAttribute.BaseRound(this, ret.QtyOnHand.GetValueOrDefault() * ret.UnitCost.GetValueOrDefault());
					}
			...
			foreach (PXResult<INLocationStatus, INLocation, InventoryItem, INSite, INSubItem> res in cmd_ls.Select())
			{
				...
				InventorySummaryEnquiryResult ret = ls_rec.CopyToNew<InventorySummaryEnquiryResult>();
				...
				var siteAggregate = new Lazy<SiteStatus>(() =>
				{
					PXCache cache = this.Caches[typeof(SiteStatus)];
					SiteStatus aggregate = new SiteStatus();
					aggregate.InventoryID = ret.InventoryID;
				...
				{
					if (item_rec.ValMethod == INValMethod.Specific
						|| filter.ExpandByLotSerialNbr == true && INLotSerClass.PK.Find(this, item_rec.LotSerClassID)?.LotSerAssign == INLotSerAssign.WhenReceived)
					{
...

				{
					...
					if (filter.ExpandByLotSerialNbr == true)
					{
						ret = ret.SubtractToNew<InventorySummaryEnquiryResult>(aggregate);
					}
			...
			if (filter.ExpandByLotSerialNbr == true)
			{
				...
				foreach (PXResult<INItemPlan, InventoryItem, INSite, INSubItem> res in lstPlans)
				{
					...
					InventorySummaryEnquiryResult ret = ss_rec.CopyToNew<InventorySummaryEnquiryResult>();
					...
					ret.Add(aggregate);
					ret = ret.SubtractToNew<InventorySummaryEnquiryResult>(origaggregate);
			...
			foreach (PXResult<INSiteStatus, InventoryItem, INSite, INSubItem> res in cmd_ss.Select())
			{
...

				{
                    ...
                    {
						ret.TotalCost = aggregate.TotalCost;
						ret.UnitCost = aggregate.QtyOnHand == 0m
							? 0m 
							: RoundUnitCost(aggregate.TotalCost / aggregate.QtyOnHand);
					}
				}
		...
		private decimal? CalculateUnitCost(ICostStatus cost, int precision)
        {
			return (cost == null || cost.QtyOnHand == null || cost.TotalCost == null) ? (decimal?)null : (cost.QtyOnHand != 0m) ? Math.Round((decimal)cost.TotalCost / (decimal)cost.QtyOnHand, precision, MidpointRounding.AwayFromZero) : 0m;
		}
