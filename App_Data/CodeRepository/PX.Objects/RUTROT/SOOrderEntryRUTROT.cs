	...
	public class SOOrderEntryRUTROT : PXGraphExtension<MobileCreatePaymentExt, CreatePaymentExt, SOOrderEntry>
	{
		...
		public override void Initialize()
		{
			...
			RRManager = new RUTROTManager<SOOrder, SOLine, SOOrderRUTROT, SOLineRUTROT>(
				this.Base, Rutrots, RRDistribution, Base.Document, Base.Transactions, () => Base.currencyinfo.Current ?? Base.currencyinfo.SelectSingle());
		}
        ...
        protected virtual void SubtractRUTROT(SOOrder order, ARPaymentEntry docgraph, bool updatePayment)
        {
            ...
            if (orderRR.IsRUTROTDeductible == true)
            {
				...
				var ordersToApplyTabExtension = docgraph.GetOrdersToApplyTabExtension(true);

				foreach (SOAdjust adj in ordersToApplyTabExtension.SOAdjustments.Select())
                {
                    ...
                    {
						ordersToApplyTabExtension.SOAdjustments.Cache.SetValueExt<SOAdjust.curyAdjgAmt>(adj, adj.CuryAdjgAmt - rutrot.CuryTotalAmt);
						ordersToApplyTabExtension.SOAdjustments.Update(adj);
