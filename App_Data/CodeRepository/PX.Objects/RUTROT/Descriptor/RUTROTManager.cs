...
namespace PX.Objects.RUTROT
{
	...
	public class RUTROTManager<Doc, Tran, DocExt, TranExt>
		where Doc : class, IBqlTable, CM.IInvoice, new()
		...
		where TranExt : PXCacheExtension<Tran>, IRUTROTableLine
	{
		...
		protected Func<CurrencyInfo> GetCurrencyInfo;

		public RUTROTManager(PXGraph graph, PXSelectBase<RUTROT> rutrot, PXSelectBase<RUTROTDistribution> distribution, 
			PXSelectBase<Doc> document, PXSelectBase<Tran> transactions, Func<CurrencyInfo> curyInfo)
		{
			this.Base = graph;
			Rutrots = rutrot;
		...
		public void Update(Doc document)
		{
			...
			WarnUndistributedAmount(documentRR, rutrot, errorLevel, GetCurrencyInfo());
        }
		...
		{
			RedistributeDeduction(RUTROTHelper.GetExtensionNullable<Doc, DocExt>(Document.Current), (RUTROT)e.Row, ARSetup.Current, GetCurrencyInfo());
		}
		...
		protected virtual void RUTROT_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (WarnUndistributedAmount(doc, rutrot, errorLevel, GetCurrencyInfo()) && treatAsError)
			{
				sender.RaiseExceptionHandling<RUTROT.curyTotalAmt>(rutrot, rutrot.CuryTotalAmt,
