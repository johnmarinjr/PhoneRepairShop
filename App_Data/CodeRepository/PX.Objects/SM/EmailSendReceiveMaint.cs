...
namespace PX.SM
{
	...
	public class EmailSendReceiveMaint : PXGraph<EmailSendReceiveMaint>
	{
		...
		public PXFilteredProcessing<EMailAccount, OperationFilter, 
										Where<EMailAccount.emailAccountType, Equal<EmailAccountTypesAttribute.standard>, 
											And<EMailAccount.isActive, Equal<True>>>>
			FilteredItems;

		public IEnumerable filteredItems()
		{
			foreach (EMailAccount account in PXSelect<EMailAccount, 
												Where<EMailAccount.emailAccountType, Equal<EmailAccountTypesAttribute.standard>,
													And<EMailAccount.isActive, Equal<True>>>>.Select(this))
			{
