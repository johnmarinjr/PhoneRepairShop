	...
	public class DropshipReturn : PXGraphExtension<SOOrderEntry>
	{
		...
		protected virtual void CreatePOReturn(POReceiptEntry receiptGraph, DocumentList<POReceipt> receiptList)
		{
			...
			for (int i = 0; i < linesToReturn.Count; i++)
			{
				...
				{
					POReceipt poReturn = receiptGraph.Document.Insert(
					...
					receiptGraph.CopyReceiptCurrencyInfoToReturn(receipt.CuryInfoID, receiptGraph.Document.Current?.CuryInfoID,
						receiptGraph.Document.Current?.ReturnInventoryCostMode == ReturnCostMode.OriginalCost);
				}

				POReceiptLine returnLine = AddReturnLine(receiptGraph, receiptLine, line);
				CurrencyInfo currencyInfo = receiptGraph.FindImplementation<IPXCurrencyHelper>().GetDefaultCurrencyInfo();
				...
				decimal? lineAmt = currencyInfo.RoundBase((decimal)(line.OrderQty * actualDiscUnitPrice));
