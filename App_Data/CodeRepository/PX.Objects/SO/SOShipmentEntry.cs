	...
	public partial class SOShipmentEntry : PXGraph<SOShipmentEntry, SOShipment>, IGraphWithInitialization
	{
		...
		protected virtual IEnumerable ConfirmShipmentAction(PXAdapter adapter)
		{
			...
			PXLongOperation.StartOperation(this, delegate ()
			{
				...
				MergeStatusCachesBetweenGraphs(docgraph, orderentry);
		...
		[PXButton(CommitChanges = true), PXUIField(Enabled = false, Visible = false, DisplayName = ShipmentActions.Messages.CancelReturn, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		{
			return CreateSplitsForAvailableLotsImpl<INLotSerialStatus, LotSerialStatus, INSiteStatus, SiteStatus>(
		...
		public virtual decimal? CreateSplitsForAvailableNonLots(
			...
			SOShipLine newline, INLotSerClass lotserclass)
		{
			return CreateSplitsForAvailableNonLotsImpl<INLocationStatus, LocationStatus, INSiteStatus, SiteStatus>(PlannedQty, origPlanType, newline, lotserclass);
		}
		...
		public virtual decimal? ShipAvailable(SOShipmentPlan plan, SOShipLine newline, PXResult<InventoryItem, INLotSerClass> item)
		{
			...
			if (initem.StkItem == false && initem.KitItem == true)
			{
				...
				copy.ShippedQty = (copy.UOM == copy.OrderUOM && kitqty == copy.BaseFullOrderQty) ? copy.FullOrderQty
					: INUnitAttribute.ConvertFromBase<SOShipLine.inventoryID>(Transactions.Cache, copy, copy.UOM, (decimal)kitqty, INPrecision.QUANTITY);
					...
					if (args.ShipmentList != null && item.ShipComplete == SOShipComplete.ShipComplete)
					{
						foreach (SOLine2 line in PXSelect<SOLine2, Where<SOLine2.orderType, Equal<Required<SOLine2.orderType>>, And<SOLine2.orderNbr, Equal<Required<SOLine2.orderNbr>>, And<SOLine2.siteID, Equal<Required<SOLine2.siteID>>, And<SOLine2.operation, Equal<Required<SOLine2.operation>>, And<SOLine2.completed, NotEqual<True>>>>>>>.Select(this, item.OrderType, item.OrderNbr, item.SiteID, item.Operation))
						{
							if (line.LineType == SOLineType.Inventory && line.ShippedQty == 0m && DateTime.Compare((DateTime)line.ShipDate, (DateTime)item.ShipDate) <= 0 && line.POSource != INReplenishmentSource.DropShipToOrder)
								throw new SOShipmentException(SOShipmentException.ErrorCode.CannotShipCompleteTraced, item, Messages.CannotShipCompleteTraced, order.OrderType, order.OrderNbr);
						}
					}
                ...
                foreach (PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact> ordres in OrderList.Select())
                {
							...
							.SelectMultiBound(docgraph, new [] { order }))
                    {
                        ...
                        if (shipline.ShipmentNbr == null &&
							(line.Completed == false || line.ShippedQty > 0m || (line.ShipDate > order.ShipDate && !cancelRemainder)))
                        {
                    ...
                    foreach (PXResult<SOLineSplit, SOShipLine> res in allocations)
                    {
                        ...
                        if (object.Equals(split.ShipmentNbr, order.ShipmentNbr))
                        {
                            ...
                            if (split.IsAllocated == true)
                            {
                                ...
                                decimal? QtyHardAvail = siteStatusQtyHardAvail + PrevAllocatedQty > 0 ? siteStatusQtyHardAvail + PrevAllocatedQty : 0;
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (SOOrderShipment order in OrderList.Select())
				{
					...
					foreach (PXResult<SOLine, SOShipLine> res in PXSelectJoin<SOLine,
						...
						OrderBy<Asc<SOLine.isFree>>>.SelectMultiBound(docgraph, new [] { order, docgraph.Document.Current }))
					{
						SOLine line = (SOLine)res;
						SOShipLine shipline = (SOShipLine)res;
											...
											Or<SOLineSplit.isAllocated, Equal<False>>>>>>>>>>.Select(docgraph, order.ShipmentNbr, line.OrderType, line.OrderNbr, line.LineNbr, order.SiteID, order.ShipDate))
								{
									...
									if ((schedule.IsAllocated == true || shline.ShipmentNbr != null) && lineShippingRule != SOShipComplete.ShipComplete && lineShippingRule != SOShipComplete.CancelRemainder && line.BaseShippedQty < line.BaseOrderQty * line.CompleteQtyMin / 100m)
									{
										...
										if (split.BaseQty > 0m)
										{
											...
											docgraph.LineSplittingAllocatedExt.InsertShipmentRemainder(split);
										}
									}
		...
		public virtual void UpdateOrigValues(SOShipLine shipline, SOLine soline, decimal? baseOrigQty)
		{
			...
			shipline.FullOrderQty = soline.OrderQty;
			shipline.BaseFullOrderQty = soline.BaseOrderQty;
		...
		public virtual void InvoiceShipment(SOInvoiceEntry docgraph, SOShipment shiporder, DateTime invoiceDate, InvoiceList list, PXQuickProcess.ActionFlow quickProcessFlow)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				PXTimeStampScope.SetRecordComesFirst(typeof(SOShipment), true);
				this.Save.Press();
		...
		{
			target.Caches[typeof(SiteStatus)] = source.Caches[typeof(SiteStatus)];
			target.Caches[typeof(LocationStatus)] = source.Caches[typeof(LocationStatus)];
			target.Caches[typeof(LotSerialStatus)] = source.Caches[typeof(LotSerialStatus)];
			...
			target.Views.Caches.Remove(typeof(SiteStatus));
			target.Views.Caches.Remove(typeof(LocationStatus));
			target.Views.Caches.Remove(typeof(LotSerialStatus));
		...
		public virtual void PostShipment(INRegisterEntryBase docgraph, PXResult<SOOrderShipment, SOOrder> sh, DocumentList<INRegister> list, ARInvoice invoice)
		{
			...
			foreach (PXResult<SOShipLine, SOShipLineSplit, SOLine, ARTran, INTran, INItemPlan> res in PXSelectJoin<SOShipLine,
				...
				LeftJoin<ARTran,
					...
					//the splits are merging to sinlge ARTRan when SOInvoiceEntry.IsShiplineMergeAllowed() == true
