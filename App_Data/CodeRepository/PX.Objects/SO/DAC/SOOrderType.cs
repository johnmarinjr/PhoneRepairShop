...
*/
	...
	public partial class SOOrderType : IBqlTable, PXNoteAttribute.IPXCopySettings
	{
		...
		[ARDocType.SOList]
		[PXUIEnabled(typeof(Where<behavior.IsNotEqual<SOBehavior.tR>>))]
		[PXFormula(typeof(ARDocType.noUpdate.When<behavior.IsEqual<SOBehavior.tR>>.Else<aRDocType>))]
		...
		[PXDBBool]
		[PXDefault(false)]
		[PXUIEnabled(typeof(Where<behavior.IsIn<SOBehavior.sO, SOBehavior.iN, SOBehavior.rM>>))]
		[PXFormula(typeof(False.When<behavior.IsNotIn<SOBehavior.sO, SOBehavior.iN, SOBehavior.rM>>.Else<creditHoldEntry>))]
		...
		[PXUIVisible(typeof(Where<behavior, In3<SOBehavior.sO, SOBehavior.tR, SOBehavior.iN, SOBehavior.cM>>))]
		...
		[Obsolete(Common.Messages.ItemIsObsoleteAndWillBeRemoved2022R1)]//also should be removed from Order Types(SO201000) screen
		...
		[PXDefault(typeof(IIf<
			Where<behavior, In3<SOBehavior.rM, SOBehavior.cM>,
				...
				And<activeOperationsCntr, Equal<int1>>>>>, True, False>))]
		...
		[PXDBBool]
		[PXFormula(typeof(IIf<Where<aRDocType, In3<ARDocType.invoice, ARDocType.debitMemo>, Or<behavior.IsEqual<SOBehavior.bL>>>, True, False>))]
	...
	public class SOBehavior
	{
		...
		{
			if (behavior.IsIn(SOBehavior.TR, SOBehavior.SO, SOBehavior.RM))
				return true;
			if (behavior.IsIn(SOBehavior.IN, SOBehavior.QT, SOBehavior.CM, SOBehavior.BL))
				return false;
			return value;
		}
		...
		{
			return behavior.IsIn(SOBehavior.IN, SOBehavior.QT, SOBehavior.CM, SOBehavior.SO, SOBehavior.TR, SOBehavior.RM, SOBehavior.BL);
		}
