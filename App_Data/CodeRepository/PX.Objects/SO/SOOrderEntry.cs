...
namespace PX.Objects.SO
{
	...
	public partial class SOOrderEntry : PXGraph<SOOrderEntry, SOOrder>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess, IGraphWithInitialization
	{
		...
		[PXCopyPasteHiddenFields(typeof(SOOrder.cancelled), typeof(SOOrder.preAuthTranNumber), typeof(SOOrder.ownerID), typeof(SOOrder.workgroupID), typeof(SOOrder.extRefNbr), typeof(SOOrder.updateNextNumber), typeof(SOOrder.emailed))]
		...
		[PXCopyPasteHiddenFields(typeof(SOLine.completed), typeof(SOLine.isLegacyDropShip))]
		...
		protected virtual IEnumerable transactions()
		{
			...
			if (sosetup.Current.MinGrossProfitValidation == MinGrossProfitValidationType.None || Document.Current?.IsTransferOrder == true)
				return null;
		...
		{
			CalculatePaymentBalance(payment, adj);
		...
		public virtual IEnumerable AddInvoice(PXAdapter adapter)
		{
			try
			{
				...
				if ((order?.IsCreditMemoOrder == true || order?.IsRMAOrder == true) && Transactions.Cache.AllowInsert && invoicesplits.AskExt() == WebDialogResult.OK)
				{
		...
		public virtual void CopyOrderProc(SOOrder sourceOrder, CopyParamFilter copyFilter)
		{
			...
			foreach (SOLine sourceSOLine in sourceSOLines)
			{
				...
				if (Document.Current.IsCreditMemoOrder != true && Document.Current.IsRMAOrder != true)
				{
		...
		public void recalculatePricesAndDiscountsFromImport()
		{
			if (Document.Current != null)
			{
				_discountEngine.AutoRecalculatePricesAndDiscounts(Transactions.Cache, Transactions, null, DiscountDetails, Document.Current.CustomerLocationID, Document.Current.OrderDate, GetDefaultSODiscountCalculationOptions(Document.Current) | DiscountEngine.DiscountCalculationOptions.CalculateDiscountsFromImport);
				Save.Press();
			}
		}
		...
		public override void InitCacheMapping(Dictionary<Type, Type> map)
		{
			...
			this.Caches.AddCacheMapping(typeof(INSiteStatus), typeof(INSiteStatus));
		...
		public virtual string GetDefaultTaxZone(Location customerLocation, bool useOrderAddress, string shipVia, int? branchID)
		{
			...
			if (IsCommonCarrier(shipVia))
			{
				...
				if (address != null && !string.IsNullOrEmpty(address.PostalCode))
					return TaxBuilderEngine.GetTaxZoneByZip(this, address.PostalCode);
			}
			else
			{
		...
		protected virtual void SOOrder_CustomerID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.multicurrency>() && !IsCopyPasteContext
		...
		protected virtual void SOOrder_PaymentMethodID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (doc?.IsPaymentInfoEnabled == true && doc.BillSeparately == false && !string.IsNullOrEmpty(doc.PaymentMethodID))
			{
		...
		protected virtual void SOOrder_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			PXDefaultAttribute.SetPersistingCheck<SOOrder.destinationSiteID>(sender, doc, doc.IsTransferOrder == true ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			...
			if (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update)
			{
				...
				if (((doc.IsInvoiceOrder == true && doc.BillSeparately == true && (doc.ARDocType == ARDocType.CashSale || doc.ARDocType == ARDocType.CashReturn)) || doc.ARDocType == ARDocType.Invoice)
				...
				{
					CustomerPaymentMethodC pmInstance = this.DefPaymentMethodInstance.Select(doc.PMInstanceID);
		...
		protected bool IsRMAOrder
		{
			get
			{
				return (soordertype.Current.Behavior == SOBehavior.RM);
			}
		}
		...
		public bool IsTransferOrder
		{
			get
			{
				return (soordertype.Current.Behavior == SOBehavior.TR) || (sooperation.Current.INDocType == INTranType.Transfer);
			}
		}
		...
		protected bool IsPaymentInfoEnabled
		{
			get
			{
				return IsCashSale || soordertype.Current.CanHavePayments == true || soordertype.Current.CanHaveRefunds == true;
			}
		}
		...
		protected virtual void SOOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (!object.ReferenceEquals(doc, _LastSelected))
			{
				...
				PXUIFieldAttribute.SetEnabled<SOLine.operation>(this.Transactions.Cache, null, soordertype.Current.ActiveOperationsCntr > 1);
				...
				PXUIFieldAttribute.SetVisible<SOLine.curyUnitCost>(this.Transactions.Cache, null, doc.IsRMAOrder == true || doc.IsCreditMemoOrder == true);
				PXUIFieldAttribute.SetEnabled<SOLine.curyUnitCost>(this.Transactions.Cache, null, doc.IsRMAOrder == true || doc.IsCreditMemoOrder == true);
				_LastSelected = doc;
			}
			...
			this.prepareInvoice.SetEnabled(doc.Hold == false && doc.Cancelled == false &&
				...
				(soordertype.Current.RequireShipping == false));
			...
			else
			{
				...
				{
					cpmRow = this.DefPaymentMethodInstance.Select(doc.PMInstanceID);
				...
				if (soordertype.Current != null)
				{
					...
					bool enableCreateCC = (doc.IsPaymentInfoEnabled == true && doc.CustomerID.HasValue && !hasActiveCCTran);
                    ...
                    PXUIFieldAttribute.SetEnabled<SOOrder.billSeparately>(cache, doc, !isPMCreditCard && doc.IsNoAROrder != true);
					...
					bool enableCCSelection = (doc.IsPaymentInfoEnabled == true && doc.CustomerID.HasValue & doc.CreatePMInstance != true && !hasActiveCCTran);
				...
				Adjustments.Cache.AllowInsert = IsAddingPaymentsAllowed(doc, soordertype.Current) && doc.HasLegacyCCTran != true;
				Adjustments.Cache.AllowDelete = applicationsEnabled;
				Adjustments.Cache.AllowUpdate = applicationsEnabled && doc.HasLegacyCCTran != true;
			...
			PXUIFieldAttribute.SetVisible<SOLine.invoiceType>(Transactions.Cache, null, doc.IsCreditMemoOrder == true || doc.IsRMAOrder == true);
			PXUIFieldAttribute.SetVisible<SOLine.invoiceNbr>(Transactions.Cache, null, doc.IsCreditMemoOrder == true || doc.IsRMAOrder == true);
			PXUIFieldAttribute.SetEnabled<SOLine.reasonCode>(Transactions.Cache, null, true);
			addInvoice.SetEnabled((doc.IsCreditMemoOrder == true || doc.IsRMAOrder == true) && Transactions.Cache.AllowInsert);
			...
			SalesPerTran.View.AllowSelect = doc.IsTransferOrder != true;
			...
			PXUIFieldAttribute.SetVisible<SOOrder.cCCardNumber>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.createPMInstance>(cache, doc, doc.IsTransferOrder != true);
			DefPaymentMethodInstance.View.AllowSelect = doc.IsTransferOrder != true;
			DefPaymentMethodInstanceDetails.View.AllowSelect = doc.IsTransferOrder != true;
			PXUIFieldAttribute.SetVisible<SOOrder.cCPaymentStateDescr>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.pCResponseReasonText>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.preAuthTranNumber>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.cCAuthExpirationDate>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.curyCCPreAuthAmount>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.curyCCCapturedAmt>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.captureTranNumber>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.refTranExtNbr>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.curyCCCapturedAmt>(cache, doc, doc.IsTransferOrder != true);
			PXUIFieldAttribute.SetVisible<SOOrder.captureTranNumber>(cache, doc, doc.IsTransferOrder != true);
		...
		protected virtual void SiteStatus_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			...
			if (Document.Current != null && Document.Current.Behavior != SOBehavior.QT && itemSettings != null && itemSettings.INItemSiteExists != true)
		...
		{
			var soline = (SOLine)e.Row;
			if (soline != null && string.IsNullOrEmpty(soline.UOM) == false && (soline.LineType == SOLineType.NonInventory || soline.LineType == SOLineType.MiscCharge) && Document.Current != null)
			{
				InventoryItem initem = InventoryItem.PK.Find(this, soline.InventoryID);
				var branch = Branch.PK.Find(this, soline.BranchID);
				var itemCurySettings = InventoryItemCurySettings.PK.Find(this, soline.InventoryID, branch?.BaseCuryID);

				if (initem != null)
				{
					if (itemCurySettings?.StdCostDate <= Document.Current.OrderDate)
						e.NewValue = itemCurySettings?.StdCost ?? 0m;
					else
						e.NewValue = itemCurySettings?.LastStdCost ?? 0m;
					e.Cancel = true;
				}
			}
		}
		...
		{
			var row = (SOLine)e.Row;
			if ((decimal?)e.NewValue < row.ClosedQty && row.RequireShipping == true
				&& row.LineType.IsIn(SOLineType.Inventory, SOLineType.NonInventory))
			{
				throw new PXSetPropertyException(CS.Messages.Entry_GE, row.ClosedQty.Value.ToString("0.####"));
			}
		}
			...
			foreach (SOOrder order in list.OrderBy(o => o.OrderType).ThenBy(o => o.OrderNbr))
			{
				try
				{
						...
						GetOrderShipments(docgraph, order))
					{
						...
						else if (order.OpenLineCntr == 0 &&
							(order.ShipmentCntr == 0 || (order.OrderQty == 0 && order.UnbilledOrderQty > 0)))
						{
							if (shipment.ShipmentNbr == null)
							{
								shipment = (SOOrderShipment)order;
					...
					foreach (PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact, SOOrderType, SOOrderTypeOperation> res in shipments)
					{
						...
						using (var ts = new PXTransactionScope())
						{
							...
							{
								Clear();
								Document.Cache.SetStatus(Document.Current = ie.soorder.Current, PXEntryStatus.Updated);
								Save.Press();
							}
							ts.Complete();
						}
		...
		protected virtual void PersistImpl()
		{
			...
			foreach (SOOrder doc in Document.Cache.Inserted
				.Concat_(Document.Cache.Updated)
				.Cast<SOOrder>().Where(doc => doc.Completed == false))
			{
				...
				foreach (PXResult<SOAdjust, ARRegisterAlias, ARPayment> res in Adjustments_Raw.View.SelectMultiBound(new object[] { doc }))
				{
					...
					if (adj?.Voided == false)
					{
						...
						if (doc.CuryDocBal - CuryApplAmt < 0m && CuryApplAmt > 0m)
						{
							...
							if (!ExternalTaxRecalculationScope.IsScoped() && (appliedToOrderUpdated || ((decimal?)Document.Cache.GetValueOriginal<SOOrder.curyOrderTotal>(doc) != doc.CuryOrderTotal)))
							{
			...
			using (LineSplittingExt.SuppressedModeScope(true))
			{
				...
				else
				{
					...
					else if (shipline.ShipmentNbr != null || lineShippingRule != SOShipComplete.ShipComplete)
					{
						...
						if (Document.Current.OpenLineCntr <= 0)
						{
							Document.Current.MarkCompleted();
						}
			...
			Dictionary<int?, (SOLine, decimal?, decimal?)> lineOpenQuantities)
		{
			...
			decimal? OpenQty = 0m, BaseOpenQty = 0m;
			if ((shipLine.ShippedQty ?? 0m) == 0m)
			{
				OpenQty = line.OpenQty = line.OrderQty - line.ShippedQty;
				BaseOpenQty = line.BaseOpenQty = line.BaseOrderQty - line.BaseShippedQty;
				line.ClosedQty = line.ShippedQty;
				line.BaseClosedQty = line.BaseShippedQty;
			}
			else
			{
				...
				{
					OpenQty = line.OpenQty = line.OrderQty - line.ShippedQty;
					BaseOpenQty = line.BaseClosedQty = line.BaseOpenQty = line.BaseOrderQty - line.BaseShippedQty;
					line.ClosedQty = line.ShippedQty;
					line.BaseClosedQty = line.BaseShippedQty;
				}
				...
				line.BaseClosedQty -= shipLine.BaseShippedQty;
				...
				if (line.POCreate == true && line.ShippedQty != line.Qty)
				{
					foreach (SOLineSplit split in PXParentAttribute.SelectChildren(splits.Cache, line, typeof(SOLine)))
					{
							...
							&& split.POReceiptNbr == null && split.Completed != true && split.IsAllocated != true)
						{
							...
							BaseOpenQty -= split.BaseUnreceivedQty;
							OpenQty -= unreceivedQty;
						}
					}
				}
				lineOpenQuantities.Add(line.LineNbr, (line, BaseOpenQty, OpenQty));
			}
