...
namespace PX.Objects.SO.WMS
{
	...
	public partial class PickPackShip : WMSBase
	{
		...
		public virtual string GetCommandOrShipmentOnlyPrompt() => CommandOrShipmentOnlyState.Msg.UseCommandOrShipmentToContinue;
		#endregion
		...
		public sealed class CommandOrShipmentOnlyState : CommandOnlyStateBase<PickPackShip>
		{
			...
			public override string Prompt => Basis.GetCommandOrShipmentOnlyPrompt();
		...
		public sealed class ConfirmShipmentCommand : ScanCommand
		{
			...
			public class Logic : ScanExtension
			{
				...
				protected static void ConfirmShipmentHandler(string shipmentNbr, bool confirmAsIs, SOPickPackShipSetup setup, SOPickPackShipUserSetup userSetup, SOPackageDetailEx autoPackageToConfirm)
					=> PXGraph.CreateInstance<SOShipmentEntry>().FindImplementation<PickPackShipShipmentConfirmation>().ApplyPickedQtyAndConfirmShipment(shipmentNbr, confirmAsIs, setup, userSetup, autoPackageToConfirm);
			...
			public class PickPackShipShipmentConfirmation : PXGraphExtension<SOShipmentEntry>
			{
				...
				public virtual void ApplyPickedQtyAndConfirmShipment(string shipmentNbr, bool confirmAsIs, SOPickPackShipSetup setup, SOPickPackShipUserSetup userSetup, SOPackageDetailEx autoPackageToConfirm)
				{
					...
					{
						PXView.Dummy view = PXView.Dummy.For<SOShipment>(Base);
						...
						if (userSetup.PrintShipmentLabels == true)
							WithSuppressedRedirects(() => Base.PrintCarrierLabels(new List<SOShipment>() { Base.Document.Current }, deviceHubAdapter));

						if (userSetup.PrintShipmentConfirmation == true)
							WithSuppressedRedirects(() => Base.PrintShipmentConfirmation(deviceHubAdapter));
					}
				}
				...
				protected virtual void CloseShipmentUserLinks(string shipmentNbr)
				{
						...
						View.Select(Base, shipmentNbr))
					{
						...
						Base.Caches<SOShipmentProcessedByUser>().Update(shipByUser);
					}
				}
