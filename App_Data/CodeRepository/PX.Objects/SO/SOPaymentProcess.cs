...
namespace PX.Objects.SO
{
	...
	public class SOPaymentProcess : PXGraph<SOPaymentProcess>
	{
		...
		protected virtual void ProcessPayments(List<SOPaymentProcessResult> payments, string action)
		{
			...
			ARPaymentEntryPaymentTransaction paymentTransactionExt = paymentEntry.GetExtension<ARPaymentEntryPaymentTransaction>();
			int paymentIndex = 0;
		...
		protected virtual void ProcessInvoice(string action, SOInvoiceEntry entry,
			...
			ARPaymentEntry paymentEntry, ARPaymentEntryPaymentTransaction paymentTransactionExt, SOPaymentProcessResult payment)
		{
			entry.Clear();
			...
			ARPaymentEntry paymentEntry, ARPaymentEntryPaymentTransaction paymentTransactionExt, SOPaymentProcessResult payment)
		{
			...
			var adjustments = paymentEntry.GetOrdersToApplyTabExtension(true).SOAdjustments.SelectMain().Where(a => a.AdjAmt > 0m);
			var adjustment = adjustments.FirstOrDefault();
			if (adjustments.Count() != 1 ||
		...
		protected virtual void RunAction<TGraph, TFirstGraph, TDocument, TDocumentAdjust, TPaymentAdjust>(string action,
			ARPaymentEntry paymentEntry, ARPaymentEntryPaymentTransaction paymentTransactionExt, TDocumentAdjust adjustment,
		...
		protected virtual string GetErrorDescription(ARPaymentEntry paymentEntry, ARPaymentEntryPaymentTransaction paymentTransactionExt, SOPaymentProcessResult payment)
		{
			try
			{
