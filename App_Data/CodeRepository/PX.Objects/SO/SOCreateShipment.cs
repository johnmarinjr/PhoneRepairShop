...
namespace PX.Objects.SO
{
	...
	public class SOCreateShipment : PXGraph<SOCreateShipment>
	{
		...
		public virtual IEnumerable orders()
		{
			...
			foreach (PXResult<SOOrder> res in cmd.View.Select(null, null, PXView.Searches, PXView.SortColumns, PXView.Descendings, newFilters, ref startRow, PXView.MaximumRows, ref totalRows))
			{
				SOOrder order = res;
				SOOrder cached;
		...
		protected virtual PXSelectBase<SOOrder> BuildCommandCreateShipment(SOOrderFilter filter)
		{
			...
			if (filter.DateSel == SOOrderFilter.dateSel.ShipDate)
			{
				...
				cmd.WhereAnd<
					Where<Sub<Sub<Sub<SOOrder.shipmentCntr,
						...
						Or2<Where<SOOrder.orderQty, Equal<decimal0>,
								And<Where<SOOrder.curyUnbilledMiscTot, Greater<decimal0>, Or<SOOrder.unbilledOrderQty, Greater<decimal0>>>>>,
						Or<Where<SOOrderType.requireShipping, Equal<boolFalse>>>>>>();
			}
			else
			{
				cmd.WhereAnd<
