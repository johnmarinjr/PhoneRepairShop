...
namespace PX.Objects.SO
{
		...
		And<ARTranAsSplit.lineType, NotEqual<SOLineType.freight>, And<ARTranAsSplit.lineType, NotEqual<SOLineType.discount>>>>>>
	{
		...
		protected virtual ReturnedQtyResult MemoAvailabilityCheckQty(PXCache sender, ARTran row)
		{
			...
			return row.InvtMult == 0 ? new ReturnedQtyResult(true) :
		...
		public virtual IStatus AvailabilityFetchSite(PXCache sender, ARTran row, AvailabilityFetchMode fetchMode)
        {
			ARTranAsSplit detail = Convert(row);
			PXParentAttribute.SetParent(DetailCache, detail, typeof(ARTran), row);

			var availability = AvailabilityFetchSite(sender, detail, fetchMode);
			
			if(fetchMode.HasFlag(AvailabilityFetchMode.ExcludeCurrent))
				availability = DeductAllocated<SiteStatus>(sender, row, availability);
			
			return availability;
		}

		public virtual IStatus DeductAllocated<TStatus>(PXCache sender, ARTran row, IStatus result)
			where TStatus : class, IQtyAllocatedBase, IBqlTable, new()
		{
			...
			INItemPlanIDAttribute.GetInclQtyAvail<TStatus>(sender, row, out signQtyAvail, out signQtyHardAvail);

			if (signQtyAvail != 0m)
			{
				lineQtyAvail -= signQtyAvail * (row.BaseQty ?? 0m);
			}

			if (signQtyHardAvail != 0m)
			{
				lineQtyHardAvail -= signQtyHardAvail * (row.BaseQty ?? 0m);
			}
