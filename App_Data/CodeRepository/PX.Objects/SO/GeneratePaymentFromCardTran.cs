	...
	public class GeneratePaymentFromCardTran : PXGraph<GeneratePaymentFromCardTran>
	{
		...
		public virtual IEnumerable externalTransactionList()
		{
			var query = new PXSelectJoin<ExternalTransactionExt,
				...
				Where<ExternalTransactionExt.active, Equal<True>,
					...
					And<Where<Not<ExternalTransactionExt.procStatus, NotEqual<ExtTransactionProcStatusCode.captureSuccess>,
						And<ExternalTransactionExt.procStatus, NotEqual<ExtTransactionProcStatusCode.captureFailed>,
						And<ExternalTransactionExt.procStatus, NotEqual<ExtTransactionProcStatusCode.captureDeclined>,
						...
						And<ExternalTransactionExt.procStatus, NotEqual<ExtTransactionProcStatusCode.authorizeSuccess>,
						And<ExternalTransactionExt.procStatus, NotEqual<ExtTransactionProcStatusCode.authorizeHeldForReview>>>>>>>>>>>>>, 
			...
			foreach (ExternalTransactionExt tran in res)
			{
				...
				tran.PreAuthorized = state.IsPreAuthorized || state.ProcessingStatus == ProcessingStatus.AuthorizeExpired;
		...
		public class SOInvoicePaymentCreator : PaymentCreator
		{
			...
			public override void CreateAuthorizedPayment()
			{
				using (PXTransactionScope scope = new PXTransactionScope())
				{
					...
					UpdateCurrentTransactionIfExpired();
			...
			private void AddSOAdjustLinkARAdjust(decimal? transferred)
			{
				...
				var ordersToApplyTabExtension = paymentGraph.GetOrdersToApplyTabExtension(true);
				...
				try
				{
					...
					if (transferred != 0)
					{
						adj.CuryAdjgAmt
							...
							= adj.OrigAdjAmt
							= payment.CuryDocBal;
						adj = ordersToApplyTabExtension.SOAdjustments.Update(adj);
			...
			private ARAdjust2 LinkPaymentWithDoc(decimal? pCuryDocBal)
			{
				...
				PXSelectorAttribute.StoreCached<ARAdjust2.adjgRefNbr>(invoiceGraph.Adjustments.Cache, adj,
					payment);
			...
			private void UpdateCurrentTransactionIfExpired()
			{
				var procStatus = ExternalTranHelper.GetTransactionState(paymentGraph, tran).ProcessingStatus;
				if (procStatus == ProcessingStatus.AuthorizeExpired)
				{
					PXDatabase.Update<ExternalTransaction>(
						new PXDataFieldAssign("ProcessingStatus", ExtTransactionProcStatusCode.AuthorizeExpired),
						new PXDataFieldAssign("Active", 0),
						new PXDataFieldRestrict("TransactionID", PXDbType.Int, 4, tran.TransactionID, PXComp.EQ)
					);
				}
			}
		}
		...
		public class SOOrderPaymentCreator : PaymentCreator
		{
			...
			private void LinkPaymentWithDoc()
			{
				...
				adj.AdjdOrderNbr = tran.OrigRefNbr;
				adj.CuryAdjdAmt = amt;
				paymentGraph.GetOrdersToApplyTabExtension(true).SOAdjustments.Insert(adj);
			}
