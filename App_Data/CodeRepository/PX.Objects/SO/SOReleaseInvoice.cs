	...
	public class SOReleaseInvoice : PXGraph<SOReleaseInvoice>
	{
		...
		public virtual void _(Events.RowUpdated<SOInvoiceFilter> e)
		{
			...
			if (!e.Cache.ObjectsEqual<SOInvoiceFilter.action, SOInvoiceFilter.definePrinterManually, SOInvoiceFilter.printWithDeviceHub>(e.Row, e.OldRow)
				&& PXAccess.FeatureInstalled<FeaturesSet.deviceHub>()
				&& Filter.Current.With(r => r.PrintWithDeviceHub == true && r.DefinePrinterManually == true
				&& (PXContext.GetSlot<AUSchedule>() == null || !(r.PrinterID != null && e.OldRow.PrinterID == null))))
			{
			...
			switch (filter.Action)
			{
				...
				case WellKnownActions.SOInvoiceScreen.CreateAndCapturePayment:
				{
					return new
						...
						InnerJoin<CustomerPaymentMethod>.On<CustomerPaymentMethod.pMInstanceID.IsEqual<ARInvoice.pMInstanceID>>.
						InnerJoin<CCProcessingCenter>.On<CCProcessingCenter.processingCenterID.IsEqual<CustomerPaymentMethod.cCProcessingCenterID>>.
						Where<
							...
							And<CCProcessingCenter.isExternalAuthorizationOnly.IsNotEqual<True>>>.
						View(this);
				}
				...
				case WellKnownActions.SOInvoiceScreen.PrintInvoice:
				{
					if (filter.ShowPrinted != true)
						return new
							SelectFrom<ARInvoice>.
							Where<
								ARInvoice.printInvoice.IsEqual<True>.
						...
						return new
							SelectFrom<ARInvoice>.
							Where<
								ARInvoice.dontPrint.IsNotEqual<True>.
								And<ARInvoice.released.IsEqual<False>>>.
							View(this);
				}
				default:
