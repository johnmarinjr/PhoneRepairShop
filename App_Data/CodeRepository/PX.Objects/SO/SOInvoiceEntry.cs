...
namespace PX.Objects.SO
{
	...
	public class BaseBilledQtyFormula :
		...
		Mult<ARTran.baseQty, shortMinus1>,
		ARTran.baseQty>
	{
	}
	public class CuryBilledAmtFormula :
		IIf<Where<ARTran.sOOrderLineOperation, Equal<SOOperation.receipt>, And<ARTran.drCr, Equal<DrCr.credit>,
			Or<ARTran.sOOrderLineOperation, Equal<SOOperation.issue>, And<ARTran.drCr, Equal<DrCr.debit>>>>>,
		Mult<ARTran.curyTranAmt, shortMinus1>,
		ARTran.curyTranAmt>
	{
	}
	...
	public partial class SOInvoiceEntry : ARInvoiceEntry
	{
		...
		[IN.SiteAvail(typeof(ARTran.inventoryID), typeof(ARTran.subItemID), DocumentBranchType = typeof(ARInvoice.branchID))]
		...
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXUnboundFormula(typeof(BaseBilledQtyFormula), typeof(AddCalc<SOLine2.baseBilledQty>))]
		[PXUnboundFormula(typeof(BaseBilledQtyFormula), typeof(SubCalc<SOLine2.baseUnbilledQty>))]
		[PXUnboundFormula(typeof(BaseBilledQtyFormula), typeof(AddCalc<SOMiscLine2.baseBilledQty>))]
		[PXUnboundFormula(typeof(BaseBilledQtyFormula), typeof(SubCalc<SOMiscLine2.baseUnbilledQty>))]
		protected virtual void ARTran_BaseQty_CacheAttached(PXCache sender)
		{
		}

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXUnboundFormula(typeof(CuryBilledAmtFormula), typeof(AddCalc<SOMiscLine2.curyBilledAmt>))]
		[PXUnboundFormula(typeof(CuryBilledAmtFormula), typeof(SubCalc<SOMiscLine2.curyUnbilledAmt>))]
		...
		protected virtual List<PXResult<ARPayment, CurrencyInfo, ExternalTransaction>> CollectPaymentsToApply(int applcount)
		{
			...
			{
				cmd.WhereAnd<Where<ARPayment.docType, In3<ARDocType.payment, ARDocType.prepayment, ARDocType.creditMemo>>>();
			}
					...
					foreach (SOLine soLine in discountCodesWithApplicableSOLines.Value.RightsFor(docGroupDisc))
					{
						foreach (ARTran tran in Transactions.Select())
						{
							bool linkedToSOLine = soLine.OrderType == tran.SOOrderType &&
												  ...
												  orderShipment.ShipmentNbr.Trim() == tran.SOShipmentNbr.Trim();
							...
							if (customerDiscountsFeatureEnabled && linkedToSOLine)
								break;
						}
					}
		...
		public virtual ARInvoice FindOrCreateInvoice(DateTime orderInvoiceDate, InvoiceOrderArgs args)
		{
			...
			SOOrder soOrder = args.SOOrder;
			SOOrderType soOrderType = SOOrderType.PK.Find(this, soOrder.OrderType);
			string invoiceDocType = GetInvoiceDocType(soOrderType, args.GroupByDefaultOperation ? soOrderType.DefaultOperation : orderShipment.Operation);
