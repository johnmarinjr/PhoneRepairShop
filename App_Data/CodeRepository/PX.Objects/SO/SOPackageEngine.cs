	...
	public class SOPackageEngine
	{
		...
		public virtual List<SOPackageInfoEx> PackByWeightMixedItems(List<INItemBoxEx> boxes, List<ItemInfo> items, Dictionary<int, List<INItemBoxEx>> boxesByInventoryLookup, Dictionary<string, List<int>> boxItemsLookup, bool restrictByVolume, int? siteID, decimal? declaredValue)
		{
			...
			if (restrictByVolume)
			{
				...
				PackIntoSmallerBoxes(boxes, boxItemsLookup, advancedboxes, true);
			}
			else
			{
				foreach (ItemInfo item in items)
				...
				PackIntoSmallerBoxes(boxes, boxItemsLookup, advancedboxes, false);
			}
		...
		/// <summary>
		/// Second pass. Trying to pack items into smaller boxes.
		/// </summary>
		private void PackIntoSmallerBoxes(List<INItemBoxEx> boxes, Dictionary<string, List<int>> boxItemsLookup, List<advancedINItemBoxEx> advancedboxes, bool packByWeightAndVolume)
		{
			foreach (advancedINItemBoxEx advancedbox in advancedboxes)
			{
				foreach (INItemBoxEx box in boxes)
				{
					...
					//checking if all the items can potentially be packed into this particular box
					bool isBoxApplicable = true;
					foreach (int itemlist in advancedbox.InvenoryList)
					{
						if (!boxItemsLookup[box.BoxID].Contains(itemlist))
						{
							isBoxApplicable = false;
							break;
						}
					}
					if (!isBoxApplicable)
					...
					bool itemsFitByWeight = advancedbox.CurrentWeight < box.MaxNetWeight;
					bool itemsFitByVolume = advancedbox.CurrentVolume <= box.MaxVolume;

					bool smallerBoxFound = false;
					if (packByWeightAndVolume)
					{
						if (itemsFitByWeight && itemsFitByVolume && (box.MaxWeight < advancedbox.MaxWeight || //new box is smaller by max weight
							(box.MaxWeight == advancedbox.MaxWeight && (box.MaxVolume < advancedbox.MaxVolume || //new box has the same max weight, but smaller by volume
							box.MaxVolume == advancedbox.MaxVolume && box.BoxWeight < advancedbox.BoxWeight)))) // new box has the same max weight and max volume, but lighter by itself
							smallerBoxFound = true;
					}
					else
					{
						if (itemsFitByWeight && (box.MaxWeight < advancedbox.MaxWeight || //new box is smaller by max weight
							(box.MaxWeight == advancedbox.MaxWeight && box.BoxWeight < advancedbox.BoxWeight))) // new box has the same max weight, but lighter by itself
							smallerBoxFound = true;
					}
