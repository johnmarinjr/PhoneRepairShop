...
namespace PX.Objects.RQ
{
	public class RQRequisitionEntry : PXGraph<RQRequisitionEntry, RQRequisition>, IGraphWithInitialization
	{
		...
		#region Declarations
		private bool? _overrideVendorCuryOrRate;
		#endregion
		...
		private void CreateSOOrderAndCloseAllQuotations(PO4SO po4so, RQRequisition requisition)
		{
				 ...
				 .Select(this, requisition.ReqNbr))
			{
				...
				if (sograph.Document.Current != null && sograph.Document.Current.OrderType == Setup.Current.QTOrderType)
				{
		...
		private void CreateSOOrderAndSOLines(SOOrderEntry sograph, RQRequisition requisition, RQSOSource rqSourceForSO, PO4SO po4so)
		{
				...
				|| sograph.Document.Current.CustomerLocationID != rqSourceForSO.CustomerLocationID)
			{
				...
				string soOrderType = Setup.Current.SOOrderType;

				SOOrderType OrderType = SOOrderType.PK.Find(this, soOrderType);

				if (!PXAccess.FeatureInstalled<FeaturesSet.inventory>() && OrderType?.Behavior != SOBehavior.IN)
					soOrderType = "IN";
				...
				order.CuryInfoID = CopyCurrenfyInfoOld(sograph, requisition.CuryInfoID);
		...
		protected virtual SOLine CreateSOLineFromPO4SOLine(SOOrderEntry soGraph, POLine poLine, decimal qty, RQRequisition requisition, RQSOSource rqSourceForSO)
		{
			SOLine quote =
				...
				.Select(this, Setup.Current.QTOrderType, poLine.RQReqNbr, poLine.RQReqLineNbr);
		...
		protected virtual void CreateQTOrderAndLines(SOOrderEntry sograph, RQRequisition requisition, RQRequisitionLine rqLine, InventoryItem inventoryItem, List<SOOrder> newSOOrders)
		{
			...
			if (sograph.Document.Current == null)
			{
				...
				SOOrderType quoteType = SOOrderType.PK.Find(this, Setup.Current.QTOrderType);

				if (quoteType == null || quoteType.Active != true)
				{
					throw new PXException(Messages.UnableToCreateQTOrderOrderTypeInactive, Setup.Current.QTOrderType);
				}

				order.OrderType = Setup.Current.QTOrderType;
		...
		public virtual IEnumerable chooseVendor(PXAdapter adapter)
		{
			foreach (RQRequisition item in adapter.Get<RQRequisition>())
			{
					...
					(item.Status == RQRequisitionStatus.Bidding || item.Status == RQRequisitionStatus.Hold))
				{
					...
					this.Document.SetValueExt<RQRequisition.vendorID>(upd, this.Vendors.Current.VendorID);

					if (upd.VendorID == this.Vendors.Current.VendorID)
					{
					...
					upd.RemitContactID = this.Vendors.Current.RemitContactID;
						...
						if (upd.BiddingComplete == true)
							RQRequisition.Events.Select(e => e.BiddingCompleted).FireOn(this, upd);
				...
				foreach (RQBiddingVendor vndr in source ?? Vendors.Select(order.ReqNbr))
				{
					...
					if (currentVendor == true || vndr.Status != true)
					{
						...
						try
						{
							...
							BidActivity.SendNotification(APNotificationSource.Vendor, notificationCD, order.BranchID, parameters, adapter.MassProcess);
						}
						catch (Exception e)
						{
							if (currentVendor)
		...
		protected virtual void RQRequisition_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			Lines.Cache.AllowInsert = row.EmployeeID != null;
		...
		protected virtual void RQRequisition_CustomerID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.multicurrency>())
			{
				...
				{
                if (sender.GetValuePending<RQRequisition.vendorID>(e.Row) != PXCache.NotSetValue)
						return;
		...
		protected virtual void RQRequisition_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (row.VendorID != newRow.VendorID && PXAccess.FeatureInstalled<FeaturesSet.multicurrency>())
			{
				...
				if (rVendor != null)
				{
					...
					RQBiddingVendor bidder =
						PXSelect<RQBiddingVendor,
							...
							.SelectWindowed(this, 0, 1, newRow.ReqNbr, newRow.VendorID, newRow.VendorLocationID);
					...
					if (vendorInfo.CuryID != info.CuryID && this._overrideVendorCuryOrRate == false)
					{
						info.CuryID = vendorInfo.CuryID;
						update = true;
					}

					if (vendorInfo.CuryRateTypeID != info.CuryRateTypeID && this._overrideVendorCuryOrRate == false)
					{
					...
					{ 
						try
						{
							...
							var sharedID = CurrencyCollection.GetBaseCurrency()?.CuryInfoID;

							if (sharedID == info.CuryInfoID)
							{
					...
					info = currencyinfo.Insert(info);
								...
								sender.SetValueExt<RQRequisition.curyInfoID>(newRow, info.CuryInfoID);
								sender.SetValueExt<RQRequisition.curyID>(newRow, info.CuryID);
							}
							else
							{
						...
						if (string.IsNullOrEmpty(message) == false)
						{
							sender.RaiseExceptionHandling<RQRequisition.orderDate>(e.Row, ((RQRequisition)e.Row).OrderDate, new PXSetPropertyException(message, PXErrorLevel.Warning));
						}
		...
		protected virtual void _(Events.FieldVerifying<RQRequisition.vendorID> e)
		{
			RQRequisition row = (RQRequisition)e.Row;
			Vendor vendor = Vendor.PK.Find(e.Cache.Graph, (int?)e.NewValue);
			...
			if (vendor != null && PXAccess.FeatureInstalled<FeaturesSet.multicurrency>())
			{
				...
				RQBiddingVendor biddingVendor = Vendors.Search<RQBiddingVendor.reqNbr, RQBiddingVendor.vendorID, RQBiddingVendor.vendorLocationID>(row.ReqNbr, e.NewValue, vendor.DefLocationID);
				CurrencyInfo biddingVendorCurrency = biddingVendor != null ? currencyinfo.Select(biddingVendor.CuryInfoID) : null;

				string vendorCuryID;
				string vendorCuryRateTypeID;

				if (biddingVendorCurrency != null)
				{
					vendorCuryID = biddingVendorCurrency.CuryID;
					vendorCuryRateTypeID = biddingVendorCurrency.CuryRateTypeID;
				}
				else
				{
					vendorCuryID = vendor.CuryID;
					vendorCuryRateTypeID = vendor.CuryRateTypeID;
				}

				if (vendorCuryID == null)
					vendorCuryID = company.Current.BaseCuryID;

				if (vendorCuryRateTypeID == null)
					vendorCuryRateTypeID = cmsetup.Current.APRateTypeDflt;

				string message = null;

				if (vendorCuryID != current.CuryID)
				{
					message = PXMessages.LocalizeFormatNoPrefixNLA(Messages.RequisitionVendorCuryIDValidation, vendorCuryID);
				}
				else if (vendorCuryID != company.Current.BaseCuryID && vendorCuryRateTypeID != null && vendorCuryRateTypeID != current.CuryRateTypeID)
				{
					message = PXMessages.LocalizeFormatNoPrefixNLA(Messages.RequisitionVendorCuryRateIDValidation, vendorCuryRateTypeID);
				}

				this._overrideVendorCuryOrRate = false;

				if (message != null)
				{
					if(PXLongOperation.IsLongOperationContext())
					{
						if(row.SkipValidateWithVendorCuryOrRate == true)
						{
							this._overrideVendorCuryOrRate = true;
						}
						else
						{
							this._overrideVendorCuryOrRate = vendor.AllowOverrideCury == true || vendor.AllowOverrideRate == true;
						}
					}
					else if(this.Document.Ask(row, Messages.Warning, message, MessageButtons.YesNo) != WebDialogResult.Yes)
					{
						if (vendor.AllowOverrideCury == true || vendor.AllowOverrideRate == true)
						{
							this._overrideVendorCuryOrRate = true;
						}
						else
						{
							e.NewValue = row.VendorID;
							e.Cancel = true;
						}
					}
				}
		...
		{
			var helper = graph.FindImplementation<ICurrencyHelperEx>();
			CM.Extensions.CurrencyInfo curryInfo = helper.GetCurrencyInfo(SourceCuryInfoID);
			return helper.CloneCurrencyInfo(curryInfo).CuryInfoID;
		}

		private long? CopyCurrenfyInfoOld(PXGraph graph, long? SourceCuryInfoID)
		{
	...
	public partial class RQRequestLineSelect : IBqlTable
	{
		...
		[PXDBString(Common.Constants.TranDescLength, IsUnicode = true, BqlField = typeof(RQRequestLine.description))]
		...
		[PXDefault(
			...
			Search2<Vendor.bAccountID,
				LeftJoin<InventoryItemCurySettings,
							On<InventoryItemCurySettings.preferredVendorID, Equal<Vendor.bAccountID>,
						And<InventoryItemCurySettings.inventoryID, Equal<Current<RQRequestLine.inventoryID>>,
			...
			Or<
					Where<Current<RQRequest.vendorID>, IsNull, And<InventoryItemCurySettings.preferredVendorID, IsNotNull>>>>>),
