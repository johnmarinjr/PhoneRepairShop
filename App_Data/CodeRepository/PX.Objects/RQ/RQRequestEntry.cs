...
namespace PX.Objects.RQ
{
	public class RQRequestEntry : PXGraph<RQRequestEntry, RQRequest>, IGraphWithInitialization
	{
		...
		*/
		...
		[PXUIField(DisplayName = "Print Request", MapEnableRights = PXCacheRights.Select)]
		...
		protected virtual void RQRequest_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			Lines.Cache.AllowInsert =
				row.EmployeeID != null && row.LocationID != null
				&& (row.CustomerRequest == true || row.DepartmentID != null);
		...
		protected virtual void RQRequestLine_OrderQty_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			var row = (RQRequestLine)e.Row;
			if (row != null)
			{
				sender.SetValueExt<RQRequestLine.openQty>(row, (row.OpenQty ?? 0m) + ((row.OrderQty ?? 0m) - ((decimal?)e.OldValue ?? 0m)));
			}
		}
		...
		protected virtual void RQRequestLine_UOM_FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			...
			if (row != null && row.ReqQty != 0)
			{
				...
				{
					//Consider reworking all this logic in upcoming RQ module refactoring
					...
					row.ReqQty = INUnitAttribute.ConvertFromBase(sender,
																   row.InventoryID, UOM, row.BaseReqQty.GetValueOrDefault(),
																   INPrecision.QUANTITY);

					//Not used anywhere
					row.OriginQty = INUnitAttribute.ConvertFromBase(sender,
																   row.InventoryID, UOM, row.BaseOriginQty.GetValueOrDefault(),
																   INPrecision.QUANTITY);

					row.OpenQty = INUnitAttribute.ConvertFromBase(sender,
																   row.InventoryID, UOM, row.BaseOpenQty.GetValueOrDefault(),
																   INPrecision.QUANTITY);

					//Not used anywhere
					row.IssuedQty = INUnitAttribute.ConvertFromBase(sender,
																   row.InventoryID, UOM, row.BaseIssuedQty.GetValueOrDefault(),
																   INPrecision.QUANTITY);
		...
		protected virtual void RQRequestLine_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				PXUIFieldAttribute.SetEnabled<RQRequestLine.orderQty>(sender, row, row.ReqQty == 0);
		...
		public class AffectedInRequisition : ProcessAffectedEntitiesInPrimaryGraphBase<AffectedInRequisition, RQRequisitionEntry, RQRequest, RQRequestEntry>
		{
			...
			protected override RQRequest ActualizeEntity(RQRequestEntry primaryGraph, RQRequest entity) => primaryGraph.Document.Search<RQRequest.orderNbr>(entity.OrderNbr);
		}
		#endregion
	}
	...
	[PXProjection(typeof(
		...
		LeftJoin<InventoryItemCurySettings,
						On<InventoryItemCurySettings.inventoryID, Equal<InventoryItem.inventoryID>,
							And<InventoryItemCurySettings.curyID, EqualBaseCuryID<Current2<RQRequest.branchID>>>>,
		LeftJoin<Vendor,
