    ...
    public class EmployeeCostEngine
	{
		...
		public class Rate
		{
			...
			public Rate(int? employeeID, string type, decimal? hourlyRate, string uom, decimal? regularHours, decimal? rateByType)
			{
				...
				Type = string.IsNullOrEmpty(type) ? RateTypesAttribute.Hourly : type;
				RegularHours = regularHours ?? 0;
				RateByType = rateByType;
			}

			public Rate(int? employeeID, string type, decimal? hourlyRate, string uom, decimal? regularHours, decimal? rateByType, string curyID)
			{
				EmployeeID = employeeID;
				UOM = uom;
				HourlyRate = hourlyRate ?? 0m;
		...
		public virtual Rate GetEmployeeRate(int? laborItemID, int? projectID, int? projectTaskID, bool? certifiedJob, string unionID, int? employeeId, DateTime? date)
		{
			...
			return new Rate(employeeId, maxRate.EmploymentType, maxRate.Rate, defaultUOM, regularHours, 
				maxRate.EmploymentType == RateTypesAttribute.Hourly ? maxRate.Rate : maxRate.Rate * regularHours, maxRate.CuryID);
		}
		
       	public virtual LaborCost CalculateEmployeeCost(string timeCardCD, string earningTypeID, int? laborItemID, int? projectID, int? projectTaskID, bool? certifiedJob, string unionID, int? employeeID, DateTime date, int? shiftID = null)
		{
			decimal? cost;
			...
			if (shiftID != null && cost != null)
			{
				cost = EPShiftCodeSetup.CalculateShiftCosting(graph, shiftID, date, cost.Value, overtimeMult);
			}

			return new LaborCost(cost, overtimeMult, employeeRate.CuryID);
		}
