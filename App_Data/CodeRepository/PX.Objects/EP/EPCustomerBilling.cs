...
namespace PX.Objects.EP
{
	...
	public class EPCustomerBillingProcess : PXGraph<EPCustomerBillingProcess>
	{
			...
			foreach (PXResult<EPExpenseClaimDetails, Contract, Account> res in resultset)
			{
				...
				else
				{
					if (arDoc == null || arDoc.BranchID != row.BranchID)
					{
						...
						arDoc = (ARInvoice)arGraph.Document.Cache.Insert(new ARInvoice
						{
							OrigModule = BatchModule.EP,
							DocType = signOperation < 0 ? ARDocType.CreditMemo : ARDocType.Invoice,
							CustomerID = row.CustomerID,
							CustomerLocationID = row.CustomerLocationID,
							DocDate = filter.InvoiceDate,
							OrigRefNbr = row.RefNbr
						});
		...
		private static void SetAmount(ARInvoiceEntry arGraph, decimal? curyTranAmt, decimal? tranAmt, decimal? qty, decimal signOperation, CurrencyInfo curyInfo, ARTran tran)
		{
			...
			{
				curyamount = arGraph
					.FindImplementation<Extensions.MultiCurrency.IPXCurrencyHelper>()
					.GetDefaultCurrencyInfo()
					.CuryConvCury(tranAmt.GetValueOrDefault());
			}
