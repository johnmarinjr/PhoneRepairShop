...
namespace PX.Objects.EP
{
	public class ActivityService : IActivityService
	{
		...
		private void CreateActivity(int classId, Guid? refNoteID, string typeCode, int? owner, PXRedirectHelper.WindowMode windowMode = PXRedirectHelper.WindowMode.NewWindow)
		{
			...
			if (owner == null)
				owner = EmployeeMaint.GetCurrentOwnerID(cache.Graph);
			...
			if (type != null)
			{
				UDFHelper.CopyAttributes(cache.Graph.Caches[type], entity, cache, activity, activity?.Type);
			}
