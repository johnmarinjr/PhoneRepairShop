...
namespace PX.Objects.EP
{
	public class EmployeeMaint : PXGraph<EmployeeMaint>
	{
		...
		public PXSelectJoin<VendorPaymentMethodDetail, 
                    ...
                    Where<VendorPaymentMethodDetail.bAccountID, Equal<Current<Location.bAccountID>>,
			            And<VendorPaymentMethodDetail.locationID, Equal<Current<Location.locationID>>,
    			            And<VendorPaymentMethodDetail.paymentMethodID, Equal<Current<Location.vPaymentMethodID>>>>>,
		...
		public EmployeeMaint()
		{
			...
			Action.AddMenuAction(generateTimeCards);
                    ...
                    PXLongOperation.StartOperation(this, () =>
                    {
                        ...
                        do
                        {
                            if (newCard != null)
                            {
								...
								newCard = tcgraph.Document.Insert(newCard);
								tcgraph.Document.Cache.SetValueExt<EPTimeCard.status>(newCard, EPTimeCardStatusAttribute.ReleasedStatus);
		...
		protected virtual void _(Events.FieldVerifying<EPEmployee, EPEmployee.vStatus> e)
		{
			EPEmployee row = (EPEmployee)e.Row;

			if (Accessinfo.UserID == row.UserID)
			{
				Employee.Cache.RaiseExceptionHandling<EPEmployee.vStatus>(e.Row, row.VStatus, 
					new PXSetPropertyException(Messages.CantDeactivateYourself));
		...
		protected virtual void _(Events.FieldUpdated<EPEmployee, EPEmployee.vStatus> e)
		{
			...
			var contact = Contact.SelectSingle();

			if (contact != null)
			{
				contact.IsActive = row.VStatus != VendorStatus.Hold && row.VStatus != VendorStatus.Inactive;
				Contact.Update(contact);
			}
		}
		...
		protected virtual void _(Events.FieldUpdated<Location, Location.vPaymentMethodID> e)
		{
			Location row = e.Row;
			...
			e.Cache.SetDefaultExt<Location.vCashAccountID>(e.Row);
		...
		protected virtual void _(Events.RowInserted<Location> e)
		{
		...
		protected virtual void _(Events.RowSelected<VendorPaymentMethodDetail> e)
		{
			if (e.Row != null)
			{
				...
				PaymentMethodDetail paymentMethodDetail = this.FindTemplate(row);

				bool isRequired = (paymentMethodDetail != null) && (paymentMethodDetail.IsRequired ?? false);
		...
		protected virtual void _(Events.RowPersisting<VendorPaymentMethodDetail> e)
		{
			if (e.Cancel)
			{
				return;
			...
			bool validationResult = IsLocationPaymentMethodValid(e.Cache, null);
		...
		public virtual PaymentMethodDetail FindTemplate(VendorPaymentMethodDetail vendorPaymentMethodDetail) =>
			PXSelect<
				PaymentMethodDetail,
				Where<
					...
					And<Where<PaymentMethodDetail.useFor, Equal<PaymentMethodDetailUsage.useForVendor>,
						Or<PaymentMethodDetail.useFor, Equal<PaymentMethodDetailUsage.useForAll>>>>>>>
				.Select(this, vendorPaymentMethodDetail.PaymentMethodID, vendorPaymentMethodDetail.DetailID);

		public virtual bool IsLocationPaymentMethodValid(PXCache cache, Location location)
		{
			bool isLocationPaymentMethodValid = true;
			
			foreach (VendorPaymentMethodDetail vendorPaymentMethodDetail in this.PaymentDetails.Select())
			{
				if (!IsPaymentDetailValid(vendorPaymentMethodDetail))
				{
					isLocationPaymentMethodValid = false;
				}
			}

			return isLocationPaymentMethodValid;
		}

		public virtual bool IsPaymentDetailValid(VendorPaymentMethodDetail vendorPaymentMethodDetail)
		{
			PaymentMethodDetail paymentMethodDetail = this.FindTemplate(vendorPaymentMethodDetail);

			if (
				paymentMethodDetail != null
				&& (paymentMethodDetail.IsRequired ?? false)
				&& String.IsNullOrEmpty(vendorPaymentMethodDetail.DetailValue)
			)
			{
				this.PaymentDetails.Cache.RaiseExceptionHandling<VendorPaymentMethodDetail.detailValue>(
					vendorPaymentMethodDetail,
					vendorPaymentMethodDetail.DetailValue,
					new PXSetPropertyException(CA.Messages.ERR_RequiredValueNotEnterd)
				);
		...
		{
			foreach (VendorPaymentMethodDetail vendorPaymentMethodDetail in PaymentDetails.Select())
			{
				bool doDelete = true;

				if (clearNewOnly)
				{
					PXEntryStatus status = PaymentDetails.Cache.GetStatus(vendorPaymentMethodDetail);
					doDelete = (status == PXEntryStatus.Inserted);
				}

				if (doDelete)
				{
					PaymentDetails.Delete(vendorPaymentMethodDetail);
				}
			}
		}
