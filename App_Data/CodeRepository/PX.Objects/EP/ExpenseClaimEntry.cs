...
namespace PX.Objects.EP
{
	public class ExpenseClaimEntry : PXGraph<ExpenseClaimEntry, EPExpenseClaim>, PXImportAttribute.IPXPrepareItems
	{
		...
		{
			EPExpenseClaim.Events.Select(e => e.Submit).FireOn(this, claim);
		}
		...
		private bool CurrencyInfoRowUpdatedScope = false;
		protected virtual void CurrencyInfo_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			foreach (EPExpenseClaimDetails detail in ExpenseClaimDetails.Select())
			{
					...
					if (detail.CreatedFromClaim == true &&
						...
						!CurrencyInfoRowUpdatedScope)
					{
						try
						{
							CurrencyInfoRowUpdatedScope = true;
						...
						{
							CurrencyInfoRowUpdatedScope = false;
						}
					}
					else
				{
        ...
        protected virtual void EPExpenseClaim_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
			...
			if (info != null && info.CuryPrecision == null)
			{
				this.Caches<CurrencyInfo>().RaiseFieldDefaulting<CurrencyInfo.curyPrecision>(info, out object newValue);
				info.CuryPrecision = newValue as short?;
			}
			...
			expenseClaimPrint.SetEnabled(cache.GetStatus(doc) != PXEntryStatus.Inserted);
		...
		{
			PXUIFieldAttribute.SetError<EPExpenseClaimDetails.uOM>(e.Cache, e.Row, null);
			var value = PXFormulaAttribute.Evaluate<EPExpenseClaimDetails.uOM>(e.Cache, e.Row);
			e.Cache.SetValueExt<EPExpenseClaimDetails.uOM>(e.Row, value);
		}

		protected virtual void _(Events.FieldUpdated<EPExpenseClaimDetails, EPExpenseClaimDetails.uOM> e)
		{
			if (e.Row == null)
				return;

			string oldUom = (string)e.OldValue;
			decimal curyUnitCost = e.Row.CuryUnitCost ?? 0m;
			if (!string.IsNullOrEmpty(oldUom) && !string.IsNullOrEmpty(e.Row.UOM) && oldUom != e.Row.UOM)
			{
				if (curyUnitCost != 0m)
				{
					curyUnitCost = INUnitAttribute.ConvertFromBase<EPExpenseClaimDetails.inventoryID>(e.Cache, e.Row, oldUom, curyUnitCost, INPrecision.NOROUND);
					curyUnitCost = INUnitAttribute.ConvertToBase<EPExpenseClaimDetails.inventoryID>(e.Cache, e.Row, e.Row.UOM, curyUnitCost, INPrecision.UNITCOST);
				}
				e.Cache.SetValueExt<EPExpenseClaimDetails.curyUnitCost>(e.Row, curyUnitCost);
			}
		}
		#endregion
		...
		private void TryValidateRequireUniqueVendorRefWhenCardInvolved()
		{
			...
			foreach (EPExpenseClaimDetails item in ExpenseClaimDetails.Select())
			{
				...
				{
					CACorpCard card = CACorpCard.PK.Find(this, item.CorpCardID);
					if (card == null)
				{
						return;
					}
		...
		public virtual string GetDefaultTaxZone(EPExpenseClaimDetails row)
		{
			...
			result = ExpenseClaimCurrent.Current.TaxZoneID;
