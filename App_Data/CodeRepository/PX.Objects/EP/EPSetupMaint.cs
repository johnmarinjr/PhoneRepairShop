...
namespace PX.Objects.EP
{
	public class EPSetupMaint : PXGraph<EPSetupMaint>
	{
		...
		{
			string postingOption = GetPostingOption(graph, setup, employeeID);

			return postingOption != EPPostOptions.DoNotPost && postingOption != EPPostOptions.PostPMAndGLFromPayroll;
		}
		...
		public static bool GetPostToOffBalance(PXGraph graph, EPSetup setup, int? employeeID)
		{
			...
			return postingOption == EPPostOptions.PostToOffBalance || postingOption == EPPostOptions.OverridePMInPayroll;
		}
	...
	protected virtual void EPSetup_OvertimeMultiplier_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		...
		protected virtual void EPCustomWeek_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
		{
			...
			if ((EPTimeCard)PXSelect<EPTimeCard>.Search<EPTimeCard.weekId>(this, row.WeekID) != null
				|| (PMTimeActivity)PXSelect<PMTimeActivity>.Search<PMTimeActivity.weekID>(this, row.WeekID) != null)
			{
