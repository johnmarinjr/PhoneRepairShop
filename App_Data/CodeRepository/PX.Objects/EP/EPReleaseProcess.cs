...
namespace PX.Objects.EP
{
	public class EPReleaseProcess : PXGraph<EPReleaseProcess>
	{
		...
		public virtual void ReleaseDocProc(EPExpenseClaim claim)
		{
			...
			{
				throw new PXException(Messages.UnapprovedWhenRelease);
			}
			...
			using (var ts = new PXTransactionScope())
			{
				if (receipts.Any())
		        {
	                foreach (var receiptGroup in receipts.GroupBy(receipt => receipt.PaidWith))
					{
						apDocs.AddRange(ReleaseClaimDetailsReceipts(claim, expenseClaimGraph, receiptGroup));
					}
				}
		        else
		        {
		...
		private List<APRegister> ReleaseClaimDetailsReceipts(EPExpenseClaim claim, ExpenseClaimEntry expenseClaimGraph, IGrouping<string, EPExpenseClaimDetails> receiptGroup)
		{
			switch (receiptGroup.Key)
			{
				case EPExpenseClaimDetails.paidWith.PersonalAccount:
					return ReleaseClaimDetails<Invoice, InvoiceMapping, APInvoiceEntry, APInvoiceEntry.APInvoiceEntryDocumentExtension>(expenseClaimGraph, claim, receiptGroup, receiptGroup.Key);
				case EPExpenseClaimDetails.paidWith.CardCompanyExpense:
					return ReleaseClaimDetails<PaidInvoice, PaidInvoiceMapping, APQuickCheckEntry, APQuickCheckEntry.APQuickCheckEntryDocumentExtension>(expenseClaimGraph, claim, receiptGroup, receiptGroup.Key);
				case EPExpenseClaimDetails.paidWith.CardPersonalExpense:
					return ReleaseClaimDetails<Invoice, InvoiceMapping, APInvoiceEntry, APInvoiceEntry.APInvoiceEntryDocumentExtension>(expenseClaimGraph, claim, receiptGroup, receiptGroup.Key);
			...
			where TAPDocumentGraphExtension : InvoiceBaseGraphExtension<TGraph, TAPDocument, TInvoiceMapping>
		{
			...
			using (var r = PXDatabase.SelectSingle<Account>(
					new PXDataField<Account.accountCD>(),
					new PXDataField("DeletedDatabaseRecord"),
					new PXDataFieldValue<Account.accountID>(PXDbType.Int, emplocation.VAPAccountID)))
			{
				if (r == null || (r != null && r.GetBoolean(1).GetValueOrDefault()))
				{
					throw new PXException(Messages.ExpenseClaimCannotBeReleasedAccountHasBeenDeleted, claim.RefNbr, (r != null) ? r.GetString(0) : string.Empty, employee.AcctCD);
				}
			}
			...
			foreach (var receiptGroup in receiptsForDocument)
			{
				...
				info = apDocumentGraphExtension.CurrencyInfo.Insert(CM.Extensions.CurrencyInfo.GetEX(info)).GetCM();
			...
			where TGraph : PXGraph, new()
		{
			...
			else if (typeof(TGraph) == typeof(APQuickCheckEntry))
			{
				...
				apQuickCheckEntryGraph.Document.Cache.RaiseFieldUpdated<AP.Standalone.APQuickCheck.branchID>((AP.Standalone.APQuickCheck)invoice.Base, invoice.BranchID);
			}
		}
