...
namespace PX.Objects.EP
{
	public class ExpenseClaimDetailEntry : PXGraph<ExpenseClaimDetailEntry, EPExpenseClaimDetails>
	{
        ...
        [PXUIField(DisplayName = Messages.Submit, MapEnableRights = PXCacheRights.Update)]
		...
		protected virtual void EPExpenseClaimDetails_EmployeeID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			cache.SetDefaultExt<EPExpenseClaimDetails.corpCardID>(row);
		}
		...
		[PXRestrictor(typeof(Where<CREmployee.userID, IsNotNull>), PX.Objects.EP.Messages.EmployeeIsNotLinkedToUser)]
		...
		public virtual string GetDefaultTaxZone(EPExpenseClaimDetails row)
		{
			...
			if (row?.RefNbr != null && epsetup?.Current?.AllowMixedTaxSettingInClaims == false)
			{
				result = CurrentClaim.SelectSingle(row.RefNbr).TaxZoneID;
			}
