...
namespace PX.Objects.EP
{
	public class EmployeeActivitiesEntry : PXGraph<EmployeeActivitiesEntry>
	{
		...
		protected virtual void _(Events.FieldUpdated<PMTimeActivityFilter, PMTimeActivityFilter.ownerID> e)
		{
			PMTimeActivityFilter row = e.Row;

			if (row != null && row.OwnerID != null)
			{
				BAccount account = PXSelect<BAccount>.Search<BAccount.defContactID>(e.Cache.Graph, e.NewValue);
				PXUIFieldAttribute.SetWarning<PMTimeActivityFilter.ownerID>(e.Cache, row, account.VStatus == VendorStatus.Inactive ? Messages.InactiveMemberTimeEntry : null);
			}
		}
		...
		public class PMTimeActivityFilter : OwnedFilter
		{
			...
			[PXRestrictor(typeof(Where<EPEmployee.userID, IsNotNull>), PX.Objects.EP.Messages.EmployeeIsNotLinkedToUser)]
