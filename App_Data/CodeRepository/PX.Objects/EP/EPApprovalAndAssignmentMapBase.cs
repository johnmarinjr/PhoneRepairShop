...
namespace PX.Objects.EP
{
	public class EPApprovalAndAssignmentMapBase<TGraph> : PXGraph<TGraph> where TGraph : PXGraph
	{
		...
		[PXMergeAttributes(Method = MergeMethod.Merge)]
		protected virtual void _(Events.CacheAttached<POOrder.ownerWorkgroupID> e) { }
		...
		protected PXFieldState CreateFieldStateForFieldName(object returnState, Type entityType, Type gType, string viewName, string cacheName, int? fieldLength, EPRuleBaseCondition condition)
		{
            ...
            if (entityType != null)
			{
				...
				var workflowStatePropertyName = !String.IsNullOrEmpty(screenID) ?
																				WorkflowService.GetWorkflowStatePropertyName(screenID) :
																				String.Empty;

				foreach (var item in fields.OrderBy(i => i.Value))
				{
					if ((!item.Key.EndsWith("_Attributes") && item.Key.Contains('_')) || item.Key.Equals(workflowStatePropertyName, StringComparison.OrdinalIgnoreCase)) continue;
