...
namespace PX.Objects.CA
{
	...
	public class CAReleaseProcess : PXGraph<CAReleaseProcess>
	{
			...
			where TCADocument : class, ICADocument, IBqlTable, new()
		{
			...
			CAAdj cashAdjustment = doc as CAAdj;

			if (cashAdjustment != null)
			{
				OnBeforeRelease(cashAdjustment);
			}
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (PXResult<CATran, CashAccount, Currency, CurrencyInfo> res in resultset)
				{
					...
					if (object.Equals(prev_tran, catran) == false)
					{
						...
						foreach (PXResult<CATaxTran, Tax> r in CATaxTran_TranType_RefNbr.Select(caadj.AdjTranType, caadj.AdjRefNbr))
						{
							...
							{
								if (IsPostUseAndSalesTaxesByProjectKey(this, salestax) && salestax.IsExternal == false)
								{
					...
					cashAdjustment = doc as CAAdj;
					if (cashAdjustment != null)
					{
						cashAdjustment = CommitExternalTax(cashAdjustment);
					}
					#endregion
					...
					if (batch != null && batch.CuryCreditTotal == batch.CuryDebitTotal)
					{
						...
						if (catran.TranID.HasValue && catran.CuryTranAmt != 0 && catran.TranAmt != 0)
						{
							...
							if (cashacct.AccountID.HasValue)
							{
								accountIDs.Add(cashacct.AccountID.Value);
							}
						}
				...
				CheckBoundGLAccountIsCashAccount(accountIDs);
			...
			decimal docInclTaxDiscrepancy)
		{
			if (docInclTaxDiscrepancy != 0)
			{
				...
				diffTran.TranClass = GLTran.tranClass.RealizedAndRoundingGOL;
		...
		{
			foreach(Account account in PXSelectReadonly<Account, Where<Account.accountID, In<Required<Account.accountID>>,
														And<Account.isCashAccount, Equal<False>>>>.Select(this, accountIDs.ToArray()))
			{
				var errorMsg = string.Format(Messages.GLAccountIsNotCashAccount, account.AccountCD);
