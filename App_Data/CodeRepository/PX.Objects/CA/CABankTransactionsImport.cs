...
{
	public class CABankTransactionsImport : PXGraph<CABankTransactionsImport, CABankTranHeader>, PXImportAttribute.IPXPrepareItems, ICaptionable
	{
		[Serializable]
		...
		public PXSelect<CABankTran, Where<CABankTran.headerRefNbr, Equal<Current<CABankTranHeader.refNbr>>,
			...
			And<Where<CABankTran.processed, Equal<True>, Or<CABankTran.documentMatched, Equal<True>>>>>>> MatchedOrProcessedDetails;
		public PXSelect<CABankTran,
		Where<CABankTran.headerRefNbr, Equal<Current<CABankTranHeader.refNbr>>,
			And<CABankTran.tranType, Equal<Current<CABankTranHeader.tranType>>,
			And<CABankTran.documentMatched, Equal<True>>>>> MatchedDetails;
		...
		public virtual IEnumerable ProcessTransactions(PXAdapter adapter)
		{
			...
			if (headerRow?.CashAccountID != null)
			{
				...
				graph.TaxTrans.Cache.Clear();
				graph.TranFilter.Cache.SetValueExt<CABankTransactionsMaint.Filter.cashAccountID>(graph.TranFilter.Current, headerRow.CashAccountID);
		...
		{
			this.Save.Press();
			bool isAnyMatchedTran = MatchedDetails.Any();

			if (isAnyMatchedTran)
			{
				CABankTran processedTran = MatchedDetails.Select().RowCast<CABankTran>().Where(x => x.Processed == true).FirstOrDefault();

				if(processedTran != null && !AskToUnmatchProcessedBankTransaction(processedTran, unmatchAll: true))
				{
					return adapter.Get();
				}
				...
				{
					CABankTransactionsImport cABankTransactionsImport = PXGraph.CreateInstance<CABankTransactionsImport>();
					cABankTransactionsImport.Header.Current = Header.Current;
		...
		{
			CABankTranHeader currentItem = this.Header.Current;
		...
		protected virtual void CABankTranHeader_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			bool hasMatchedDetails = MatchedOrProcessedDetails.Any();
