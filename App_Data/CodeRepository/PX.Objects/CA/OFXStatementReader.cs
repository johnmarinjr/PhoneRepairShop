...
//		protected abstract void ReadOFXMessage(string aOFXMessage);
	...
	public class OFXStatementReader : OFXReaderBase, IStatementReader
	{
			...
			where T : CABankTransactionsImport
		{
			...
			var notExistCashAccounts = "";
			var notSelectedCashAccounts = "";

			foreach (STMTRS iAcctStatement in this.Content.Stmtrs)
			{
				ExportNew(graph, iAcctStatement, updateCurrent, ref notExistCashAccounts, ref notSelectedCashAccounts, allowEmptyFITID);
				aStatements.Add(graph.Header.Current);
			}

			if (!string.IsNullOrEmpty(notExistCashAccounts) && !string.IsNullOrEmpty(notSelectedCashAccounts))
			{
				throw new PXException(Messages.CashAccountWithExtRefNbrIsNotFoundInTheSystemAndImportedStatementIsMadeForAnotherAccount, notExistCashAccounts ,notSelectedCashAccounts);
			}

			if (!string.IsNullOrEmpty(notExistCashAccounts))
			{
				throw new PXException(Messages.CashAccountWithExtRefNbrIsNotFoundInTheSystem, notExistCashAccounts);
			}

			if (!string.IsNullOrEmpty(notSelectedCashAccounts))
			{
				throw new PXException(Messages.ImportedStatementIsMadeForAnotherAccount, notSelectedCashAccounts);
			}
		}
			...
			where T : CABankTransactionsImport
		{
			...
			ExportNew(aGraph, aAcctStatement, updateCurrent, ref notExistCashAccounts, ref notSelectedCashAccounts, allowEmptyFITID);
		...
		protected void ExportNew<T>(T aGraph, STMTRS aAcctStatement, bool updateCurrent, ref string notExistCashAccounts, ref string notSelectedCashAccounts, bool allowEmptyFITID=false)
			where T : CABankTransactionsImport
		{
			...
			else
			{
				...
				else
				{
					...
					if (header.CashAccountID.HasValue)
					{
						...
						{
							notSelectedCashAccounts += string.Format("\n{0}-'{1}'", acct.CashAccountCD, acct.Descr);
							return;
						}
					}
				}
