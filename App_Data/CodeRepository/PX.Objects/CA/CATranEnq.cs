...
namespace PX.Objects.CA
{
	...
	public class CATranEnq : PXGraph<CATranEnq>
	{
		...
		public partial class CATranExt : CATran
		{
			...
			#endregion
			#region BatchPaymentRefNbr
			public abstract class batchPaymentRefNbr : BqlString.Field<batchPaymentRefNbr> { }

			[PXDBScalar(typeof(Search<CABatchDetail.batchNbr, Where<CATranExt.origModule, Equal<CABatchDetail.origModule>,
													And<CATranExt.origTranType, Equal<CABatchDetail.origDocType>,
		...
		#region ViewBatchPayment
		public PXAction<CAEnqFilter> viewBatchPayment;
		[PXUIField(Visible = false, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		[PXLookupButton]
		public virtual IEnumerable ViewBatchPayment(PXAdapter adapter)
		{
			if (CATranListRecords.Current?.OrigModule == BatchModule.AP)
			{
				var caBatchEntry = PXGraph.CreateInstance<CABatchEntry>();
				caBatchEntry.Document.Current = CABatch.PK.Find(caBatchEntry, CATranListRecords.Current.BatchPaymentRefNbr);
				PXRedirectHelper.TryRedirect(caBatchEntry, PXRedirectHelper.WindowMode.NewWindow);
			}
		...
		public virtual IEnumerable addDet(PXAdapter adapter)
		{
			...
			if (AddFilter.View.Answer == WebDialogResult.OK || AddFilter.View.Answer == WebDialogResult.Yes)
			{
				using (new PXTimeStampScope(this.TimeStamp))
				{
					bool releaseTransaction = AddFilter.View.Answer == WebDialogResult.Yes;
					CATran catran = AddTrxFilter.VerifyAndCreateTransaction(this, AddFilter, currencyinfo, releaseTransaction);
					if (catran != null)
					{
		...
		#endregion
		#region Button ReleaseTransaction
		public PXAction<CAEnqFilter> ReleaseTransaction;
		[PXUIField(DisplayName = "Release", MapEnableRights = PXCacheRights.Insert, MapViewRights = PXCacheRights.Insert)]
		[PXButton]
		public virtual IEnumerable releaseTransaction(PXAdapter adapter)
		{
			return adapter.Get();
		}
		#endregion
		...
		#region BAccountR Events
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), "DisplayName", CR.Messages.BAccountCD)]
		...
		public virtual void AddTrxFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			ReleaseTransaction.SetEnabled(row.Hold == false);
		}
		#endregion
	}
