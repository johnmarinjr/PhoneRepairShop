...
namespace PX.Objects.CA
{
    ...
    public class PaymentReclassifyProcess : PXGraph<PaymentReclassifyProcess>
    {
		...
		protected virtual void _(Events.FieldUpdated<Filter, Filter.branchID> e)
		{
			...
			PXFieldState accFieldState = e.Cache.GetValueExt<Filter.accountID>(e.Row) as PXFieldState;

			if (accFieldState == null) return;
			CashAccount currentCashAccount = PXSelectorAttribute.Select<Filter.accountID>(e.Cache, row, accFieldState.Value) as CashAccount;
			...
			{
				e.Cache.SetValue<Filter.accountID>(row, null);
				e.Cache.SetValueExt<Filter.accountID>(row, null);
				e.Cache.SetValuePending<Filter.accountID>(row, null);
				e.Cache.RaiseExceptionHandling<Filter.accountID>(row, null, null);
			}
		}
        ...
        public static CATran AddAPTransaction(ICADocSource parameters, CurrencyInfo aCuryInfo, IList<ICADocAdjust> aAdjustments, bool aOnHold)
        {
            if (parameters.OrigModule == GL.BatchModule.AP)
            {
				...
				CurrencyInfo refInfo = aCuryInfo
					?? PXSelectReadonly<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<CurrencyInfo.curyInfoID>>>>.Select(te, parameters.CuryInfoID);
                
                if (refInfo != null)
                {
					CM.Extensions.CurrencyInfo new_info = CM.Extensions.CurrencyInfo.GetEX(refInfo);
					new_info.CuryInfoID = te.GetExtension<APPaymentEntry.MultiCurrency>().GetDefaultCurrencyInfo().CuryInfoID;
		...
		public static CATran AddARTransaction(ICADocSource parameters, CurrencyInfo aCuryInfo, IEnumerable<ARAdjust> aAdjustments, bool aOnHold)
		{
            if (parameters.OrigModule == GL.BatchModule.AR)
            {
                ...
                if (refInfo != null)
                {
					foreach (CM.Extensions.CurrencyInfo info in te.currencyinfo.Select())
                    {
