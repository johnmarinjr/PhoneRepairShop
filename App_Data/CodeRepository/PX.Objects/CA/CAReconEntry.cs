...
{
	public class CAReconEntry : PXGraph<CAReconEntry>, ICaptionable
	{
		...
		{
			CARecon currentItem = this.CAReconRecords.Current;
			if (currentItem == null)
			{
				return string.Empty;
			}
		...
		public PXSelectJoin<CATranExt,
				LeftJoin<CR.BAccountR, On<CATranExt.referenceID, Equal<CR.BAccountR.bAccountID>>>,
		...
		protected virtual IEnumerable carecontranrecords()
		{
			...
			var transToReconcile = PXSelectJoin<CATranExt,
												LeftJoin<CR.BAccountR, On<CATranExt.referenceID, Equal<CR.BAccountR.bAccountID>>>,
												...
												And<Where<CATranExt.reconNbr, Equal<Current<CARecon.reconNbr>>,
													...
													And<Where<CATranExt.tranDate, LessEqual<Current2<CARecon.loadDocumentsTill>>,
														Or<Current2<CARecon.loadDocumentsTill>, IsNull>>>>>>>>>.Select(this).AsEnumerable().Select(result => new { CATran = result.GetItem<CATranExt>(), BAccount = result.GetItem<CR.BAccountR>() });
			if (transToReconcile.Any())
			{
				Dictionary<long?, CABankTran> bankTrans;
				Dictionary<long?, CATranVoiding> voidingTrans;
				SelectAdditionalTables(doc, transToReconcile.Select(result => result.CATran), out bankTrans, out voidingTrans);
				foreach (var item in transToReconcile)
				{
					CATranExt tran = item.CATran;
				...
				this.Caches[typeof(CATranExt)].Hold(tran);
					...
					{
						catrans.Add(tran.TranID.Value);
						yield return new PXResult<CATranExt, CR.BAccountR>(tran, item.BAccount);
					}
					else
					{
						continue;
		...
		[PXRemoveBaseAttribute(typeof(AP.BatchRefAttribute))]
		protected virtual void _(Events.CacheAttached<CABatch.batchSeqNbr> e) { }
		#endregion
		...
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), "DisplayName", CR.Messages.BusinessAccount)]
		protected virtual void _(Events.CacheAttached<BAccountR.acctCD> sender) { }

		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), "DisplayName", CR.Messages.BAccountName)]
		protected virtual void _(Events.CacheAttached<BAccountR.acctName> sender) { }
		#endregion
