...
namespace PX.Objects.CA
{
    ...
    public class CashAccountMaint : PXGraph<CashAccountMaint, CashAccount>
	{
        ...
        public CashAccountChangeID ChangeID;
		public class CashAccountChangeID : PXChangeID<CashAccount, CashAccount.cashAccountCD>
		{
			public CashAccountChangeID(PXGraph graph, string name) : base(graph, name)
			{
			}
		...
		public PXAction<CashAccount> redirectToCashAccountDetails;
		[PXUIField(DisplayName = "Cash Account Details", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		public virtual IEnumerable RedirectToCashAccountDetails(PXAdapter adapter)
		{
            ...
            if(cashAccount != null)
			{
                CATranEnq graph = PXGraph.CreateInstance<CATranEnq>();
                graph.Filter.Current.AccountID = cashAccount.CashAccountID;
                throw new PXRedirectRequiredException(graph, true, Messages.CashAccountDetails);
            }
            return adapter.Get();
        }

        public PXAction<CashAccount> redirectToCashFlowForecast;
        [PXUIField(DisplayName = "Cash Flow Forecast", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton(Category = "Inquiries")]
        public virtual IEnumerable RedirectToCashFlowForecast(PXAdapter adapter)
        {
            ...
            if (cashAccount != null)
            {
                CashFlowEnq graph = PXGraph.CreateInstance<CashFlowEnq>();
                graph.Filter.Current.CashAccountID = cashAccount.CashAccountID;
                throw new PXRedirectRequiredException(graph, true, Messages.CashForecastEntry);
            }
            return adapter.Get();
        }

        public PXAction<CashAccount> redirectToReconciliationStatementHistory;
        [PXUIField(DisplayName = "Reconciliation Statement History", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton(Category = "Inquiries")]
        public virtual IEnumerable RedirectToReconciliationStatementHistory(PXAdapter adapter)
        {
            ...
            if (cashAccount != null)
            {
                CAReconEnq graph = PXGraph.CreateInstance<CAReconEnq>();
                graph.Filter.Current.AccountID = cashAccount.CashAccountID;
                throw new PXRedirectRequiredException(graph, true, Messages.ReconciliationStatementHistory);
            }
            return adapter.Get();
        }

        public PXAction<CashAccount> redirectToBankTransactionsHistory;
        [PXUIField(DisplayName = "Bank Transactions History", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton(Category = "Inquiries")]
        public virtual IEnumerable RedirectToBankTransactionsHistory(PXAdapter adapter)
        {
            ...
            if (cashAccount != null)
            {
                CABankTransactionsEnq graph = PXGraph.CreateInstance<CABankTransactionsEnq>();
                graph.TranFilter.Current.CashAccountID = cashAccount.CashAccountID;
                throw new PXRedirectRequiredException(graph, true, Messages.BankTransactionsHistory);
            }
            return adapter.Get();
        }
        ...
        protected virtual void CashAccount_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            bool isExistingRecord = !(status == PXEntryStatus.Inserted && row.CashAccountID < 0);

            redirectToCashAccountDetails.SetEnabled(isExistingRecord);
            redirectToCashFlowForecast.SetEnabled(isExistingRecord);
            redirectToReconciliationStatementHistory.SetEnabled(isExistingRecord);
            redirectToBankTransactionsHistory.SetEnabled(isExistingRecord);
		}
        ...
        protected virtual void CashAccount_AccountID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            ...
            if (e.NewValue != null)
            {
				...
				List<string> modulesWithOpenDocument = GetAccountUsage(acct);

				if (modulesWithOpenDocument.Any())
				{
					e.NewValue = acct.AccountCD;
					throw new PXSetPropertyException(Messages.AccountCannotBeUsedAsOpenDocumentsExists, acct.AccountCD, String.Join(", ", modulesWithOpenDocument));
				}
		...
		protected virtual void _(Events.FieldUpdated<CashAccount.accountID> e)
		{
			if (e.OldValue != null && e.NewValue != e.OldValue)
			{
				Account account = Account_AccountID.SelectSingle((int?)e.OldValue);
				account = (Account)Account_AccountID.Cache.Locate(account);
				Account_AccountID.Cache.Remove(account);
			}
		}

		private List<string> GetAccountUsage(Account acct)
		{
			List<string> modulesWithOpenDocument = new List<string>();

			if (SelectFrom<APInvoice>.Where<APInvoice.status.IsEqual<APDocStatus.open>
					.And<Brackets<APInvoice.aPAccountID.IsEqual<@P.AsInt>>.Or<APInvoice.retainageAcctID.IsEqual<@P.AsInt>>>>
					.View.Select(this, acct.AccountID, acct.AccountID).Any())
			{
				modulesWithOpenDocument.Add(BatchModule.AP);
			}

			if (SelectFrom<ARInvoice>.Where<ARInvoice.status.IsEqual<ARDocStatus.open>
					.And<Brackets<ARInvoice.aRAccountID.IsEqual<@P.AsInt>>.Or<ARInvoice.retainageAcctID.IsEqual<@P.AsInt>>>>
					.View.Select(this, acct.AccountID, acct.AccountID).Any())
			{
				modulesWithOpenDocument.Add(BatchModule.AR);
			}

			return modulesWithOpenDocument;
		}
        ...
        protected virtual void _(Events.FieldUpdated<CashAccount, CashAccount.branchID> e)
        {
            ...
            if (ValidateClearingAccount(e.Row.BranchID))
        ...
        {
            if (CashAccount.Current != null
                && CashAccount.Cache.GetStatus(CashAccount.Current) != PXEntryStatus.Deleted
                && CashAccount.Cache.GetStatus(CashAccount.Current) != PXEntryStatus.InsertedDeleted
                && !ValidateClearingAccount(CashAccount.Current.BranchID)) return;
        ...
        protected virtual bool ValidateClearingAccount(int? branchId)
        {
            ...
            foreach (CashAccountDeposit cur in Deposits.Select())
            {
                CashAccount cashAcc = PXSelect<CashAccount,
                    Where<CashAccount.cashAccountID,
                        Equal<Required<CashAccount.cashAccountID>>>>
                    .Select(this, cur.DepositAcctID);

                if (branch.BaseCuryID != cashAcc.BaseCuryID)
                {
                    Deposits.Cache.RaiseExceptionHandling<CashAccountDeposit.depositAcctID>(cur, cashAcc.CashAccountCD, new PXSetPropertyException(
                        Messages.ClearingAccountDifferentBaseCury,
                        PXErrorLevel.Error,
                        PXAccess.GetBranchCD(cashAcc.BranchID), cashAcc.CashAccountCD, branch.BranchCD));
                    result = false;
                }
            }
            return result;
