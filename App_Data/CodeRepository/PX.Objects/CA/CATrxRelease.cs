...
namespace PX.Objects.CA
{
	...
	public partial class CARegister : PX.Data.IBqlTable
	{
		...
		[CM.PXDBCury(typeof(CARecon.curyID))]
		[PXDefault(TypeCode.Decimal, "0.0")]
			...
			if (aTran != null)
			{
				...
				switch (aTran.OrigModule)
				{
					case GL.BatchModule.AP:
						...
						if (apReg == null) throw new Exception(Messages.DocNotFound);
						if (apReg.Hold != false) throw new PXException(Messages.DocumentStatusInvalid);
						if (apReg.Approved == false) throw new PXException(Messages.CannotReleasePendingApprovalDocument);

						if (apReg.Released == false)
							APDocumentRelease.ReleaseDoc(new List<APRegister> { apReg }, false, externalPostList);
						break;
					...
					case GL.BatchModule.AR:
						...
						if (arReg == null) throw new Exception(Messages.DocNotFound);
						if (arReg.Hold != false) throw new PXException(Messages.DocumentStatusInvalid);
						if (arReg.Approved == false) throw new PXException(Messages.CannotReleasePendingApprovalDocument);
						if (arReg.Released == false)
						{
							using (new PXTimeStampScope(null))
								ARDocumentRelease.ReleaseDoc(new List<ARRegister> { arReg }, false, externalPostList);
						}
						break;
					case GL.BatchModule.CA:
						switch (aTran.OrigTranType)
						{
							case CATranType.CAAdjustment:
								...
								if (docAdj == null) throw new Exception(Messages.DocNotFound);
								if (docAdj.Hold != false) throw new PXException(Messages.DocumentStatusInvalid);
								if (docAdj.Approved == false) throw new PXException(Messages.CannotReleasePendingApprovalDocument);

								if (docAdj.Released == false)
									ReleaseDoc(docAdj, i, externalPostList, CAAdj.Events.Select(ev => ev.ReleaseDocument));
							...
							case CATranType.CATransferExp:
								...
								if (docTransfer == null) throw new Exception(Messages.DocNotFound);
								if (docTransfer.Hold != false) throw new PXException(Messages.DocumentStatusInvalid);

								if (docTransfer.Released == false)
									ReleaseDoc(docTransfer, i, externalPostList, CATransfer.Events.Select(ev => ev.ReleaseDocument));
							...
							case CATranType.CAVoidDeposit:
								...
								if (docDeposit == null) throw new Exception(Messages.DocNotFound);
								if (docDeposit.Hold != false) throw new PXException(Messages.DocumentStatusInvalid);
