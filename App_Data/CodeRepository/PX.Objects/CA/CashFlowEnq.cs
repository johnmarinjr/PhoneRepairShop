...
namespace PX.Objects.CA
{
	...
	public class CashFlowEnq : PXGraph<CashFlowEnq>
	{
		...
		public IEnumerable cashFlow()
		{
			...
			var baseCuryID = PXAccess.GetBranchByBAccountID(filter.OrgBAccountID)?.BaseCuryID
								?? PXAccess.GetOrganizationByBAccountID(filter.OrgBAccountID)?.BaseCuryID;

			this.baseCurrency = PXSelect<Currency, Where<Currency.curyID, Equal<Required<Branch.baseCuryID>>>>.Select(this, baseCuryID);
		...
		public void CalcCashAccountBalance(DateTime startDate, CashFlowFilter filter, Dictionary<int, CashFlowForecast> result)
		{
			...
			PXSelectBase<CashAccount> acctBalanceSelect = new PXSelectJoinGroupBy<CashAccount, 
																			LeftJoin<CADailySummary, On<CADailySummary.cashAccountID, Equal<CashAccount.cashAccountID>,
																				...
																				InnerJoin<Branch, On<Branch.branchID, Equal<CashAccount.branchID>>,
																				InnerJoin<Currency, On<Currency.curyID, Equal<Branch.baseCuryID>>>>>,
			...
			foreach (PXResult<CashAccount,CADailySummary, Branch, Currency> iRes in acctBalanceSelect.Select(startDate, cashAccountID))
			{
		...
		{
			bool isQuickCheck = doc.DocType == APDocType.QuickCheck || doc.DocType == APDocType.VoidQuickCheck;

			Decimal? curyAmount = isQuickCheck ? doc.CuryOrigDocAmt : doc.CuryDocBal;
			Decimal? baseAmount = isQuickCheck ? doc.OrigDocAmt : doc.DocBal;
