...
using PX.Objects.AP;
using PX.Api;
using static PX.Objects.PM.ProFormaProcess.PMProFormaAction;
	...
	public class PaymentMethodMaint : PXGraph<PaymentMethodMaint, PaymentMethod>
	{
		...
		protected virtual void PaymentMethod_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			if (oldRow.PaymentType != row.PaymentType)
			{
				...
				row.ARIsProcessingRequired = PXAccess.FeatureInstalled<CS.FeaturesSet.integratedCardProcessing>() && row.PaymentType == PaymentMethodType.CreditCard;
			}
