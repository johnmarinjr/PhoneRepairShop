	...
	public class LaborCostRateMaint : PXGraph<LaborCostRateMaint>, PXImportAttribute.IPXPrepareItems
	{
		...
		{			
			...
			{				
				...
				PXUIFieldAttribute.SetEnabled<PMLaborCostRate.employeeID>(e.Cache, e.Row, e.Row.Type == PMLaborCostRateType.Employee);
				...
				PXUIFieldAttribute.SetEnabled<PMLaborCostRate.wageRate>(e.Cache, e.Row, e.Row.EmploymentType == EP.RateTypesAttribute.Hourly);
				PXUIFieldAttribute.SetEnabled<PMLaborCostRate.rate>(e.Cache, e.Row, e.Row.EmploymentType == EP.RateTypesAttribute.Hourly || PXAccess.FeatureInstalled<FeaturesSet.payrollModule>());
			}
		}
        ...
        protected virtual void _(Events.RowUpdating<PMLaborCostRate> e)
        {
            PMLaborCostRate newRow = e.NewRow;
            PMLaborCostRate originalRow = e.Row;
            if (newRow == null || originalRow == null)
                return;

            if (newRow.EffectiveDate != originalRow.EffectiveDate)
            {
                PMLaborCostRate latestDuplicate = GetLatestDuplicate(newRow);

                if (latestDuplicate != null && latestDuplicate.EffectiveDate != null)
                {
                    if (newRow.EffectiveDate != null && newRow.EffectiveDate.Value.Date <= latestDuplicate.EffectiveDate.Value.Date)
                    {
                        Items.Cache.RaiseExceptionHandling<PMLaborCostRate.effectiveDate>(newRow, newRow.EffectiveDate, new PXSetPropertyException<PMLaborCostRate.effectiveDate>(Messages.EffectiveDateShouldBeGreater, latestDuplicate.EffectiveDate));
                        newRow.EffectiveDate = originalRow.EffectiveDate;
                    }
                }
                else
                {
                    DateTime? minEffectiveDate = GetMinEffectiveDate(e.Row);
                    if (newRow.EffectiveDate != null && minEffectiveDate != null && newRow.EffectiveDate.Value.Date > minEffectiveDate.Value.Date)
                    {
                        Items.Cache.RaiseExceptionHandling<PMLaborCostRate.effectiveDate>(newRow, newRow.EffectiveDate, new PXSetPropertyException<PMLaborCostRate.effectiveDate>(Messages.EffectiveDateShouldBeGreater, minEffectiveDate.Value.AddDays(1)));
                        newRow.EffectiveDate = originalRow.EffectiveDate;
                    }
                }
            }
            else
            {
                ...
                {
                    if (newRow.EffectiveDate != null && newRow.EffectiveDate.Value.Date <= latestDuplicate.EffectiveDate.Value.Date)
                        throw new PXException(Messages.LaborCostRateIsDuplicated);
                }
            }
        }
		...
		// Handler is left for backward compatibility
		...
		public virtual PMLaborCostRate GetLatestDuplicate(PMLaborCostRate row)
		{
			...
			{
                List<object> args = new List<object>(new object[] { row.ProjectID, row.InventoryID, row.Type, row.RecordID });
                PXSelectBase<PMLaborCostRate> select = new PXSelect<PMLaborCostRate, Where<PMLaborCostRate.projectID, Equal<Required<PMLaborCostRate.projectID>>,
						And<PMLaborCostRate.inventoryID, Equal<Required<PMLaborCostRate.inventoryID>>,
						And<PMLaborCostRate.type, Equal<Required<PMLaborCostRate.type>>,
                                                                                        And<PMLaborCostRate.recordID, NotEqual<Required<PMLaborCostRate.recordID>>>>>>,
