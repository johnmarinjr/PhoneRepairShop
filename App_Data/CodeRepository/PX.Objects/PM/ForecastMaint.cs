	...
	public class ForecastMaint : PXGraph<ForecastMaint, PMForecast>, PXImportAttribute.IPXPrepareItems
	{
		...
		public virtual IEnumerable AddPeriods(PXAdapter adapter)
		{
			if (Items.Current != null)
			{
				...
				if (budget != null)
				{
					string startPeriodID = AddPeriodDialog.Current.StartPeriodID;
					string endPeriodID = AddPeriodDialog.Current.EndPeriodID;

					if (startPeriodID == null)
					{
						startPeriodID = GetMinStartPeriodID(budget);
					}

					if (endPeriodID == null)
					{
						endPeriodID = GetMaxEndPeriodID(budget);
						if (endPeriodID == null)
							endPeriodID = startPeriodID;
					}

					GeneratePeriodsForBudget(budget, startPeriodID, endPeriodID);
				}
			}
		...
		[PXButton(Category = Messages.BudgetForecasting, DisplayOnMainToolbar = true)]
		public virtual IEnumerable GeneratePeriods(PXAdapter adapter)
		{
			foreach (PMBudgetInfo budget in Budgets.Select())
			{
				string startPeriodID = GetMinStartPeriodID(budget);
				string endPeriodID = GetMaxEndPeriodID(budget);
				if (endPeriodID == null)
					endPeriodID = startPeriodID;

				if (int.Parse(endPeriodID) < int.Parse(startPeriodID))
				{
		...
		[PXButton(Category = Messages.Other, DisplayOnMainToolbar = true)]
		...
		[PXUIField(DisplayName = "Generate Forecast", MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Select)]
		[PXButton(Category = Messages.BudgetForecasting, DisplayOnMainToolbar = true)]
		...
		private int GetBasePrecision() => ServiceLocator.Current.GetInstance<Func<PXGraph, IPXCurrencyService>>()(this).BaseDecimalPlaces();
		...
		protected virtual void DistributeForecastRedistribution(PMForecastDistributeDialogInfo filterdata, Summary summary)
		{
			...
			int decimalPlaces = GetBasePrecision();
			decimal amt = Math.Round(DistibuteRound(amtRaw, summary.Details.Count - 1), decimalPlaces);
			decimal revisedAmt = Math.Round(DistibuteRound(revisedAmtRaw, summary.Details.Count - 1), decimalPlaces);
		...
		protected virtual string GetMinStartPeriodID(PMBudgetInfo row)
		{
			...
			if (first != null)
			{
				...
				{
					minDate = first.Date;
					minFinPeriod = first.FinPeriodID;
				}
			...
			if (minFinPeriod == null)
			{
				minFinPeriod = FinPeriodRepository.GetPeriodIDFromDate(minDate, FinPeriod.organizationID.MasterValue);
			}
		...
		protected virtual string GetMaxEndPeriodID(PMBudgetInfo row)
		{
			...
			if (maxFinPeriod == null && maxDate !=  null)
			{
				maxFinPeriod = FinPeriodRepository.GetPeriodIDFromDate(maxDate, FinPeriod.organizationID.MasterValue);
			}

			return maxFinPeriod;
		}
				
		protected virtual void GeneratePeriodsForBudget(PMBudgetInfo row, string startPeriodID, string endPeriodID)
		{
			...
			foreach (FinPeriod period in AllPeriodsBetweenInclusive(startPeriodID, endPeriodID, FinPeriod.organizationID.MasterValue))
			{
		...
		public IEnumerable<FinPeriod> AllPeriodsBetweenInclusive(string startPeriodID, string endPeriodID, int? organizationID)
		{
			...
			return SelectFrom<FinPeriod>.
				Where<
					FinPeriod.finPeriodID.IsBetween<@P.AsString, @P.AsString>.
					And<FinPeriod.organizationID.IsEqual<@P.AsInt>>>.
					View.Select(this, startPeriodID, endPeriodID, organizationID).
					RowCast<FinPeriod>();
		}
		...
		protected virtual void _(Events.FieldDefaulting<PMForecastAddPeriodDialogInfo, PMForecastAddPeriodDialogInfo.startPeriodID> e)
		{
			if (Items.Current != null)
			{
				...
				if (budget != null)
				{
					string finPeriod = GetMinStartPeriodID(budget);
						if (finPeriod != null)
						{
						e.NewValue = FinPeriodIDAttribute.FormatForDisplay(finPeriod);
					}
				}
			}
		}
		...
		protected virtual void _(Events.FieldDefaulting<PMForecastAddPeriodDialogInfo, PMForecastAddPeriodDialogInfo.endPeriodID> e)
		{
			if (Items.Current != null)
			{
				PMBudgetInfo budget = GetBudgetInfoFromKey(BudgetKeyTuple.Create(Items.Current));
				{
					string finPeriod = GetMaxEndPeriodID(budget);
						if (finPeriod != null)
						{
						e.NewValue = FinPeriodIDAttribute.FormatForDisplay(finPeriod);
					}
		...
		protected virtual void _(Events.FieldDefaulting<PMForecastAddPeriodDialogInfo, PMForecastAddPeriodDialogInfo.startDate> e)
		{
			...
			{
				e.NewValue = FinPeriodRepository.FindByID(FinPeriod.organizationID.MasterValue, e.Row.StartPeriodID).StartDateUI;
			}
		}
