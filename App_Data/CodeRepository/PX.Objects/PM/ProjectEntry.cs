...
namespace PX.Objects.PM
{
    ...
    public class ProjectEntry : PXGraph<ProjectEntry, PMProject>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public PXSelectJoin<PMCostCenter, 
			InnerJoin<PMTask, On <PMTask.taskID, Equal<PMCostCenter.taskID>>,
			InnerJoin<IN.INLocation, On<IN.INLocation.locationID, Equal<PMCostCenter.locationID>>>>,
			Where<PMCostCenter.projectID, Equal<Current<PMProject.contractID>>>> CostCenters;
		...
		protected virtual void _(Events.FieldUpdated<PMRevenueBudget, PMRevenueBudget.projectTaskID> e)
		{
			...
			{
				PMTask task = PMTask.PK.Find(this, e.NewValue as int?);
		...
		public virtual void RecalculateRevenueBudget(PMRevenueBudget row)
		{
			if (row != null)
			{
				...
				try
				{
					...
					if (row.ProgressBillingBase == ProgressBillingBase.Amount && ContainsProgressiveBillingRule(projectTaskID))
					{
					...
					{
						var newQty = (row.CompletedPct.GetValueOrDefault() / 100.0m * row.RevisedQty.GetValueOrDefault()) - (row.InvoicedQty.GetValueOrDefault() + row.ActualQty.GetValueOrDefault());
		...
		{
			foreach (PXResult<PMCostCenter, PMTask, IN.INLocation> res in CostCenters.Select())
			{
				PMCostCenter costCenter = (PMCostCenter)res;
