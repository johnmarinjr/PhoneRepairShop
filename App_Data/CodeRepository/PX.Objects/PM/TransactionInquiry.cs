...
namespace PX.Objects.PM
{
	...
	public class TransactionInquiry : PXGraph<TransactionInquiry>
	{
		...
		public virtual IEnumerable transactions()
		{
				...
				filter.TranID != null))
			{
				...
				PXSelectBase<PMTran> select = new PXSelectJoin<PMTran,
					...
					Where<True, Equal<True>, And2<Where<RegisterReleaseProcess.OffsetPMAccountGroup.groupID, IsNull, Or<Match<RegisterReleaseProcess.OffsetPMAccountGroup, Current<AccessInfo.userName>>>>, 
				...
				{
					select.WhereAnd<Where<PMTran.accountGroupID, Equal<Current<TranFilter.accountGroupID>>, Or<PMTran.offsetAccountID, Equal<Current<TranFilter.accountGroupID>>>>>();
				}

				if (filter.AccountID != null)
				{
					select.WhereAnd<Where<PMTran.accountID, Equal<Current<TranFilter.accountID>>, Or<PMTran.offsetAccountID, Equal<Current<TranFilter.accountID>>>>>();
				}
        ...
        public TransactionInquiry()
        {
			...
			PXUIFieldAttribute.SetDisplayName<BAccount.acctName>(Caches[typeof(BAccount)], Messages.EmployeeName);
			PXUIFieldAttribute.SetVisible<BAccount.acctName>(Caches[typeof(BAccount)], null, false);
		}
