...
namespace PX.Objects.PM
{
	...
	public class RateMaint : PXGraph<RateMaint, PMRateSequence>
	{
		...
		{
			if (RateSequence.Current != null)
			{
				int? rateTableIDIndex = null;
				int? rateTypeIDIndex = null;
				int? sequenceIndex = null;
				int? rateCodeIDIndex = null;
				for (int i = 0; i <= adapter.SortColumns.Length - 1; i++)
				{
					if (adapter.SortColumns[i].ToLower() == typeof(PMRateSequence.rateTableID).Name.ToLower())
					{
						rateTableIDIndex = i;
					}
					else if (adapter.SortColumns[i].ToLower() == typeof(PMRateSequence.rateTypeID).Name.ToLower())
					{
						rateTypeIDIndex = i;
					}
					else if (adapter.SortColumns[i].ToLower() == typeof(PMRateSequence.sequence).Name.ToLower())
					{
						sequenceIndex = i;
					}
					else if (adapter.SortColumns[i].ToLower() == typeof(PMRateSequence.rateCodeID).Name.ToLower())
					{
						rateCodeIDIndex = i;
					}
				}

				PMRateSequence rs = null;
				if (RateSequence.Current.RateTableID != null && RateSequence.Current.RateTypeID != null)
				{
					if (rateTableIDIndex != null && PXLocalesProvider.CollationComparer.Compare(adapter.Searches[rateTableIDIndex.Value], RateSequence.Current.RateTableID) != 0)
					{
						rs = PXSelect<PMRateSequence,
								Where<PMRateSequence.rateTableID, Equal<Required<PMRateSequence.rateTableID>>>,
								OrderBy<Asc<PMRateSequence.rateTypeID, Asc<PMRateSequence.sequence>>>>.SelectWindowed(this, 0, 1, adapter.Searches[rateTableIDIndex.Value]).RowCast<PMRateSequence>().FirstOrDefault();
					}
					else if (rateTypeIDIndex != null && PXLocalesProvider.CollationComparer.Compare(adapter.Searches[rateTypeIDIndex.Value], RateSequence.Current.RateTypeID) != 0)
					{
						rs = PXSelect<PMRateSequence,
								Where<PMRateSequence.rateTypeID, Equal<Required<PMRateSequence.rateTypeID>>>,
								OrderBy<Asc<PMRateSequence.rateTableID, Asc<PMRateSequence.sequence>>>>.SelectWindowed(this, 0, 1, adapter.Searches[rateTypeIDIndex.Value]).RowCast<PMRateSequence>().FirstOrDefault();
					}
				}

				if (rs != null)
				{
					if (rateTableIDIndex != null)
					{
						adapter.Searches[rateTableIDIndex.Value] = rs.RateTableID;
					}
					if (rateTypeIDIndex != null)
					{
						adapter.Searches[rateTypeIDIndex.Value] = rs.RateTypeID;
					}
					if (sequenceIndex != null)
					{
						adapter.Searches[sequenceIndex.Value] = rs.Sequence;
					}
					if (rateCodeIDIndex != null)
					{
						adapter.Searches[rateCodeIDIndex.Value] = rs.RateCodeID;
					}
				}
			}

			foreach (PMRateSequence rateSequence in (new PXCancel<PMRateSequence>(this, "Cancel")).Press(adapter))
			{
				return new object[] { rateSequence };
			}
