...
namespace PX.Objects.PM
{
	public class ChangeOrderEntry : PXGraph<ChangeOrderEntry, PMChangeOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public virtual IEnumerable Send(PXAdapter adapter)
		{
			...
			{
				PXLongOperation.StartOperation(this, delegate () {
					SendReport(ChangeOrderNotificationCD, Document.Current, adapter.MassProcess);
				});
			}
		...
		public virtual void SendReport(string notificationCD, PMChangeOrder doc, bool massProcess = false)
		{
			if (doc != null)
			{
				...
				{
					Activity.SendNotification(PMNotificationSource.Project, notificationCD, Project.Current.DefaultBranchID, mailParams, massProcess);
					this.Save.Press();
		...
		protected virtual void _(Events.FieldVerifying<PMChangeOrder, PMChangeOrder.projectID> e)
		{
			...
			if (project != null)
			{
				if (project.Status == ProjectStatus.Completed || project.Status == ProjectStatus.Cancelled || project.Status == ProjectStatus.OnHold)
				{
		...
		protected virtual void ModifyExistsingLineInOrder(POOrderEntry target, PMChangeOrderLine line)
		{
			...
			bool wasCancelled = poLine.Cancelled == true;
			...
			if (wasCancelled && poLine.Cancelled != true)
			{
		...
		private decimal GetAmountInProjectCurrency(string fromCuryID, decimal? value)
		{
