...
namespace PX.Objects.PM
{
	public class PMQuoteMaint : PXGraph<PMQuoteMaint>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		...
		public IEnumerable SendQuote(PXAdapter adapter)
		{
			foreach (PMQuote item in adapter.Get<PMQuote>())
			{
				...
				Activities.SendNotification(PMNotificationSource.Project, DefaultNotificationCD, item.BranchID, parameters, adapter.MassProcess);
		...
		{
			if (Quote.Current != null && !string.IsNullOrEmpty(Quote.Current.TaxZoneID))
			{
				var row = Quote.Cache.GetExtension<Extensions.SalesTax.Document>(Quote.Current);
				if (row != null)
				{
		...
		public virtual void ConvertQuoteToProject(PMQuote row, ConvertToProjectFilter settings)
		{
			...
			projectEntry.DefaultFromTemplate(project, row.TemplateID, new ProjectEntry.DefaultFromTemplateSettings() { CopyProperties = true, CopyAttributes = true, CopyEmployees=true, CopyEquipment=true, CopyNotification=true, CopyCurrency = false });
		...
		public virtual void AddingShippingInfoToProject(PMQuote row, ProjectEntry projectEntry)
		{
			...
			if (shippingAddress != null && shippingAddress.OverrideAddress == true)
			{
				...
				if (projectAddress != null)
				{
					...
					projectAddress = projectEntry.Site_Address.Update(projectAddress);
				}
			}
		}
		...
		public class ContactAddress : CROpportunityContactAddressExt<PMQuoteMaint>
		{
			...
			protected override void _(Events.FieldUpdated<CR.Extensions.CROpportunityContactAddress.Document, CR.Extensions.CROpportunityContactAddress.Document.contactID> e)
			{
				...
				if (bAccount != null)
				{
					Base.Quote.Cache.SetValueExt<PMQuote.taxZoneID>(e.Row.Base, bAccount.TaxZoneID);
	...
	public class EPApprovalActionExtensionPersistentRequired<SourceAssign, Approved, Rejected, ApprovalMapID, ApprovalNotificationID, SubmitCancelled, RequiredField1, RequiredField2, RequiredField3>
		...
		where RequiredField3 : class, IBqlField
	{
		...
		public override IEnumerable Submit(PXAdapter adapter)
		{
			foreach (var item in adapter.Get<SourceAssign>())
			{
				...
				var requiredData3 = cache.GetValue<RequiredField3>(item);
				...
				else if (requiredData3 == null)
				{
					cache.RaiseExceptionHandling<RequiredField3>(item, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{typeof(RequiredField3).Name}]"));
					cache.SetValue<SubmitCancelled>(item, true);
					yield return item;
				}
