	...
	public class ProformaEntry : PXGraph<ProformaEntry, PMProforma>, IGraphWithInitialization
	{
		...
		public virtual void SendReport(string notificationCD, PMProforma doc, bool massProcess = false)
		{
			if (doc != null)
			{
				...
				{
					Activity.SendNotification(PMNotificationSource.Project, notificationCD, doc.BranchID, mailParams, massProcess);
					this.Save.Press();
		...
		protected virtual void _(Events.RowSelected<PMProformaProgressLine> e)
		{
			if (e.Row != null)
			{
				...
				if(billingBase == ProgressBillingBase.Amount)
				{
				...
				else if(billingBase == ProgressBillingBase.Quantity)
				{
		...
		protected virtual Tuple<decimal, decimal> CalculateContractRetainageOnLine(PMProject project, PMProformaProgressLine line, decimal totalRetainageUptoDate, decimal contractAmount, decimal totalRetainageOnInvoice, decimal roundingOverflow)
		{
			...
			decimal lineRetainage = GetAmountInProjectCurrency(line.CuryLineTotal.GetValueOrDefault() * project.RetainagePct.GetValueOrDefault() * 0.01m);
		...
		public virtual Dictionary<ProformaTotalsCounter.AmountBaseKey, decimal> GetBilledRetainageToDate(DateTime? cutoffDate)
		{
			...
			{
				var key = new ProformaTotalsCounter.AmountBaseKey(tran.TaskID.GetValueOrDefault(), tran.CostCodeID.GetValueOrDefault(CostCodeAttribute.DefaultCostCode.GetValueOrDefault()), tran.InventoryID.GetValueOrDefault(PMInventorySelectorAttribute.EmptyInventoryID), 0);
