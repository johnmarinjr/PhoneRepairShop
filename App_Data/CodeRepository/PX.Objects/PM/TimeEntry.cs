	...
	public class TimeEntry : PXGraph<TimeEntry, PMTimeActivity>
	{
		...
		public virtual void RecalculateEmployeeCost(PMTimeActivity row)
		{
			if (row != null && row.Date != null)
			{
				...
				var cost = CostEngine.CalculateEmployeeCost(null, row.EarningTypeID, row.LabourItemID, row.ProjectID, row.ProjectTaskID, row.CertifiedJob, row.UnionID, employeeID, row.Date.Value, row.ShiftID);
