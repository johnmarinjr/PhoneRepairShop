...
{
    [PMSiteStatusAccumulatorAttribute]
    [Serializable]
    [PXCacheName(Messages.PMSiteStatus)]
    public class PMSiteStatusAccum : PMSiteStatus, IQtyAllocated, ICostStatus
    {
        #region Keys
        public new class PK : PrimaryKeyOf<PMSiteStatusAccum>.By<inventoryID, subItemID, siteID, projectID, taskID>
        {
            public static PMSiteStatusAccum Find(PXGraph graph, int? inventoryID, int? subItemID, int? siteID, int? projectID, int? taskID)
