...
{
	/// <summary>
	/// Represents the base class for the project budget line.
	...
	public class PMBudget : IBqlTable, IProjectFilter, IQuantify
	{
		...
		public virtual Decimal? Performance
		{
			get
			{
				if(ProgressBillingBase == PM.ProgressBillingBase.Amount)
				{
					return CuryRevisedAmount != 0.0m ? (CuryActualAmount / CuryRevisedAmount) * 100.0m : 0.0m;
				}
				else if(ProgressBillingBase == PM.ProgressBillingBase.Quantity)
				{
					return RevisedQty != 0.0m ? (ActualQty / RevisedQty) * 100.0m : 0.0m;
				}
				return 0.0m;
			}
		}
