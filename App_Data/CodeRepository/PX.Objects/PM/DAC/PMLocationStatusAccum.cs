...
{
	[PMLocationStatusAccumulatorAttribute]
    [Serializable]
    [PXCacheName(Messages.PMLocationStatus)]
    public class PMLocationStatusAccum : PMLocationStatus, IQtyAllocated, ICostStatus
    {
        #region Keys
        public new class PK : PrimaryKeyOf<PMLocationStatusAccum>.By<inventoryID, subItemID, siteID, locationID, projectID, taskID>
        {
            public static PMLocationStatusAccum Find(PXGraph graph, int? inventoryID, int? subItemID, int? siteID, int? locationID, int? projectID, int? taskID)
        ...
        [PXBool()]
        [PXDefault(typeof(Search<INLocation.inclQtyAvail, Where<INLocation.locationID, Equal<Current<locationID>>>>), CacheGlobal = true, PersistingCheck = PXPersistingCheck.Nothing)]
