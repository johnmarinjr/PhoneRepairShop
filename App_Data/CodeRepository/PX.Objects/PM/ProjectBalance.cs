...
namespace PX.Objects.PM
{
	public class ProjectBalance
	{
		...
		public virtual List<Result> Calculate(PMProject project, PMTran tran, Account acc, PMAccountGroup ag, Account offsetAcc, PMAccountGroup offsetAg)
		{
			...
			{
				int invert = 1;
				if (tran.TranType == BatchModule.PM &&
					IsFlipRequired(acc.Type, ag.Type) &&
					(ag.Type != AccountType.Liability || offsetAg == null) &&
					offsetAg?.Type != AccountType.Expense)
				{
				...
				else
				{
					...
					{
						bool isCreditAccountGroupSpecified = tran.OffsetAccountGroupID != null;
						//DEBIT
						if (acc.Type == AccountType.Income || acc.Type == AccountType.Liability)
							list.Add(Calculate(project, tran, ag, acc.Type, -1 * invert, isCreditAccountGroupSpecified ? -1 * invert : 1));
						else
							list.Add(Calculate(project, tran, ag, acc.Type, 1 * invert, isCreditAccountGroupSpecified ? 1 * invert : 1));
					}
				}
				...
				{
					int offsetInvert = 1;
					if (ag.Type != AccountType.Expense &&
						offsetAg.Type != AccountType.Liability &&
						IsFlipRequired(offsetAcc.Type, offsetAg.Type))
					{
						offsetInvert = -1;
					}
