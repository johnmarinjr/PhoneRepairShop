	...
	public class PMBillEngine : PXGraph<PMBillEngine>, IRateTable
	{
			...
			foreach (KeyValuePair<string, Tuple<int?, List<BillingData>>> kv in list)
			{
				foreach (BillingData data in kv.Value.Item2)
				{
					...
					else
					{
						...
						{
							if (data.Tran.CostCodeID == null)
							{
								data.Tran.CostCodeID = CostCodeAttribute.DefaultCostCode;
							}
						}
		...
		public virtual List<PMTask> SelectBillableTasks(PMProject project)
		{
			...
			PXSelectBase<PMTask> selectTasks = new PXSelect<PMTask,
				...
				And<PMTask.isActive, Equal<True>>>>, OrderBy<Asc<PMTask.taskCD>>>(this);
		...
		{
			CM.Extensions.CurrencyInfo currencyInfo = InvoiceEntry.FindImplementation<Extensions.MultiCurrency.IPXCurrencyHelper>().GetDefaultCurrencyInfo();

			foreach (BillingData data in unbilled)
			{
				ARTran tran = new ARTran
				{
					BranchID = data.Tran.BranchID,
					InventoryID = data.Tran.InventoryID == PMInventorySelectorAttribute.EmptyInventoryID ? null : data.Tran.InventoryID,
					TranDesc = data.Tran.Description,
					UOM = data.Tran.UOM,
					Qty = data.Tran.Qty * mult,
					CuryExtPrice = MultiCurrencyService.GetValueInBillingCurrency(InvoiceEntry, project, currencyInfo, data.Tran.CuryAmount * mult),
					CuryUnitPrice = MultiCurrencyService.GetValueInBillingCurrency(InvoiceEntry, project, currencyInfo, data.Tran.CuryUnitPrice),
					ProjectID = data.Tran.ProjectID,
					TaskID = data.Tran.TaskID,
					CostCodeID = data.Tran.CostCodeID,
					Date = data.Tran.Date,
					AccountID = data.Tran.AccountID,
					SubID = data.Tran.SubID,
					TaxCategoryID = data.Tran.TaxCategoryID,
					RetainagePct = project.RetainageMode == RetainageModes.Contract ? project.RetainagePct : data.Tran.RetainagePct,
				...
				if (data.Tran.IsPrepayment != true && data.Tran.AccountGroupID != null)
				{
					...
					ext.SubtractValuesToInvoice(newTran, data.Tran.AccountGroupID, (int)ARDocType.SignAmount(newTran.TranType).GetValueOrDefault(1));
				}
		...
		public virtual PMProformaLine InsertTransaction(PMProject project, PMProformaLine tran, string subCD, string note, Guid[] files)
		{
			...
			CurrencyInfo currencyInfo = ProformaEntry.FindImplementation<IPXCurrencyHelper>().GetDefaultCurrencyInfo();
			tran.CuryAmount = MultiCurrencyService.GetValueInBillingCurrency(ProformaEntry, project, currencyInfo, tran.CuryAmount);
			tran.CuryBillableAmount = MultiCurrencyService.GetValueInBillingCurrency(ProformaEntry, project, currencyInfo, tran.CuryBillableAmount);
			tran.CuryUnitPrice = MultiCurrencyService.GetValueInBillingCurrency(ProformaEntry, project, currencyInfo, tran.CuryUnitPrice);
			...
			else
			{
				...
				{					
					...
					{
						ProformaEntry.SubtractValuesToInvoice(newTran, newTran.CuryAmount, newTran.Qty);
					}
		...
		public virtual PMProforma InsertNewProformaDocument(string finPeriod, Customer customer, PMProject project, DateTime billingDate, string docDesc, int? locationID)
		{
			...
			ProformaEntry.Document.Cache.SetValueExt<PMProforma.curyID>(doc, project.BillingCuryID);
			...
			else
			{
				...
				//changes to _CacheAttributes are not restored on Clear().

				PX.Objects.TX.TaxBaseAttribute.SetTaxCalc<PMProformaTransactLine.taxCategoryID>(ProformaEntry.TransactionLines.Cache, null, PX.Objects.TX.TaxCalc.Calc);
				PX.Objects.TX.TaxBaseAttribute.SetTaxCalc<PMProformaProgressLine.taxCategoryID>(ProformaEntry.ProgressiveLines.Cache, null, PX.Objects.TX.TaxCalc.Calc);
			}

			PMAddress shippingAddressPM = (PMAddress)PXSelect<PMAddress, Where<PMAddress.addressID, Equal<Required<PMProject.siteAddressID>>>>.Select(this, project.SiteAddressID);
			if (shippingAddressPM != null && shippingAddressPM.IsDefaultAddress != true && !string.IsNullOrWhiteSpace(project.RevenueTaxZoneID) && Setup.Current.CalculateProjectSpecificTaxes == true)
			{
				PMShippingAddress addressPM = ProformaEntry.Shipping_Address.Current;
				PMAddress billingAddress = (PMAddress)PXSelect<PMAddress, Where<PMAddress.addressID, Equal<Required<PMProject.billAddressID>>>>.Select(this, project.BillAddressID);

				if (addressPM == null)
				{
					PMShippingAddress shipAddress = (PMShippingAddress)ProformaEntry.Shipping_Address.Cache.CreateInstance();
					AddressAttribute.Copy(shipAddress, shippingAddressPM);

					shipAddress.IsValidated = shippingAddressPM.IsValidated;
					shipAddress.BAccountAddressID = billingAddress?.BAccountAddressID;
					shipAddress.IsDefaultAddress = true;

					ProformaEntry.Shipping_Address.Current = shipAddress = ProformaEntry.Shipping_Address.Insert(shipAddress);

					ProformaEntry.Document.Current.ShipAddressID = shipAddress.AddressID;
				}
				else
				{
					addressPM.BAccountAddressID = billingAddress?.BAccountAddressID;
					addressPM.BAccountID = shippingAddressPM.BAccountID;
					addressPM.RevisionID = shippingAddressPM.RevisionID;
					addressPM.IsDefaultAddress = shippingAddressPM.IsDefaultAddress;
					addressPM.AddressLine1 = shippingAddressPM.AddressLine1;
					addressPM.AddressLine2 = shippingAddressPM.AddressLine2;
					addressPM.AddressLine3 = shippingAddressPM.AddressLine3;
					addressPM.City = shippingAddressPM.City;
					addressPM.State = shippingAddressPM.State;
					addressPM.PostalCode = shippingAddressPM.PostalCode;
					addressPM.CountryID = shippingAddressPM.CountryID;
					addressPM.IsValidated = shippingAddressPM.IsValidated;
					addressPM.Latitude = shippingAddressPM.Latitude;
					addressPM.Longitude = shippingAddressPM.Longitude;
					addressPM.IsDefaultAddress = true;
				}
			}
			...
			DateTime billingDate, string docDesc, int? locationID)
		{
			...
			InvoiceEntry.Document.SetValueExt<ARInvoice.customerID>(invoice, customer.BAccountID);//SetValueExt needed for correct BaseCuryID (MBC) defaulting.
			...
			InvoiceEntry.Document.Cache.SetDefaultExt<ARInvoice.taxZoneID>(invoice);
			InvoiceEntry.Document.SetValueExt<ARInvoice.curyID>(invoice, project.BillingCuryID);
			...
			if (addressPM != null && addressPM.IsDefaultAddress != true)
			{
				...
				if (addressAR == null)
				{
					ARAddress billAddress = (ARAddress)InvoiceEntry.Billing_Address.Cache.CreateInstance();
					AddressAttribute.Copy(billAddress, addressPM);

					billAddress.IsValidated = addressPM.IsValidated;
					billAddress.BAccountAddressID = addressPM.BAccountAddressID;
					billAddress.IsDefaultAddress = true;

					InvoiceEntry.Billing_Address.Current = billAddress = InvoiceEntry.Billing_Address.Insert(billAddress);

					InvoiceEntry.Document.Current.BillAddressID = billAddress.AddressID;
				}
				else
				{
			...
			PMAddress shippingAddressPM = (PMAddress)PXSelect<PMAddress, Where<PMAddress.addressID, Equal<Required<PMProject.siteAddressID>>>>.Select(this, project.SiteAddressID);
			if (shippingAddressPM != null && shippingAddressPM.IsDefaultAddress != true && !string.IsNullOrWhiteSpace(project.RevenueTaxZoneID))
			{
				ARAddress addressAR = InvoiceEntry.Shipping_Address.Current;

				if (addressAR == null)
				{
					ARShippingAddress shipAddress = (ARShippingAddress)InvoiceEntry.Shipping_Address.Cache.CreateInstance();
					AddressAttribute.Copy(shipAddress, shippingAddressPM);

					shipAddress.IsValidated = shippingAddressPM.IsValidated;
					shipAddress.BAccountAddressID = addressPM.BAccountAddressID;
					shipAddress.IsDefaultAddress = true;

					InvoiceEntry.Shipping_Address.Current = shipAddress = InvoiceEntry.Shipping_Address.Insert(shipAddress);

					InvoiceEntry.Document.Current.ShipAddressID = shipAddress.AddressID;
				}
				else
				{
					addressAR.BAccountAddressID = addressPM.BAccountAddressID;
					addressAR.BAccountID = shippingAddressPM.BAccountID;
					addressAR.RevisionID = shippingAddressPM.RevisionID;
					addressAR.IsDefaultAddress = shippingAddressPM.IsDefaultAddress;
					addressAR.AddressLine1 = shippingAddressPM.AddressLine1;
					addressAR.AddressLine2 = shippingAddressPM.AddressLine2;
					addressAR.AddressLine3 = shippingAddressPM.AddressLine3;
					addressAR.City = shippingAddressPM.City;
					addressAR.State = shippingAddressPM.State;
					addressAR.PostalCode = shippingAddressPM.PostalCode;
					addressAR.CountryID = shippingAddressPM.CountryID;
					addressAR.IsValidated = shippingAddressPM.IsValidated;
					addressAR.Latitude = shippingAddressPM.Latitude;
					addressAR.Longitude = shippingAddressPM.Longitude;
					addressAR.IsDefaultAddress = true;
				}
			}
		...
		public virtual Tuple<string, bool> GetDocTypeAndRetainage(PMProject project, List<BillingData> value)
		{
			...
			{
				amount += data.Tran.CuryAmount.GetValueOrDefault();
			...
			else
			{
				...
				foreach (Group group in grouping.BreakIntoGroups(billingBase))
				{
					...
					foreach (PMTran tr in group.List)
					{
						...
						sumAmt += tr.Billable == true ? tr.ProjectCuryInvoicedAmount.GetValueOrDefault() : 0;
		...
		{
			return SelectFrom<PMCostCode>
				.Where<PMCostCode.costCodeID.IsEqual<P.AsInt>>
				.View
				.SelectSingleBound(this, null, line.CostCodeID);
		}

		private InventoryItem SelectInventoryItem(PMRevenueBudget line)
		{
			return SelectFrom<InventoryItem>
				.Where<InventoryItem.inventoryID.IsEqual<P.AsInt>>
				.View
				.SelectSingleBound(this, null, line.InventoryID);
		}
		...
		public virtual List<BillingData> BillFixPriceTask(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate)
		{
			...
			var budgetLines = select.Select(task.TaskID)
				.Cast<PXResult<PMRevenueBudget, PMAccountGroup>>()
				.ToList();

			ValidateProgressBillingBase(budgetLines.Select(r => new InvalidBillingBaseException.BillingComponent(
					project.BudgetLevel,
					r.GetItem<PMRevenueBudget>().ProgressBillingBase,
					task.TaskCD,
					r.GetItem<PMAccountGroup>().GroupCD,
					() => SelectInventoryItem(r),
					() => SelectCostCode(r)))
				);

			foreach (PXResult<PMRevenueBudget, PMAccountGroup> res in budgetLines)
			{
				PMRevenueBudget line = res;

				decimal unbilledAmount = 0;//In Project currency
				...
				if (Setup.Current.AutoCompleteRevenueBudget == true)
				{
					...
					if (ratios.TryGetValue(key, out decimal? ratio) && ratio != null)//autocalculated ratio.
					{
						unbilledAmount = Math.Max(0, (line.CuryRevisedAmount.GetValueOrDefault() * ratio.GetValueOrDefault() / 100m) - line.CuryInvoicedAmount.GetValueOrDefault());
					}
				}
				else
				{
					unbilledAmount = line.CuryAmountToInvoice.GetValueOrDefault();
				}
				...
				if (!string.IsNullOrEmpty(rule.DescriptionFormula))
				{
					...
					PMDataNavigator navigator = new PMDataNavigator(this, new List<PMTran>(new PMTran[1] { container }));
					CalculateFormulas(project, navigator, rule, container, out decimal? qty, out decimal? amt, out description);
				}
				else
				{
				...
				proformaLine.Qty = line.QtyToInvoice;
		...
		public virtual List<BillingData> BillPrepayment(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate)
		{
			...
			if(rule.Type == PMBillingType.Budget)
			{
				ValidateProgressBillingBase(budgetLines.Select(r => new InvalidBillingBaseException.BillingComponent(
					project.BudgetLevel,
					r.GetItem<PMRevenueBudget>().ProgressBillingBase,
		...
		private static void ValidateProgressBillingBase(IEnumerable<InvalidBillingBaseException.BillingComponent> lines)
		{
			var invalidLines = lines.Where(l => l.ProgressBillingBase.IsNotIn(ProgressBillingBase.Amount, ProgressBillingBase.Quantity)).ToList();

			if (invalidLines.Count > 0)
			{
				throw new InvalidBillingBaseException(invalidLines);
			}
		}
		...
		public virtual void Transform(PMProject project, List<PMTran> billingBase, PMBillingRule rule, PMTask task)
		{
			foreach (PMTran tran in billingBase)
			{
				...
				CalculateFormulas(project, navigator, rule, tran, out decimal? qty, out decimal? amt, out string desc);
		...
		public virtual decimal? GetPrice(PMTran tran)
		{
			...
			if (tran.InventoryID != null && tran.InventoryID != PMInventorySelectorAttribute.EmptyInventoryID)
			{
				...
				CM.CurrencyInfo dummy = GetCurrencyInfo(project, billingDate).GetCM();
		...
		public decimal? ConvertAmountToCurrency(string fromCuryID, string toCuryID, string rateType, DateTime? effectiveDate, decimal? value)
		{
			...
			IPXCurrencyService currencyService = ServiceLocator.Current.GetInstance<Func<PXGraph, IPXCurrencyService>>()(this);
		...
		public virtual CurrencyInfo GetCurrencyInfo(PMProject project, DateTime? effectiveDate)
		{
			...
			if (curyID == project.BaseCuryID)
			{
				...
				curyInfo.CuryMultDiv = CM.CuryMultDivType.Mult;
			}
			else if(project.CuryID == project.BillingCuryID && curyID != project.BaseCuryID)
			{
				//Ex: Base=USD, BillingCuryID = EUR, ProjectCuryID = EUR
				//use the fxRate of the Project,
				curyInfo = GetExtension<MultiCurrency>().GetCurrencyInfo(project.CuryInfoID);
			}
			else
			{
		...
		{
			PMTranComparer comparer = new PMTranComparer(rule.GroupByItem, rule.GroupByVendor, rule.GroupByDate, rule.GroupByEmployee, false, true, false, true);
		...
		public class InvalidBillingBaseException : PXException
		{
			public class BillingComponent
			{
				...
				private readonly Lazy<InventoryItem> Item;
				private readonly Lazy<PMCostCode> CostCode;
				public string ProgressBillingBase { get; }
				public BillingComponent(string revenueBudgetLevel,
					string progressBillingBase,
					string taskCD,
					string accountGroupCD,
					Func<InventoryItem> itemGetter,
					Func<PMCostCode> costCodeGetter)
				{
					ProgressBillingBase = progressBillingBase;
					RevenueBudgetLevel = revenueBudgetLevel;
					AccountGroupCD = accountGroupCD;
					TaskCD = taskCD;
					Item = new Lazy<InventoryItem>(itemGetter);
					CostCode = new Lazy<PMCostCode>(costCodeGetter);
				}

				public override string ToString()
				{
					bool includeCostCode = RevenueBudgetLevel.IsIn(BudgetLevels.CostCode, BudgetLevels.Detail);
					bool includeInventory = RevenueBudgetLevel.IsIn(BudgetLevels.Item, BudgetLevels.Detail);

					var builder = new System.Text.StringBuilder($@"{TaskCD ?? string.Empty}-{AccountGroupCD ?? string.Empty}");
					if(includeInventory)
					{
						builder.Append($"-{Item?.Value?.InventoryCD ?? string.Empty}");
					}

					if (includeCostCode)
					{
						builder.Append($"-{CostCode?.Value?.CostCodeCD ?? string.Empty}");
					}
					return builder.ToString();
				}
			}

			private static string GetFormatValue(IEnumerable<BillingComponent> lines)
			{
			...
			public InvalidBillingBaseException(IEnumerable<BillingComponent> lines) :
				base(Messages.ProgressBillingBaseNotValid, GetFormatValue(lines))
			{ }
