...
namespace PX.Objects.AP
{
	...
	public class VendorMaint : PXGraph<VendorMaint, VendorR>
	{
		...
		[PXCopyPasteHiddenFields(typeof(VendorR.vStatus))]
		...
		public virtual IEnumerable NewBillAdjustment(PXAdapter adapter)
		{
			...
			if (vendor != null && vendor.BAccountID > 0L)
			{
				...
				if (vendor.CuryID != null) invEntry.Document.Cache.SetValueExt<APInvoice.curyID>(newDoc, vendor.CuryID);
		...
		protected virtual void Vendor_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			VendorBalanceByBaseCurrency.AllowSelect = false;
			VendorBalanceByBaseCurrency.AllowDelete = false;
			VendorBalanceByBaseCurrency.AllowInsert = false;
			VendorBalanceByBaseCurrency.AllowUpdate = false;
		...
		protected virtual void Vendor_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
		{
			...
			if (row != null)
			{
				List<string> associatedTaxes = new List<string>();
				
				foreach (Tax tax in PXSelect<PX.Objects.TX.Tax, Where<PX.Objects.TX.Tax.taxVendorID, Equal<Current<Vendor.bAccountID>>>>.Select(this)) {
					associatedTaxes.Add(tax.TaxID);
				}

				if (associatedTaxes.Any())
				{
					e.Cancel = true;
					throw new PXException(Messages.TaxVendorDeleteErr, string.Join(", ", associatedTaxes));
				}
		...
		public class PaymentDetailsExt : PXGraphExtension<VendorMaint>
		{
			...
			protected virtual void _(Events.RowPersisting<VendorPaymentMethodDetail> e)
			{
				...
				bool validationResult = ValidateLocationPaymentMethod(e.Cache, null);
			...
			VendorR, VendorR.bAccountID, VendorR.primaryContactID>
		{
			...
			protected virtual void _(Events.FieldUpdated<VendorR, VendorR.acctName> e)
			{
				VendorR row = e.Row;
