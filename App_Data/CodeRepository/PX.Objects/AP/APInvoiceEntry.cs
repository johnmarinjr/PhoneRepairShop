...
{ 
	...
	public partial class APInvoiceEntry : APDataEntryGraph<APInvoiceEntry, APInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public class APInvoiceEntryDocumentExtension : InvoiceGraphExtension<APInvoiceEntry, APAdjust>
	    {
			...
			public override PXSelectBase<APAdjust> AppliedAdjustments => Base.Adjustments;
		...
		public class MultiCurrency : APMultiCurrencyGraph<APInvoiceEntry, APInvoice>
		{
			...
			protected override PXSelectBase[] GetChildren()
			{
				return new PXSelectBase[]
				{
					...
					Base.Adjustments,
					Base.Adjustments_Raw
				};
			}

			protected override bool AllowOverrideCury()
			{
				if (ShouldBeDisabledDueToDocStatus()) return false;
				else if (Base.Document.Current.DocType == APDocType.Prepayment)
					return !IsPOLinkedAPBill.Ensure(Base.Document.Cache, Base.Document.Current);
			...
			protected override void _(Events.RowUpdated<CurrencyInfo> e)
			{
				...
				if (e.Row?.CuryRate == null || Base.Document.Current == null) return;

				if (Base.Document.Current?.ReleasedOrPrebooked != true)
				{
					Base.RecalculateApplicationsAmounts(this);
				}
			}
			...
			protected override void _(Events.FieldUpdated<Extensions.MultiCurrency.Document, Extensions.MultiCurrency.Document.bAccountID> e)
			{
				...
				Base.SetDefaultsAfterVendorIDChanging(Base.Document.Cache, (APInvoice)base.Documents.Cache.GetMain(e.Row));
			}
		}
		...
		public class SkipUpdAdjustments : Common.Scopes.FlaggedKeyModeScopeBase<SkipUpdAdjustments>
		{
			public SkipUpdAdjustments(string key) : base(key) { }
		}
		#endregion
		...
		{
			APRegister reversingDoc = PXSelect<APRegister,
			...
			if (reversingDoc != null)
			{
				string localizedMsg = PXMessages.LocalizeFormatNoPrefix(
					...
					APDocType.GetDisplayName(APDocType.DebitAdj),
					reversingDoc.RefNbr);
					...
					origDoc.IsChildRetainageDocument())
			{
				...
				foreach (APTran itr in this.Transactions.Select())
				{
					if ((!string.IsNullOrEmpty(itr.ReceiptNbr) || !string.IsNullOrEmpty(itr.PONbr)) && 
						(!(itr.POOrderType == POOrderType.ProjectDropShip) || (!POLineType.IsService(itr.LineType) && itr.DropshipExpenseRecording == DropshipExpenseRecordingOption.OnReceiptRelease)))
					{
						isPOReferenced = true;
						break;
					}
		...
		public virtual IEnumerable ViewPODocument(PXAdapter adapter)
		{
			if (this.Transactions.Current != null)
			{
				...
				if (!String.IsNullOrEmpty(row.ReceiptNbr))
				{
					...
					docGraph.Document.Current = docGraph.Document.Search<POReceipt.receiptNbr>(row.ReceiptNbr, row.ReceiptType);
		...
		public virtual IEnumerable AutoApply(PXAdapter adapter)
		{
			...
			{
				foreach (PXResult<APAdjust, APPayment, Standalone.APRegisterAlias, CurrencyInfo> res in SelectAdjustmentsRaw())
				{
					APAdjust adj = res;
		...
		public PXSelectJoin<APPayment,InnerJoin<Standalone.APRegisterAlias,
			On<Standalone.APRegisterAlias.docType, Equal<APPayment.docType>,
			And<Standalone.APRegisterAlias.refNbr,Equal<APPayment.refNbr>>>,
				InnerJoin<CurrencyInfo,
					On<CurrencyInfo.curyInfoID, Equal<Standalone.APRegisterAlias.curyInfoID>>>>,
			Where<Standalone.APRegisterAlias.vendorID, Equal<Current<APInvoice.vendorID>>,
				And<Standalone.APRegisterAlias.docDate, LessEqual<Current<APInvoice.docDate>>,
					And<Standalone.APRegisterAlias.tranPeriodID, LessEqual<Current<APRegister.tranPeriodID>>,
						And<Standalone.APRegisterAlias.released, Equal<True>,
							And<Standalone.APRegisterAlias.openDoc, Equal<True>,
								And<Standalone.APRegisterAlias.hold, NotEqual<True>>>>>>>> AvailablePayments;

		private bool AppendAdjustmentsRawTail(APAdjust adj, PXResult<APPayment> paymentResult)
		{
			Adjustments_Raw.StoreTailResult(
				paymentResult,
				new[] { adj },
				Document.Current.DocType, Document.Current.RefNbr,//Sensfull parameters
				false, false, false //senseless parameters
				);
			return true;
		}

		protected IEnumerable<PXResult<APAdjust>> SelectAdjustmentsRaw()
		{
			Adjustments_Raw.Cache.Cached.OfType<APAdjust>().Join(
				AvailablePayments.Select(),
				_ => _.AdjgDocType + _.AdjgRefNbr,
				_ => _.GetItem<APPayment>().DocType + _.GetItem<APPayment>().RefNbr,
		...
		public virtual void ReverseInvoiceProc(APRegister doc)
		{
			...
			foreach (PXResult<APInvoice, CurrencyInfo, Terms, Vendor> res in APInvoice_CurrencyInfo_Terms_Vendor.Select(this, (object)doc.DocType, doc.RefNbr))
			{
				...
				if (invoice.RefNbr == null)
				{
					...
					if (duplicate != null)
					{
						...
						throw new PXException(ErrorMessages.RecordExists);
							Document.Cache.SetValueExt<APInvoice.refNbr>(invoice, duplicatefilter.Current.RefNbr);
						}
					}
					else
					Document.Cache.SetValueExt<APInvoice.refNbr>(invoice, invoice.OrigRefNbr);
				...
				if (info != null)
				{
					...
					b_info.BaseCuryID = info.BaseCuryID;
		...
		public virtual IEnumerable adjustments()
		{
			...
			foreach (PXResult<APAdjust, APPayment, Standalone.APRegisterAlias, CurrencyInfo> res in SelectAdjustmentsRaw())
			{
				...
				if (adj != null)
				{
					...
					PXSelectorAttribute.StoreResult<APAdjust.displayRefNbr>(Adjustments.Cache, adj, (Standalone.APRegisterAlias)res);
					PopulateAdjDisplayFields(res, res);
				...
				&& !IsImport)
			{
				...
				{
					foreach (PXResult<Standalone.APRegisterAlias, CurrencyInfo, Branch, APAdjust, APPayment> res in 
						PXSelectJoin<
							Standalone.APRegisterAlias,
								InnerJoin<CurrencyInfo, 
									On<CurrencyInfo.curyInfoID, Equal<Standalone.APRegisterAlias.curyInfoID>>, 
								InnerJoin<Branch,
									On<Branch.branchID, Equal<Standalone.APRegisterAlias.branchID>,
						...
						.Select(this))
					{
						...
						{
							VendorID = Document.Current.VendorID,
						...
						if (Adjustments.Cache.Locate(adj) == null)
						{
							...
							PopulateAdjDisplayFields(payment, adj);
							//TODO: try to remove on merge to 2022 (MC in PO) or remove Adjustments_Raw from GetChildren
							Adjustments_Raw.StoreTailResult(new PXResult<APPayment, Standalone.APRegisterAlias, CurrencyInfo>(res, res,  res),
								new[] { adj },
								adj.AdjdDocType, adj.AdjdRefNbr, false, false, false);
		...
		private static void PopulateAdjDisplayFields(APPayment payment, APAdjust adj)
		{
		...
		protected virtual void APInvoice_CuryDocBal_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (!SkipUpdAdjustments.IsActive(Document.Current?.DocType + Document.Current?.RefNbr)
					&& (status == PXEntryStatus.Inserted || status == PXEntryStatus.Updated)
				&& apdoc.PaymentsByLinesAllowed == true
				&& apdoc.DocType == APDocType.Invoice
				&& apdoc.Released != true && apdoc.Prebooked != true)
			{
				...
				{
					RecalculateApplicationsAmounts(GetExtension<MultiCurrency>());
				}
			}
		}
		...
		protected virtual void _(Events.RowUpdating<APInvoice> e)
		{
			...
			{
				APInvoice origDocument = PXSelect<APInvoice,
					Where<APInvoice.docType, Equal<Required<APInvoice.docType>>,
						And<APInvoice.refNbr, Equal<Required<APInvoice.refNbr>>>>>
					.Select(this, e.Row.OrigDocType, e.Row.OrigRefNbr);

				if (origDocument.FinPeriodID.CompareTo(e.NewRow.FinPeriodID) > 0)
				{
					e.Cache.RaiseExceptionHandling<APInvoice.finPeriodID>(e.NewRow, PeriodIDAttribute.FormatForDisplay(e.NewRow.FinPeriodID),
						new PXSetPropertyException(Messages.PeriodEarlierThanPeriodOfOriginalDocument, PXErrorLevel.Error, PeriodIDAttribute.FormatForError(origDocument.FinPeriodID)));
		...
		protected virtual void APInvoice_VendorLocationID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			try
			{
				...
				{
					SetDefaultsAfterVendorIDChanging(sender, (APInvoice)e.Row);
				}
			}
			finally
			{
		...
		{
			sender.SetDefaultExt<APInvoice.aPAccountID>(row);
			sender.SetDefaultExt<APInvoice.aPSubID>(row);
			sender.SetValue<APInvoice.payLocationID>(row, sender.GetValue<APInvoice.vendorLocationID>(row));
			sender.SetDefaultExt<APInvoice.payTypeID>(row);
			sender.SetDefaultExt<APInvoice.separateCheck>(row);
			sender.SetDefaultExt<APInvoice.taxCalcMode>(row);
			sender.SetDefaultExt<APInvoice.taxZoneID>(row);
			sender.SetDefaultExt<APInvoice.prebookAcctID>(row);
			sender.SetDefaultExt<APInvoice.prebookSubID>(row);
			sender.SetDefaultExt<APInvoice.paymentsByLinesAllowed>(row);
		}
		...
		protected virtual void APInvoice_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (ShouldPerformBalanceVerification(doc) && doc.CuryDocBal != doc.CuryOrigDocAmt)
			{
        ...
        protected virtual void APInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			else
			{
				...
				PXUIFieldAttribute.SetEnabled<APInvoice.hold>(cache, document, document.Scheduled == false);
			...
			if (document.Hold != true && document.Released != true && document.Prebooked != true && document.RoundDiff != 0)
			{
				if (checkControlTaxTotal || PXAccess.FeatureInstalled<FeaturesSet.invoiceRounding>() && document.TaxRoundDiff == 0)
				{
					...
					{
						currencyinfo.Current = currencyinfo.SelectSingle();
					}

						string baseCuryID = currencyinfo.Current?.BaseCuryID ?? Accessinfo.BaseCuryID;
					decimal roundingLimit = CM.CurrencyCollection.GetCurrency(baseCuryID).RoundingLimit.Value;

					if (Math.Abs(document.RoundDiff.Value) > Math.Abs(roundingLimit))
					{
		...
		protected virtual void APInvoice_TaxZoneID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			APInvoice row = e.Row as APInvoice;
			if (row != null)
			{
		...
		public virtual string GetDefaultTaxZone(APInvoice row)
		{
			...
			var vendorRelationsEnabled = PXAccess.FeatureInstalled<FeaturesSet.vendorRelations>();
			if (vendorRelationsEnabled == true)
			{
				Location suppliedByVendorLocation = PXSelect<Location, Where<Location.bAccountID, Equal<Required<Location.bAccountID>>, And<Location.locationID, Equal<Required<Location.locationID>>>>>
					.Select(this, row.SuppliedByVendorID, row.SuppliedByVendorLocationID);
				if (suppliedByVendorLocation != null)
				{
					if (!string.IsNullOrEmpty(suppliedByVendorLocation.VTaxZoneID))
					{
						result = suppliedByVendorLocation.VTaxZoneID;
					}
				}
			}
			else
		...
		protected virtual void APInvoice_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (doc.Released != true)
			{
				...
				if (ShouldPerformBalanceVerification(doc))
				{
					if (doc.CuryDocBal != doc.CuryOrigDocAmt && !this.IsImport)
					{
						sender.RaiseExceptionHandling<APInvoice.curyOrigDocAmt>(doc, doc.CuryOrigDocAmt, new PXSetPropertyException(Messages.DocumentOutOfBalance));
					}
		...
		{
			return doc.Hold != true && doc.Released != true && doc.Prebooked != true;
		}
                ...
                if (tran.ManualPrice != true || tran.CuryUnitCost == null)
                {
                    ...
                    if (tran.InventoryID != null && tran.UOM != null)
                    {
                        ...
                        vendorUnitCost = APVendorPriceMaint.CalculateUnitCost(
							...
							tran.InventoryID, 
							tran.SiteID, 
							((CurrencyInfo)currencyinfo.Select()).GetCM(), 
							tran.UOM,
							tran.Qty,
							date,
							tran.CuryUnitCost
		...
		protected virtual void _(Events.FieldDefaulting<APTran, APTran.box1099> e)
		{
			APTran tran = e.Row as APTran;
			if (tran == null) return;
		...
		public virtual List<POReceiptLineS> GetInvoicePOReceiptLines(POReceipt receipt, APInvoice doc)
		{
			return PXSelectReadonly2<POReceiptLineS,
					...
					Where<
						POReceiptLineS.receiptType, Equal<Required<POReceiptLineS.receiptType>>,
						And<POReceiptLineS.receiptNbr, Equal<Required<POReceiptLineS.receiptNbr>>,
				...
				.Select(this, receipt.ReceiptType, receipt.ReceiptNbr, doc.RefNbr, doc.DocType).RowCast<POReceiptLineS>().ToList();
		}
			...
			bool errorIfUnreleasedAPExists = true)
		{
			if (errorIfUnreleasedAPExists)
			{
				var unreleasedAPTrans = PXSelectJoinGroupBy<POReceiptLine,
					...
					.Select(this, receipt.ReceiptType, receipt.ReceiptNbr)
			...
			string orderCuryID = null;
			string termsID = null;
			string description = null;

			var receiptLinks = PXSelectGroupBy<POOrderReceiptLink,
				Where<
					POOrderReceiptLink.receiptType, Equal<Required<POOrderReceiptLink.receiptType>>,
					And<POOrderReceiptLink.receiptNbr, Equal<Required<POOrderReceiptLink.receiptNbr>>>>, 
				Aggregate<
			...
			if (receiptLinks.Any())
			{
				...
				if (usePOParameters)
				{
					...
					if (receiptLinks.Any(x => x.CuryID != firstOrderReceipt.CuryID))
						...
						orderCuryID = firstOrderReceipt.CuryID;
			...
			if (list != null)
			{
				...
				else
				{
					...
					Document.Cache.SetValueExt<APInvoice.payLocationID>(newdoc, newdoc.VendorLocationID);
					PXFieldState state3 = (PXFieldState)Document.Cache.GetStateExt<APInvoice.payLocationID>(newdoc);
					if (state3 != null && !string.IsNullOrEmpty(state3.Error))
						throw new PXException(state3.Error);
					...
					try
					{
						newdoc = PXCache<APInvoice>.CreateCopy(Document.Update(newdoc));
					}
					finally
					{
					...
					bool useCuryRateFromPOReceipt = usePOParameters && posetup.Current != null
						&& (orderCuryID == null || string.Equals(orderCuryID, receipt.CuryID));
					newdoc.CuryID = useCuryRateFromPOReceipt ? receipt.CuryID : (orderCuryID ?? defaultedCuryID);
					newdoc = Document.Update(newdoc);

					if (useCuryRateFromPOReceipt)
					{
		...
		public virtual void AttachPrepayment(List<POOrder> orders)
		{
			...
			if (Document.Current != null && (Document.Current.DocType == APDocType.Invoice || Document.Current.DocType == APDocType.CreditAdj) && Document.Current.Released == false && Document.Current.Prebooked == false)
			{
				...
				{
					decimal? invoiceAmtToPay = Document.Current.CuryDocBal - Adjustments.Select().Sum(a => ((APAdjust)a).CuryAdjdAmt);
					if (invoiceAmtToPay == 0) return;
							...
							.Select(this, payment.DocType, payment.RefNbr))
						{
								...
								&& lazyTrans.Value.Any(t => t.POOrderType == orderPrepay.OrderType && t.PONbr == orderPrepay.OrderNbr))
							{
								...
								if ((existing = Adjustments.Locate(adj)) == null ||
									Adjustments.Cache.GetStatus(adj).IsIn(PXEntryStatus.Deleted, PXEntryStatus.InsertedDeleted))
								{
								...
								if (!existing.CuryAdjdAmt.HasValue) throw new ArgumentNullException(nameof(existing.CuryAdjdAmt));
								...
								decimal? appAmt = new[] { existing.CuryDocBal, orderAppliedAmt, invoiceAmtToPay }.Min();
								if (appAmt == 0) continue;
								if (!appAmt.HasValue) throw new ArgumentNullException(nameof(appAmt));

								existing.CuryAdjdAmt = Math.Max((existing.CuryAdjdAmt + appAmt).Value, 0);
								existing = Adjustments.Update(existing);
								invoiceAmtToPay -= appAmt;
							}
						}
					}
				}
		...
		public virtual APTran AddPOReceiptLine(IAPTranSource aLine, HashSet<APTran> checkForDuplicates)
		{
			...
			if (areCurrenciesSame)
			{
				...
				newtran.CuryLineAmt = keepAmounts ? curyAmtToBill : billCuryInfo.RoundCury( curyAmtToBill ?? 0m);
				newtran.CuryDiscAmt = keepAmounts ? curyDiscAmt : billCuryInfo.RoundCury(curyDiscAmt ?? 0m);
				newtran.CuryRetainageAmt = keepAmounts ? curyRetainAmt : 0m;
			}
			else
			{
				newtran.CuryUnitCost = curyUnitCost = billCuryInfo.CuryConvCury(unitCost, this.commonsetup.Current.DecPlPrcCst.Value);
				newtran.CuryLineAmt = billCuryInfo.CuryConvCury(amtToBill.Value);
				newtran.CuryDiscAmt = billCuryInfo.CuryConvCury(discAmt.Value);
				newtran.CuryRetainageAmt = keepAmounts ? billCuryInfo.CuryConvCury(retainAmt.Value) : 0m;
			}
        ...
        public virtual void AutoRecalculateDiscounts()
		{
			...
			_discountEngine.AutoRecalculatePricesAndDiscounts(
				...
				: Document.Current.VendorLocationID, 
				Document.Current.DocDate,
				GetDefaultAPDiscountCalculationOptions(Document.Current) | DiscountEngine.DiscountCalculationOptions.EnableOptimizationOfGroupAndDocumentDiscountsCalculation);
			
				RecalculateTotalDiscount();
		}

		private void RecalculateApplicationsAmounts(MultiCurrency MultiCurrencyExt)
		{
			...
			{
				CurrencyInfo documentCurrencyInfo = MultiCurrencyExt.GetCurrencyInfo(application.AdjdCuryInfoID);
				decimal baseAdjustedAmount = documentCurrencyInfo.CuryConvBase(application.CuryAdjdAmt.Value);

				CurrencyInfo applicationCurrencyInfo = MultiCurrencyExt.GetCurrencyInfo(application.AdjgCuryInfoID);

				decimal curyAdjustingAmount = string.Equals(applicationCurrencyInfo.CuryID, documentCurrencyInfo.CuryID)
					? application.CuryAdjdAmt.Value
					: applicationCurrencyInfo.CuryConvCury(baseAdjustedAmount);

				decimal baseAdjustingAmount =
					Equals(applicationCurrencyInfo.CuryID, documentCurrencyInfo.CuryID) &&
					...
					? baseAdjustedAmount
					: applicationCurrencyInfo.CuryConvBase(curyAdjustingAmount);
		...
		public partial class APAdjust : PX.Objects.AP.APAdjust
		{
			...
			[PXDBLong()]
			[CurrencyInfo(typeof(AP.APInvoice.curyInfoID))]
