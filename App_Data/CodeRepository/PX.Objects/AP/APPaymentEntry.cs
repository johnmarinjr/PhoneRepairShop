	...
	public partial class APPaymentEntry : APDataEntryGraph<APPaymentEntry, APPayment>
	{
				...
				return new CurySourceMapping(typeof(CashAccount))
	    {
			...
			protected virtual void _(Events.FieldUpdated<APPayment, APPayment.cashAccountID> e)
			{
				...
				else SourceFieldUpdated<APPayment.curyInfoID, APPayment.curyID, APPayment.adjDate>(e.Cache, e.Row);
			}
		...
		public PXSelectJoin<
			...
			Where<Standalone.APAdjust.adjgDocType, Equal<Current<APPayment.docType>>,
				...
				And<APAdjust.released, Equal<Required<APAdjust.released>>>>>> 
			Adjustments_Balance;
		...
		{
			APAdjust adj = PXSelect<APAdjust, Where<APAdjust.noteID, Equal<Current<APTranPostBal.refNoteID>>>>.Select(this).RowCast<APAdjust>().SingleOrDefault();
			if (adj != null)
			{
				APInvoice invoice = PXSelect<APInvoice, Where<APInvoice.refNbr, Equal<Required<APAdjust.pPDDebitAdjRefNbr>>,
					And<APInvoice.docType, Equal<APDocType.debitAdj>>>>.Select(this,adj.PPDDebitAdjRefNbr)?.First();
		...
		protected virtual IEnumerable adjustments()
		{
			...
			if (Document.Current == null || (Document.Current.DocType != APDocType.Refund && Document.Current.DocType != APDocType.VoidRefund))
			{
						...
						ref totalRows))
				{
					...
					ret.Add(mapper.CreateResult(new PXResult<APAdjust, APInvoice, APTran>(res, fullInvoice, tran)));
				}
				PXView.StartRow = 0;
				return ret;
			}
		...
		private Dictionary<long, CurrencyInfo> createdInfo { get; } = new Dictionary<long, CurrencyInfo>();
		...
		protected virtual void AddBalanceCache(APAdjust adj, PXResult res)
		{
			...
			var adjd_info = PXResult.Unwrap<CM.CurrencyInfo2>(res);
			var register = PXResult.Unwrap<Standalone.APRegisterAlias>(res);
			CurrencyInfo_CuryInfoID.StoreResult(info_copy);
			CurrencyInfo_CuryInfoID.StoreResult(adjd_info);
