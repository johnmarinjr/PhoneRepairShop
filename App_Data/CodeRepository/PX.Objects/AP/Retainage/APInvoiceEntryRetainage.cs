...
namespace PX.Objects.AP
{
					...
					foreach (APInvoiceExt docLine in group)
					{
						...
						foreach (APTran detail in details)
						{
							...
							else
							{
								...
								curyLineAmt = Base.FindImplementation<IPXCurrencyHelper>().RoundCury((detail.CuryRetainageAmt ?? 0m) * retainagePercent);
							}
	...
	{
		public Condition IsNotRetenageApplied => GetOrCreate(b => b.FromBql<
			Not<APRegister.docType.IsEqual<APDocType.invoice>
		...
		>());
		public Condition IsNotInvoice => GetOrCreate(b => b.FromBql<
			APInvoice.docType.IsNotEqual<APDocType.invoice>
		>());
	}
					...
					.WithActions(actions =>
					{
						actions.Add<APInvoiceEntryRetainage>(g => g.releaseRetainage,
							c => c.InFolder(processingCategory, g => g.payInvoice)
								...
								.IsHiddenWhen(conditions.IsNotInvoice || apconditions.IsMigrationMode)
