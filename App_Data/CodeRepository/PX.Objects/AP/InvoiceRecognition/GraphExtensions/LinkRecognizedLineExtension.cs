...
namespace PX.Objects.AP.InvoiceRecognition
{
	...
	public class LinkRecognizedLineExtension : PXGraphExtension<APInvoiceRecognitionEntry>
	{
		internal static string[] APTranPOFields =
		{
			...
			nameof(APRecognizedTran.ProjectID),
			nameof(APRecognizedTran.TaskID),
			nameof(APRecognizedTran.CostCodeID)
		};
		...
		public PXSelectJoin<
			...
			Where<POReceiptLineS.receiptType, Equal<Required<LinkLineReceipt.receiptType>>,
				And<POReceiptLineS.receiptNbr, Equal<Required<LinkLineReceipt.receiptNbr>>,
			...
			else
			{
				...
				if ((res = linkLineFilter.AskExt(
						(graph, view) =>
						{
							...
							if (Base.Document.Current?.VendorLocationID != null)
								location = Base.VendorLocation.Select();
		...
		private static void LinkToReceipt(APRecognizedTran apTran, POReceiptLineS receipt)
		{
			...
			if (PXAccess.FeatureInstalled<CS.FeaturesSet.projectAccounting>())
			{
		...
		public virtual List<PXResult<POReceiptLineS, POReceipt>> GetAvailableReceiptLines(APRecognizedTran currentRecognizedTran, string pONbr, int? inventoryID, string uOM)
		{
			...
			foreach (LinkLineReceipt item in GetLinkLineReceipts(pONbr, inventoryID, uOM, Base.Document.Current?.VendorLocationID))
			{
				...
				{
					var res = (PXResult<POReceiptLineS, POReceipt>)ReceipLineLinked.Select(item.ReceiptType, item.ReceiptNbr, item.ReceiptLineNbr);
		...
		private PXResultset<LinkLineReceipt> GetLinkLineReceipts(string pONbr, int? inventoryID, string uOM, int? vendorLocationID)
		{
			...
			else
			{
				cmd.WhereAnd<Where<LinkLineReceipt.vendorID, Equal<Current<APRecognizedInvoice.vendorID>>,
					And<LinkLineReceipt.vendorLocationID, Equal<Required<APRecognizedInvoice.vendorLocationID>>>>>();
			}

			return cmd.Select(pONbr, pONbr, inventoryID, uOM, uOM, vendorLocationID);
		}
		...
		public virtual List<PXResult<POLineS, POOrder>> GetAvailableOrderLines(APRecognizedTran currentRecognizedTran, string pONbr, int? inventoryID, string uOM)
		{
			...
			foreach (LinkLineOrder item in GetLinkLineOrders(pONbr, inventoryID, uOM, Base.Document.Current?.VendorLocationID).RowCast<LinkLineOrder>().AsEnumerable()
		...
		private PXResultset<LinkLineOrder> GetLinkLineOrders(string pONbr, int? inventoryID, string uOM, int? vendorLocationID)
		{
			...
			else
			{
				cmd.WhereAnd<Where<LinkLineOrder.vendorID, Equal<Current<APRecognizedInvoice.vendorID>>,
					And<LinkLineOrder.vendorLocationID, Equal<Required<APRecognizedInvoice.vendorLocationID>>>>>();
			}
		...
		protected virtual void _(Events.RowSelected<APRecognizedInvoice> e)
		{
			...
			linkLine.SetEnabled(e.Row.DocType == APDocType.Invoice && e.Row.VendorID != null && e.Row.VendorLocationID != null &&
