...
namespace PX.Objects.AP.InvoiceRecognition.Feedback
{
	internal class DocumentFeedbackBuilder
	{
		...
		public DocumentFeedbackBuilder(PXCache recognizedInvoiceCache, HashSet<string> primaryFields, HashSet<string> detailFields)
		{
			...
			var invoiceCache = PXGraph.CreateInstance<PXGraph>().Caches[typeof(APInvoice)];
		...
		{
			var feedbackList = new List<DocumentFeedback>();
		...
		private DocumentFeedback GetRowsBoundFeedback(string detailViewName, short page, short table,
		...
		private DocumentFeedback GetColumnBoundFeedback(string detailViewName, short page, short table, string columnName, List<short> columnIndexes)
		{
			var detail = new Detail
			{
				Ocr = new List<DetailOcr>
		...
		{
			var feedback = new DocumentFeedback
			{
		...
		public DocumentFeedback ToRecordSavedFeedback(PXView primaryView, APInvoice primaryRow,
			PXView detailView, IEnumerable<APTran> detailRows, IEntitySearchService entitySearchService)
		{
			...
			var recordSavedFeedback = new DocumentFeedback
			{
				Documents = new List<Document>()
			};
			...
			IEntitySearchService entitySearchService)
		{
			foreach (var field in trackedFields)
			{
				...
				var fullTextTerm = isVendorInvoiceField ?
					GetVendorFullTextTerm(view.Cache, row, entitySearchService) :
					null;

				FillDocumentField(documentField, value, type, isVendorInvoiceField, fullTextTerm);
			}
		}
		...
		private (FieldTypes? Type, object Value) GetFieldInfo(PXCache cache, string fieldName, object row, bool isVendorInvoiceField)
		{
			...
			{
				value = cache.GetValue(row, fieldName);
				return (FieldTypes.Entity, value);
			}
		...
		private void FillDocumentField(TypedField field, object value, FieldTypes? type, bool isVendorInvoiceField, FullTextTerm fullTextTerm)
		{
			if (isVendorInvoiceField)
			{
				if (value is int entityIdInt)
				{
					field.EntityId = entityIdInt.ToString();
				}
		...
		private FullTextTerm GetVendorFullTextTerm(PXCache cache, object row, IEntitySearchService entitySearchService)
		{
