...
namespace PX.Objects.AP
{
	...
	public class PaymentRefAttribute : PXEventSubscriberAttribute, IPXRowPersistingSubscriber, IPXFieldDefaultingSubscriber, IPXFieldVerifyingSubscriber 
	{
		...
		{
			return sender.GetValueOriginal(Row, _FieldName) as string;
		}
	...
	public class APTaxAttribute : ManualVATAttribute
	{
		...
		public override object Insert(PXCache cache, object item)
		{
			...
			PXRowInserted inserted = delegate(PXCache sender, PXRowInsertedEventArgs e)
			{
				...
				PXSelect<APTax,
					...
					.StoreCached(cache.Graph, new PXCommandKey(new object[] { TranType, RefNbr }), recordsList);
					//.StoreResult(sender.Graph, recordsList, PXQueryParameters.ExplicitParameters(TranType, RefNbr));
			};
		...
		public override object Update(PXCache cache, object item)
		{
			...
			PXRowUpdated updated = delegate (PXCache sender, PXRowUpdatedEventArgs e)
			{
				var comparer = sender.GetComparer();
			...
			cache.Graph.RowUpdated.AddHandler<APTax>(updated);
			...
			{
				cache.Graph.RowUpdated.RemoveHandler<APTax>(updated);
			}
		}
		...
		protected virtual bool FilterParent(PXCache cache, object detrow)
		{
			...
			if (_ChildType == typeof(APTran))
			{
				...
				return orderReceipt != null && (orderReceipt.POType != tran.POOrderType || orderReceipt.PONbr != tran.PONbr
					|| (orderReceipt.ReceiptType != tran.ReceiptType || orderReceipt.ReceiptNbr != tran.ReceiptNbr) && tran.POAccrualType == POAccrualType.Receipt);
			}
			return false;
		}
		...
		protected override decimal? GetCuryTranAmt(PXCache sender, object row, string TaxCalcType)
		{
			...
			return MultiCurrencyCalculator.RoundCury(sender,row, (decimal)value);
		}
        ...
        protected virtual bool IsRoundingNeeded(PXGraph graph) 
        {
			...
			CM.Currency currency = MultiCurrencyCalculator.GetCurrentCurrency(graph);

			bool allowRounding = currency?.UseAPPreferencesSettings == false 
				?  currency.APInvoiceRounding != RoundingType.Currency
				: ((APSetup)cache.Current).InvoiceRounding != RoundingType.Currency;
        ...
        protected override void ParentSetValue(PXGraph graph, string fieldname, object value)
        {
            ...
            if (fieldname == _CuryDocBal && cache.Current != null && IsRoundingNeeded(graph))
            {
                ...
                decimal oldbaseval;
                decimal baseval;
				MultiCurrencyCalculator.CuryConvBase(ParentCache(graph), ParentRow(graph), (decimal)oldval, out oldbaseval);
				MultiCurrencyCalculator.CuryConvBase(ParentCache(graph), ParentRow(graph), (decimal)value, out baseval);
		...
		protected override List<object> SelectTaxes<WhereType>(PXGraph graph, object row, PXTaxCheck taxchk, params object[] parameters)
		{
			...
			object[] selectParameters = new object[]
			{
				GetDocDate(ParentCache(graph), row)
			};
			...
			switch (taxchk)
			{
				case PXTaxCheck.Line:
					...
					tails = CollectInvolvedTaxes<WhereType>(graph, taxDetails, selectTaxes, currents, parameters, selectParameters);

					int? lineNbr = (int?)graph.Caches[typeof(APTran)].GetValue<APTran.lineNbr>(row);

					foreach (APTax record in taxDetails)
					{
				...
				case PXTaxCheck.RecalcTotals:
					...
					tails = CollectInvolvedTaxes<WhereType>(graph, taxDetails, selectTaxes, currents, parameters, selectParameters);
					foreach (APTaxTran record in taxDetails)
					{
						InsertTax(graph, taxchk, record, tails, ret);
					}
					return ret;
	...
	public class PXDBVendorCuryAttribute : PXDBDecimalAttribute
	{
		...
		public PXDBVendorCuryAttribute(Type vendorID)
			: base(BqlCommand.Compose(typeof(Search2<,,>), typeof(Vendor.taxReportPrecision),
				typeof(LeftJoin<CM.Currency, On<CM.Currency.curyID, Equal<Vendor.curyID>>,
						LeftJoin<CM.Currency2, On<CM.Currency2.curyID, Equal<Current<AccessInfo.baseCuryID>>>>>),
				typeof(Where<,>), typeof(Vendor.bAccountID), typeof(Equal<>), typeof(Current<>), vendorID))
		{			
		}

		public PXDBVendorCuryAttribute(Type vendorID, Type branchID)
			: base(BqlCommand.Compose(typeof(Search2<,,>), typeof(Vendor.taxReportPrecision),
				typeof(LeftJoin<,,>), typeof(Branch), typeof(On<,>), typeof(Branch.branchID), typeof(Equal<>), typeof(Current<>), branchID,
				typeof(LeftJoin<CM.Currency, On<CM.Currency.curyID, Equal<Vendor.curyID>>,
						LeftJoin<CM.Currency2, On<CM.Currency2.curyID, Equal<Branch.baseCuryID>>>>),
				typeof(Where<,>), typeof(Vendor.bAccountID), typeof(Equal<>), typeof(Current<>), vendorID))
		{
		}
		...
		{
			dynamic result = item as PX.Data.PXResult<Vendor, CM.Currency, CM.Currency2>;
			if (result == null) result = item as PX.Data.PXResult<Vendor, Branch, CM.Currency, CM.Currency2>;
			...
			{
				return
					(short?)view.Graph.Caches[typeof(CM.Currency)].GetValue<CM.Currency.decimalPlaces>((CM.Currency)result) ??
					(short?)view.Graph.Caches[typeof(CM.Currency2)].GetValue<CM.Currency2.decimalPlaces>((CM.Currency2)result);
			}
        ...
        where WhereSelect : IBqlWhere, new()
	{
		...
		{
			foreach (PXResult<ChargeTable, CashAccountETDetail> paychargeresult in PXSelectJoin<ChargeTable,
				LeftJoin<CashAccountETDetail, 
					On<CashAccountETDetail.entryTypeID, Equal<EntryTypeID>,
			...
			{
				ChargeTable paycharge = paychargeresult.GetItem<ChargeTable>();
				CashAccountETDetail cashAccountEntryTypeDetail = paychargeresult.GetItem<CashAccountETDetail>();

				if(cashAccountEntryTypeDetail.EntryTypeID == null)
				{
					int? cashAccountID = (int?)_Graph.Caches[typeof(PaymentTable)].GetValue<CashAccountID>(_Graph.Caches[typeof(PaymentTable)].Current);
					var cashAccount = CashAccount.PK.Find(_Graph, cashAccountID);

					throw new PXException(CA.Messages.ChargeCantBeReversedEntryTypeNotConfiguredForCashAccount, paycharge.EntryTypeID, cashAccount.CashAccountCD);
				}
