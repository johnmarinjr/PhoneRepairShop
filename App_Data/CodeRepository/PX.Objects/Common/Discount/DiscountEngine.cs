...
namespace PX.Objects.Common.Discount
{
		...
		where TDiscountDetail : class, IBqlTable, IDiscountDetail, new()
	{
			...
			DiscountCalculationOptions discountCalculationOptions = DefaultDiscountCalculationParameters)
		{
			...
			if ((dLine.AutomaticDiscountsDisabled != true || discountCalculationOptions.HasFlag(DiscountCalculationOptions.ExplicitlyAllowToCalculateAutomaticLineDiscounts)) &&
				IsDiscountCalculationNeeded(cache, line, DiscountType.Line, discountDetails) &&
			...
			if (!discountCalculationOptions.HasFlag(DiscountCalculationOptions.DisableGroupAndDocumentDiscounts) && 
			    ((!cache.Graph.IsImport || discountCalculationOptions.HasFlag(DiscountCalculationOptions.CalculateDiscountsFromImport)) || cache.Graph.IsMobile)
			    && dLine.SkipDisc != true && dLine.AutomaticDiscountsDisabled != true)
			{
			...
			bool needDiscountID)
		{
			...
			HashSet<DiscountSequenceKey> applicableDiscountSequences = SelectApplicableEntityDiscounts(entities, DiscountType.Line, !needDiscountID, true);
				...
				if (recalcFilter.RecalcTarget == RecalcDiscountsParamFilter.AllLines)
				{
					...
					for (int i = 0; i < documentDetails.Count(); i++)
					{
						...
						if (discountCalculationOptions.HasFlag(DiscountCalculationOptions.EnableOptimizationOfGroupAndDocumentDiscountsCalculation))
						{
							...
							if (i != documentDetails.Count() - 1)
								dFields.SkipDisc = true;

							try
							{
								//We still need to call this for each line to calculate prices and line discounts. 
								RecalculatePricesAndDiscountsOnLine(cache, documentDetails[i], recalcFilter, discountCalculationOptions);
							}
							finally
							{
								dFields.SkipDisc = false;
							}
						}
						else
						{
		...
		protected virtual void RecalculatePricesAndDiscountsOnLine(PXCache cache, TLine line, RecalcDiscountsParamFilter recalcFilter, DiscountCalculationOptions discountCalculationOptions)
		{
			...
			if ((bool)recalcFilter.RecalcDiscounts &&
				(dFields.AutomaticDiscountsDisabled != true || recalcFilter.CalcDiscountsOnLinesWithDisabledAutomaticDiscounts == true || discountCalculationOptions.HasFlag(DiscountCalculationOptions.ExplicitlyAllowToCalculateAutomaticLineDiscounts)))
			{
				if (recalcFilter.OverrideManualDiscounts == true)
					dFields.ManualDisc = false;
				if (dFields.ManualDisc != true)
					dFields.DiscountID = null;
				if (recalcFilter.CalcDiscountsOnLinesWithDisabledAutomaticDiscounts == true)
					dFields.AutomaticDiscountsDisabled = false;
		...
		public virtual void RecalculateGroupAndDocumentDiscounts(
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			DiscountLineFields dLine = DiscountLineFields.GetMapFor(currentLine, cache);
			if (branchID == null || locationID == null || date == null || dLine.AutomaticDiscountsDisabled == true || !IsDiscountFeatureEnabled(discountCalculationOptions))
				return;
		...
		protected virtual bool SetGroupDiscounts(
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			if (!IsDiscountCalculationNeeded(cache, currentLine, DiscountType.Group, discountDetails))
				return false;
			...
			bool skipManual = false)
		{
			...
			foreach (TLine line in documentDetails)
			{
				...
				if (discountedLine.AutomaticDiscountsDisabled == true ||
					(discountedLine.DiscountID != null && cachedDiscountTypes[discountedLine.DiscountID].ExcludeFromDiscountableAmt))
					continue;
		...
		/// <summary>
		/// Links new group discount details with document lines. Preserves existing document discounts, but ignores existing group discounts.
		/// </summary>
		[Obsolete("Use MatchLinesWithDiscounts() instead.")]
		public virtual TwoWayLookup<TDiscountDetail, TLine> MatchLinesWithGroupDiscounts(
			...
			PXSelectBase<TDiscountDetail> discountDetails)
		{
			return MatchLinesWithDiscounts(cache, discountCodesWithApplicableLines, documentDetails, discountDetails, true, false);
		}

		/// <summary>
		/// Links new discount details with document lines.
		/// </summary>
		/// <param name="newDiscountCodesWithApplicableLines">Discount codes linked to document details lines.</param>
		/// <param name="ignoreExistingGroupDiscounts">Set true to ignore all existing group discounts. Lines will have links to new group discounts only.</param>
		/// <param name="ignoreExistingDocumentDiscounts">Set true to ignore all existing document discounts. Lines will have links to new document discounts only.</param>
		/// <returns>Returns TwoWayLookup with discount lines matched with document lines.</returns>
			...
			bool ignoreExistingDocumentDiscounts)
		{
			...
			foreach (TLine line in documentDetails)
			{
				...
				foreach (TDiscountDetail discount in newDiscountDetails)
				{
					...
					if ((discount.IsOrigDocDiscount == true || (discount.Type == DiscountType.Group && !ignoreExistingGroupDiscounts) || (discount.Type == DiscountType.Document && !ignoreExistingDocumentDiscounts)) 
						&& discountFields.DiscountsAppliedToLine != null && discountFields.DiscountsAppliedToLine.Contains(discount.LineNbr ?? 0))
					{
						discountsWithApplicableLines.Link(discount, line);
					}
					else if (discount.IsOrigDocDiscount != true && newDiscountCodesWithApplicableLines.RightValues.Contains(line) &&
						newDiscountCodesWithApplicableLines.LeftsFor(line).Contains(dsKey))
					{
						discountsWithApplicableLines.Link(discount, line);
					}
				}
		...
		/// </summary>
		/// <param name="documentDetailsSelectParams">Parameters for document details select query.</param>
		/// <param name="discountDetailsSelectParams">Parameters for discount details select query.</param>
		...
		{
			List<TDiscountDetail> discountDetails = GetDiscountDetailsByType(cache, discountDetailsSelect, null, discountDetailsSelectParams);
			if (!discountDetails.Any())
			{
				return new TwoWayLookup<TDiscountDetail, TLine>(leftComparer: new TDiscountDetailComparer());
			}
			List<TLine> documentDetails = GetDocumentDetails(cache, documentDetailsSelect, documentDetailsSelectParams);
		...
		public virtual TwoWayLookup<TDiscountDetail, TLine> GetListOfLinksBetweenDiscountsAndDocumentLines(PXCache cache, List<TLine> documentDetails, List<TDiscountDetail> discountDetails)
		{
			...
			{
				DiscountLineFields discountFields = DiscountLineFields.GetMapFor(line, cache);
				if (discountFields.AutomaticDiscountsDisabled == true)
					continue;
		...
		public virtual TwoWayLookup<TDiscountDetail, TLine> GetListOfDiscountsAppliedToLine(PXCache cache, TLine line, List<TDiscountDetail> discountDetails)
		{
			...
			if (discountFields.AutomaticDiscountsDisabled != true && discountFields.DiscountsAppliedToLine != null)
			{
				foreach (TDiscountDetail dDetail in discountDetails)
				{
		...
		/// </summary>
		/// <param name="lines">Query that allows to select document lines.</param>
		/// <param name="documentLinesSelectParams">Parameters for document lines select query.</param>
		...
		{
			List<TLine> documentDetails = GetDocumentDetails(cache, lines, documentLinesSelectParams);
		...
		public virtual void CalculateGroupDiscountRate(
			...
			bool recalcAll = true)
		{
			CalculateGroupDiscountRate(cache, documentDetails, currentLine, MatchLinesWithDiscounts(cache, newGroupDiscountCodesWithApplicableLines, documentDetails, discountDetails, true, false), recalculateTaxes, recalcAll);
		}
		...
		/// <summary>
		/// This methos verifies, whether the discount calculation is needed and will help to improve performance in the process
		/// Ex: If there are no discounts, there is no need to iterate through documentDetails and create mappers
		/// </summary>
		/// <param name="cache"></param>
		/// <param name="currentLine"></param>
		/// <param name="discountDetails"></param>
		/// <returns></returns>
		public bool IsDocumentDiscountRateCalculationNeeded(PXCache cache, TLine currentLine, PXSelectBase<TDiscountDetail> discountDetails)
		{			
			bool anyDiscount = discountDetails.Select().Any();

			if (!anyDiscount && !discountDetails.Cache.Cached.Any_())
			{
				AmountLineFields lineAmtFields = AmountLineFields.GetMapFor(currentLine, cache, false);
				DiscountLineFields discountFields = DiscountLineFields.GetMapFor(currentLine, cache);

				lineAmtFields.OrigDocumentDiscountRate = 1;
				lineAmtFields.DocumentDiscountRate = 1;
				lineAmtFields.OrigGroupDiscountRate = 1;
				lineAmtFields.GroupDiscountRate = 1;
				discountFields.DiscountsAppliedToLine = new ushort[] { };

				bool anyTaxAttributes = cache.GetAttributesReadonly<AmountLineFields.taxCategoryID>().OfType<TX.TaxAttribute>().Any();
				...
				foreach (TDiscountDetail discount in discountsLinkedToLine.LeftsFor(line).Where(x => x.Type.IsIn(DiscountType.Document, DiscountType.ExternalDocument) && x.SkipDiscount != true))
				{
					...
					else
					{
						foreach (TLine dline in discountsLinkedToLine.RightsFor(discount))
						{
							...
							if (dlineDiscountFields.AutomaticDiscountsDisabled != true &&
								(dlineDiscountFields.DiscountID == null || cachedDiscountTypes.ContainsKey(dlineDiscountFields.DiscountID) &&
			...
			bool skipParameters = false)
		{
			...
			foreach (TLine line in documentDetails)
			{
				AmountLineFields lineAmtFields = AmountLineFields.GetMapFor(line, cache, qtyDiscountByBaseUOM);
				DiscountLineFields discountFields = DiscountLineFields.GetMapFor(line, cache);
				if (discountFields.AutomaticDiscountsDisabled != true && (discountFields.DiscountID == null
			...
			foreach (TLine line in documentDetails)
			{
				...
				if (updLineTotal != 0m && totalDocumentDiscountAmt != 0m && lineAmtFields.CuryLineAmount != 0m
					&& (discountFields.AutomaticDiscountsDisabled != true &&
					(discountFields.DiscountID == null || cachedDiscountTypes.ContainsKey(discountFields.DiscountID) && !cachedDiscountTypes[discountFields.DiscountID].ExcludeFromDiscountableAmt)))
			...
			TLine line)
		{
			...
			ushort[] newValue = new ushort[0];
			if (discountFields.AutomaticDiscountsDisabled != true) //DiscountsAppliedToLine should always be reset to 0 for lines, excluded from discount calculation
			{
				...
				newValue = discountLineNbrs.Where(x => x.HasValue).Select(x => x.Value).ToArray();
			...
			if (discountFields.DiscountsAppliedToLine == null || !discountFields.DiscountsAppliedToLine.SequenceEqual(newValue))
			{
			...
			string type)
		{
			...
			newDiscountDetail.CuryDiscountAmt = RoundCury(cache, newDiscountDetail, rawValue);
		...
		protected virtual void SetDocumentDiscount(
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			if (!IsDiscountCalculationNeeded(cache, currentLine, DiscountType.Document, discountDetails))
			{
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			...
			if (cachedDiscountTypes.ContainsKey(manualDiscountID) && documentDetails.Count != 0)
			{
				...
				{
					var newDiscountCodesWithLines = new TwoWayLookup<DiscountSequenceKey, TLine>();
					foreach (TLine line in documentDetails)
					{
						...
						if (discountedLine.DiscountID != null)
							excludeFromDiscountableAmount = cachedDiscountTypes[discountedLine.DiscountID].ExcludeFromDiscountableAmt || discountedLine.AutomaticDiscountsDisabled == true;

						if (!excludeFromDiscountableAmount)
						{
							CombineDiscountsAndDocumentLines(cache, line, selectedGroupDiscounts, ref newDiscountCodesWithLines);
						}
					}

					foreach (DiscountSequenceKey applicableGroup in newDiscountCodesWithLines.LeftValues)
					{
						DiscountLineFields discountedLine = DiscountLineFields.GetMapFor(documentDetails[0], cache);
					...
					TwoWayLookup<TDiscountDetail, TLine> newDiscountDetailsMatchedWithLines = MatchLinesWithDiscounts(cache, newDiscountCodesWithLines, documentDetails, discountDetails, false, false);
					CalculateGroupDiscountRate(cache, documentDetails, currentLine, newDiscountDetailsMatchedWithLines, true, false);
				}
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			...
			if ((cache.Graph.IsCopyPasteContext || currentDiscountDetailLine.CuryDiscountAmt != oldDiscountDetailLine.CuryDiscountAmt) && 
				...
				currentDiscountDetailLine.DiscountPct = Math.Round(((currentDiscountDetailLine.CuryDiscountAmt ?? 0m) * 100 / currentDiscountDetailLine.CuryDiscountableAmt) ?? 0m, 6, MidpointRounding.AwayFromZero);
		...
		protected virtual UnitPriceVal GetUnitPrice(PXCache cache, TLine line, int? locationID, string curyID, DateTime date)
		{
			...
			if (arsetup.ApplyLineDiscountsIfCustomerPriceDefined == false || arsetup.ApplyLineDiscountsIfCustomerClassPriceDefined == false)
			{
				...
				if (eFields.CustomerID != null && eFields.InventoryID != null)
				{
					...
					SalesPriceItem salesPrice = GetSalesPrice(cache, 
		...
		{
			return IsDiscountCalculationNeeded(cache, line, discountType, null);
		}

		/// <summary>
		/// Checks if discount calculation is needed for the given combination of entity line and discount type
		/// </summary>
		/// <param name="cache"></param>
		...
		/// <param name="discountDetails"></param>
		/// <returns></returns>
		protected virtual bool IsDiscountCalculationNeeded(PXCache cache, TLine line, string discountType, PXSelectBase<TDiscountDetail> discountDetails)
		{
			ConcurrentDictionary<string, DiscountCode> cachedDiscountTypes = GetCachedDiscountCodes();
			...
			switch (discountType)
			{
				...
				case DiscountType.Document:
					...
					if (docLineAmountsFields.DocumentDiscountRate != 1m && (discountDetails == null || GetDiscountDetailsByType(cache, discountDetails, discountType).Any()))
			{
					return true;
			}
					break;
		...
		public HashSet<KeyValuePair<object, string>> GetDiscountEntitiesDiscounts(PXCache cache, TLine line, int? locationID, bool isLineOrGroupDiscount, int? branchID = null, int? inventoryID = null, int? customerID = null)
		{
			LineEntitiesFields lineEntities = LineEntitiesFields.GetMapFor(line, cache);
		...
		protected virtual void SumAmounts(PXCache cache, List<TLine> lines, out decimal totalLineAmt, out decimal curyTotalLineAmt)
		{
			...
			foreach (TLine line in lines)
			{
				...
				{
					if (discountedLine.AutomaticDiscountsDisabled == true || (discountedLine.DiscountID != null && cachedDiscountTypes[discountedLine.DiscountID].ExcludeFromDiscountableAmt))
