...
namespace PX.Objects.Common.Discount
{
	public abstract class DiscountEngine : PXGraph<DiscountEngine>
	{
		...
		protected virtual HashSet<DiscountSequenceKey> SelectApplicableEntityDiscounts(
			...
			bool skipManual)
		{
			return SelectApplicableEntityDiscounts(entities, discountType, skipManual, false);
		}
			...
			bool appliedToDR = false)
		{
			...
			foreach (DiscountSequenceKey discountSequence in allFoundDiscounts)
			{
				if (cachedDiscountTypes.ContainsKey(discountSequence.DiscountID)
					...
					&& (!appliedToDR || cachedDiscountTypes[discountSequence.DiscountID].IsAppliedToDR)
			...
			bool ignoreCurrency = false)
		{
			...
			if (!ignoreCurrency && dLine?.MappedLine != null
				&& dLine.Cache.GetValue(dLine.MappedLine, nameof(CM.CurrencyInfo.curyInfoID)) != null)
		...
		protected class DCCache : IPrefetchable
		{
			...
			private void StoreARDiscounts()
			{
				foreach (PXDataRecord discountType in PXDatabase.SelectMulti<ARDiscount>(
					...
					new PXDataField<ARDiscount.isAppliedToDR>(),
					...
					var type = new DiscountCode
								{
									...
									IsAppliedToDR = (bool)discountType.GetBoolean(3),
		...
		public static decimal GetLineDiscountOnly<TLine>(
			...
			where TLine : class, IBqlTable, new()
		{
			return DiscountEngineProvider.GetEngineFor<TLine, NoDiscountDetail>().GetLineDiscountOnlyImpl(
				cache,
				line,
				dLine,
				unitPrice,
		...
		public enum DiscountCalculationOptions
		{
			...
			[Description("Set this flag to enable calculation of automatic Line discounts when AutomaticDiscountsDisabled is enabled. Default state: not set")]
			ExplicitlyAllowToCalculateAutomaticLineDiscounts = 512
		}
