...
namespace PX.Objects.CN.ProjectAccounting.PO.GraphExtensions
{
    public class PoOrderEntryExt : PXGraphExtension<POOrderEntry>
    {
        ...
        protected virtual void _(Events.RowSelected<POOrder> e)
        {
            ...
            PXUIFieldAttribute.SetVisible<CostBudgetFilter.projectID>(ProjectItemFilter.Cache, null, !Base.RequireSingleProject(e.Row));
        }
        }
        ...
        protected virtual void _(Events.FieldUpdated<POOrder, POOrder.projectID> e)
        {
            ...
            if (Base.RequireSingleProject(e.Row))
        ...
        {
            // Acuminator disable once PX1074 SetupNotEnteredExceptionInEventHandlers [False positive.]
            if (Base.RequireSingleProject(Base.Document.Current) && ProjectDefaultAttribute.IsProject(Base, Base.Document.Current?.ProjectID))
            {
		...
		[PXOverride]
		public virtual IEnumerable CreateAPInvoice(PXAdapter adapter, CreateAPInvoiceDelegate handler)
		{
			if (Base.Document.Current != null &&
				 (POOrderType.IsNormalType(Base.Document.Current.OrderType)
					|| Base.Document.Current.OrderType == POOrderType.DropShip))
			{
				...
				{
					APSetup apsetup = PXSelect<APSetup>.Select(Base);
					if (apsetup.RequireSingleProjectPerDocument == true)
					{
						PMProject project = PXSelectorAttribute.Select<CostBudgetFilter.projectID>(Base.Document.Cache, Base.Document.Current) as PMProject;
						if (project.IsActive != true)
						{
							PXUIFieldAttribute.SetError<POOrder.projectID>(Base.Document.Cache, Base.Document.Current,
								PX.Objects.PO.Messages.ProjectIsNotActive, project.ContractCD);
						}
					}
				}
			}
