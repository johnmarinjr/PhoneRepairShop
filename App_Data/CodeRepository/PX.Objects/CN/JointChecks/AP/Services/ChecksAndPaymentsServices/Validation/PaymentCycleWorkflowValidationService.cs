...
namespace PX.Objects.CN.JointChecks.AP.Services.ChecksAndPaymentsServices.Validation
{
    ...
    {
        public PaymentCycleWorkflowValidationService(APPaymentEntry graph, IJointCheckErrorHandlingStrategy jointCheckErrorHandlingStrategy)
		...
		public void Validate(List<APAdjust> adjustments)
		{
			foreach (var adjustment in adjustments)
			{
				...
				if (invoiceJCExt.IsJointPayees == true)
				{
					...
					{
						var errorHandlingParams = new ShowErrorOnlyParams(JointCheckMessages.PaymentCycleWorkflowIsStarted, adjustment);
						errorHandlingStrategy.HandleError<APAdjust.adjdRefNbr>(errorHandlingParams);
					}
				}
			}
		}
