...
namespace PX.Objects.CN.JointChecks.AP.Services.ChecksAndPaymentsServices.Validation
{
    ...
    {
        public DebitAdjustmentsValidationService(APPaymentEntry graph, IJointCheckErrorHandlingStrategy jointCheckErrorHandlingStrategy)
        ...
        private void ShowErrorMessageOnDebitAdjustments(decimal? totalVendorPreparedBalance)
        {
            ...
            {
                var showErrorHandlingParams = new ShowErrorOnlyParams(JointCheckMessages.MaxDebitAmountApplication, 
                    adjustment, totalVendorPreparedBalance);
                errorHandlingStrategy.HandleError<APAdjust.curyAdjgAmt>(showErrorHandlingParams);
            }
            var errorHandlingParams = 
                new ThrowErrorOnlyParams(JointCheckMessages.MaxDebitAmountApplication, Graph.Adjustments.Cache.DisplayName);
