...
namespace PX.Objects.CN.JointChecks.AP.GraphExtensions.PaymentEntry
{
    public class ApPaymentEntryReverseExtension : PaymentRevertBaseExtension
    {
        ...
        {
            var originalAdjustment =  PXSelect<APAdjust, 
                    Where<APAdjust.noteID, Equal<Current<APTranPostBal.refNoteID>>>>
                .SelectSingleBound(Base, new object[]{});;
        ...
        {
            var billReferenceNumbers = Base.APPost.SelectMain()
                .Select(adj => adj.RefNbr).Cast<object>().ToArray();
