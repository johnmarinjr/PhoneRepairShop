...
namespace PX.Objects.CT
{
	public class CTBillEngine : PXGraph<CTBillEngine>, IContractInformation
	{
		...
		public PXSetupOptional<CM.CMSetup> cmsetup; 
				...
				foreach (UsageData item in invData.UsageData.Where(IsBillable).OrderBy(item=>item.ContractDetailsLineNbr))
				{
					...
					if (item.IsTranData != false)
					{
						...
						else if (item.PriceOverride != null)
						{
							...
							tran.CuryExtPrice = invoiceEntry.FindImplementation<IPXCurrencyHelper>().GetDefaultCurrencyInfo().RoundCury(extPriceRaw);
						}
					}
				...
				if (data.Count > 0)
				{
					...
					foreach (UsageData item in data.Where(IsBillable).OrderBy(item => item.ContractDetailsLineNbr))
					{
						...
						else // item.IsTranData == false
						{
							...
							tran.CuryExtPrice = invoiceEntry.FindImplementation<IPXCurrencyHelper>().GetDefaultCurrencyInfo().RoundCury(extPrice);
						}
		...
		protected virtual bool ProcessDollarUsage(Contract contract, TranNotePair tran, string UOM, decimal? BillableQty, int? EmployeeID, PXResult<ContractDetail, ContractItem, InventoryItem> res, List<UsageData> addedData)
		{
			...
			if (billing != null && item != null && item.DepositItemID != null)
			{
				...
				if (availableDeposit.TryGetValue(item.DepositItemID, out available))
				{
					...
					decimal amount = CM.PXDBCurrencyAttribute.BaseRound(this, (decimal)(baseQty * billing.FixedRecurringPriceVal));
					...
					else if (available > 0m && baseQty > 0m)
					{
						...
						amount = CM.PXDBCurrencyAttribute.BaseRound(this, (decimal)((baseQty - fixedBaseQty) * billing.UsagePriceVal));
						available = -amount;
					}
					else
					{
						...
						amount = CM.PXDBCurrencyAttribute.BaseRound(this, (decimal)(baseQty * billing.UsagePriceVal));
						available -= amount;
					}
					...
					if (deposit != null)
					{
						deposit.DepositUsedTotal += CM.PXDBCurrencyAttribute.BaseRound(this, (decimal)(availableDeposit[item.DepositItemID] - available));
		...
		{
			SetBillingTarget(contract, out Customer customer, out Location location);

			DateTime effectiveDate = (DateTime) (/*tranData.TranDate ?? */Accessinfo.BusinessDate);
			PXCache<CM.CurrencyInfo> ciCache = this.Caches<CM.CurrencyInfo>();
			CM.CurrencyInfo ci = (CM.CurrencyInfo)ciCache.Insert(new CM.CurrencyInfo
			{
