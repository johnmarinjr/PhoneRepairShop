...
namespace PX.Objects.CT
{
	public class TemplateMaint : PXGraph<TemplateMaint, ContractTemplate>
	{
		...
		protected virtual void ContractTemplate_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			if (e.Row == null) return;
			ContractTemplate row = e.Row as ContractTemplate;
			ValidateRefundPeriod(sender, row);
		}
        ...
        private int GetMaximumDurationInDays(ContractTemplate contract)
        {
			switch (contract.DurationType)
			{
				...
				case Contract.durationType.Monthly:
					return contract.Duration.Value * 31;
		...
		{
			if (contract != null && contract.Refundable == true && contract.Type != Contract.type.Unlimited)
			{
				if (contract.RefundPeriod > GetMaximumDurationInDays(contract))
				{
					sender.RaiseExceptionHandling<ContractTemplate.refundPeriod>(contract, contract.RefundPeriod,
						new PXSetPropertyException(Messages.RefundDurationExceed));
				}
			}
		}
		#endregion
