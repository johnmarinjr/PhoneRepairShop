...
namespace PX.Objects.CR
{
	...
	public class CRPopupValidator
	{
		...
		public class Generic<Table> : CRPopupValidator where Table : class, IBqlTable, new()
		{
			...
			{
				WebDialogResult? result = AskExt(initializeHandler, reset, additionalValidators);
				return TryValidate(additionalValidators?.ToArray()) ? result : null;
			}

			public WebDialogResult AskExt(PXView.InitializePanel initializeHandler = null, bool reset = true, IEnumerable<CRPopupValidator> additionalValidators = null)
			{
				...
				return Filter.AskExt(initalizer, reset);
			}
		}
	}
