...
namespace PX.Objects.CR
{
	public class CREmailActivityMaint : CRBaseActivityMaint<CREmailActivityMaint, CRSMEmail>
	{
		...
		protected virtual IEnumerable send(PXAdapter adapter)
		{
			...
			VerifyEmailFieldsOrThrow(activity);
			Actions.PressSave();
		...
		[PXButton(Tooltip = EP.Messages.CancelSendingTooltip)]
		...
		[PXUIField(DisplayName = "Select Template", MapEnableRights = PXCacheRights.Select)]
		...
		public virtual void loadEmailSource()
		{
			...
			if (res == WebDialogResult.OK)
			{
				...
				var graphType = PXBuildManager.GetType(PXSiteMap.Provider.FindSiteMapNodeByScreenID(notification.ScreenID).GraphType, false);
				var templateGraph = PXGraph.CreateInstance(graphType);
				var type = templateGraph.PrimaryItemType;
				object row = new EntityHelper(this).GetEntityRow(type, Message.Current.RefNoteID);
				if (row == null)
				{
					row = new EntityHelper(this).GetEntityRow(Message.Current.RefNoteID, true);
					if (row == null) return;
					type = row.GetType();
					PXPrimaryGraphAttribute.FindPrimaryGraph(this.Caches[type], true, ref row, out graphType);
					type = row.GetType(); // it could be changed
					templateGraph = PXGraph.CreateInstance(graphType);
				}

				PXSelectBase generalInfo;
				if (!templateGraph.Views.ContainsKey(GeneralInfoSelect.ViewName))
				{
					generalInfo = new GeneralInfoSelect(templateGraph);
				...
				else
				{
					...
					bool isReplyEmail = Message.Current.ParentNoteID != null && Message.Current.Outgoing == true;
					var signature = GetSignature(!isReplyEmail);

					if (!string.IsNullOrEmpty(signature))
					{
						Message.Current.Body = Tools.AppendToHtmlBody(Message.Current.Body, "<br />", signature);
					}
				}
			}
		...
		[PXRestrictor(typeof(Where<EMailAccount.isActive, Equal<True>>), ErrorMessages.EmailAccountNotActive, ShowWarning = true)]
		[PXDefault]
		[PXMergeAttributes(Method = MergeMethod.Merge)]
		...
		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PM.ProjectTask(typeof(PMTimeActivity.projectID), "TA", DisplayName = "Project Task", AllowNull = true)]
		...
		protected virtual void CRSMEmail_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			LoadEmailSource.SetEnabled(!isIncome && row.MPStatus == MailStatusListAttribute.Draft && row.RefNoteID != null);
		...
		private void VerifyEmailFieldsOrThrow(CRSMEmail row)
		{
			var res = true;
