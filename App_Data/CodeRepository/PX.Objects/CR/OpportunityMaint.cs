...
namespace PX.Objects.CR
{
    public class OpportunityMaint : PXGraph<OpportunityMaint>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public virtual IEnumerable CreateQuote(PXAdapter adapter)
		{
			foreach (CROpportunity opportunity in adapter.Get<CROpportunity>())
			{
				...
				if ((QuoteInfo.Current.QuoteType == CRQuoteTypeAttribute.Distribution))
					PXLongOperation.StartOperation(this, () => clone.CreateNewQuote(opportunity, QuoteInfo.Current, result));
				else
					PXLongOperation.StartOperation(this, () => clone.CreateNewProjectQuote(opportunity, QuoteInfo.Current, result));
		...
		protected virtual void CreateNewSalesQuote(QuoteMaint graph, CROpportunity opportunity, CreateQuotesFilter param, WebDialogResult result)
		{
			...
			{
				CloneView(Billing_Contact.View, graph, quote.QuoteID, nameof(CRBillingContact.ContactID));
			...
			{
				CloneView(Billing_Address.View, graph, quote.QuoteID, nameof(CRBillingAddress.AddressID));
		...
		public class ContactAddress : CROpportunityContactAddressExt<OpportunityMaint>
		{
			...
			protected override void _(Events.FieldUpdated<Extensions.CROpportunityContactAddress.Document, Extensions.CROpportunityContactAddress.Document.bAccountID> e)
			{
				...
				BAccount bAccount = PXSelect<BAccount, Where<BAccount.bAccountID, Equal<Current<CROpportunity.bAccountID>>>>.SelectSingleBound(Base, null);

				var opportunity = e.Row.Base as CROpportunity;

				if (bAccount != null)
				{
					Base.Opportunity.Cache.SetValueExt<CROpportunity.taxZoneID>(opportunity, bAccount.TaxZoneID);
				}

				Base.Opportunity.Cache.SetDefaultExt<CROpportunity.curyID>(opportunity);
																...
																.Select(Base).RowCast<CROpportunityRevision>())
				{
					...
					if (bAccount != null)
					{
						Base.OpportunityRevision.Cache.SetValueExt<CROpportunityRevision.taxZoneID>(opportunityRevision, bAccount.TaxZoneID);
					}
