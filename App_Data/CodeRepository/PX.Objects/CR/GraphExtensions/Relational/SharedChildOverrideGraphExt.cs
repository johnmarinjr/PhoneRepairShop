...
namespace PX.Objects.CR.Extensions.Relational
{
		...
		where TThis : SharedChildOverrideGraphExt<TGraph, TThis>
	{
		...
		{
			UpdateRelated(e.NewRow, e.Row);
		}

		public virtual void UpdateRelated(Document row, Document oldRow)
		{
			...
			if (row.ChildID != oldRow.ChildID)
			{
				...
				if (row.ChildID == null)
				{
					...
					{
						relatedChild = GetChildByID(oldRow.ChildID);
						child = relatedChild != null
							? (Child)ChildDocument.Cache.CreateCopy(relatedChild)
							: (Child)ChildDocument.Cache.CreateInstance();
					}
