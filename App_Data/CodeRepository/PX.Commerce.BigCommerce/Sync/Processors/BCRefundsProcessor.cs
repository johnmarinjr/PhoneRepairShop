...
namespace PX.Commerce.BigCommerce
{
	...
	public class BCRefundsProcessor : BCOrderBaseProcessor<BCRefundsProcessor, BCRefundsBucket, MappedRefunds>
	{
		...
		public override EntityStatus GetBucketForImport(BCRefundsBucket bucket, BCSyncStatus syncstatus)
		{
			...
			orderData.Refunds = orderRefundsRestDataProvider.GetAll(orderData.Id.ToString())?.ToList() ?? new List<OrderRefund>();
			...
			orderData.OrdersCoupons = orderCouponsRestDataProvider.GetAll(syncstatus.ExternID)?.ToList() ?? new List<OrdersCouponData>();
			orderData.OrderProducts = orderProductsRestDataProvider.GetAll(syncstatus.ExternID)?.ToList() ?? new List<OrdersProductData>();
			orderData.Taxes = orderTaxesRestDataProvider.GetAll(syncstatus.ExternID).ToList()?.ToList() ?? new List<OrdersTaxData>();
			orderData.Transactions = orderTransactionsRestDataProvider.GetAll(syncstatus.ExternID)?.ToList() ?? new List<OrdersTransactionData>();
					...
					if (transaction?.IsDeclined == false)
					{
						...
						else
						{
							...
							else // if refund payment gateway matches original payment 
							{
								...
								{
									helper.AddCreditCardProcessingInfo(currentPayment, refundPayment, ccrefundTransaction.Event, ccrefundTransaction.PaymentInstrumentToken, ccrefundTransaction.CreditCard);
		...
		public virtual void CreateRefundOrders(BCRefundsBucket bucket, MappedRefunds existing)
		{
			...
			Where<SOOrderType.orderType, Equal<Required<SOOrderType.orderType>>>>.Select(this, currentBindingExt.ReturnOrderType).Cast<PXResult<SOOrderType, SOOrderTypeOperation>>().FirstOrDefault();
			...
			foreach (OrderRefund data in refunds)
			{
					...
					if (data.TotalTax > 0 && orderData.Taxes.Count > 0)// if acumatica original SO has tax and Refunds has tax then process tax
				{
					...
					string taxType = helper.DetermineTaxType(orderData.Taxes.Select(i => i.Name).ToList());
					foreach (OrdersTaxData tax in orderData.Taxes)
					{
						...
						var taxAmount = helper.RoundToStoreSetting(taxable * tax.Rate / 100);
						if (inserted == null)
						{
				...
				else
				{
					if (existing.Local.TaxDetails?.Count > 0 && data.TotalTax > 0 && orderData.Taxes.Count == 0)// In case of full refunds order taxes count become zero
					{
						...
						foreach (TaxDetail tax in existing.Local.TaxDetails)
						{
							...
							var taxAmount = helper.RoundToStoreSetting((decimal)(data.TotalTax * tax.TaxRate.Value / taxRateTotal)); // just get tax amount based on totaltax inrefund and taxrate from Acumatica salesorder
							if (inserted == null)
							{
								impl.TaxDetails.Add(new TaxDetail()
								{
				...
				if (payments != null)
					foreach (var transaction in payments)
					{
						...
						refundPayment.Hold = false.ValueField();
		...
		public virtual Dictionary<string, decimal?> AddSOLine(BCRefundsBucket bucket, SalesOrder impl, OrderRefund data, MappedRefunds existing, StringValue branch, SalesOrder presentCROrder)
		{
			...
			foreach (var item in data.RefundedItems.Where(x => x.ItemType == RefundItemType.Product || x.ItemType == RefundItemType.GiftWrapping))
			{
				...
				{
					inventoryCD = helper.GetInventoryCDByExternID(
