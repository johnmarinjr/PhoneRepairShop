...
namespace PX.Commerce.BigCommerce.API.REST
{
	...
	[CommerceDescription(BigCommerceCaptions.OrderData, FieldFilterStatus.Filterable, FieldMappingStatus.ImportAndExport)]
	...
	public class OrderData : BCAPIEntity
	{
		public OrderData()
		{
			...
			MetaFields = new List<OrdersMetaFieldData>();
		}
		...
		public virtual string StoreDefaultCurrencyCode { get; set; }
		...
		public virtual decimal StoreDefaultToTransactionalExchangeRate { get; set; }
		...
		[CommerceDescription(BigCommerceCaptions.OrderProducts, FieldFilterStatus.Skipped, FieldMappingStatus.Import)]
		...
		[CommerceDescription(BigCommerceCaptions.TotalExcludingTax, FieldFilterStatus.Filterable, FieldMappingStatus.ImportAndExport)]
		...
		[CommerceDescription(BigCommerceCaptions.TotalIncludingTax, FieldFilterStatus.Filterable, FieldMappingStatus.ImportAndExport)]
		...
		[CommerceDescription(BigCommerceCaptions.Metafields, FieldFilterStatus.Skipped, FieldMappingStatus.Skipped)]
		public virtual IList<OrdersMetaFieldData> MetaFields { get; set; }
		public bool ShouldSerializeMetaFields()
		{
			return false;
		}
		...
		public override string CalculateHash()
		{
			...
			sb.Append(this.Id ?? 0);
			sb.Append(this.TotalIncludingTax);
			sb.Append(this.TotalTax);
			sb.Append(this.DiscountAmount);
			sb.Append(this.CouponDiscount);
			foreach (var item in this.OrderProducts ?? new List<OrdersProductData>())
			{
				sb.Append(item.Id ?? 0);
				sb.Append(item.Quantity + item.QuantityRefund);
			}
			foreach (var address in this.OrderShippingAddresses ?? new List<OrdersShippingAddressData>())
			{
				sb.Append(address.FirstName ?? string.Empty);
				sb.Append(address.LastName ?? string.Empty);
				sb.Append(address.Street1 ?? string.Empty);
				sb.Append(address.ZipCode ?? string.Empty);
				sb.Append(address.FirstName ?? string.Empty);
				sb.Append(address.Phone ?? string.Empty);
			}
