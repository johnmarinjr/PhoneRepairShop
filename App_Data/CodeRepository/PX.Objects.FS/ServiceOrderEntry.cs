...
namespace PX.Objects.FS
{
    public class ServiceOrderEntry : ServiceOrderBase<ServiceOrderEntry, FSServiceOrder>, IGraphWithInitialization
    {
        ...
        #endregion
        #region FSAppointment CacheAttached
        [PXMergeAttributes(Method = MergeMethod.Merge)]
        [PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.DisplayName), "Billable Total")]
        ...
        public PXSetup<FSContractPeriod>.Where<
               Where<
                   ...
                   And<Current<FSBillingCycle.billingBy>, Equal<FSBillingCycle.billingBy.ServiceOrder>>>>> BillServiceContractPeriod;
        ...
        public static void UpdateServiceOrderUnboundFieldsInt(PXGraph graph, FSServiceOrder fsServiceOrderRow, FSBillingCycle fsBillingCycleRow, PXGraph appointmentGraph, FSAppointment fsAppointmentRow, bool disableServiceOrderUnboundFieldCalc, bool calcPrepaymentAmount = true)
        {
            ...
            if (fsServiceOrderRow.SOID != null
                && (fsBillingCycleRow == null 
                    || fsServiceOrderRow.BillingBy == null
                    || fsServiceOrderRow.BillingBy != fsBillingCycleRow.BillingBy))
            {
            ...
            FSSrvOrdType srvOrdType = FSSrvOrdType.PK.Find(graph, fsServiceOrderRow.SrvOrdType);
            if (srvOrdType == null
                || srvOrdType.Behavior == FSSrvOrdType.behavior.Values.Quote
                || srvOrdType.Behavior == FSSrvOrdType.behavior.Values.InternalAppointment)
            {
                fsServiceOrderRow.BillingBy = null;
            }
            else
            {
                ...
                {
                    if (fsServiceOrderRow.BillingBy != null && fsServiceOrderRow.BillingBy != fsBillingCycleRow.BillingBy)
                    {
                        //throw new PXInvalidOperationException();
                    }

                    fsServiceOrderRow.BillingBy = fsServiceOrderRow.BillingBy ?? fsBillingCycleRow.BillingBy;
                }
            }

            if (fsServiceOrderRow.BillingBy == null 
                && fsServiceOrderRow.BillCustomerID != null
                && srvOrdType != null
                && srvOrdType.Behavior != FSSrvOrdType.behavior.Values.Quote
                && srvOrdType.Behavior != FSSrvOrdType.behavior.Values.InternalAppointment)
            {
            ...
            PXResultset<FSAppointment> appointmentList = PXSelect<FSAppointment,
                        Where<
                        ...
                        .Select(appointmentGraph ?? graph, fsServiceOrderRow.SOID, currentAppointmentID, currentAppointmentID);
            ...
            foreach (FSAppointment row in appointmentList)
            {
            ...
            if (fsServiceOrderRow.BillingBy == ID.Billing_By.APPOINTMENT)
            {
            ...
            if (graph is ServiceOrderEntry)
            {
                ...
                fsServiceOrderRow.CuryEffectiveCostTotal = 0;

                foreach (FSProfitability row in soGraph.GetProfitabilityRecords(fsServiceOrderRow))
                {
                    fsServiceOrderRow.CuryEffectiveCostTotal += row.CuryExtCost;
                }

                if (fsServiceOrderRow.CuryEffectiveBillableLineTotal != null && fsServiceOrderRow.CuryEffectiveCostTotal != null ) { 
                    decimal curyProfit = Math.Round((decimal)fsServiceOrderRow.CuryEffectiveBillableLineTotal, 2) - Math.Round((decimal)fsServiceOrderRow.CuryEffectiveCostTotal, 2);

                    if (fsServiceOrderRow.CuryEffectiveLogBillableTranAmountTotal != null) 
                    {
                        curyProfit += Math.Round((decimal)fsServiceOrderRow.CuryEffectiveLogBillableTranAmountTotal, 2);
                    }

                    fsServiceOrderRow.ProfitPercent = fsServiceOrderRow.CuryEffectiveCostTotal == 0.0m ? 0.0m : (curyProfit / fsServiceOrderRow.CuryEffectiveCostTotal) * 100;
                }
            }
		...
		protected virtual void _(Events.FieldUpdated<FSServiceOrder, FSServiceOrder.billContractPeriodID> e)
		{
                ...
                && BillServiceContractRelated.Current?.BillingType == FSServiceContract.billingType.Values.StandardizedBillings)
            {
                ...
                {
                    if (fsServiceOrderRow.BillingBy == ID.Billing_By.SERVICE_ORDER)
                    {
        ...
        public class SalesTax : TaxGraph<ServiceOrderEntry, FSServiceOrder>
        {
            ...
            protected virtual void _(Events.FieldDefaulting<FSServiceOrder, FSServiceOrder.taxZoneID> e)
            {
				...
				e.NewValue = ((ServiceOrderEntry)e.Cache.Graph).GetDefaultTaxZone(row);
			}
            #endregion
        }
        #endregion
			...
			if (row == null)
                ...
                if (customerLocation != null)
                {
                    ...
                    else
                    {
                        ...
                        {
						result = TaxBuilderEngine.GetTaxZoneByZip(this, address.PostalCode);
                        }
                    }
                }
