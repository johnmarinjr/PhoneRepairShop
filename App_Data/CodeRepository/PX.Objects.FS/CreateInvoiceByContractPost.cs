...
namespace PX.Objects.FS
{
    public class CreateInvoiceByContractPost : PXGraph<CreateInvoiceByContractPost>
    {
        ...
        protected virtual IEnumerable contracts()
        {
            ...
            var results = PXSelectJoin<ContractPeriodToPost,
                    ...
                    InnerJoinSingleTable<Customer,
                        ...
                        And2<
                            Where2<
                                Where<ContractPeriodToPost.billingType, Equal<FSServiceContract.billingType.Values.standardizedBillings>,
                                And<
                                ...
                                Or<Where2<
                                        Where<ContractPeriodToPost.billingType, Equal<FSServiceContract.billingType.Values.fixedRateBillings>,
                                            Or<ContractPeriodToPost.billingType, Equal<FSServiceContract.billingType.Values.fixedRateAsPerformedBillings>>>,
                                    And<
                                        Where<CurrentValue<InvoiceContractPeriodFilter.upToDate>, IsNull,
                                        Or<ContractPeriodToPost.startPeriodDate, LessEqual<CurrentValue<InvoiceContractPeriodFilter.upToDate>>>>>>>>,
                        And <
            ...
            foreach (PXResult<ContractPeriodToPost, FSServiceOrder, FSAppointment> it in results)
            {
                ...
                if (contractPeriodToPostRow.BillingType == FSServiceContract.billingType.Values.StandardizedBillings
                        && (fsServiceOrderRow?.SOID != null || fsAppointmentRow?.AppointmentID != null))
                {
                    continue;
                }
                else
        ...
        public CreateInvoiceByContractPost()
        {
            ...
            Contracts.SetProcessDelegate(
                delegate (List<ContractPeriodToPost> contractPeriodToPostRows)
                {
                    ...
                    IInvoiceContractGraph fixedRateInvoiceGraph = null;
                    ...
                    foreach (ContractPeriodToPost contractPeriodToPostRow in contractPeriodToPostRows)
                    {
                        ...
                        if (contractPeriodLines.Count > 0 && fsServiceContractRow != null && fsContractPeriodRow !=null)
                        {
                            ...
                            if(fixedRateInvoiceGraph == null
                                && fsServiceContractRow.IsFixedRateContract == true)
                            {
                                if(fsSetupRow.ContractPostTo == ID.Batch_PostTo.AR)
                                {
                                    fixedRateInvoiceGraph = invoiceGraph;
                                }
                                else
                                {
                                    fixedRateInvoiceGraph = PXGraph.CreateInstance<ARInvoiceEntry>().GetExtension<SM_ARInvoiceEntry>();
                                }
                            }
                            ...
                            try
                            {
                                using (var ts = new PXTransactionScope())
                                {
                                    ...
                                    IInvoiceContractGraph invoiceContractGraph = (fsServiceContractRow.BillingType == FSServiceContract.billingType.Values.StandardizedBillings) ? invoiceGraph : fixedRateInvoiceGraph;
                                    fsContractPostDocRow = invoiceContractGraph.CreateInvoiceByContract(graphCreateInvoiceByServiceOrderPost, filter.InvoiceDate, filter.InvoiceFinPeriodID, fsContractPostBatchRow, fsServiceContractRow, fsContractPeriodRow, contractInvoiceLines);
                                    ...
                                    InsertFixedRateBillRelatedDocuments(fsServiceContractRow, fsContractPostDocRow.ContractPeriodID);
                            ...
                            catch (PXOuterException ex)
                            {
                                ...
                                {
                                    errorMessage += Environment.NewLine + err;
                                }

                                PXProcessing<ContractPeriodToPost>.SetError(rowIndex, errorMessage);
                            }
                            catch (Exception e)
                            {
        ...
        public virtual void InsertFixedRateBillRelatedDocuments(FSServiceContract fsServiceContractRow, int? contractPeriodID)
        {
            if (fsServiceContractRow.BillingType != FSServiceContract.billingType.Values.FixedRateBillings)
            {
                return;
            }

            PXCache cacheFSBillHistory = Caches[typeof(FSBillHistory)];

            foreach (PXResult<FSAppointment, FSContractPostDoc, FSBillHistory> row in 
                    PXSelectJoin<FSAppointment, 
                        InnerJoin<FSContractPostDoc,
                                On<FSContractPostDoc.serviceContractID, Equal<FSAppointment.billServiceContractID>,
                                And<FSContractPostDoc.contractPeriodID, Equal<FSAppointment.billContractPeriodID>>>,
                        InnerJoin<FSBillHistory,
                                On<FSBillHistory.childDocType, Equal<FSContractPostDoc.postDocType>,
                                And<FSBillHistory.childRefNbr, Equal<FSContractPostDoc.postRefNbr>>>>>,
                        ...
                        .Select(this, fsServiceContractRow.ServiceContractID, contractPeriodID))
            {
                FSAppointment appointment = (FSAppointment)row;
                FSBillHistory contractBillHistory = (FSBillHistory)row;

                FSBillHistory billHistory = new FSBillHistory();

                billHistory.ServiceContractRefNbr = contractBillHistory.ServiceContractRefNbr;
                billHistory.SrvOrdType = appointment.SrvOrdType;
                ...
                cacheFSBillHistory.Insert(billHistory);
            }

            foreach (PXResult<FSServiceOrder, FSContractPostDoc, FSBillHistory> row in
                    PXSelectJoin<FSServiceOrder,
                    InnerJoin<FSContractPostDoc,
                            On<FSContractPostDoc.serviceContractID, Equal<FSServiceOrder.billServiceContractID>,
                            And<FSContractPostDoc.contractPeriodID, Equal<FSServiceOrder.billContractPeriodID>>>,
                    InnerJoin<FSBillHistory,
                            On<FSBillHistory.childDocType, Equal<FSContractPostDoc.postDocType>,
                            And<FSBillHistory.childRefNbr, Equal<FSContractPostDoc.postRefNbr>>>>>,
                    ...
                    .Select(this, fsServiceContractRow.ServiceContractID, contractPeriodID))
            {
                ...
                billHistory.ServiceContractRefNbr = contractBillHistory.ServiceContractRefNbr;
                billHistory.SrvOrdType = serviceOrder.SrvOrdType;
        ...
        public virtual List<ContractInvoiceLine> GetInvoiceLines(List<ContractInvoiceLine> invoiceLine, List<ContractInvoiceLine> contractPeriodLines, List<ContractInvoiceLine> contractInvoiceDetLines, string PostTo)
        {
            ...
            foreach (ContractInvoiceLine periodDet in periodInvoiceLines)
            {
                if (periodDet.Processed != true)
                {
                    ...
                    if (periodDet.Qty >= 0 
                            || periodDet.BillingType == FSServiceContract.billingType.Values.FixedRateBillings
                            || periodDet.BillingType == FSServiceContract.billingType.Values.FixedRateAsPerformedBillings)
                    { 
