...
namespace PX.Objects.FS
{
        ...
        where TPrimary : class, PX.Data.IBqlTable, new()
    {
        ...
        public class ServiceOrderAppointments_View : PXSelect<FSAppointment,
                                                     Where<
                                                         FSAppointment.srvOrdType, Equal<Current<FSServiceOrder.srvOrdType>>,
                                                         And<FSAppointment.soRefNbr, Equal<Current<FSServiceOrder.refNbr>>>>,
                                                     OrderBy<
        ...
        public class AppointmentDetails_View : PXOrderedSelect<FSAppointment, FSAppointmentDet,
                                               Where<
													FSAppointmentDet.srvOrdType, Equal<Current<FSAppointment.srvOrdType>>,
		...
		{
			return serviceOrderDetails.Select()
										.RowCast<FSSODet>()
										.Where(x => (x.SiteID != (int?)serviceOrderDetails.Cache.GetValueOriginal<FSSODet.siteID>(x)
													|| x.SiteLocationID != (int?)serviceOrderDetails.Cache.GetValueOriginal<FSSODet.siteLocationID>(x)))
										.ToList();
		}

		public virtual void UpdateDetailSiteLocationInAppt(AppointmentEntry graphAppointmentEntry, List<FSSODet> detailsForUpdate)
		{
			...
			foreach (FSSODet soDetRow in detailsForUpdate)
			{
				foreach (FSAppointmentDet apptDetRow in graphAppointmentEntry.AppointmentDetails.Select()
																							.RowCast<FSAppointmentDet>()
																							.Where(x => x.SODetID == soDetRow.SODetID))
				{
					graphAppointmentEntry.AppointmentDetails.Current = apptDetRow;

					foreach (FSApptLineSplit apptSplitRow in graphAppointmentEntry.Splits
															.Select()
															.RowCast<FSApptLineSplit>()
															.Where(x => x.LineNbr == apptDetRow.LineNbr
																&& x.InventoryID == apptDetRow.InventoryID))
					{
						if (apptSplitRow != null)
						{
							graphAppointmentEntry.Splits.Delete(apptSplitRow);

							apptDetRow.LotSerialNbr = null;
							graphAppointmentEntry.AppointmentDetails.Update(apptDetRow);
						}
					}
                                                                ...
                                                                bool forceAppointmentCheckings)
        {
            ...
            if (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update)
            {
				...
				List<FSSODet> detailsForUpdate = DetailSiteLocationForUpdate(serviceOrderDetails);
                    ...
                    if (serviceOrderAppointments.Select().Count() > 0)
                    {
                        ...
                        foreach (FSAppointment fsAppointmentRow in serviceOrderAppointments.Select())
                        {
                            if (fsAppointmentRowBeingSaved == null || fsAppointmentRowBeingSaved.AppointmentID != fsAppointmentRow.AppointmentID)
                            {
								...
								if (detailsForUpdate.Count > 0)
								{
									UpdateDetailSiteLocationInAppt(graphAppointmentEntry, detailsForUpdate);
								}
				...
				ValidateServiceContractDates(cacheServiceOrder, fsServiceOrderRow, graphServiceOrderEntry.BillServiceContractRelated.Current);
			}
            else
            {
                ...
                where DAC : class, IBqlTable, IFSSODetBase, new()
        {
			...
			if (fsServiceOrderRow?.AllowInvoice == false)
			{
				isEnabled = true;
			}

			SetEnabledAndPersistingCheck<FSSODet.branchID>(cache, eRow, eventType, isEnabled, persistingCheck: null);
                                                                       ...
                                                                       FSServiceContract fsServiceContractRow = null)
        {
            ...
            EnableDisable_LineType(cacheAppointmentDet, fsAppointmentDetRow, fsSetupRow, fsAppointmentRow, fsSrvOrdTypeRow, fsServiceContractRow);
                                                     ...
                                                     FSServiceOrder fsServiceOrder)
        {
            ...
            {
                throw new PXRedirectToBoardRequiredException(Paths.ScreenPaths.MULTI_EMPLOYEE_DISPATCH, GetServiceOrderUrlArguments(fsServiceOrder), PXBaseRedirectException.WindowMode.Same);
            }
            else
            {
                                                   ...
                                                   bool allowCustomerChange = false)
        {
            ...
            bool customerRequired = fsServiceOrderRow.BAccountRequired == true;
            bool contactRequired = fsSrvOrdTypeRow.RequireContact == true;
            bool internalSOType = fsSrvOrdTypeRow.Behavior == FSSrvOrdType.behavior.Values.InternalAppointment;
            bool enableServiceContract = PXAccess.FeatureInstalled<FeaturesSet.equipmentManagementModule>()
                                                        || PXAccess.FeatureInstalled<FeaturesSet.routeManagementModule>();

            bool enableContractPeriod = enableServiceContract 
            ...
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.locationID>(cacheServiceOrder, fsServiceOrderRow, customerRequired && !isAllowedForInvoiceOrInvoiced);

            PXUIFieldAttribute.SetEnabled<FSServiceOrder.billServiceContractID>(cacheServiceOrder, fsServiceOrderRow, enableServiceContract
                                                                                                                        && fsServiceOrderRow.AllowInvoice == false);
            PXUIFieldAttribute.SetVisible<FSServiceOrder.billServiceContractID>(cacheServiceOrder, fsServiceOrderRow, enableServiceContract);
            PXUIFieldAttribute.SetVisible<FSServiceOrder.billContractPeriodID>(cacheServiceOrder, fsServiceOrderRow, enableContractPeriod && fsServiceOrderRow.BillServiceContractID != null);
            ...
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.customerID>(cacheServiceOrder, fsServiceOrderRow, cacheServiceOrder.GetStatus(fsServiceOrderRow) == PXEntryStatus.Inserted
																											&& customerRequired && detailsCount == 0
            ...
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.billCustomerID>(cacheServiceOrder, fsServiceOrderRow, noClosedOrCompletedAppointments && unrestrictedAppointmentEdition
																												&& !internalSOType && !isAllowedForInvoiceOrInvoiced);
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.billLocationID>(cacheServiceOrder, fsServiceOrderRow, noClosedOrCompletedAppointments && unrestrictedAppointmentEdition
																												&& !internalSOType && !isAllowedForInvoiceOrInvoiced);
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.branchID>(cacheServiceOrder, fsServiceOrderRow, noClosedOrCompletedAppointments && unrestrictedAppointmentEdition && !isAllowedForInvoiceOrInvoiced);
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.branchLocationID>(cacheServiceOrder, fsServiceOrderRow, noClosedOrCompletedAppointments && unrestrictedAppointmentEdition && !isAllowedForInvoiceOrInvoiced);
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.projectID>(cacheServiceOrder, fsServiceOrderRow, noClosedOrCompletedAppointments && unrestrictedAppointmentEdition && !isAllowedForInvoiceOrInvoiced);
            ...
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.dfltProjectTaskID>(cacheServiceOrder, fsServiceOrderRow, noClosedOrCompletedAppointments && unrestrictedAppointmentEdition && !ProjectDefaultAttribute.IsNonProject(fsServiceOrderRow.ProjectID) && !isAllowedForInvoiceOrInvoiced);
            ...
            PXUIFieldAttribute.SetEnabled<FSServiceOrder.taxZoneID>(cacheServiceOrder, fsServiceOrderRow, !isAllowedForInvoiceOrInvoiced);
			PXUIFieldAttribute.SetEnabled<FSServiceOrder.custPORefNbr>(cacheServiceOrder, fsServiceOrderRow, !isAllowedForInvoiceOrInvoiced);
			PXUIFieldAttribute.SetEnabled<FSServiceOrder.custWorkOrderRefNbr>(cacheServiceOrder, fsServiceOrderRow, !isAllowedForInvoiceOrInvoiced);
			PXUIFieldAttribute.SetEnabled<FSServiceOrder.allowOverrideContactAddress>(cacheServiceOrder, fsServiceOrderRow, !(fsServiceOrderRow.CustomerID == null && fsServiceOrderRow.ContactID == null) && !isAllowedForInvoiceOrInvoiced);
			PXUIFieldAttribute.SetEnabled<FSServiceOrder.salesPersonID>(cacheServiceOrder, fsServiceOrderRow, !isAllowedForInvoiceOrInvoiced);
			PXUIFieldAttribute.SetEnabled<FSServiceOrder.commissionable>(cacheServiceOrder, fsServiceOrderRow, !isAllowedForInvoiceOrInvoiced);
		}
        ...
        public virtual void SetBillCustomerAndLocationID(PXCache cache, FSServiceOrder fsServiceOrderRow)
        {
			...
			if (fsServiceOrderRow.BillServiceContractID != null && fsServiceOrderRow.BillServiceContractID != 0)
			{
				FSServiceContract serviceContract = FSServiceContract.PK.Find(cache.Graph, fsServiceOrderRow.BillServiceContractID);
				if (serviceContract != null && serviceContract.ServiceContractID != null)
				{
					billCustomerID = serviceContract.BillCustomerID;
					billLocationID = serviceContract.BillLocationID;
				}
			}
			if (billCustomerID == null && (bAccountRow == null || bAccountRow.Type != BAccountType.ProspectType))
			{
            ...
            where TField : IBqlField
        {
            if (row == null
                ...
                || fsSrvOrdTypeRow.Behavior == FSSrvOrdType.behavior.Values.Quote
                || fsSrvOrdTypeRow.Behavior == FSSrvOrdType.behavior.Values.InternalAppointment
				|| (row is FSServiceOrder
					&& (row as FSServiceOrder).Quote == true ))
            {
                return true;
            }
        ...
        public virtual void CreatePrepaymentDocument(FSServiceOrder fsServiceOrderRow, FSAppointment fsAppointmentRow, out PXGraph target, string paymentType = ARPaymentType.Payment)
        {
            ...
            {
                arPaymentRow.CuryOrigDocAmt = fsServiceOrderRow.CuryDocTotal ?? 0m;
            }
            else
            {
                arPaymentRow.CuryOrigDocAmt = graphARPaymentEntry
                    .FindImplementation<IPXCurrencyHelper>()
                    .GetDefaultCurrencyInfo()
                    .CuryConvCury(fsServiceOrderRow.DocTotal.Value);
            }
                                                              ...
                                                              FSAppointment fsAppointmentRow)
        {
			...
			bool isTimeEntryEnabled = string.IsNullOrEmpty(fsLogRow.TimeCardCD)
										|| fsLogRow.TimeActivityStatus == ActivityStatusListAttribute.Open
										|| fsLogRow.TimeActivityStatus == ActivityStatusListAttribute.Completed;
			...
			PXUIFieldAttribute.SetEnabled<FSAppointmentLog.dateTimeEnd>(cache, fsLogRow, isTimeDurationNegative == false && isTimeEntryEnabled);
			PXUIFieldAttribute.SetEnabled<FSAppointmentLog.dateTimeBegin>(cache, fsLogRow, isTimeEntryEnabled);
        ...
        public virtual void EnableDisable_LineType(PXCache cache, FSAppointmentDet fsAppointmentDetRow, FSSetup fsSetupRow, FSAppointment fsAppointmentRow, FSSrvOrdType fsSrvOrdTypeRow, FSServiceContract fsServiceContractRow)
        {
            // TODO: AC-142850
            ...
            switch (fsAppointmentDetRow.LineType)
            {
                ...
                case ID.LineType_ServiceTemplate.NONSTOCKITEM:
                    ...
                    PXUIFieldAttribute.SetVisibility<FSSODet.contractRelated>(cache, null, showStandardContractColumns ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
                    PXUIFieldAttribute.SetVisibility<FSSODet.coveredQty>(cache, null, showStandardContractColumns ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
                    PXUIFieldAttribute.SetVisibility<FSSODet.extraUsageQty>(cache, null, showStandardContractColumns ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
                    PXUIFieldAttribute.SetVisibility<FSSODet.curyExtraUsageUnitPrice>(cache, null, showStandardContractColumns ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
            ...
            where SODetType : FSSODet, new()
        {
            ...
            if (fsSODetRow != null)
            {
                ...
                graphAppointment.CopyAppointmentLineValues<AppointmentDetType, FSSODet>(cacheAppointmentDet,
                                                                                        ...
                                                                                        fsAppointmentRow == null || fsAppointmentRow.BillServiceContractID == null);
		...
		public virtual void ValidateContact(FSServiceOrder row)
		{
			...
			Contact contact = Contact.PK.Find(this, row.ContactID);
        ...
        public static void CalculateBillHistoryUnboundFieldsInt(PXCache cache, FSBillHistory fsBillHistoryRow)
        {
            ...
            else if (fsBillHistoryRow.ChildEntityType == FSEntityType.SOInvoice || fsBillHistoryRow.ChildEntityType == FSEntityType.SOCreditMemo)
            {
                ...
                if (soInvoiceRow != null)
                {
					...
					fsBillHistoryRow.ChildAmount = soInvoiceRow.CuryOrigDocAmt;
        ...
        public virtual void InsertDeleteRelatedFixedRateContractBill(PXCache docCache, object document, PXDBOperation operation, PXSelectBase<FSBillHistory> invoiceRecords)
        {
            bool billContractInfoUpdated = false;
            int? billServiceContractID = null;
            int? billContractPeriodID = null;

            if (document is FSAppointment)
            {
                FSAppointment appointment = (FSAppointment)document;
                billServiceContractID = appointment?.BillServiceContractID;
                billContractPeriodID = appointment?.BillContractPeriodID;
            }
            else if (document is FSServiceOrder)
            {
                FSServiceOrder serviceOrder = (FSServiceOrder)document;
                billServiceContractID = serviceOrder?.BillServiceContractID;
                billContractPeriodID = serviceOrder?.BillContractPeriodID;
            }

            if (operation == PXDBOperation.Update
                    && (billServiceContractID != (int?)docCache.GetValueOriginal<FSAppointment.billServiceContractID>(document)
                        || billContractPeriodID != (int?)docCache.GetValueOriginal<FSAppointment.billContractPeriodID>(document)))
            {
                DeleteRelatedFixedRateContractBills(docCache, document, invoiceRecords);
                billContractInfoUpdated = true;
            }

            if (billServiceContractID != null
                    && (operation == PXDBOperation.Insert || billContractInfoUpdated))
            {
                InsertRelatedFixedRateContractBill(docCache, document, invoiceRecords);
            }
        }

        public virtual void DeleteRelatedFixedRateContractBills(PXCache docCache, object document, PXSelectBase<FSBillHistory> invoiceRecords)
        {
            int? oldBillServiceContractID = (int?)docCache.GetValueOriginal<FSAppointment.billServiceContractID>(document);
            ...
            FSServiceContract serviceContract = PXSelect<FSServiceContract,
                                                    ...
                                                    And<FSServiceContract.isFixedRateContract, Equal<True>>>>
                                                .Select(this, oldBillServiceContractID);

            if (serviceContract != null)
            {
                foreach (FSBillHistory row in invoiceRecords.Select().RowCast<FSBillHistory>().Where(x => x.ServiceContractRefNbr == serviceContract.RefNbr))
                {
                    invoiceRecords.Delete(row);
                }

                invoiceRecords.Cache.Persist(PXDBOperation.Delete);
            }
        }

        public virtual void InsertRelatedFixedRateContractBill(PXCache docCache, object document, PXSelectBase<FSBillHistory> invoiceRecords)
        {
            ...
            int? billServiceContractID = serviceOrder?.BillServiceContractID ?? appointment?.BillServiceContractID;
            int? billContractPeriodID = serviceOrder?.BillContractPeriodID ?? appointment?.BillContractPeriodID;
            string srvOrdType = serviceOrder?.SrvOrdType ?? appointment?.SrvOrdType;

            var contractPeriodInfoRow =
                PXSelectJoin<FSBillHistory,
                    InnerJoin<FSServiceContract,
                            On<FSServiceContract.refNbr, Equal<FSBillHistory.serviceContractRefNbr>>,
                    ...
                    InnerJoin<FSContractPostDoc,
                        On<FSContractPostDoc.serviceContractID, Equal<FSServiceContract.serviceContractID>,
                        And<FSContractPostDoc.contractPeriodID, Equal<FSContractPeriod.contractPeriodID>,
                        And<FSContractPostDoc.postDocType, Equal<FSBillHistory.childDocType>,
                        And<FSContractPostDoc.postRefNbr, Equal<FSBillHistory.childRefNbr>>>>>>>>,
                    Where<FSServiceContract.serviceContractID, Equal<Required<FSServiceContract.serviceContractID>>,
                        ...
                        And<FSBillHistory.serviceOrderRefNbr, IsNull>>>>>>
                .Select(docCache.Graph, billServiceContractID, billContractPeriodID);

            if (contractPeriodInfoRow != null)
            {
                //There should be only 1 record for the current invoiced period.
                if (contractPeriodInfoRow.Count() == 1)
                {
                    FSBillHistory contractBillHistory = (FSBillHistory)contractPeriodInfoRow;

                    if (invoiceRecords.Select().RowCast<FSBillHistory>()
                                              .Where(x => x.ServiceContractRefNbr == contractBillHistory.ServiceContractRefNbr
                                                        && x.SrvOrdType == srvOrdType
                                                        && x.AppointmentRefNbr == appointment?.RefNbr
                                                        && x.ServiceOrderRefNbr == serviceOrder?.RefNbr
                                                        && x.ChildDocType == contractBillHistory.ChildDocType
                                                        && x.ChildRefNbr == contractBillHistory.ChildRefNbr).Any() == false)
                    {
                        FSBillHistory billHistory = new FSBillHistory();

                        billHistory.ServiceContractRefNbr = contractBillHistory.ServiceContractRefNbr;
                        billHistory.SrvOrdType = srvOrdType;
                        billHistory.AppointmentRefNbr = appointment?.RefNbr;
                        billHistory.ServiceOrderRefNbr = serviceOrder?.RefNbr;
                        billHistory.ChildEntityType = contractBillHistory.ChildEntityType;
                        billHistory.ChildDocType = contractBillHistory.ChildDocType;
                        billHistory.ChildRefNbr = contractBillHistory.ChildRefNbr;

                        invoiceRecords.Cache.Insert(billHistory);
                        invoiceRecords.Cache.Persist(PXDBOperation.Insert);
                        invoiceRecords.Cache.Clear();
                    }
                }
                else
                {
                    //This case should not occur unless some inconsistency exists.
                    if (contractPeriodInfoRow.Count() > 1)
                    {
        ...
        {
            bool oldAllowInsert = documentCache.AllowInsert;
            ...
            documentCache.AllowInsert = oldAllowInsert;
        }
        ...
        {
            return row.Status != ID.Status_SODet.CANCELED
                && IsValidForProfitability(row.LineType, row.CuryUnitCost);
        }

        public virtual bool IsValidForProfitability(FSAppointmentDet row)
        {
            return row.Status != null
                && row.Status.IsNotIn(FSAppointmentDet.status.CANCELED,
                                        FSAppointmentDet.status.NOT_PERFORMED,
                                        FSAppointmentDet.status.RequestForPO)
                && IsValidForProfitability(row.LineType, row.CuryUnitCost);
        }

        public virtual List<FSProfitability> ProfitabilityRecords_INItems(PXGraph graph, FSServiceOrder serviceOrder, FSAppointment appointment)
        {
            var result = new List<FSProfitability>();

            if (serviceOrder == null && appointment == null)
            {
                return result;
            }
            ...
            if (appointment != null) 
            {
                var qResult = PXSelect<FSAppointmentDet,
                    Where<
                        FSAppointmentDet.srvOrdType, Equal<Required<FSAppointmentDet.srvOrdType>>,
                        And<FSAppointmentDet.refNbr, Equal<Required<FSAppointmentDet.refNbr>>>>,
                    OrderBy<
                        Asc<FSAppointmentDet.sortOrder,
                        Asc<FSAppointmentDet.lineNbr>>>>
                    .Select(graph, appointment.SrvOrdType, appointment.RefNbr)
                    .RowCast<FSAppointmentDet>()
                    .Where(x => IsValidForProfitability(x));

                foreach (FSAppointmentDet row in qResult)
                {
                    result.Add(PrepareProfitabilityRow(row));
                }
            }
            else if(serviceOrder != null) 
            { 
                var qResult = PXSelect<FSSODet,
                        Where<
                        ...
                        OrderBy<
                            Asc<FSSODet.sortOrder,
                            Asc<FSSODet.lineNbr>>>>
                        .Select(graph, serviceOrder.SrvOrdType, serviceOrder.RefNbr)
                        .RowCast<FSSODet>()
                        .Where(x => IsValidForProfitability(x));

                foreach (FSSODet row in qResult)
                {
                    result.Add(PrepareProfitabilityRow(row, serviceOrder.BillingBy));
                }
            }
        ...
        public virtual List<FSProfitability> ProfitabilityRecords_Logs(PXGraph graph, FSServiceOrder serviceOrder)
        {
            var result = new List<FSProfitability>();
            ...
            var appointments = PXSelect<FSAppointment,
                                    Where<
                                        FSAppointment.srvOrdType, Equal<Required<FSAppointment.srvOrdType>>,
                                        And<FSAppointment.soRefNbr, Equal<Required<FSAppointment.soRefNbr>>>>,
                                    OrderBy<
                                        Asc<FSAppointment.srvOrdType,
                                        Asc<FSAppointment.refNbr>>>>
                                    .Select(graph, serviceOrder.SrvOrdType, serviceOrder.RefNbr)
                                    .RowCast<FSAppointment>()
                                    .Where(x => x.InProcess == true
                                            || x.Paused == true
                                            || x.Completed == true
                                            || x.Closed == true);

            foreach (FSAppointment appointment in appointments)
            {
                result = result.Concat(ProfitabilityRecords_Logs(this, appointment)).ToList();
            }

            return result;
        }

        public virtual List<FSProfitability> ProfitabilityRecords_Logs(PXGraph graph, FSAppointment appointment)
        {
            var result = new List<FSProfitability>();
            ...
            var qResult = PXSelect<FSAppointmentLog,
                    Where<
                        FSAppointmentLog.docType, Equal<Required<FSAppointmentLog.docType>>,
                        And<FSAppointmentLog.docRefNbr, Equal<Required<FSAppointmentLog.docRefNbr>>>>,
                    OrderBy<
                        Asc<FSAppointmentLog.lineNbr>>>
                    .Select(graph, appointment.SrvOrdType, appointment.RefNbr)
                    .RowCast<FSAppointmentLog>()
                    .Where(x => x.BAccountID != null
                                && x.CuryUnitCost != 0m);

            foreach (FSAppointmentLog row in qResult)
            {
                result.Add(PrepareProfitabilityRow(graph, row));
            }

            return result;
        }

        public virtual FSProfitability PrepareProfitabilityRow(PXGraph graph, FSAppointmentLog appointmentLog)
        {
            FSProfitability profitabilityRow = new FSProfitability(appointmentLog);
            InventoryItem inventoyItemRow = InventoryItem.PK.Find(graph, appointmentLog.LaborItemID);

            profitabilityRow.Descr = inventoyItemRow.Descr;

            return profitabilityRow;
        }

		public virtual void ValidateServiceContractDates(PXCache docCache, object document, FSServiceContract fsServiceContractRow)
		{
			int? billServiceContractID = null;
			DateTime? actualDocDate = null;
			...
			if (document is FSAppointment)
			{
				FSAppointment row = document as FSAppointment;
				billServiceContractID = row.BillServiceContractID;
				actualDocDate = row.ExecutionDate;
				fieldName = typeof(FSAppointment.executionDate).Name;
			}
			else if (document is FSServiceOrder)
			{
				FSServiceOrder row = document as FSServiceOrder;
				billServiceContractID = row.BillServiceContractID;
				actualDocDate = row.OrderDate;
				fieldName = typeof(FSServiceOrder.orderDate).Name;
			}
			...
			if(fsServiceContractRow == null)
				fsServiceContractRow = FSServiceContract.PK.Find(docCache.Graph, billServiceContractID);

			if (actualDocDate == null)
			{
				docCache.RaiseExceptionHandling(fieldName, document, actualDocDate, new PXSetPropertyException(TX.Error.DocumentDateLesserThanServiceContractDate, PXErrorLevel.Error));
				return;
			}
			...
			{ 
				if (fsServiceContractRow.StartDate > actualDocDate)
				{
					docCache.RaiseExceptionHandling(fieldName, document, actualDocDate, new PXSetPropertyException(TX.Error.DocumentDateLesserThanServiceContractDate, PXErrorLevel.Error));
				}
				else if (fsServiceContractRow.EndDate < actualDocDate)
				{
					if (fsServiceContractRow.ExpirationType == ID.Contract_ExpirationType.EXPIRING)
					{
						docCache.RaiseExceptionHandling(fieldName, document, actualDocDate, new PXSetPropertyException(TX.Error.DocumentDateGreaterThanServiceContractEndDate, PXErrorLevel.Error));
					}
					else
					{
						docCache.RaiseExceptionHandling(fieldName, document, actualDocDate, new PXSetPropertyException(TX.Warning.DocumentEndDateLaterThanServiceContractExpirationDate, PXErrorLevel.Warning));
					}
				}
			}
		}
