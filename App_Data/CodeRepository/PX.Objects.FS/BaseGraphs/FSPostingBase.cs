        ...
        where TGraph : PXGraph
    {
        ...
        public virtual void CleanPostingInfoLinkedToDoc(object createdDoc)
        {
            ...
            foreach (FSPostRegister postRegister in postRegisterRows)
            {
                ...
                {
                    CleanPostingInfoOnServiceOrderAppointment(cleanerGraph, apptGraph, soGraph, postRegister);
                }
            }
        }
                                                                      ...
                                                                      FSPostRegister postRegister)
        {
            ...
            {
				srvOrdGraph.Clear(PXClearOption.ClearAll);

				FSServiceOrder srvOrd = srvOrdGraph.ServiceOrderRecords.Current = srvOrdGraph.ServiceOrderRecords
																				  .Search<FSServiceOrder.refNbr>
																				  (refNbr, srvOrdType);

				srvOrd = (FSServiceOrder)srvOrdGraph.ServiceOrderRecords.Cache.CreateCopy(srvOrd);

				srvOrd.PostedBy = null;
				srvOrd.Billed = false;
				srvOrd.PendingAPARSOPost = true;

				srvOrdGraph.ServiceOrderRecords.Cache.Update(srvOrd);
				srvOrdGraph.ServiceOrderRecords.Cache.SetValue<FSServiceOrder.finPeriodID>(srvOrd, null);
