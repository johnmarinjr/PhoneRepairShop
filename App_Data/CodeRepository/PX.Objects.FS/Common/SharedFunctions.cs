...
namespace PX.Objects.FS
{
    public static class SharedFunctions
    {
        ...
        public static InventoryItem GetInventoryItemRow(PXGraph graph, int? inventoryID)
        {
            ...
            return InventoryItem.PK.Find(graph, inventoryID);
        }
        #endregion
        ...
        public static void SetEquipmentActionFromInventory(PXGraph graph, object row, int? inventoryID)
        {
			...
			if ((row is SOLine
				...
				|| (row is FSSODet
					&& SharedFunctions.SetScreenIDToDotFormat(ID.ScreenID.SERVICE_ORDER) != graph.Accessinfo.ScreenID))
            {
        ...
        public static DateTime? GetNextExecution(PXCache cache, FSSchedule fsScheduleRow)
        {
            ...
            DateTime? nextOccurence = generator.GenerateNextOccurrence(mapScheduleResults, fsScheduleRow.LastGeneratedElementDate ?? fsScheduleRow.StartDate.Value, fsScheduleRow.EndDate, out expired);
			return nextOccurence?.Date;
        }
        ...
        public static void SetVisibleEnableProjectField<projectRelatedField>(PXCache cache, object row, int? projectID)
            where projectRelatedField : IBqlField
        {
            bool nonProject = ProjectDefaultAttribute.IsNonProject(projectID);
            PXUIFieldAttribute.SetVisible<projectRelatedField>(cache, row, !nonProject);
            PXUIFieldAttribute.SetEnabled<projectRelatedField>(cache, row, !nonProject);
            PXUIFieldAttribute.SetRequired<projectRelatedField>(cache, !nonProject);
            PXDefaultAttribute.SetPersistingCheck<projectRelatedField>(cache, row, !nonProject ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
        }
		...
		public static List<SM_ARReleaseProcess.ItemInfo> GetDifferentItemList(PXGraph graph, ARTran arTran, bool createDifferentEntriesForQtyGreaterThan1)
		{
			...
			var lotSerialList = new List<SM_ARReleaseProcess.ItemInfo>();
			...
			{
				var itemInfo = new SM_ARReleaseProcess.ItemInfo(arTran);
				...
				&& SOShipment.UK.Find(graph, arTran.SOShipmentType, arTran.SOShipmentNbr) != null) // this is because when there is no Shipment (OrderType == IN), SOShipmentNbr has '<NEW>'
			{
				...
				{
					var split = new SM_ARReleaseProcess.ItemInfo(shipLineSplit);
			...
			else if (arTran.SOOrderType != null && arTran.SOOrderNbr != null && arTran.SOOrderLineNbr != null)
			{
				...
				{
					var split = new SM_ARReleaseProcess.ItemInfo(soLineSplit);
		...
		public static List<SM_ARReleaseProcess.ItemInfo> GetVerifiedDifferentItemList(PXGraph graph, ARTran arTran, List<SM_ARReleaseProcess.ItemInfo> lotSerialList)
		{
