...
namespace PX.Objects.FS
{
    public class SM_APInvoiceEntry : FSPostingBase<APInvoiceEntry>
    {
        ...
        {
            return MessageHelper.GetErrorInfo<APTran>(Base.Document.Cache, Base.Document.Current, Base.Transactions, typeof(APTran));
        }
		...
		public virtual void CopyCustomizationFieldsToAPTran(APTran apTranToFill, IAPTranSource poSourceLine, bool areCurrenciesSame
							, Action<APTran, IAPTranSource, bool> baseMethod)
		{
			if (apTranToFill == null || apTranToFill.ReceiptNbr == null)
			{
				if (baseMethod != null) baseMethod(apTranToFill, poSourceLine, areCurrenciesSame);
				return;
			}
			string relatedEntityType = null;
			Guid? relatedNoteId = null;

			FSAppointment relatedAP = SelectFrom<FSAppointment>
							.InnerJoin<FSApptLineSplit>
								.On<FSAppointment.refNbr.IsEqual<FSApptLineSplit.apptNbr>
									.And<FSAppointment.srvOrdType.IsEqual<FSApptLineSplit.srvOrdType>>>
							.Where<FSApptLineSplit.pOReceiptNbr.IsEqual<@P.AsString>>
							.View
							.SelectSingleBound(Base, null, apTranToFill.ReceiptNbr);

			if (relatedAP != null && relatedAP.NoteID != null)
			{
				relatedEntityType = ID.FSEntityType.Appointment;
				relatedNoteId = relatedAP.NoteID;
			}
			else
			{
				FSServiceOrder relatedSO = SelectFrom<FSServiceOrder>
								.InnerJoin<FSSODetSplit>
									.On<FSServiceOrder.refNbr.IsEqual<FSSODetSplit.refNbr>
										.And<FSServiceOrder.srvOrdType.IsEqual<FSSODetSplit.srvOrdType>>>
								.Where<FSSODetSplit.pOReceiptNbr.IsEqual<@P.AsString>>
								.View
								.SelectSingleBound(Base, null, apTranToFill.ReceiptNbr);

				if (relatedSO != null && relatedSO.NoteID != null)
				{
					relatedEntityType = ID.FSEntityType.ServiceOrder;
					relatedNoteId = relatedSO.NoteID;
				}
			}
			FSxAPTran extRow = Base.Transactions.Cache.GetExtension<FSxAPTran>(apTranToFill);

			extRow.RelatedEntityType = relatedEntityType;
			extRow.RelatedDocNoteID = relatedNoteId;
        ...
        public virtual void EnableDisableRelatedSMFields(PXCache cache, APTran row)
        {
            ...
            if (item?.StkItem == false)
            {
                ...
                enableRelatedFSFields = row.InventoryID != null
                                        ...
                                        && (row.TranType == APDocType.Invoice || row.TranType == APDocType.DebitAdj || row.TranType == APDocType.CreditAdj)
                                        && row.PONbr == null;
            }
        ...
        public virtual void InsertUpdateDeleteDocDetail(PXCache cache, IEnumerable<IFSSODetBase> rowList, APInvoice apInvoiceRow, APTran apTranRow, PXEntryStatus apTranStatus, bool IsDocBilledOrClosed)
        {
            ...
            if (apTranStatus != PXEntryStatus.Deleted && apTranRow.InventoryID != null)
            {
                ...
                decimal sign = apInvoiceRow.DocType == APInvoiceType.DebitAdj ? -1 : 1;

                if (detRow.CuryUnitCost != sign * apTranRow.CuryUnitCost)
                {
                    detRow.CuryUnitCost = sign * apTranRow.CuryUnitCost;
                    rowUpdated = true;
                }

                if (detRow.CuryExtCost != sign * apTranRow.CuryLineAmt)
                {
                    detRow.CuryExtCost = sign * apTranRow.CuryLineAmt;
                    rowUpdated = true;
                }
