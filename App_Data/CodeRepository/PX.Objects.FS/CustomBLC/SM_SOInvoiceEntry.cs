...
namespace PX.Objects.FS
{
    public class SM_SOInvoiceEntry : FSPostingBase<SOInvoiceEntry>
    {
        ...
        public override void CreateInvoice(PXGraph graphProcess, List<DocLineExt> docLines, short invtMult, DateTime? invoiceDate, string invoiceFinPeriodID, OnDocumentHeaderInsertedDelegate onDocumentHeaderInserted, OnTransactionInsertedDelegate onTransactionInserted, PXQuickProcess.ActionFlow quickProcessFlow)
        {
            ...
            try
            {
				...
				foreach (DocLineExt docLineExt in docLines)
				{
					...
					bool nextDoc = docLineExt.fsAppointment == null
									? (soDet == null || soDet.RefNbr != docLineExt.fsServiceOrder.RefNbr)
									: (appointmentDet == null || appointmentDet.RefNbr != docLineExt.fsAppointment.RefNbr);
        ...
        public virtual ARTran InsertSOInvoiceLine(PXGraph graphProcess,
												  ...
												  bool firstInADoc /*indicates if the docLineExt is the first one in current Service Order or Appointment*/)
        {
            IDocLine docLine;
            ARTran arTranRow;
            ...
            decimal? curyExtPrice = docLine.GetQty(FieldType.BillableField) != 0 ? ((docLine.CuryBillableExtPrice * invtMult) / docLine.GetQty(FieldType.BillableField)) * arTranRow.Qty : 0m;
