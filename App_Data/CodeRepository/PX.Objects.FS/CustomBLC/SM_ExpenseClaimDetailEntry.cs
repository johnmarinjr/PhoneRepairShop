...
namespace PX.Objects.FS
{
    public class SM_ExpenseClaimDetailEntry : PXGraphExtension<ExpenseClaimDetailEntry>
    {
		...
		protected virtual void _(Events.FieldUpdated<FSxEPExpenseClaimDetails.fsEntityNoteID> e)
		{
			if (e.Row == null)
			{
				return;
			...
			if (extRow.FSEntityNoteID.HasValue)
			{
				EPExpenseClaimDetails row = (EPExpenseClaimDetails)e.Row;
				FSServiceOrder srvOrder = GetServiceOrder(extRow);

				if (srvOrder != null && row?.CustomerID != srvOrder?.BillCustomerID)
				{
					e.Cache.SetValueExt<EPExpenseClaimDetails.customerID>(e.Row, srvOrder.BillCustomerID);
					e.Cache.SetValueExt<EPExpenseClaimDetails.customerLocationID>(e.Row, srvOrder.BillLocationID);
				}
			}
		}
		#endregion
		...
		{
			FSServiceOrder so = null;

			if (row.FSEntityType == ListField_FSEntity_Type.ServiceOrder)
			{
				so = SelectFrom<FSServiceOrder>
					.InnerJoin<Customer>.On<FSServiceOrder.customerID.IsEqual<Customer.bAccountID>>
					.Where<FSServiceOrder.noteID.IsEqual<@P.AsGuid>>
					.View.SelectSingleBound(Base, null, new object[] { row.FSEntityNoteID });
			}
			else if (row.FSEntityType == ListField_FSEntity_Type.Appointment)
			{
				so = SelectFrom<FSServiceOrder>
					.InnerJoin<Customer>.On<FSServiceOrder.customerID.IsEqual<Customer.bAccountID>>
					.InnerJoin<FSAppointment>.On<FSAppointment.soRefNbr.IsEqual<FSServiceOrder.refNbr>
						.And<FSAppointment.srvOrdType.IsEqual<FSServiceOrder.srvOrdType>>>
					.Where<FSAppointment.noteID.IsEqual<@P.AsGuid>>
					.View.SelectSingleBound(Base, null, new object[] { row.FSEntityNoteID });
			}
        ...
        protected virtual void _(Events.RowSelected<EPExpenseClaimDetails> e)
        {
            ...
            PXUIFieldAttribute.SetEnabled<FSxEPExpenseClaimDetails.fsBillable>(e.Cache, e.Row, extRow.FSEntityNoteID != null && extRow.IsDocBilledOrClosed == false);
        ...
        public virtual bool IsFSDocumentBilledOrClosed(object row, string fsEntityType) 
			=> IsFSDocumentBilledOrClosedInt(row, fsEntityType);
        ...
        public virtual bool IsFSDocumentRelatedToProjects(object row, string fsEntityType)
			=> IsFSDocumentRelatedToProjectsInt(row, fsEntityType);
        ...
        public virtual FSServiceOrder GetServiceOrderRelated(PXGraph graph, string srvOrdType, string refNbr)
			=> GetServiceOrderRelatedInt(graph, srvOrdType, refNbr);

		public static FSServiceOrder GetServiceOrderRelatedInt(PXGraph graph, string srvOrdType, string refNbr)
        {
        ...
        public virtual FSServiceOrder UpdateServiceOrderDetail(ServiceOrderEntry graph, 
			...
			=> UpdateServiceOrderDetailInt(graph = GetServiceOrderGraph(false), serviceOrder, row, extRow);

		public static FSServiceOrder UpdateServiceOrderDetailInt(ServiceOrderEntry graph, 
                                                     FSServiceOrder serviceOrder, 
                                                     EPExpenseClaimDetails row, 
                                                     FSxEPExpenseClaimDetails extRow)
        {
            if (serviceOrder != null)
            {
                ...
                InsertUpdateDocDetailInt<FSSODet>(graph.ServiceOrderDetails, sODet, row, extRow);
        ...
        public virtual void InsertUpdateDocDetail<DAC>(PXSelectBase dacView, object dacRow, EPExpenseClaimDetails epExpenseClaimRow, FSxEPExpenseClaimDetails fsxEPExpenseClaimDetails)
			...
			=> InsertUpdateDocDetailInt<DAC>(dacView, dacRow, epExpenseClaimRow, fsxEPExpenseClaimDetails);

		public static void InsertUpdateDocDetailInt<DAC>(PXSelectBase dacView, object dacRow, EPExpenseClaimDetails epExpenseClaimRow, FSxEPExpenseClaimDetails fsxEPExpenseClaimDetails)
            where DAC : class, IBqlTable, IFSSODetBase, new()
        {
        ...
        public virtual FSAppointment UpdateAppointmentDetail(AppointmentEntry graph,
                                                    ...
                                                    EPExpenseClaimDetails row, 
                                                    FSxEPExpenseClaimDetails extRow)
			=> UpdateAppointmentDetailInt(graph = GetAppointmentGraph(false), appointment, row, extRow);

		public static FSAppointment UpdateAppointmentDetailInt(AppointmentEntry graph,
                                                    FSAppointment appointment, 
                                                    EPExpenseClaimDetails row, 
                                                    FSxEPExpenseClaimDetails extRow)
        {
            if (appointment != null)
            {
                ...
                InsertUpdateDocDetailInt<FSAppointmentDet>(graph.AppointmentDetails, appDet, row, extRow);
