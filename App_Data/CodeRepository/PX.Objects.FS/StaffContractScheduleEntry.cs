...
namespace PX.Objects.FS
{
    ...
    {
        public PXSelect<FSStaffSchedule, Where<FSStaffSchedule.entityType, Equal<FSStaffSchedule.entityType.Employee>>> StaffScheduleRecords;
        public PXSelect<FSStaffSchedule, Where<FSStaffSchedule.entityType, Equal<FSStaffSchedule.entityType.Employee>, And<FSStaffSchedule.scheduleID, Equal<Current<FSStaffSchedule.scheduleID>>>>> StaffScheduleSelected;
        ...
        #endregion
		#region RollbackScheduleGeneration
		public PXAction<FSStaffSchedule> rollbackScheduleGeneration;
		[PXUIField(DisplayName = "Roll Back Schedule Generation", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		[PXButton()]
		protected virtual IEnumerable RollbackScheduleGeneration(PXAdapter adapter)
		{
			Save.Press();

			List<FSStaffSchedule> list = new List<FSStaffSchedule>();
			foreach (FSStaffSchedule rule in adapter.Get<FSStaffSchedule>())
			{
				list.Add(rule);
			}
			...
			{
				StaffContractScheduleEntry docgraph = PXGraph.CreateInstance<StaffContractScheduleEntry>();
				TimeSlotMaint timeSlotMaintGraph = PXGraph.CreateInstance<TimeSlotMaint>();

				foreach (FSStaffSchedule row in list)
				{
					if (row.EntityType != ID.Schedule_EntityType.EMPLOYEE)
						continue;

					FSStaffSchedule item = docgraph.StaffScheduleRecords.Search<FSStaffSchedule.refNbr>(row.RefNbr);

					item = (FSStaffSchedule)docgraph.StaffScheduleRecords.Cache.CreateCopy(item);

					foreach (FSTimeSlot fsTimeSlotRow in PXSelect<FSTimeSlot,
														 Where<
															 FSTimeSlot.scheduleID, Equal<Required<FSTimeSlot.scheduleID>>>>
														 .Select(this, item.ScheduleID))
					{
					...
					item.LastGeneratedElementDate = null;
					item.NextExecutionDate = null;

					docgraph.StaffScheduleRecords.Update(item);
        ...
        protected virtual void _(Events.FieldDefaulting<FSStaffSchedule, FSStaffSchedule.recurrenceDescription> e)
        {
        ...
        protected virtual void _(Events.RowSelected<FSStaffSchedule> e)
        {
            ...
            bool existAnyGenerationProcess = SharedFunctions.ShowWarningScheduleNotProcessed(cache, e.Row);
            openStaffContractScheduleProcess.SetEnabled(existAnyGenerationProcess == false);
			rollbackScheduleGeneration.SetEnabled(existAnyGenerationProcess == true && e.Row.ScheduleID > 0);
        }
        ...
        protected virtual void _(Events.RowPersisting<FSStaffSchedule> e)
        {
            ...
            if (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update)
            {
                ...
                if (e.Operation == PXDBOperation.Insert && IsImport == false)
                {
