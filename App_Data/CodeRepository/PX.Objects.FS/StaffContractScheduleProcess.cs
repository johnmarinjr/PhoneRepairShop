        ...
        protected virtual void generateTimeSlotAndUpdateStaffSchedule(List<Schedule> scheduleRules, string recordType, DateTime? fromDate, DateTime? toDate, FSSchedule fsScheduleRow)
        {
            ...
            using (PXTransactionScope ts = new PXTransactionScope())
            {
                try
                {
                    ...
                    DateTime? lastGeneratedTimeSlotDate = null;

                    if (timeSlots.Count > 0)
                    {
                        lastGeneratedTimeSlotDate = timeSlots.Max(a => a.DateTimeBegin).Date;
                    }
                    ...
                    createContractGenerationHistory((int)nextGenerationID,
                                                    ...
                                                    processEndDate,
                                                    lastGeneratedTimeSlotDate,
                                                    recordType);

                    //Update Last Generated Time Slot Date
                    updateGeneratedSchedule(scheduleRules.ElementAt(0).ScheduleID, processEndDate, lastGeneratedTimeSlotDate, fsScheduleRow);
                }
                catch (PXException e)
                {
