...
namespace PX.Data
{
	...
	public class PXCacheNameAttribute : PXNameAttribute
	{
		...
		/// </returns>
		public static string GetName(Type dacType)
		{
			var cacheNameAttribute = dacType.CheckIfNull(nameof(dacType)).GetCustomAttributes<PXCacheNameAttribute>(inherit: true)
																		 .FirstOrDefault();
			return cacheNameAttribute?.GetName();
		}
	...
	/// </summary>
	/// <remarks>
	/// If you want an object to be visible to the web service APIs, you can set the <see cref="ServiceVisible"/> property to <see langword="true"/>.
	/// The hidden feature of a class is not inherited. If you derive a class from another class with <see cref="PXHiddenAttribute"/>, the derived class is not hidden.
	/// </remarks>
	/// <example>
			...
			where TBatchTable : class, IBqlTable, new()
		{
				...
				IDictionary<string, StateInfo> statesCache, out KeyValuePair<IDictionary, IDictionary> result)
			{
				...
				foreach (PXImportColumnsSettings property in _importColumnsSettings.SelectMulti(_viewName))
				{
					...
					string contentValue = contentReader.GetValue((int)property.ColumnIndex).TrimEnd();
			...
			else
			{
				...
				else
				{
					if (_sourceField != null)
					{
						...
						if (sourceCache.Current != null)
						{
							var sourceValue = sourceCache.GetValue(sourceCache.Current, _sourceField);
							if (sourceValue != null)
								_LastDefaultValue.Value = sourceValue;
						}
					}
					else
					{
