...
namespace PX.Data
{
	...
	{
		internal HashSet<string> variableLengthStrings = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
		...
		public T PlaceInserted(T value, out bool wasDeleted)
		{
			...
			for (int i = this.buckets[hash % this.buckets.Length]; i >= 0; i = this.entries[i].next)
			{
				if ((this.entries[i].hashCode == hash) && _Cache.ObjectsEqual(this.entries[i].value, value))
				{
					...
					if (this.entries[i].status == PXEntryStatus.InsertedDeleted || this.entries[i].status == PXEntryStatus.Modified)
					{
						this.entries[i].status = PXEntryStatus.Inserted;
					}
					else
					{
						this.entries[i].status = PXEntryStatus.Updated;
						if (!String.IsNullOrEmpty(_Cache._Timestamp) && _Cache.GetValue(value, _Cache._Timestamp) == null)
						{
							_Cache.SetValue(value, _Cache._Timestamp, _Cache.GetValue(this.entries[i].value, _Cache._Timestamp));
						}
						wasDeleted = true;
					}
					this.entries[i].value = value;
