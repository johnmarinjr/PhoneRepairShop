...
namespace PX.Data
{
	...
	public abstract partial class PXCache
	{
		...
		internal protected string _Timestamp;
...
#pragma warning restore 618
		...
		{
			if (Keys.variableLengthStrings.Contains(field) && value is string svalue && svalue.StartsWith(" <"))
			{
				return true;
			}
			if (String.Equals(Identity, field, StringComparison.OrdinalIgnoreCase))
			{
				if (value is int ivalue && ivalue < 0)
				{
					return true;
				}
				if (value is long lvalue && lvalue < 0L)
				{
					return true;
				}
			}
        ...
        {
			if (Identity != null && isUnexistingKey(Identity, GetValue(data, Identity)))
            {
				return true;
            }
			foreach (string key in Keys)
            {
				if (isUnexistingKey(key, GetValue(data, key)))
				{
					return true;
				}
			}
			...
			if (extensions.Count > 0)
			{
				...
				foreach (Type ext in extensions)
				{
					foreach (PropertyInfo extpi in ext.GetProperties(BindingFlags.Public | BindingFlags.Instance)
						         .OrderBy(p => p, new PXGraph.FieldInfoComparer()))
					{
    ...
    public static class PXCacheEx
	{
		...
		/// Cases like inheritance via <see cref="PXTableAttribute">PXTable</see>, projections and so on are also handled by this method.
		/// </summary>
		/// <param name="cache">Cache that the table belongs to</param>
		/// <returns/>
		[PXInternalUseOnly]
		public static bool TableExistsInDb(this PXCache cache) => cache.GetDbTableSchemas().Any();
		...
		{
			TableHeader tableSchema = PXDatabase.Provider.GetTableStructure(cache.BqlTable.Name);

			if (tableSchema == null && cache.BqlSelect != null)
			{
				foreach (Type dacType in cache.BqlSelect.GetTables())
				{
				string tableName = PXCache.GetBqlTable(dacType)?.Name;

					if (string.IsNullOrEmpty(tableName)) continue;
					var tableHeader = PXDatabase.Provider.GetTableStructure(tableName);
					if (tableHeader != null)
		...
		/// <remarks>This method doesn't return <see cref="PXMappedCacheExtension">PXMappedCacheExtension</see>-derived types</remarks>
		[PXInternalUseOnly]
		public static IEnumerable<Type> GetDacExtensionsContainingDacField(Type[] extensionTypes, Type dacType, string fieldName)
		{
			extensionTypes.ThrowOnNull(nameof(extensionTypes));
			dacType.ThrowOnNull(nameof(dacType));
			fieldName.ThrowOnNullOrWhiteSpace(nameof(fieldName));

			foreach (Type extType in extensionTypes.Where(extType => extType.IsSubclassOf(typeof(PXCacheExtension)) && 
																	!typeof(PXMappedCacheExtension).IsAssignableFrom(extType)))
			{
				PropertyInfo property = extType.GetProperty(fieldName, BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.Instance | BindingFlags.IgnoreCase);
				Type[] genericArguments = extType.BaseType.GetGenericArguments();

				if (property != null && genericArguments.Length > 0 && genericArguments[genericArguments.Length - 1].IsAssignableFrom(dacType))
				{
		...
		internal static bool IsAttributesField(this PXCache cache, string fieldName)
			=> fieldName?.EndsWith(AttributesFieldNameSuffix, StringComparison.OrdinalIgnoreCase) == true;
