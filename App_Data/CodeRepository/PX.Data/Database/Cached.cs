	...
	public class PXView
	{
		...
		protected PXViewQueryCollection SelectQueries
		{
			get
			{
				...
				bool _isViewReadOnly = _IsReadOnly || RestrictedFields.Any();

				if (IsNonStandardView && _Delegate == null)
				{
					_SelectQueries = new PXViewQueryCollection { CacheType = CacheType, IsViewReadonly = _isViewReadOnly };
				...
				var viewKey = new ViewKey(selectType, _isViewReadOnly);
				...
				foreach (PXFilterRow fr in filters)
				{
					...
					switch (fr.Condition)
					{
						...
						case PXCondition.NI:
							...
							else if (fr.Value is string)
							{
								if (fr.Variable == FilterVariableType.CurrentUserGroups 
									...
									|| fr.Variable == FilterVariableType.CurrentOrganization)
								{
									Type bqlField = cache.GetBqlField(fr.DataField);
									if (bqlField != null && cache.GetFieldType(bqlField.Name) == typeof(int))
									{
										...
										switch (fr.Variable)
										{
											case FilterVariableType.CurrentUserGroups:
												ids = UserGroupLazyCache.Current.GetUserGroupIds(PXAccess.GetUserID()).ToArray();
												break;
											case FilterVariableType.CurrentUserGroupsTree:
												ids = UserGroupLazyCache.Current.GetUserWorkTreeIds(PXAccess.GetUserID()).ToArray();
												break;
											case FilterVariableType.CurrentOrganization:
												ids = PXAccess.GetBranchIDsForCurrentOrganization();
												break;
											default:
						...
						case PXCondition.ER:
							...
							RaiseCommandPreparingForFilterValue(noteCache, fieldName: typeof(Note.externalKey).Name, fr.Value, PXDBOperation.Select,
																typeof(Note), state, out descr);	
							if (descr != null)
							{
								fr.Description = descr;
							}
						...
						case PXCondition.NE:
							...
							if (!corrected)
							{
								...
								{
									cache.RaiseCommandPreparing(fieldName, null, null, operation, cache.GetItemType(), out var fdescr);
								...
								else if (fr.Variable == FilterVariableType.CurrentBranch)
								{
									cache.RaiseCommandPreparing(fieldName, null, null, operation, cache.GetItemType(), out var fdescr);
									if (fdescr == null || fdescr.Expr == null || fdescr.DataType != PXDbType.Int)
										continue;
									
									fr.Value = PXAccess.GetBranchID();
									...
									if (state != null && state.DataType != fr.Value.GetType())
									{
										...
										value = PXFieldState.UnwrapValue(value);
										fr.UseExt = fr.UseExt | value != null && value != fr.Value;
										fr.Value = value ?? PXAccess.GetBranchID();
									}
								}
							}
							if (fr.Value != null)
							{
								...
								if (fr.Value != null)
								{
									try
									{
										...
										if (!updatingFailed)
										{
											RaiseCommandPreparingForFilterValue(cache, fieldName, fr.Value, operation, state, out descr);
										}
										...
										{
											RaiseCommandPreparingForFilterValue(cache, fieldName, fr.Value, PXDBOperation.Select | PXDBOperation.External,
																				state, out descr);
									...
									if (fr.Variable == FilterVariableType.CurrentUser || fr.Variable == FilterVariableType.CurrentBranch)
									{
						...
						case PXCondition.RLIKE:
							if (fr.Value is string s)
							{
								...
								try
								{
									RaiseCommandPreparingForFilterValue(cache, fieldName, filterValue: s, PXDBOperation.Select | PXDBOperation.External,
																		state, out descr);
						...
						case PXCondition.BETWEEN:
							...
							if (fr.Value != null && fr.Value2 != null)
							{
								...
								try
								{
									RaiseCommandPreparingForFilterValue(cache, fieldName, fr.Value2, PXDBOperation.Select | PXDBOperation.External,
																		state, out var descr2);
					...
					if (fr.Description != null)
					{
						...
						if (descr == null || descr.Expr == null)
						{
							...
							if (!fr.UseExt || fr.Variable == FilterVariableType.CurrentUser || fr.Variable == FilterVariableType.CurrentBranch)
							{
								object val = fr.Value;
								if (state != null)
								{
									...
									else if (fr.Variable != FilterVariableType.CurrentUser && fr.Variable != FilterVariableType.CurrentBranch
									         && fr.OrigValue != null && state.DataType == fr.OrigValue.GetType())
									{
		...
		private static void RaiseCommandPreparingForFilterValue(PXCache cache, string fieldName, object filterValue, PXDBOperation operation,
																PXFieldState state, out PXCommandPreparingEventArgs.FieldDescription fieldDescription)
		{
			Type table = cache.GetItemType();
			RaiseCommandPreparingForFilterValue(cache, fieldName, filterValue, operation, table, state, out fieldDescription);
		}

		/// <summary>
		/// Raises the cache command preparing field event for filter value with a special treatment for the <see cref="PXDBDateAttribute"/>. The <see cref="PXDBDateAttribute"/> may convert the date filter
		/// value from local time zone to UTC if its flags <see cref="PXDBDateAttribute.UseTimeZone"/> and
		/// <see cref="PXDBDateAttribute.PreserveTime"/> both are set. This results in a shift in the filter value and a value 01.09.2020 00:00 corresponding to the 1st of September 2020
		...
		/// values these flags on  <see cref="PXDBDateAttribute"/> attribute are temporarily cleared to prevent time zone conversions.
		/// </summary>
		...
		private static void RaiseCommandPreparingForFilterValue(PXCache cache, string fieldName, object filterValue, PXDBOperation operation, Type table,
																PXFieldState state, out PXCommandPreparingEventArgs.FieldDescription fieldDescription)
		{
			bool filterValueIsDate = filterValue is DateTime || filterValue is DateTime? || state?.DataType == typeof(DateTime) || state?.DataType == typeof(DateTime?); 

			if (!filterValueIsDate)
				cache.RaiseCommandPreparing(fieldName, row: null, filterValue, operation, table, out fieldDescription);
			else
			{
				var dateAttributes = cache.GetAttributesReadonly(fieldName).OfType<PXDBDateAttribute>()
																		   .Where(a => a.UseTimeZone)
																		   .ToList();
				try
				{
					dateAttributes.ForEach(a => a.UseTimeZone = false);
					cache.RaiseCommandPreparing(fieldName, row: null, filterValue, operation, table, out fieldDescription);
				}
				finally
				{
					dateAttributes.ForEach(a => a.UseTimeZone = true);
				}
			}
		}

		private static void CreateInConditionForIntArray(object dataValue, Type field, bool inCondition, out Type expression, out PXDataValue parameter)
		{
			parameter = null;

			if (dataValue is int[] ids && ids.Length > 0)
			{
				...
				for (int i = 0; i < filters.Length; i++)
				{
					...
					if (field != null && !conditionViolation)
					{
						...
						switch (fr.Condition)
						{
							...
							case PXCondition.NI:
										  ...
										  fr.Variable == FilterVariableType.CurrentOrganization))
								{
									if (fr.Description != null)
									{
										CreateInConditionForIntArray(fr.Description.DataValue, field, fr.Condition == PXCondition.IN,
											out var expression, out var parameter);
										if (expression != null)
										{
						...
						if (field == typeof (FieldNameParam) && expressions[i] != null)
						{
							...
							int idxscore = filters[i].DataField.IndexOf("__");
							...
							if (idxscore != -1 && fieldExpr.LExpr() is Column lFieldCol && lFieldCol.Table()?.Alias != null &&
								lFieldCol.Name.IndexOf("__") == -1)
							{
								lFieldCol.substituteTableName(lFieldCol.Table().Alias, filters[i].DataField.Substring(0, idxscore));
							}
			...
			if (parameters != null)
			{
				...
				for (int i = 0; i < cmdpars.Length && i < parameters.Length; i++)
				{
					...
					if (!cmdpars[i].IsArgument)
					{
						...
						{
							if (Cache.isUnexistingKey(field.Name, parameters[i]) && !cmdpars[i].NullAllowed && !_Select.HasNotEqualParameter(cmdpars[i].GetReferencedType()))
                            {
								context.BadParametersQueryNotExecuted = true;
								return ret;
                            }
		...
		private protected virtual object CreateResult(PXCache[] caches, PXDataRecord rec, bool hascount, ref bool overrideSort, ref bool extFilter)
		{
			...
			{
				items[i] = CreateItem(caches[i], rec, ref position, _IsReadOnly || i > 0 || RestrictedFields.Any(), out var wasUpdated);
				if (wasUpdated)
				{
					overrideSort = true;
					extFilter = true;
			...
			if (items[0] != null)
			{
				if (!_IsReadOnly && RestrictedFields.Any())
				{
					var item = caches[0].Locate(items[0]);
					if (item != null)
					{
						var status = caches[0].GetStatus(item);
						if (status == PXEntryStatus.Deleted || status == PXEntryStatus.InsertedDeleted)
							return null;
						if (status == PXEntryStatus.Updated || status == PXEntryStatus.Inserted || status == PXEntryStatus.Modified)
						{
							overrideSort = true;
							extFilter = true;
						}
		...
		private List<object> GetResultWindowedRO(object[] parameters, PXSearchColumn[] sorts, int skip=0, int take=0) {
			...
			EnsureCreateInstance(tables);
			cmd.Context = context;
            var query = cmd.GetQuery(_Graph, this, take, skip);
            var result = PXDatabase.Provider.Select(query, pars.ToArray());
            ...
            foreach (PXDataRecord r in result)
            {
				...
				for (int i = 0; i < caches.Length; i++) 
                {
					items[i] = caches[i].Select(rec, ref position, _IsReadOnly || i > 0 || RestrictedFields.Any(), out _);
				}
        ...
        {
            var list = new List<object>(1) { selectResult };
            if (WebConfig.StoreCachedValidation)
            {
                var prs = queryParameters.GetParameters(this);
                ValidateParametersInfer(list, prs);
			}

			StoreResult(list, queryParameters);
        ...
        {
            var prs = GetParameterValues(selectResult);
			StoreResult(new List<object>(1) { selectResult }, PXQueryParameters.ExplicitParameters(prs));
        ...
        private Type[] _InvertedParameters;
		
		internal object[] GetParameterValues(IBqlTable row)
        {
            bool validateRow = _InvertedParameters == null;

			var mapping = _InvertedParameters ?? (_InvertedParameters = GetParametersMapping());
            if (mapping.Length == 0)
            {
            ...
            var result = mapping.Select(_ => this.Cache.GetValue(row, _.Name)).ToArray();
            if (validateRow)
            {
                bool meet = _Select.Meet(this.Cache, row, result);
                if (!meet)
                {
                    _InvertedParameters = new Type[0];
		...
		{
            var mappings = _InvertedParameters ?? (_InvertedParameters = GetParametersMapping());
            ...
            {
				var bqlTable = row[mapping.DeclaringType];
				var cache = this.Graph.Caches[mapping.DeclaringType];
				if (cache != null)
                {
					res.Add(cache.GetValue(bqlTable, mapping.Name));
                }
            }
		...
		List<Type> GetParametersFromBQL()
        {
            Type command = this._Select.GetSelectType();
            var list = BqlCommand.Decompose(command);
            var result = new List<Type>();
            for (var i = 0; i < list.Length-1; i++)
            {
                var p = list[i];
                if (!typeof(IBqlParameter).IsAssignableFrom(p))
                    continue;

                var f = list[i +1];
                if (!typeof(IBqlField).IsAssignableFrom(f))
                    continue;
        ...
        Type[] GetParametersMapping()
        {
            Type command = this._Select.GetSelectType();
			//PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<CurrencyInfo.curyInfoID>>>>
			var list = BqlCommand.Decompose(command);
            var result = new List<Type>();
            for (var i = 2; i < list.Length; i++)
            {
                ...
                var eq = list[i - 1];
                if (!typeof(Equal<>).IsAssignableFrom(eq))
                    return new Type[0];
        ...
        {
            var prs = queryParameters.GetParameters(this);
            if (!SelectQueries.HasStoredResults())// single time per view
            {
                ValidateRecords(selectResult, prs);
            }

            List<object> items = (this.IsReadOnly) ? selectResult : new VersionedList(CloneResult(selectResult), Cache.Version);
            SelectQueries.SetResult(this, SelectQueries.GetAltCommandKey(prs), items );
        ...
        private void ValidateRecords(List<object> records, object[] prs)
        {
            try
            {
                var pnames = this.GetParameterNames();

                if (pnames.Length != prs.Length)
                {
                    var bqlNames = this.GetParametersFromBQL();
                    if (bqlNames.Count != prs.Length)
                    {
                        PXTrace.Logger.ForTelemetry("StoreCached", "ParametersCount")
                            .WithStack()
                            .Warning("{Names};{Params};{Query};{BQL};{BQLNames}",
                                pnames.JoinToString(",")
                                , prs.JoinToString(",")
                                , this.ToString()
                                , this.BqlSelect.ToString()
                                ,bqlNames.JoinToString(","));
					}
                ...
                else
                {
                    ...
                    {
                        bool meet = _Select.Meet(this.Cache, record, prs);
                        if (!meet)
                        {
                            PXTrace.Logger.ForTelemetry("StoreCached", "Mismatch")
                                .WithStack()
                                .Warning("{Names};{Params};{Query};{BQL}",
                                    pnames.JoinToString(",")
                                    , prs.JoinToString(",")
                                    , this.ToString()
                                    , this.BqlSelect.ToString());
        ...
        internal virtual void StoreCached(PXCommandKey queryKey, List<object> records, PXSelectOperationContext context)
        {
            if (WebConfig.StoreCachedValidation)
            {
                var prs = queryKey.GetParameters() ?? new object[0];
                ValidateParametersInfer(records, prs);
                ValidateRecords(records, prs);
			}
        ...
        private void ValidateParametersInfer(List<object> records, object[] prs)
        {
			try
            {
                ...
                {
                    var r = records[0];
                    if (r is IBqlTable row)
                    {
                        var prs2 = GetParameterValues(row);
                        if (!prs2.SequenceEqual(prs))
                        {
                            PXTrace.Logger.ForTelemetry("StoreCached", "ValidateParameters")
                                .WithStack()
                                .Warning("{Params};{Params2};{Query};{BQL}",
                                    prs.JoinToString(",")
                                    , prs2.JoinToString(",")
                                    , this.ToString()
                                    , this.BqlSelect.ToString());
            ...
            catch(Exception e)
            {
                PXTrace.Logger.ForTelemetry("StoreCached", "ValidateParameters")
                    .WithStack()
                    .Warning("{Params};{Error};{Query};{BQL}",
                        prs.JoinToString(",")
                        , e.ToString()
                        , this.ToString()
                        , this.BqlSelect.ToString());
		...
		private protected virtual object LookupItem(object item, PXCache[] caches, ref bool overrideSort, ref bool needFilterResults)
		{
			...
			if (res != null)
			{
				...
				for (int j = 0; j < res.Items.Length; j++)
				{
					...
					else
					{
						...
						CloneOriginals(caches[j], res.Items[j], clone);
						res.Items[j] = clone;
						res[j] = caches[j].Select(res[j], _IsReadOnly || j > 0 || RestrictedFields.Any(), out wasUpdated); ;
						if (wasUpdated)
						{
							overrideSort = true;
			...
			else
			{
				...
				else
				{
					...
					CloneOriginals(caches[0], item, clone);
					result = clone;
					result = caches[0].Select(result, _IsReadOnly || RestrictedFields.Any(), out wasUpdated); ;
					if (wasUpdated)
					{
						overrideSort = true;
						needFilterResults = true;
		...
		protected virtual bool emulation => Cache.Interceptor is PXUIEmulatorAttribute;
			...
			)
		{
			...
			if (_Select is IBqlJoinedSelect || !(list is VersionedList) || ((VersionedList)list).Version != version)
			{
				...
				if (!(fullKeyReferenced == true && list.Count == 1 && list[0] != null && object.ReferenceEquals(LocateInCache(list[0]), list[0])))
				{
		...
		private bool InternCache(List<object> list, ref bool sortReq)
		{
			bool anyMerged = false;

			if (!_IsReadOnly && _LegacyQueryCacheMode && (Cache.Interceptor == null || Cache.Interceptor.CacheSelected))
			{
				bool wasUpdated = false;

				Dictionary<object, object> insertedMap = null;
				if (Cache._Persisting && Cache.Inserted.Count() > 0)
				{
					insertedMap = new Dictionary<object, object>(Cache.GetComparer());
					foreach (var insertedItem in Cache.Inserted)
						insertedMap.Add(insertedItem, insertedItem);
				}
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					if (res != null)
					{
						if (res.Items[0] == null)
							continue;
						var newItem = Cache.Locate(res.Items[0]);
						if (newItem == null && insertedMap != null)
							insertedMap.TryGetValue(res.Items[0], out newItem);
						if (newItem != null)
						{
							if (!object.ReferenceEquals(newItem, res.Items[0]))
							{
								res.Items[0] = newItem;
								anyMerged = true;
							}
						...
						{
							Cache.PlaceNotChanged(res.Items[0], out wasUpdated);
							if (wasUpdated)
							{
								sortReq = true;
							}
					...
					{
						var newItem = Cache.Locate(item);
						if (newItem == null && insertedMap != null)
							insertedMap.TryGetValue(item, out newItem);
						if (newItem != null)
						{
							if (!object.ReferenceEquals(newItem, item))
							{
						...
						{
							Cache.PlaceNotChanged(item, out wasUpdated);
							if (wasUpdated)
							{
								sortReq = true;
							}
		...
		private protected virtual object LocateInCache(object item)
		{
			return Cache.Locate(item);
		}
			...
			)
		{
			...
			for (int i = 0; i < filters.Length; i++)
			{
				...
				switch (fr.Condition)
				{
					case PXCondition.IN:
					case PXCondition.NI:
						#region Filter variables (@mygroups, @myworktree, @company)
						if (fr.Variable == FilterVariableType.CurrentUserGroups)
						{
							if (fr.UseExt)
							{
						...
						else if (fr.Variable == FilterVariableType.CurrentOrganization)
						{
							if (fr.UseExt)
							{
								var branches = PXAccess.GetBranchCDsForCurrentOrganization().ToHashSet(PXLocalesProvider.CollationComparer);
							...
							{
								var branches = PXAccess.GetBranchIDsForCurrentOrganization().ToHashSet();
					...
					case PXCondition.RLIKE:
						...
						if (fr.Value is string)
						{
							...
							var fastFilterCondition = SitePolicy.GridFastFilterCondition;
							bool isFastFilter = filters.Length > 1 && filters
								.Except(fr)
								.All(f => f.Condition == fastFilterCondition && f.Value is string && String.Equals(((string)f.Value).Trim(), fValue?.Trim(), StringComparison.OrdinalIgnoreCase));
			...
			)
		{
			...
			switch (list)
			{
				case VersionedList _:
					ret = new VersionedList();
					break;
				case IPXDelegateResult delegateResult:
					ret = new PXDelegateResult
					{
						IsResultSorted = delegateResult.IsResultSorted,
						IsResultFiltered = delegateResult.IsResultFiltered,
						IsResultTruncated = delegateResult.IsResultTruncated
					};
					break;
				default:
					ret = new List<object>();
			...
			else
			{
				...
				if (list == null)
				{
					...
					if (!PXDatabase.ReadDeleted)
					{
						...
						if (isFromCache && perf != null && PXPerformanceMonitor.SaveSqlToDb && PXPerformanceMonitor.SqlProfilerIncludeQueryCache)
						{
					...
					if (list == null)
					{
						...
						if (!_LegacyQueryCacheMode)
						{
							...
							for (int i = 0; i < list.Count; i++)
							{
								...
								{
									bool wasUpdated = false;
									list[i] = caches[0].Select(item, _IsReadOnly || RestrictedFields.Any(), out wasUpdated);
									if (wasUpdated)
									{
										overrideSort = true;
										needFilterResults = true;
					...
					if (queryKey != null && _LegacyQueryCacheMode && isFromCache)
					{
						anyMerged |= InternCache(list, ref sortReq);
					}
		...
		/// indicates the clicked button.</returns>
		public WebDialogResult Ask(object row, string header, string message, MessageButtons buttons,
			IReadOnlyDictionary<WebDialogResult, string> buttonNames, MessageIcon icon, bool repaintControls)
		{
			return DialogManager.Ask(this, null, row, header, message, buttons, buttonNames, icon, repaintControls);
		}
		...
		/// <summary>Displays the dialog window configured by the
		/// <tt>PXSmartPanel</tt> control. Header is displayed as a panel title</summary>
		public WebDialogResult AskExtWithHeader(string header, InitializePanel initializeHandler, List<string> commitFields = null, bool repaintControls = false)
		{
			return DialogManager.AskExt(this, header, new DialogManager.InitializePanel(initializeHandler), commitFields, repaintControls);
		}
