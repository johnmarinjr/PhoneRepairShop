...
namespace PX.Data
{
	...
	public class PXFieldState : IDataSourceFieldSchema, ICloneable
	{
		...
		public virtual string Name { get { return _FieldName; } internal set { _FieldName = value; } }
		/// <exclude/>
		...
		protected PXFieldState(object value)
		{
			...
			else
			{
				...
				_ControlConfig = state._ControlConfig;
			}
		}
		...
		/// </summary>
		/// <remarks>
		/// If <see cref="DisplayName"/> has not been set explicitly then field name <see cref="Name"/> is returned.
		/// </remarks>
		...
		/// <summary>
		/// Gets a value indicating whether  <see cref="DisplayName"/> was explicitly specified for the field.
		/// </summary>
		/// <value>
		/// True if the <see cref="DisplayName"/> was explicitly specified for the field, false if not.
		/// </value>
		public virtual bool IsDisplayNameSpecified => _DisplayName != null;
	...
	public class PXStringState : PXFieldState
	{
		...
		protected PXStringState(object value)
			: base(value)
		{
			if (value is PXStringState state)
			{
				_Length = state._Length;
			}
		}
		...
		public virtual string[] AllowedLabels
		{
			get
			{
				...
				if (_AllowedLabels != null && _AllowedLabels.Length >= _AllowedValues.Length)
				{
		...
		public virtual string[] AllowedImages
		{
			get
			{
				...
				if (_AllowedImages != null && _AllowedImages.Length >= _AllowedValues.Length)
				{
		...
		internal bool TryGetListValue(string label, out string result)
		{
			if (this.HasFixedValuesList())
			{
				...
				{
					if (PXLocalesProvider.CollationComparer.Equals(labels[i], label))
					{
						result = values[i];
						return true;
					}
		...
		internal bool TryGetListLabel(string value, out string result)
		{
			if (this.HasFixedValuesList())
			{
				...
				{
					if (PXLocalesProvider.CollationComparer.Equals(values[i], value))
					{
	...
	public class PXIntState : PXFieldState
	{
		...
		public virtual string[] AllowedLabels
		{
			get
			{
				...
				return _AllowedValues.Select(value => value.ToString()).ToArray();
			}
			set { _AllowedLabels = value; }
		}
		...
		internal bool TryGetListValue(string label, out object result)
		{
			if (this.HasFixedValuesList())
			{
				var values = AllowedValues;
				var labels = AllowedLabels;
				bool isShort = _DataType == typeof(short);
				for (int i = 0; i < labels.Length; i++)
				{
					if (PXLocalesProvider.CollationComparer.Equals(labels[i], label))
					{
						//cast to (object) is needed to prevent a (short) value to be casted to (int)
						result = isShort ? (short)values[i] : (object)values[i];
						return true;
					}
				}
			}
