...
namespace PX.Data
{
	...
	public class PXDBStringAttribute : PXDBFieldAttribute, IPXRowSelectingSubscriber, IPXCommandPreparingSubscriber, IPXFieldUpdatingSubscriber, IPXFieldSelectingSubscriber
	{
		...
		protected bool? _IsUnicode = null;
        ...
        public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
        {
			...
			if (IsKey && !IsFixed && (e.Operation & PXDBOperation.Command) == PXDBOperation.Insert
				&& e.DataValue is string s && s.StartsWith(" ") && sender.Keys.variableLengthStrings.Contains(_FieldName))
			{
				throw new PXCommandPreparingException(_FieldName, e.DataValue, ErrorMessages.LeadingSpaceIsNotAllowed);
			}
		}

		protected PXDbType DbType => _IsFixed
			? (IsUnicode ? PXDbType.NChar : PXDbType.Char)
			: (IsUnicode ? PXDbType.NVarChar : PXDbType.VarChar);

		private bool IsUnicodeField()
		{
			if (_IsUnicode != null) return _IsUnicode.Value;

			if (BqlTable?.Name is string tableName)
			{
				var structure = PXDatabase.Provider.GetTableStructure(tableName);
				var column = structure?.Columns.SingleOrDefault(c => c.Name == this.FieldName);
				_IsUnicode = column?.Type.IsIn(SqlDbType.NChar, SqlDbType.NText, SqlDbType.NVarChar);
			}

			return _IsUnicode ?? false;
		}
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			if (IsKey && !IsFixed)
            {
				sender.Keys.variableLengthStrings.Add(_FieldName);
            }
		}
		#endregion
	}
	...
	public class PXDBLocalizableStringAttribute : PXDBStringAttribute, IPXRowPersistedSubscriber, IPXRowPersistingSubscriber
	{
		...
		public static void SetShowTranslations(PXCache cache, object data, string name, bool showTranslations)
		{
			if (data == null)
			{
				cache.SetAltered(name, true);
			}
			foreach (var attr in cache.GetAttributesOfType<PXDBLocalizableStringAttribute>(data, name))
			{
				if (attr is PXDBLocalizableStringAttribute)
				{
					((PXDBLocalizableStringAttribute)attr).ShowTranslations = showTranslations;
				}
			}
		}

		public static void SetShowTranslations<Field>(PXCache cache, object data, bool showTranslations)
					where Field : IBqlField
		{
			SetShowTranslations(cache, data, typeof(Field).Name, showTranslations);
		}
		...
		{
			Func<string, bool> tableMeet = t => tables.Contains(t);
			EnsureTranslations(tableMeet);
		}
		...
		{
			if (_Definition != null && ShowTranslations)
			{
				string currentLanguage = Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
				if (String.IsNullOrEmpty(currentLanguage) || !_Indexes.ContainsKey(currentLanguage))
				{
		...
		internal virtual string GetTranslationFromPackedValue(string value)
		{
			if (_Definition == null || !ShowTranslations)
			{
				return value;
			...
			var translations = UnpackTranslations(value);
			var languageIndex = _Indexes[currentLanguage];
			...
			if (alternatives == null)
			{
				...
				else
				{
					...
					if (!String.IsNullOrEmpty(value))
					{
						...
						SetTranslations(sender, row, alternatives);
					}
				}
			}
			return alternatives;
		...
		{
			if (_Definition != null && e.Row != null && e.NewValue != sender.GetValue(e.Row, _FieldOrdinal))
			{
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			if ((e.Operation & PXDBOperation.Command) != PXDBOperation.Insert && (e.Operation & PXDBOperation.Option) != PXDBOperation.Second) {
				...
				if (!isSelect) {
					...
					if (recordComesFirst)
					{
						value = e.Value ?? gstamp;
					}
					else
					{
						byte[] persistedTstamp = null;
						object[] persisted = PXTimeStampScope.GetPersisted(sender, e.Row);
						if (persisted != null && persisted.Length > 0) {
							persistedTstamp = persisted[0] as byte[];
						}

						if (persistedTstamp != null)
						{
							value = sender.Graph.SqlDialect.CompareTimestamps(persistedTstamp, gstamp) > 0 ? persistedTstamp : gstamp;
						}
						else if (gstamp != null && sender.Graph.PrimaryItemType != null && e.Row != null
							&& sender.Graph.PrimaryItemType.IsAssignableFrom(e.Row.GetType()))
						{
							TimeSpan ts;
							Exception msg;
							if (!sender.Graph.IsImport && PXLongOperation.GetStatus(sender.Graph.UID, out ts, out msg) == PXLongRunStatus.Completed)
							{
								var tstamp = sender.GetValue(e.Row, _FieldOrdinal) as byte[];
								if (tstamp != null) {
									if (sender.Graph.SqlDialect.CompareTimestamps(tstamp, gstamp) > 0)
		...
		public virtual void RowPersisted(PXCache sender, PXRowPersistedEventArgs e)
		{
			...
			if (e.TranStatus == PXTranStatus.Completed
				...
				if (tstamp == null) {
					...
					bool isSet = false;
					if (sender.Graph.SqlDialect.CompareTimestamps(gstamp, tstamp) > 0) {
						sender.SetValue(e.Row, _FieldOrdinal, gstamp);
						PXTimeStampScope.PutPersisted(sender, e.Row, gstamp);
						isSet = true;
					}
	...
	public class PXDBForeignIdentityAttribute : PXDBIdentityAttribute, IPXRowPersistingSubscriber
	{
		...
		/// Such field of the identity providing table should be marked with the <see cref="PXDBForeignIdentityTypeAttribute"/>.
		/// </summary>
		string _typeDiscriminatorField = null;
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			_typeDiscriminatorField = sender.Graph
				.Caches[_ForeignIdentity]
				.GetAttributesReadonly(null)
				.OfType<PXDBForeignIdentityTypeAttribute>()
		...
		public virtual void RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert)
			{
				try
				{
					if (_typeDiscriminatorField == null)
					PXDatabase.Insert(_ForeignIdentity, PXDataFieldAssign.OperationSwitchAllowed);
					else
						PXDatabase.Insert(_ForeignIdentity, GetTypeAssign(), PXDataFieldAssign.OperationSwitchAllowed);
				}
				catch (PXDbOperationSwitchRequiredException)
				{
					...
					PXDatabase.Update(_ForeignIdentity);
						...
						PXDatabase.Update(_ForeignIdentity, GetTypeAssign());
				}
				_KeyToAbort = (int?)sender.GetValue(e.Row, _FieldOrdinal);
				decimal? identity;
		...
		protected virtual PXDataFieldAssign GetTypeAssign() => new PXDataFieldAssign(_typeDiscriminatorField, PXDbType.NVarChar, _BqlTable.Name.ToString());
	...
	/// which would use the table as a source of its identity (see the <see cref="PXDBForeignIdentityAttribute"/>).
	/// </summary>
	...
	/// field of the same DAC, including those marked with <see cref="PXDBScalarAttribute">PXDBScalar</see>. You can also use BQL
	...
	/// side at runtime, use the <see cref="PXFormulaAttribute">PXFormula</see> attribute.</para>
	...
	public class PXDBCreatedByIDAttribute : PXDBBaseIDAttribute, IPXRowInsertingSubscriber
	{
		...
		public sealed class Creator : Users
		{
			...
			[PXDBScalar(typeof(Search<UserPreferences.mailSignature, Where<UserPreferences.userID.IsEqual<Creator.pKID>>>))]
	...
	public class PXDBLastModifiedByIDAttribute : PXDBCreatedByIDAttribute, IPXRowUpdatingSubscriber, IPXRowPersistingSubscriber
	{
		...
		public sealed class Modifier : Users
		{
			...
			[PXDBScalar(typeof(Search<UserPreferences.mailSignature, Where<UserPreferences.userID.IsEqual<Modifier.pKID>>>))]
			public string MailSignature { get; set; }
			#endregion
		}
