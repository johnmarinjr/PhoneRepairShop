...
namespace PX.Data
{
    ...
    public class PXNoteAttribute : PXDBGuidAttribute, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXRowDeletedSubscriber, IPXReportRequiredField
    {
		...
		[Obsolete("The constructor is obsolete. Use the parameterless constructor instead. " +
				  "The " + nameof(PXNoteAttribute) + " constructor is exactly the same as the parameterless one. " +
        ...
        protected PX.Data.EP.IActivityService ActivityService { get; set; }
        ...
        {
            var command = new Select<Note, Where<Note.noteID, Equal<Required<Note.noteID>>>>();
            var query = command.GetQuery(graph, 1);

            return PXDatabase.Provider.Select(query, new[] {new PXDataValue(id)});
        }

        protected Note GetNote(PXGraph graph, Guid? id)
        {
            if (!id.HasValue) return null;
            foreach (var record in SelectNote(graph, id.Value))
            {
                return new Note
                {
                    NoteID = record.GetGuid(0),
                    NoteText = record.GetString(1),
                    EntityType = record.GetString(2),
                    GraphType = record.GetString(3),
                    ExternalKey = record.GetString(4),
                    NotePopupText = record.GetString(5)
                };
            }
        ...
        protected int GetActivityCount(PXGraph graph, Guid? id)
        {
            ...
            if (ActivitiesCountByParent)
            {
                ...
                var fieldName = EntityHelper.GetIDField(graph.Caches[entity.GetType()]);
		...
		public virtual void RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
			...
			Note noteFilter = new Note { NoteID = id };
			PXCache noteCahce = sender.Graph.Caches[typeof(Note)];
			PXCache noteDocCache = sender.Graph.Caches[typeof(NoteDoc)];
			...
			switch (sender.GetStatus(e.Row))
			{
				...
				case PXEntryStatus.InsertedDeleted:
					{
						Note note = (Note)noteCahce.Locate(noteFilter);
						if (note != null) noteCahce.Remove(note);
						foreach (NoteDoc doc in noteDocCache.Cached.Cast<NoteDoc>().Where(_ => _.NoteID == id))
						{
							noteDocCache.Remove(doc);
						}
						return;
					}
				default:
					{
						noteCahce.Delete(noteFilter);

						foreach (NoteDoc doc in GetDocView(sender.Graph).SelectMulti(id))
						{
							noteDocCache.Delete(doc);
						}
						return;
					}
			}
		...
		public virtual void noteFilesFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
        {
            ...
            if (id != null)
            {
                ...
                PXContext.SetSlot<string>(string.Format("{0}+{1}", typeof(Note).FullName, id), id.ToString());
		...
		public virtual void noteActivityFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
        {
            ...
            {
                int count = ActivityService.GetCount(id);
                e.ReturnValue = count;
        ...
        private YaqlScalar GetValueForSubquery(PXCache sender, PXCommandPreparingEventArgs e, bool fieldRequired, bool forCRActivity)
        {
            ...
            if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select && fieldRequired)
            {
                            ...
                            (e.Operation & PXDBOperation.Option) == PXDBOperation.Normal && e.Value == null))
                {
                    ...
                    {
                        string field = EntityHelper.GetIDField(sender);
		...
		public override void RowSelecting(PXCache sender, PXRowSelectingEventArgs e)
        {
            ...
            if (e.Row != null)
            {
                ...
                if (PopupTextEnabled && popupText != null)
                {
        ...
        private void noteFilesCountFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
        {
            ...
            if (retVal == null)
            {
				PXView noteDocView = GetDocView(sender.Graph);

				int filesCount = sender.GetStatus(e.Row) == PXEntryStatus.Inserted && e.Row.GetType() != sender.Graph.PrimaryItemType
					? FiltrateCachedButNotDeletedNoteDoc(noteDocView, id).Count()
					: noteDocView.SelectMulti(id).Count();

				SetFilesExists(sender, e.Row, filesCount);
		...
		private static IEnumerable<NoteDoc> FiltrateCachedButNotDeletedNoteDoc(PXView noteDocView, Guid? noteID)
		{
			foreach (NoteDoc noteDoc in noteDocView.Cache.Cached.RowCast<NoteDoc>())
			{
				PXEntryStatus status = noteDocView.Cache.GetStatus(noteDoc);
                                                         ...
                                                         Func<Note, string> getNoteText, Action<PXCache, object, string> setNoteTextExists)
        {
            ...
            if (id.HasValue)
            {
                ...
                if (result == null)
                {
					Note note = (Note)sender.Graph.Caches<Note>().Locate(new Note { NoteID = id });
					if (note == null && sender.GetStatus(e.Row) != PXEntryStatus.Inserted)
						note = GetNote(sender.Graph, id);
	...
	public class PXUniqueNoteAttribute : PXNoteAttribute, IPXRowInsertingSubscriber
	{
		...
		[Obsolete("The constructor is obsolete. Use the parameterless constructor instead. " +
				  "The " + nameof(PXUniqueNoteAttribute) + " constructor is exactly the same as the parameterless one. " +
				  "It does not provide any additional functionality and does not save values of provided fields in the note. " +
				  "The constructor will be removed in a future version of Acumatica ERP.")]
	...
	public class PXSequentialNoteAttribute : PXNoteAttribute
	{
		...
		[Obsolete("The constructor is obsolete. Use the parameterless constructor instead. " +
				  "The " + nameof(PXSequentialNoteAttribute) + " constructor is exactly the same as the parameterless one. " +
				  "It does not provide any additional functionality and does not save values of provided fields in the note. " +
				  "The constructor will be removed in a future version of Acumatica ERP.")]
	...
	public class PXSequentialSelfRefNoteAttribute : PXSequentialNoteAttribute
	{
		...
		[Obsolete("The constructor is obsolete. Use the parameterless constructor instead. " +
				  "The " + nameof(PXSequentialSelfRefNoteAttribute) + " constructor is exactly the same as the parameterless one. " +
				  "It does not provide any additional functionality and does not save values of provided fields in the note. " +
				  "The constructor will be removed in a future version of Acumatica ERP.")]
    ...
    public static class PopupNoteManager
    {
        ...
        {
            var warnings = PXLongOperation.GetCustomInfoForCurrentThread(_longRunWarningsKey) as HashSet<PXUIWarningInfo>;

            if (warnings == null)
            {
                warnings = new HashSet<PXUIWarningInfo>();
        ...
        {
            return PXLongOperation.GetCustomInfo(key, _longRunWarningsKey) as HashSet<PXUIWarningInfo>;
        }
        ...
        {
            return GetLongRunWarnings(key)?.ToList();
        }
