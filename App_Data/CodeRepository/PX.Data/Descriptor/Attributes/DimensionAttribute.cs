...
namespace PX.Data
{
		...
		IPXFieldUpdatingSubscriber
	{
		...
		protected string GetNextNumberInt(PXCache sender, Object row)
		{
				...
				))
			{
				if (record == null)
				{
					throw new PXException(ErrorMessages.CantAutoNumber, Numbering);
				}
		...
		public static void Clear()
		{
			...
			PXContext.ClearSlot<Definition>();
		}
	...
	public class PXDimensionSelectorAttribute : PXAggregateAttribute, IPXFieldVerifyingSubscriber, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXDependsOnFields 
	{
		...
		{
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert && e.TranStatus == PXTranStatus.Open && _JustPersistedKey != null) {
				Type field = SelectorAttribute.Field;
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			if (SelectorAttribute.DirtyRead || SupportNewValues) {
