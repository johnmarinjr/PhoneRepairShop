...
namespace PX.Data
{
	...
	public class PXDefaultAttribute : PXEventSubscriberAttribute, IPXFieldDefaultingSubscriber, IPXRowPersistingSubscriber, IPXFieldSelectingSubscriber
	{
		...
		protected static Tuple<object, PXCache> SelectRow(PXCache cache, PXDefaultAttribute attr, object data)
		{
			...
			{
				var cacheKey = PXSelectorAttribute.GlobalDictionary.NormalizeKeyFieldValue(key);
				...
				if (row == null)
				{
					...
					{
						PXSelectorAttribute.CheckIntegrityAndPutGlobal(dict, view.Cache, attr._ReferencedField.Name, row, cacheKey, false);
					}
				}
	...
	public class PXParentAttribute : PXForeignReferenceAttribute, IPXRowInsertedSubscriber, IPXRowPersistedSubscriber
	{
		...
		private static object SelectParent(PXCache cache, object row, object newRow, Type ParentType, PXParentAttribute attr)
		{
			...
			else
			{
				if (row != null)
				{
					...
					{
						object cached = parentview.Cache.Locate(parent);
		...
		{
			PXView parentView = GetParentSelect(sender);
			BqlCommand selectParent = parentView.BqlSelect;
			IBqlParameter[] pars = selectParent.GetParameters();
			...
			{
				Type partype = pars[i].GetReferencedType();
				if (!Equals(sender.GetValue(row, partype.Name), sender.GetValue(oldrow, partype.Name)))
					return false;
			}
			return true;
		}

		public static bool IsParentReferenceTheSame(PXCache sender, object row, object oldrow, Type parentType, out bool attributeFound)
		{
			var parentAttrs = new List<PXParentAttribute>();
			foreach (PXParentAttribute attr in sender.GetAttributesReadonly(null).OfType<PXParentAttribute>())
			{
				if (attr._ParentType == parentType)
				{
					parentAttrs.Insert(0, attr);
				}
				else if (parentType.IsSubclassOf(attr._ParentType))
				{
			...
			attributeFound = (parentAttrs.Count > 0);
			return attributeFound
				? parentAttrs[0].IsParentReferenceTheSame(sender, row, oldrow)
				: true;
		}
	}
	#endregion
	...
	public class PXStringListAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXLocalizableList
	{
		...
		internal static string[] SplitMultiSelectValues(string values)
		{
			return values?.Split(MultiSelectValueSeparator);
		}
		...
		private SQLExpression GetConditionForValues(string[] allowedValues, SQLExpression fieldExpr)
		{
			...
			for (int i = 0; i < allowedValues.Length; i++)
			{
				...
				SQLExpression whenexp = value == null
										? fieldExpr?.IsNull()
										: fieldExpr.EQ(new SQLConst(value));

				var then = new SQLConst(_AllowedLabels[i]);
				if (IsLocalized)
				{
					//This attribute used for "value-label" columns
					//That is, almost always when we localize label part we want use unicode string for that
					//In the same time value part we use as latin charset
					//Setting DB Type will lead to play with explicit collation change
					then.SetDBType(PXDbType.NVarChar);
				}
				swexp.Case(whenexp, then);
			}

			return swexp;
		}

		private bool AllowedValuesAreFilled(string[] allowedValues) => allowedValues.Length > 0 && !allowedValues.All(val => val == null);
			...
			if ((e.Operation & PXDBOperation.External) == PXDBOperation.External && e.Value == null && _AllowedValues.Length > 0)
			{
				...
				try
				{
					...
					if (descr != null && descr.Expr != null)
					{
						...
						e.Cancel = true;
						e.Expr = AllowedValuesAreFilled(allowedValues) ? GetConditionForValues(allowedValues, fieldExpr) : fieldExpr;
		...
		protected virtual void TryLocalize(PXCache sender)
		{
			if (IsLocalizable)
			{
				...
				IsLocalized = _NeutralAllowedLabels.GetHashCodeOfSequence() != _AllowedLabels.GetHashCodeOfSequence();
			}
		}
	...
	internal class PXDisplaySelectorAttribute : PXSelectorAttribute
	{
		...
		{
			if (MacroVariablesManager != null)
				e.NewValue = MacroVariablesManager.TryResolveExt(e.NewValue, sender, FieldName, e.Row);
	...
	public class PXRestrictorAttribute : PXEventSubscriberAttribute, IPXFieldVerifyingSubscriber
	{
		...
		public static object GetItem(PXCache cache, PXRestrictorAttribute attr, object data, object key)
		{
			...
			if (attr._CacheGlobal && key != null)
			{
				...
				lock ((dict).SyncRoot)
				{
					...
					object cacheKey = keysCount == 1
						? PXSelectorAttribute.GlobalDictionary.NormalizeKeyFieldValue(key)
			...
			if (row == null && (key == null || key.GetType() == cache.GetFieldType(attr._FieldName)))
			{
				...
				{
					PXSelectorAttribute.CheckIntegrityAndPutGlobal(dict, @select.Cache, ((IBqlSearch)attr._OriginalCmd).GetField().Name, item, PXSelectorAttribute.GlobalDictionary.NormalizeKeyFieldValue(key), false);
				}
			}
			return row;
		}
		...
		protected virtual PXException TryVerify(PXCache sender, PXFieldVerifyingEventArgs e, Boolean IsErrorValueRequired)
		{
			...
			if (item != null)
			{
				...
				if (!_AlteredCmd.Meet(view.Cache, item, pars))
				{
					...
					object[] messageParams = GetMessageParameters(sender, itemres, e.Row);
		...
		public virtual object[] GetMessageParameters(PXCache sender, object itemres, object row)
		{
			...
			foreach(Type paramType in _MsgParams)
			{
				Type cacheType = BqlCommand.GetItemType(paramType);
				PXCache cache = sender.Graph.Caches[cacheType];
				object value = cache.GetStateExt(PXResult.Unwrap(itemres, cacheType) ?? cache.Current, paramType.Name);
	...
	public class PXUnboundFormulaAttribute : PXFormulaAttribute
	{
		...
		public override void RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			if (_Formula != null)
			{
				UpdateParent(sender, e.Row, e.OldRow, EnsureParent);
		...
		public override void RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			if (_Formula != null)
			{
				...
				UpdateParent(sender, e.Row, null, (cache, row) => parent);
			}
		}
	...
	public class PXFormulaAttribute : PXEventSubscriberAttribute, IPXRowUpdatedSubscriber, IPXRowInsertedSubscriber, IPXRowDeletedSubscriber, IPXDependsOnFields
	{
		...
		/// </summary>
		public virtual bool SkipZeroUpdates
		{
			get;
			set;
		} = true;
		...
		public static void SetAggregate<Field>(PXCache sender, Type aggregateType, Type parentFieldType = null)
			where Field : IBqlField
		{
			foreach (PXFormulaAttribute attr in sender.GetAttributes<Field>().OfType<PXFormulaAttribute>()
				.Where(attr => parentFieldType == null || parentFieldType == attr.ParentField))
			{
		...
		public static object Evaluate<Field>(PXCache sender, object row)
			where Field : IBqlField
			=> Evaluate(sender, typeof(Field).Name, row);
		...
		protected object EnsureParent(PXCache cache, object Row) => EnsureParent(cache, Row, null);
		...
		protected virtual object EnsureParent(PXCache cache, object Row, object NewRow)
		{
			...
			return PXParentAttribute.SelectParent(cache, Row, NewRow, parentType)
		...
		protected virtual void dependentFieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e, Type dependentField)
		{
			...
			else
			{
				...
				bool? result = null;
				object value = null;
				BqlFormula.ItemContainer item = new BqlFormula.ItemContainer(sender.CreateCopy(e.Row), 
					dependentField, 
					e.ExternalCall, 
					sender.GetValuePending(e.Row, _FieldName));
				try
				{
		...
		{
			return _Aggregate is IBqlZeroValueChecker chk ? chk.IsZeroValue(val) : false;
		}
		...
		protected virtual void UpdateParent(PXCache sender, object row, object oldrow, Func<PXCache, object, object> ensureNewParentFunc = null)
		{
			...
			Type ParentType = BqlCommand.GetItemType(_ParentFieldType);
			bool isParentRefTheSame = PXParentAttribute.IsParentReferenceTheSame(sender, row, oldrow, ParentType, out bool attributeFound);
			Lazy<object> lazyCalcedDiff = Lazy.By(() => CalcAggregate(sender, row, oldrow, -1));

			if (!attributeFound || SkipZeroUpdates && (isParentRefTheSame || oldrow == null) && IsZeroValue(lazyCalcedDiff.Value))
				return;

			object newparent = null;
			if (ensureNewParentFunc != null)
			{
				newparent = ensureNewParentFunc(sender, row);
			}
			else if (row != null)
			{
			...
			if (oldrow != null && !isParentRefTheSame)
			{
		...
		IPXRowPersistingSubscriber
	{
		...
		public virtual void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			if ((sender.Graph.IsImport && !sender.Graph.IsMobile) || sender.Graph.IsContractBasedAPI || sender.Graph.IsCopyPasteContext)
				return;
			if ((_AttributeLevel == PXAttributeLevel.Item || e.IsAltered)
	...
	public class PXDBScalarAttribute : PXDBFieldAttribute
	{
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
				...
				!_recursiveCall)
			{
				...
				(e.Operation & PXDBOperation.Option) == PXDBOperation.External)
				{
					...
					e.BqlTable = _BqlTable;
					if (!isMutable && !TableChangingScope.IsScoped)
					{
						lock (((ICollection)dictExpr).SyncRoot)
						{
		...
		public override void RowSelecting(PXCache sender, PXRowSelectingEventArgs e)
		{
			if (e.Row != null)
			{
				var dbValue = e.Record.GetValue(e.Position, typeOfProperty);
	...
	public class PXRSACryptDataProviderPasswordParameterAttribute : PXRSACryptStringAttribute
	{
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
				...
				(e.Operation & PXDBOperation.Command) == PXDBOperation.Update)
			{
				...
				isEncryptionRequired = (sender.GetValue(e.Row, _isEncryptedField.Name) as bool?) ?? false;
			}
