...
namespace PX.Objects.PJ.Common.Services
{
    public abstract class BaseZipService<TDocument>
    {
        ...
        protected void CreateZipArchive(List<Guid> fileIds, Action<ZipArchiveWrapper, List<Guid>> addFilesToZipArchive)
        {
            var zipFile = GetZipArchive(fileIds, addFilesToZipArchive, ZipFileName);
            throw new PXRedirectToFileException(zipFile, true);
        }

        protected static FileInfo GetZipArchive(List<Guid> fileIds, Action<ZipArchiveWrapper, List<Guid>> addFilesToZipArchive, string fileName)
        {
	        using (var memoryStream = new MemoryStream())
	        {
		        using (var zipArchive = new ZipArchiveWrapper(memoryStream))
		        {
        ...
        protected static void AddDocumentsFilesToZipArchive(ZipArchiveWrapper zipArchive, List<Guid> fileIds)
        {
            ...
            foreach (var (id, fileName) in fileNames)
            {
				...
				zipArchive[fileName] = fileData.BinData;

			}
        }

        protected static void AddDocumentFilesToZipArchive(ZipArchiveWrapper zipArchive, List<Guid> fileIds)
        {
            var graph = PXGraph.CreateInstance<UploadFileMaintenance>();
            foreach (var file in fileIds.Select(fileId => graph.GetFile(fileId)))
            {
				zipArchive[file.Name] = file.BinData;
			}
        }
