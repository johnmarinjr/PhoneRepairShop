...
namespace SP.Objects.IN
{
    ...
    public class InventoryLineMaint : PXGraph<InventoryLineMaint>
	{
        ...
        public InventoryLineMaint()
        {
            ...
            currentCustomer = ReadBAccount.ReadCurrentCustomer(this);
        ...
        protected virtual void InventoryLineFilter_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
        {
            ...
            if (row != null)
            {
                row.CurrencyStatus = currentCustomer.CuryID ?? currentCustomer.BaseCuryID;
            }
        }
		...
		public virtual void CalculateDiscount(InventoryLines row, decimal price)
        {
	        ...
	        DiscountEngine.SetLineDiscountOnly(FilteredItems.Cache, row,
                                new DiscountLineFields
									...
									DiscountLineFields.automaticDiscountsDisabled>(FilteredItems.Cache, row), 
                                    row.BaseDiscountID,
                                    price, 
                                    row.Qty * price, 
                                    row.Qty, 
		...
		protected virtual void PersistCardLines(bool inserted, bool updated)
		{
			if (inserted || updated)
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					PersistCardLinesInTranScope(inserted, updated);
					ts.Complete(this);
				}
		...
		protected virtual void PersistCardLinesInTranScope(bool inserted, bool updated)
		{
			if (inserted)
