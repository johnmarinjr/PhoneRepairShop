...
namespace SP.Objects.IN
{
    ...
    public class SOOrderInquire : PXGraph<SOOrderInquire>
    {
            ...
            if (this.Items.Current != null)
            {
                ...
                if (currentline != null)
                {
                                ...
                                currentline.OrderNbr))
                    {
                        ...
                        {
							var currentlineincart1 = InsertOrderLineToCart(_soLine, _inventoryItem);
                            CalculateDiscountPriceCard(currentlineincart1, (decimal)_soLine.CuryUnitPrice);
                            PersistInsertedPortalCardLines(currentlineincart1);
                        }
		...
		protected virtual bool PersistInsertedPortalCardLines(PortalCardLines portalCardLines)
		{
			return CardLines.Cache.PersistInserted(portalCardLines);
		}

		protected virtual PortalCardLines InsertOrderLineToCart(SOLine soLine, InventoryItem inventoryItem)
		{
			...
			currentlineincart1.SiteID = soLine.SiteID;
			currentlineincart1.InventoryID = soLine.InventoryID;
			currentlineincart1.InventoryIDDescription = inventoryItem.InventoryCD;
			currentlineincart1.Descr = inventoryItem.Descr;
			currentlineincart1.StkItem = inventoryItem.StkItem;

			currentlineincart1.Qty = Convert.ToInt32(soLine.Qty);
			currentlineincart1.UOM = soLine.UOM;
			using (new PXReadBranchRestrictedScope())
			{
				currentlineincart1 = CardLines.Insert(currentlineincart1);
			}
			return currentlineincart1;
		}
		...
		public virtual CurrencyInfo GetCurrencyInfo()
        {
            ...
            info.CuryID = currentCustomer.CuryID ?? currentCustomer.BaseCuryID;
        ...
        public virtual void CalculateDiscountPriceCard(PortalCardLines row, decimal price)
        {
	        ...
	        DiscountEngine.SetLineDiscountOnly(CardLines.Cache, row,
                                new DiscountLineFields
									...
									DiscountLineFields.automaticDiscountsDisabled>(CardLines.Cache, row),
                                    row.BaseDiscountID,
                                    price,
                                    row.Qty * price,
                                    row.Qty,
                                    ...
                                    currentCustomer.CuryID ?? currentCustomer.BaseCuryID,
