...
namespace SP.Objects.IN
{
    public class SOOrderEntryExt : PXGraphExtension<SOOrderEntry>
    {
		...
		public override void Initialize()
        {
            ...
            currentCustomer = ReadBAccount.ReadCurrentCustomer(Base);
            finalConfirm.CurrencyStatus = currentCustomer.CuryID ?? currentCustomer.BaseCuryID;
		...
		/// Overrides <see cref="SOOrderEntry.CalculateFreight(PXAdapter)"/>
		[PXOverride]
        public virtual IEnumerable CalculateFreight(PXAdapter adapter, Func<PXAdapter, IEnumerable> baseImpl)
        {
			try
			{
				return baseImpl(adapter);
			}
			catch (Exception e)
			{
				PXTrace.WriteError(e);

				throw new PXSetPropertyException(e, PXErrorLevel.Error, Messages.AtleastOnePackageIsRequired);
			}
        }
        #endregion
            ...
            PXRowInserted ins)
        {
            ...
            if (row != null && row.CustomerID == null)
            {
                ...
                Base.Document.Cache.SetValueExt<SOOrder.curyID>(soorder, currentCustomer.CuryID ?? currentCustomer.BaseCuryID);
        ...
        public virtual decimal CalculatePriceCard(PortalCardLines row, SOOrder soOrder)
        {
            // цена
