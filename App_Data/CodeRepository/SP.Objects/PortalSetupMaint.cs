...
namespace SP.Objects
{
	public class PortalSetupMaint : PXGraph<PortalSetupMaint>
	{
		...
		protected virtual void PortalSetup_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			var documentRestrictionEnabled = PXAccess.FeatureInstalled<FeaturesSet.branch>() || PXAccess.FeatureInstalled<FeaturesSet.multiCompany>();
			...
			if (row.DisplayFinancialDocuments == FinancialDocumentsFilterAttribute.BY_BRANCH && row.RestrictByBranchID != null)
			{
				...
				if (row.SellingBranchID != row.RestrictByBranchID)
				{
					// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Legacy]
					sender.SetValueExt<PortalSetup.sellingBranchID>(row, row.RestrictByBranchID);
				}
			}
		...
		protected virtual void PortalSetup_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
														 ...
														 PortalSetup.restrictByOrganizationID>(oldRow, row))
			{
				if (!sender.ObjectsEqual<PortalSetup.displayFinancialDocuments>(oldRow, row))
				{
					...
					sender.SetValueExt<PortalSetup.sellingBranchID>(row, null);
				}
				else
				{
					var sellingBranchID = GetDefaultSellingBranchId(row.RestrictByOrganizationID);
					sender.SetValueExt<PortalSetup.sellingBranchID>(row, sellingBranchID);
				}
			}
		}
