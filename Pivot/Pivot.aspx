...
<asp:Content ID="contDS" ContentPlaceHolderID="phDS" runat="Server">
    <script type="text/javascript">
		...
		{
			if (__px(window).topWin() != window && (__win(this).suppressChanges || !__px_cm(window).checkDataChanges()))
				__px_cm(window).sendPageUnloadRequest(true);
		}

		function addHandlers(args)
		{
			__px(window).addEventHandler(window, "beforeunload", handle_BeforeUnload);
		}
	...
	<px:PXPivotDataSource ID="ds" runat="server" Visible="false" Width="100%" PageLoadBehavior="PopulateSavedValues" PivotControlID="pt">
        <ClientEvents Initialize="addHandlers"/>
		<CallbackCommands>
		</CallbackCommands>
