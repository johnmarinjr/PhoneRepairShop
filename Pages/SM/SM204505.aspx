		...
		<Levels>
			<px:PXGridLevel DataMember="Projects">
				...
				<Columns>
					...
					<px:PXGridColumn DataField="CertificationStatus" Width="50px" />
					<%--<px:PXGridColumn DataField="DevelopedBy" Width="50px" />--%>
					<px:PXGridColumn DataField="Initials" Width="50px" />
...
<%--					<px:PXGridColumn AllowUpdate="False" DataField="CreatedDateTime" Width="90px" />
    ...
    <script type="text/javascript">
        ...
        function onDSCommandPerformed(src, args) {
			if (args.command == "ReloadPage" && src.longRunMessage == "RUN-PACKAGES-VALIDATION") {
				src.longRunMessage = "";
                setTimeout(function () { px_alls.ds.executeCallback("AutoValidateCertStatus"); }, 200);
			} 
			else
            if (args.command.toLowerCase() == "actionvalidatecertstatus" || args.command == "AutoValidateCertStatus") {
                setTimeout(function () { px_alls.grid.refresh(); }, 200);
			}
		}
    </script>
