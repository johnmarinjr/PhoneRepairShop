...
<asp:Content ID="cont1" ContentPlaceHolderID="phDS" runat="Server">
    <pxa:DynamicDataSource ID="ds" runat="server" Visible="True" Width="100%" PrimaryView="EMailAccounts" TypeName="PX.SM.EMailAccountMaint" HeaderDescriptionField="Description">
		<CallbackCommands>
			...
			<px:PXDSCallbackCommand Name="checkEMailAccount" PopupVisible="True" StartNewGroup="True" CommitChanges="true" />
...
<asp:Content ID="cont2" ContentPlaceHolderID="phF" runat="Server">
    <px:PXFormView ID="form" runat="server" DataSourceID="ds" Style="z-index: 100" Width="100%"
		...
		<Template>
			...
			<px:PXTextEdit ID="edDescriptionOld" runat="server" DataField="Description" CommitChanges="true" />
			...
			<px:PXSelector ID="edPluginTypeName" runat="server" DataField="PluginTypeName" CommitChanges="true" />
			<px:PXCheckBox ID="edIsOfPluginType" runat="server" DataField="IsOfPluginType" Visible="False" />
		</Template>
	</px:PXFormView>
</asp:Content>
<asp:Content ID="cont3" ContentPlaceHolderID="phG" runat="Server">
    <px:PXTab ID="tab" runat="server" DataSourceID="ds" Height="387px" Style="z-index: 100" Width="100%" DataMember="CurrentEMailAccounts" DefaultControlID="edDescription">
		<Items>
			<px:PXTabItem Text="Plug-In" BindingContext="form" VisibleExp="DataControls[&quot;edIsOfPluginType&quot;].Value == true" >
				<Template>
					<px:PXGrid ID="PXGridSettings" runat="server" DataSourceID="ds" AllowFilter="False" Width="100%" SkinID="DetailsInTab" Height="500px" MatrixMode="True">
						<ActionBar>
							<Actions>
								<NoteShow MenuVisible="False" ToolBarVisible="False" />
								<FilesMenu MenuVisible="False" ToolBarVisible="False" />
							</Actions>
							<CustomItems>
								<px:PXToolBarButton CommandName="ReloadSettings" CommandSourceID="ds" Text="Reload Settings" />
							</CustomItems>
						</ActionBar>
						<Levels>
							<px:PXGridLevel DataMember="Details">
								...
								<Columns>
									...
									<px:PXGridColumn DataField="Value" AllowSort="False" />
								</Columns>
							</px:PXGridLevel>
						</Levels>
			...
			<px:PXTabItem Text="Service" LoadOnDemand="True" BindingContext="form" VisibleExp="DataControls[&quot;edIsOfPluginType&quot;].Value == false" >
				<Template>
					<px:PXLayoutRule runat="server" StartColumn="True" LabelsWidth="SM" ControlSize="L" />
					...
					<px:PXCheckBox CommitChanges="True" ID="chkOutcomingAuthenticationRequest" runat="server" DataField="OutcomingAuthenticationRequest" />
					<px:PXCheckBox CommitChanges="True" ID="chkOutcomingAuthenticationDifferent" runat="server" DataField="OutcomingAuthenticationDifferent" />
					...
					<px:PXLayoutRule runat="server" StartGroup="True" GroupCaption="Receiving" ControlSize="XM" LabelsWidth="SM" SuppressLabel="True"/>
					<px:PXDropDown ID="edFetchingBehavior" runat="server" DataField="FetchingBehavior" CommitChanges="true" SuppressLabel="false" />
				</Template>
			</px:PXTabItem>
			<px:PXTabItem Text="Sender Name" RepaintOnDemand="false">
				<Template>
					<px:PXLayoutRule runat="server" StartColumn="True" LabelsWidth="SM" ControlSize="L" />
					<px:PXLayoutRule runat="server" StartGroup="True" GroupCaption="Server Information" />
			...
			<px:PXTabItem Text="Incoming Mail Processing" BindingContext="form" VisibleExp="DataControls[&quot;edIsOfPluginType&quot;].Value == false">
				<Template>
					...
					<px:PXSelector ID="edConfirmReceiptTemplate" runat="server" PopulateOnDemand="True" DataField="ConfirmReceiptNotificationID" SuppressLabel="False" Size="L" DisplayMode="Text">
						<AutoCallBack Enabled="True" Command="Save" Target="tab" />
					</px:PXSelector> 
					...
					<px:PXSelector ID="edRespnseTemplate" runat="server" PopulateOnDemand="True" DataField="ResponseNotificationID" SuppressLabel="False" Size="L" DisplayMode="Text">
						<AutoCallBack Enabled="True" Command="Save" Target="tab" />
					</px:PXSelector>
					...
					<px:PXDropDown ID="edTypeDelete" runat="server" DataField="TypeDelete" AllowNull="false" Size="XS" />
					<px:PXLabel ID="PXLabel3" runat="server" Text="processing" />
					<px:PXLayoutRule ID="PXLayoutRule3" runat="server" />
					<px:PXLayoutRule ID="PXLayoutRule4" runat="server" LabelsWidth="SM" ControlSize="M" SuppressLabel="True" />
					<px:PXCheckBox ID="chkAddUpInformation" runat="server" DataField="AddUpInformation" />
				</Template>
			</px:PXTabItem>	
			<px:PXTabItem Text="Content" BindingContext="form" VisibleExp="DataControls[&quot;edIsOfPluginType&quot;].Value == false">
				<Template>
					...
					<px:PXLabel ID="lblCommentAttach" runat="server" Text="Specify the extensions for allowed attachment types separating them by commas. Example: .csv, .jpg, .png, .xls" SuppressLabel="False"/>	 
					<px:PXTextEdit ID="edIncomingAttachmentType" runat="server" DataField="IncomingAttachmentType" Size="XXL" SuppressLabel="True" />
				</Template>
			</px:PXTabItem>
			<px:PXTabItem Text="Assignment Settings" BindingContext="form" VisibleExp="DataControls[&quot;edIsOfPluginType&quot;].Value == false">
				<Template>
					<px:PXLayoutRule ID="PXLayoutRule1" runat="server" StartColumn="True" LabelsWidth="M" ControlSize="L" />
					<px:PXSelector ID="edDefaultAssignmentMapID" runat="server" DataField="DefaultEmailAssignmentMapID" TextField="Name" AllowEdit="True" AutoRefresh="True">
						<GridProperties>
							<Layout ColumnsMenu="False"></Layout>
						</GridProperties>
					</px:PXSelector>
					<px:PXSelector ID="WorkgroupID" runat="server" DataField="DefaultWorkgroupID" CommitChanges="True" TextMode="Search" DisplayMode="Text" FilterByAllFields="True" AutoRefresh="True" />
					<px:PXSelector ID="OwnerID" runat="server" DataField="DefaultOwnerID" TextMode="Search" CommitChanges="True" DisplayMode="Text" FilterByAllFields="True" AutoRefresh="True" />
				</Template>
			</px:PXTabItem>
		</Items>
		<AutoSize Container="Window" Enabled="True" MinHeight="250" MinWidth="300" />
	</px:PXTab>
	<px:PXSmartPanel ID="pnlSendTestEmail" runat="server" Caption="Send Test Email" CaptionVisible="true" LoadOnDemand="true" Key="SendTestEmailFilter"
		AutoCallBack-Enabled="true" AutoCallBack-Command="Refresh" CallBackMode-CommitChanges="True"
		CallBackMode-PostData="Page">
		<div style="padding: 5px">
			<px:PXFormView ID="frmSendTestEmail" runat="server" DataSourceID="ds" CaptionVisible="False" DataMember="SendTestEmailFilter">
				<ContentStyle BackColor="Transparent" BorderStyle="None" />
				<Template>
					<px:PXTextEdit ID="edEmailAddress" runat="server" DataField="EmailAddress" Required="True"/>
				</Template>
			</px:PXFormView>
		</div>
