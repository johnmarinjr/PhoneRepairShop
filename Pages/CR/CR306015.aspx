<%@ Page Language="C#" MasterPageFile="~/MasterPages/FormDetail.master" AutoEventWireup="true" ValidateRequest="false" CodeFile="CR306015.aspx.cs" Inherits="Page_CR306015" Title="Case Activity" %>
...
<asp:Content ID="cont1" ContentPlaceHolderID="phDS" runat="Server">
	...
	<px:PXDataSource ID="ds" runat="server" Width="100%" Visible="true" PrimaryView="Message" TypeName="PX.Objects.CR.CREmailActivityMaint">
		<CallbackCommands>
			...
			<px:PXDSCallbackCommand CommitChanges="True" Name="send" PopupVisible="True" StartNewGroup="True" ClosePopup="True" RepaintControls="All" />
	...
	<px:PXUploadDialog ID="dlgUpload" runat="server" Height="76px" Style="position: static" Width="550px" Caption="File Upload" AllowPostBack="False" />
</asp:Content>
<asp:Content ID="cont2" ContentPlaceHolderID="phF" runat="server">
	<px:PXFormView
			ID="message" runat="server" DataSourceID="ds" Width="100%" DataMember="Message" OverflowY="Hidden" NoteIndicator="true" NoteField="NoteText"
			OnDataBound="on_data_bound" FilesIndicator="true" FilesField="NoteFiles" DefaultControlID="edSubject" OnSelect="message_OnSelect" AllowCollapse="false">
		...
		<Template>
			...
			<px:PXSelector ID="edMailFrom" runat="server" DataField="MailAccountID" ValueField="EmailAccountID" DataSourceID="ds" ReadOnly="true" Width="100%" DisplayMode="Text" CommitChanges="true">
				<GridProperties>
					<Columns>
						<px:PXGridColumn DataField="Address" />
						<px:PXGridColumn DataField="Description" />
						<px:PXGridColumn DataField="EmailAccountType" />
					</Columns>
					<PagerSettings Mode="NextPrevFirstLast" />
				</GridProperties>
			</px:PXSelector>
			<px:PXTextEdit ID="edMailFromTe" runat="server" DataField="MailFrom" ReadOnly="true" Width="100%" />
			<px:PXMultiSelector ID="edMailTo" runat="server" SkinID="email" TextField="SearchSuggestion" ValueField="EMail" DataSourceID="ds" DataField="MailTo" Width="100%" AutoGenerateColumns="false" Hidden="True" CommitChanges="True">
			...
			<px:PXTextEdit ID="edMailToTe" runat="server" DataField="MailTo" Width="100%" ReadOnly="true" Hidden="True" />
			<px:PXMultiSelector ID="edMailCc" runat="server" SkinID="email" DataField="MailCc" Hidden="True" TextField="SearchSuggestion" ValueField="EMail" DataSourceID="ds" Width="100%" CommitChanges="True">
				<GridProperties FastFilterFields="DisplayName">
					<Columns>
						<px:PXGridColumn DataField="DisplayName" />
						<px:PXGridColumn DataField="ContactType" />
			...
			<px:PXTextEdit ID="edMailCcTe" runat="server" DataField="MailCc" Hidden="True" Width="100%" ReadOnly="true" />
			<px:PXMultiSelector ID="edMailBcc" runat="server" SkinID="email" DataField="MailBcc" TextField="SearchSuggestion" ValueField="EMail" DataSourceID="ds" Width="100%" Hidden="True" CommitChanges="True">
				<GridProperties FastFilterFields="DisplayName">
					<Columns>
						<px:PXGridColumn DataField="DisplayName" />
						<px:PXGridColumn DataField="ContactType" />
			...
			<px:PXTextEdit ID="edMailBccTe" runat="server" DataField="MailBcc" Width="100%" ReadOnly="true" Hidden="True" />
			<px:PXTextEdit ID="edSubject" runat="server" DataField="Subject" Width="570px" />
			<px:PXTextEdit ID="edRecipientNotes" runat="server" DataField="RecipientNotes" CommitChanges="True" TextMode="MultiLine" Height="56px" />
			<px:PXLayoutRule runat="server" ColumnWidth="35%" StartColumn="True" SuppressLabel="True" />
			<px:PXHtmlView ID="edEntityDescription" runat="server" DataField="EntityDescription" TextMode="MultiLine" SkinID="Label" Width="250px" Style="background-color: White; height: 132px;">
			</px:PXHtmlView>
		</Template>
	</px:PXFormView>
</asp:Content>
<asp:Content ID="cont3" ContentPlaceHolderID="phG" runat="Server">
		...
		<Items>
			...
			<px:PXTabItem Text="Details">
				<Template>
					...
					<px:PXDateTimeEdit ID="edStartDate_Date" runat="server" DataField="StartDate_Date" CommitChanges="True" />
					<px:PXDateTimeEdit ID="edStartDate_Time" runat="server" DataField="StartDate_Time" TimeMode="true" SuppressLabel="true" Width="84" CommitChanges="True" />
					...
					<pxa:PXRefNoteSelector ID="edRefEntity" runat="server" DataField="Source" NoteIDDataField="RefNoteID" MaxValue="0" MinValue="0" ValueType="Int64">
						...
						<LookupPanel DataMember="Message$RefNoteView" DataSourceID="ds" TypeDataField="Type" IDDataField="RefNoteID" />
					...
					<px:PXLayoutRule runat="server" ControlSize="XM" LabelsWidth="SM" />
					<px:PXLayoutRule runat="server" StartGroup="True" GroupCaption="SendGrid" />
					<px:PXTextEdit ID="edCategories" runat="server" DataField="Categories" Placeholder="comma-separated list" />
					<px:PXCheckBox ID="chkBypassSuppressionChecks" runat="server" DataField="BypassSuppressionChecks" />
					<px:PXCheckBox ID="chkTrackEmailOpens" runat="server" DataField="TrackEmailOpens" />
					<px:PXCheckBox ID="chkTrackClicksInEmail" runat="server" DataField="TrackClicksInEmail" />
                    ...
                    <px:PXSelector ID="edShiftID" runat="server" DataField="TimeActivity.ShiftID" />
			...
			<px:PXTabItem Text="Tracking" >
				<Template>
					<px:PXGrid ID="grdRecipientsData" runat="server" DataSourceID="ds" Height="323px" Style="z-index: 100; left: 0px; position: absolute; top: 0px"
						Width="100%" AllowSearch="True" AllowPaging="true" AdjustPageSize="Auto" BorderWidth="0" SkinID="Details" MatrixMode="true">
						<Levels>
							<px:PXGridLevel DataMember="SendGridRecipients">
								<Columns>
									<px:PXGridColumn DataField="Address" />
									<px:PXGridColumn DataField="Name" Width="160" />
									<px:PXGridColumn DataField="Status" Width="140" />

									<px:PXGridColumn DataField="OpenedCount" />
									<px:PXGridColumn DataField="ClickedCount" />
									<px:PXGridColumn DataField="ReportedAsSpamCount" Width="140" />
									<px:PXGridColumn DataField="OptedOutCount" Width="90" />

									<px:PXGridColumn DataField="MailServiceReply" />
								</Columns>
							</px:PXGridLevel>
						</Levels>
	...
	<px:PXSmartPanel ID="PanelSelectNotificatonTemplate" runat="server"
		Style="z-index: 108; position: absolute; left: 27px; top: 99px; height: 110px; width: 355px;"
		...
		<px:PXFormView ID="formSelectNotificationTemplate" runat="server" DataSourceID="ds" Style="z-index: 100" Width="100%"
			...
			<Template>
				...
				<px:PXSelector CommitChanges="True" ID="edNtificationName" runat="server" DataField="NotificationName">
					<GridProperties FastFilterFields="Description, SiteMap__Title, ScreenID, Subject" />
				</px:PXSelector>
	...
	<px:PXSmartPanel runat="server" ID="panelAttach" Height="400px" Width="950px" LoadOnDemand="True" AutoReload="True" CaptionVisible="True" Caption="Attach Files" Key="AttachedFiles" CommandSourceID="ds" AutoCallBack-Command="Attach" AutoCallBack-Target="grdFiles" CloseButtonDialogResult="Abort">
		<px:PXGrid runat="server" ID="grdFiles" Height="200px" SkinID="Inquire" TabIndex="17500" Width="100%" AllowPaging="True" PageSize="10">
			<Mode AllowAddNew="False" />
			<Levels>
				<px:PXGridLevel DataMember="AttachedFiles">
					<Columns>
						<px:PXGridColumn DataField="IsAttached" Width="50" Type="CheckBox" TextAlign="Center" CommitChanges="True" AllowCheckAll="True" />
						<px:PXGridColumn DataField="FileName" Width="120" />
						<px:PXGridColumn DataField="FileSource" Width="120" AllowShowHide="Server" />
						<px:PXGridColumn DataField="DrawingLogCd" Width="120" />
						<px:PXGridColumn DataField="Number" Width="120" />
						<px:PXGridColumn DataField="Revision" Width="120" />
						<px:PXGridColumn DataField="IsDrawingLogCurrentFile" Width="120" Type="CheckBox" />
					</Columns>
				</px:PXGridLevel>
			</Levels>
		</px:PXGrid>
		<px:PXPanel runat="server" ID="DialogButtons" SkinID="Buttons">
			<px:PXButton runat="server" ID="btnOk" DialogResult="OK" Text="Attach" />
			<px:PXButton runat="server" ID="btnCancel" DialogResult="Abort" Text="Cancel" />
		</px:PXPanel>
	</px:PXSmartPanel>
