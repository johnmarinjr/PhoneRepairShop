...
<asp:Content ID="cont1" ContentPlaceHolderID="phDS" runat="Server">
	<script type="text/javascript">
		function onDSCommandPerformed(src, args) {
			if (args.command == "ReloadPage" || args.command == "Cancel") {
				var x = navigator.plugins.length;var txt = [];
				for (var i = 0; i < x; i++) {
					txt.push(navigator.plugins[i].name.split(" ").join("%20"));
				}

				var scaling = window.screen.scaling || 1;
				var clientScreens = ("width=" + window.screen.width + "&height=" + window.screen.height + "&scaling-factor=" + scaling + "&colour-depth=" + window.screen.colorDepth).toString();
				var windowSize = ("width=" + window.innerWidth + "&height=" + window.innerHeight).toString();
				var ds = px_alls['ds'];
				var actionName = 'calculate';
				var clientIP ='';
				...
				function executeCalculateCallback(){
    ...
    <px:PXDataSource ID="ds" runat="server" Visible="True" Width="100%" TypeName="PX.Objects.Localizations.GB.HMRC.VATMaint" PrimaryView="Period_Header" PageLoadBehavior="PopulateSavedValues">
        <CallbackCommands>
	        <px:PXDSCallbackCommand Name="Calculate" Visible="false" CommitChanges="true" PostData="Self" />
	        <px:PXDSCallbackCommand Name="SendVATReturn"  Visible="true" CommitChanges="true" PostData="Self" />
	        <px:PXDSCallbackCommand Name="TestFraudHeaders"  Visible="true" CommitChanges="true" PostData="Self" />
...
<asp:Content ID="cont2" ContentPlaceHolderID="phF" runat="Server">
    <px:PXFormView ID="form" runat="server" Width="100%" DataMember="Period_Header" Caption="Selection" TabIndex="4500">
        <Template>
            ...
            <px:PXSelector AutoRefresh="True" Size="S" ID="edTaxPeriodID" runat="server" DataField="TaxPeriodID" CommitChanges="true" ></px:PXSelector>
